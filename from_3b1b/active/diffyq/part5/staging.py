...
class IntroductionOfExp(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(Restore(derivative), FadeInFrom(ic, LEFT))
        self.wait()
        self.play(
            FadeOut(questions[0]),
            FadeOut(top_cross),
    ...
    def get_deriv_and_ic(self, const=None):
        if const:
            ...
            mult_str = "{}"
        args = [
            "{d \\over d{t}}", "e^{", const_str, "{t}}", "=", const_str,
            mult_str, "e^{", const_str, "{t}}"
        ]
        derivative = TexMobject(*args, **tex_config)
        if const:
...
class IntroducePhysicalModel(IntroductionOfExp):
    ...
    def setup_number_line(self):
        nl = self.number_line = NumberLine(**self.number_line_config, )
        nl.to_edge(LEFT)
    ...
    def setup_vectors(self):
        ...
        position_vect.add_updater(lambda m: m.put_start_and_end_on(
            number_line.cn2p(0),
    ...
    def show_formulas(self):
        ...
        self.play(ShowCreation(rhs_rect), FadeInFrom(rhs_word, UP),
...
class ConstantEquals2(IntroducePhysicalModel):
    ...
    def point_out_chain_rule(self):
        ...
        words = VGroup(
            *map(TextMobject, ["Velocity", self.const_text, "Position"]))
    ...
    def show_double_vector(self):
        ...
        self.play(ReplacementTransform(p_copy, v_vect),
                  TransformFromCopy(
                      self.v_word,
                      self.velocity_label,
                  ))
...
class NegativeConstant(ConstantEquals2):
    ...
    def show_vector_manipulation(self):
        ...
        self.velocity_label.add_updater(lambda m: m.next_to(
            self.velocity_vect,
            UP,
            buff=SMALL_BUFF,
        ))
        ...
        self.wait()
        self.play(p_copy.scale, 0.5, {"about_point": p_copy.get_start()},
                  p_copy.match_color, v_vect, FadeIn(times_point5))
        self.wait()
        self.play(
...
class ImaginaryConstant(ConstantEquals2):
    ...
    def setup_number_line(self):
        ...
        nl.shift(0.5 * DOWN)
        shift_distance = (4 * nl.tick_size + nl.numbers.get_height() +
                          SMALL_BUFF)
    ...
    def add_initial_objects(self):
        ...
        self.tip_label.add_updater(lambda m: m.next_to(
        ...
        self.play(ShowCreation(p_rect), FadeInFrom(p_label, UP))
    ...
    def transition_to_complex_plane(self):
        ...
        corner_group.target[-1].next_to(corner_group.target,
                                        DOWN,
                                        buff=0.75,
                                        aligned_edge=LEFT)
    ...
    def show_vector_field(self):
        ...
        foreground = VGroup(self.corner_group, )
...
class PiMinuteMark(Scene):
    def construct(self):
        text = TexMobject("\\pi \\text{ minutes} \\approx \\text{3:08}")
...
class ReferenceWhatItMeans(PiCreatureScene):
    def construct(self):
        ...
        epii = TexMobject("e^{{i} \\pi} = -1", **tex_config)
        epii.scale(2)
        ...
        nonsense = TextMobject("\\dots because that's " "literal nonsense!")
        ...
        series = TexMobject("{({i} \\pi )^0 \\over 0!} + ",
                            "{({i} \\pi )^1 \\over 1!} + ",
                            "{({i} \\pi )^2 \\over 2!} + ",
                            "{({i} \\pi )^3 \\over 3!} + ",
                            "{({i} \\pi )^4 \\over 4!} + ", "\\cdots = -1",
                            **tex_config)
        ...
        self.wait()
        self.play(FadeOutAndShift(randy, DOWN), FadeInFrom(down_arrow, UP),
