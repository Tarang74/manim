...
class TwoDBodyWithManyTemperatures(ThreeDScene):
    ...
    def introduce_body(self):
        ...
        rows = VGroup(*[
            ...
            ]).arrange(RIGHT, buff=buff) for y in range(self.cells_per_side)
    ...
    def show_temperature_at_all_points(self):
        ...
        decimal.add_updater(
            lambda d: d.set_value(40 + 50 * self.point_to_temp(get_point())))
        ...
        dot.add_updater(lambda p: p.move_to(body[-1] if
                                            (1 < len(body)) else start_corner))
...
class BringTwoRodsTogether(Scene):
    ...
    def show_rods(self):
        ...
        self.play(FadeInFrom(rod2, DOWN), Write(rod2.label))
        self.wait()
    ...
    def show_equilibration(self):
        ...
        self.play(
            Write(axes),
            rods.restore,
            rods.space_out_submobjects,
            1.1,
            FadeIn(self.time_group),
            FadeIn(self.clock),
            *[MaintainPositionRelativeTo(rod.label, rod) for rod in rods],
        )
        ...
        br1.add_updater(lambda b: b.align_to(rods[0].get_right(), LEFT))
        br2.add_updater(lambda b: b.move_to(axes.c2p(0, 10)))
        br2.add_updater(lambda b: b.align_to(rods[1].get_left(), RIGHT))
        ...
        self.play(self.get_clock_anim(self.wait_time), FadeOut(labels))
    ...
    def get_second_derivative(self, x, dx=0.001):
        ...
        ly, y, ry = [
            graph.point_from_proportion(
                inverse_interpolate(x_min, x_max, alt_x))[1]
    ...
    def get_rod(self, x_min, x_max, n_pieces=None):
        ...
        rod = VGroup(*[Square() for n in range(n_pieces)])
    ...
    def rod_point_to_graph_y(self, point):
        ...
        return axes.y_axis.p2n(graph.point_from_proportion(alpha))
    ...
    def rod_point_to_color(self, point):
        return self.y_to_color(self.rod_point_to_graph_y(point))
...
class ShowEvolvingTempGraphWithArrows(BringTwoRodsTogether):
    ...
    def temp_func(self, x, t):
        new_x = TAU * x / 10
        return 50 + 20 * np.sum([
            amp * np.sin(freq * new_x) * np.exp(-(self.alpha * freq**2) * t)
...
class TalkThrough1DHeatGraph(ShowEvolvingTempGraphWithArrows,
    ...
    def emphasize_rod(self):
        ...
        self.play(
            LaggedStart(
                *[Rotating(piece, rate_func=smooth) for piece in alt_rod],
                run_time=2,
                lag_ratio=0.01,
            ), Write(word))
        self.remove(*alt_rod)
    ...
    def show_x_axis(self):
        ...
        self.play(rod.set_opacity, 0.5, FadeInFrom(x_axis_label, UL),
                  LaggedStartMap(
                      FadeInFrom,
                      x_numbers,
                      lambda m: (m, UP),
        ...
        triangle.add_updater(lambda t: t.next_to(get_x_point(), UP))
        x_label = VGroup(
            TexMobject("x"), TexMobject("="),
            DecimalNumber(
                0,
        ...
        rod_piece = always_redraw(lambda: self.get_rod(
            get_x() - 0.05,
            get_x() + 0.05,
            n_pieces=1,
        ))
    ...
    def show_changes_over_time(self):
        ...
        self.play(self.get_graph_time_change_animation(graph, time),
    ...
    def show_surface(self):
        ...
        graph_slices = VGroup(*[
            self.get_graph(time=t).shift(t * IN)
        ...
        slicing_plane.replace(Line(axes_copy.c2p(0, 0), axes_copy.c2p(10,
                                                                      100)),
                              stretch=True)
        ...
        graph.add_updater(
            lambda g: g.become(get_time_slice(t_tracker.get_value())))
    ...
    def get_graph_time_change_animation(self, graph, new_time, **kwargs):
        ...
        return UpdateFromAlphaFunc(
            graph, lambda g, a: g.become(
                self.get_graph(interpolate(old_time, new_time, a))), **config)

    def orient_mobject_for_3d(self, mob):
        mob.rotate(90 * DEGREES, axis=RIGHT, about_point=ORIGIN)
        return mob
...
class ContrastXChangesToTChanges(TalkThrough1DHeatGraph):
    ...
    def emphasize_dimensions_of_input_space(self):
        ...
        self.play(Write(words),
                  ...
                  -90 * DEGREES,
                  run_time=1)
        self.play(ReplacementTransform(plane_copy0, plane_copy1))
        self.play(ReplacementTransform(plane_copy1, plane_copy))
        self.wait(2)
    ...
    def show_changes_with_x(self):
        ...
        line = always_redraw(lambda: self.get_tangent_line(
            self.graph,
        ...
        self.play(ShowCreation(dx_line), FadeInFrom(dx, LEFT))
        self.wait()
        self.play(ShowCreation(dT_line), FadeInFrom(dT, IN))
        self.wait()
    ...
    def show_changes_with_t(self):
        ...
        graph.set_points_smoothly(
            [gs.point_from_proportion(slice_alpha) for gs in slices])
        graph.color_using_background_image("VerticalTempGradient")
        graph.set_shade_in_3d(True)
        ...
        self.play(
            ShowCreation(graph.copy(), remover=True),
    ...
    def get_tangent_line(self, graph, alpha, d_alpha=0.001, length=2):
        ...
        line.scale(length / line.get_length())
        line.move_to(p1)
        return line
...
class TransitionToTempVsTime(ContrastXChangesToTChanges):
    ...
    def construct(self):
        ...
        self.play(
            t_axis.time_label.scale,
            1 / 1.5,
            t_axis.time_label.next_to,
            t_axis,
            IN,
            MED_LARGE_BUFF,
            t_axis.numbers.shift,
            0.7 * IN,
        )
        self.wait()
...
class ShowDelTermsAsTinyNudges(TransitionToTempVsTime):
    ...
    def show_del_x(self):
        ...
        for sym in syms:
            sym.add_updater(lambda m: m.set_width(dx_line.get_length()))
        ...
        for sym in reversed(syms):
            self.play(
                ...
                ShowCreation(sym.line.copy(), remover=True),
            )
            self.add(sym.line)
        self.play(ShowCreation(tan_line))
        for sym in syms:
            self.play(ShowCreationThenDestruction(sym.rect))
            self.wait()
        self.wait()
        self.add(line_group)
        self.play(
            dx_tracker.set_value,
            0.01,
            run_time=5,
        )
        self.play(
    ...
    def show_del_t(self):
        ...
        for sym in syms:
            ...
            sym.add_updater(lambda m: m.set_height(0.8 * dT_line.get_length()))

        del_t.add_updater(
            lambda m: m.set_height(min(0.5, m.line.get_length())))
        del_T.add_updater(lambda m: m.set_depth(min(0.5, m.line.get_length())))
        for sym in syms:
            sym.add_updater(lambda m: m.next_to(
                m.line,
                m.direction,
    ...
    #
    def get_line_group(self, graph, input_tracker, nudge_tracker,
                       corner_index):
        ...
        def update_line_group(group):
            ...
            tl.scale(self.tangent_line_length / tl.get_length())
...
class DiscreteSetup(ShowEvolvingTempGraphWithArrows):
    CONFIG = {
        "step_size":
        1,
        "rod_piece_size_ratio":
        1 / 3,
        "dashed_line_stroke_opacity":
        1.0,
        "dot_radius":
        DEFAULT_DOT_RADIUS,
        "freq_amplitude_pairs": [
    ...
    def discretize(self):
        ...
        for x in self.get_sample_inputs():
            ...
            piece.set_width(self.step_size * self.rod_piece_size_ratio)
            piece.move_to(axes.c2p(x, 0))
            piece.set_color(self.rod_point_to_color(piece.get_center()))
    ...
    def let_time_pass(self):
        ...
        dots.add_updater(
            lambda d: d.become(self.get_dots(t_tracker.get_value())))
        run_time = 5
        self.play(
            ClockPassesTime(
                self.clock,
    ...
    def show_nieghbor_rule(self):
        ...
        kw = {"buff": 0.1, "stroke_width": 2, "tip_length": 0.15}
        arrows = VGroup(
        ...
        self.play(dot.scale, 0, dot.set_opacity, 0,
                  FadeInFrom(point_label, DOWN))
        self.play(FadeInFrom(neighbors_label, DOWN), *map(GrowArrow, arrows))
        self.wait()
        ...
        v_vect = always_redraw(lambda: Vector(
            get_v() * UP,
        ...
        ).shift(d2.get_center()).set_opacity(v_vect_fader.get_value(), ))
        d2.add_updater(lambda d, dt: d.shift(get_v() * dt * UP, ))
        ...
        self.wait(4)
        self.play(FadeOut(VGroup(point_label, neighbors_label, arrows)))
    ...
    def focus_on_three_points(self):
        ...
        x_labels, T_labels = [
            VGroup(*[TexMobject("{}_{}".format(s, i))
                     for i in [1, 2, 3]]).scale(0.8) for s in ("x", "T")
        ]
        ...
        for Tl, dot in zip(T_labels, dots):
            Tl.dot = dot
            Tl.add_updater(lambda m: m.next_to(m.dot, RIGHT, SMALL_BUFF))
        ...
        average_pointer.add_updater(lambda m: m.move_to(
            0.5 * (hl1.get_start() + hl3.get_start()), RIGHT))
        ...
        average_label = TexMobject("{T_1", "+", "T_3", "\\over", "2}")
        average_label.scale(0.5)
        average_label.add_updater(
            lambda m: m.next_to(average_pointer, LEFT, SMALL_BUFF))

        average_rect = SurroundingRectangle(average_label)
        average_rect.add_updater(lambda m: m.move_to(average_label))
        ...
        self.play(FadeIn(average_pointer), Restore(mini_T1), Restore(mini_T3),
                  FadeIn(average_label[1]), FadeIn(average_label[3:]),
        ...
        self.play(GrowFromPoint(hl2, hl2.get_end()))
        self.wait()

        # Show formula
        formula = TexMobject("\\left(", "{T_1", "+", "T_3", "\\over", "2}",
                             "-", "T_2", "\\right)")
        ...
        diff_value.add_updater(lambda m: m.set_value(
            y_axis.p2n(average_pointer.get_right()) - y_axis.p2n(d2.get_center(
            ))))
        diff_value.next_to(brace, DOWN)

        self.play(
            ReplacementTransform(average_label.deepcopy(),
                                 formula[1:1 + len(average_label)]),
            TransformFromCopy(T2, formula[-2]), FadeIn(formula[-3]),
            FadeIn(formula[-1]), FadeIn(formula[0]), GrowFromCenter(brace),
            FadeIn(diff_value))
        self.wait()
        ...
        # Show derivative
        lhs = TexMobject("{dT_2", "\\over", "dt}", "=", "\\alpha")
    ...
    def show_difference_formula(self):
        ...
        new_rhs = TexMobject("=", "{\\alpha", "\\over", "2}", "\\left(", "(",
                             "T_3", "-", "T_2", ")", "-", "(", "T_2", "-",
                             "T_1", ")", "\\right)")
        big_parens = VGroup(
        ...
        lil_brace1 = always_redraw(
            lambda: Brace(Line(d1.get_left(), d2.get_left()), **kw))
        lil_brace2 = always_redraw(
            lambda: Brace(Line(d2.get_left(), d3.get_left()), **kw))
        ...
        double_difference_words = TextMobject("Difference of differences")
        double_difference_words.next_to(double_difference_brace, DOWN)
    ...
    def write_second_difference(self):
        ...
        second_difference_word = TextMobject("``Second difference''")
        ...
        self.wait()
        self.play(Write(second_difference_word), )
        self.wait()
    ...
    def emphasize_final_expression(self):
        ...
        self.play(ShowCreationThenFadeAround(VGroup(new_lhs, new_rhs)))
        self.wait()
    ...
    def get_dots(self, time=0):
        ...
        dots = VGroup(
            *[Dot(point, radius=self.dot_radius) for point in points])
    ...
    def get_dot_dashed_line(self, dot, index, color=False):
        ...
        def get_line():
            ...
            if color:
                result.color_using_background_image("VerticalTempGradient")
            return result
...
class ShowFinitelyManyX(DiscreteSetup):
    def construct(self):
        ...
        axes = self.axes
        axes.fade(1)
        points = [axes.c2p(x, 0) for x in self.get_sample_inputs()[1:]]
        x_labels = VGroup(*[
            TexMobject("x_{}".format(i)).next_to(p, DOWN)
            for i, p in enumerate(points)
        ])

        self.play(LaggedStartMap(FadeInFromLarge, x_labels))
...
class TransitionToContinuousCase(DiscreteSetup):
    ...
    def show_temperature_difference(self):
        ...
        self.play(ShowCreation(VGroup(dx_line, dT_line)),
        ...
        self.add(line_group)
        self.play(dx_tracker.set_value, 0.01, run_time=5)
        self.wait()
        self.play(
            dx_tracker.set_value,
            0.3,
        ...
        ratio = TexMobject("{\\partial T", "\\over", "\\partial x}")
        ratio[0].match_style(dT_sym)
        ratio.to_edge(UP)

        self.play(
            ShowCreationThenFadeAround(dT_sym,
        ...
        self.wait()
        self.play(
            TransformFromCopy(dT_sym, ratio.get_part_by_tex("\\partial T")),
            TransformFromCopy(dx_sym, ratio.get_part_by_tex("\\partial x")),
            Write(ratio.get_part_by_tex("\\over")))
        self.play(
            ShowCreation(tan_line.copy().set_stroke(width=2), remover=True),
            FadeOut(to_zero),
        )
        ...
        v_line, lv_line, rv_line = v_lines = VGroup(
            *[self.get_v_line(x) for x in [x0, x0 - dx, x0 + dx]])
        ...
        self.play(
            FadeOut(v_lines[1:]),
            ApplyMethod(dx_tracker.set_value, 0.01, run_time=2),
        )
    ...
    def show_second_derivative(self):
        ...
        deriv_of_deriv = TexMobject("{\\partial", "\\left(", "{\\partial T",
        ...
        v_line.add_updater(
            lambda m: m.become(self.get_v_line(x_tracker.get_value())))
        for change in [-0.1, 0.1]:
            self.play(x_tracker.increment_value, change, run_time=3)

        # Write second deriv
        second_deriv = TexMobject("{\\partial^2 T", "\\over", "\\partial x^2}")
    ...
    def show_curvature_examples(self):
        ...
        self.play(*[TransformFromCopy(v_line, nl) for nl in neighbor_lines])
        self.add(neighbor_lines)
        self.play(x_tracker.set_value,
                  5,
                  run_time=5,
    ...
    def get_line_group(
        ...
        def update_group(group):
            ...
            p0, p2 = [
                graph.point_from_proportion(
                    inverse_interpolate(self.graph_x_min, self.graph_x_max, x))
                for x in [x, x + dx]
            ]
            p1 = np.array([p2[0], *p0[1:]])
            ...
            tanl.scale(self.tangent_line_length / tanl.get_length())
            dxs.match_width(dxl)
...
class ShowManyVLines(TransitionToContinuousCase):
    ...
    def add_v_lines(self):
        ...
        x_eq.add_updater(lambda m: m.arrange(RIGHT, buff=SMALL_BUFF))
        x_eq.add_updater(
            lambda m: m[1].set_value(axes.x_axis.p2n(x_pointer.get_top())))
        x_eq.add_updater(lambda m: m.next_to(
            x_pointer, DOWN, SMALL_BUFF, submobject_to_align=x_eq[0]))
...
class ShowNewtonsLawGraph(Scene):
    ...
    def show_graph(self):
        ...
        brace = always_redraw(
            lambda: Brace(brace_line, RIGHT, buff=SMALL_BUFF))
        ...
        self.play(
            ShowCreation(graph),
            UpdateFromFunc(water_dot, lambda m: m.move_to(graph.get_end())),
            run_time=10,
            rate_func=linear,
        )
        self.wait()
    ...
    def show_equation(self):
        ...
        self.play(
            ...
            Write(VGroup(*filter(lambda p: p not in delta_T_parts, equation))))
