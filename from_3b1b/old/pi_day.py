...
class CircleConstants(Scene):
    ...
    def setup_circle(self):
        ...
        self.play(ShowCreation(self.circle_arc), Write(self.pi_equals),
                  Write(self.decimal))
    ...
    def change_arc_length(self, new_angle):
        def decimal_position_update_func(decimal):
            ...
            label_radius = self.radial_label_func(max_radius, min_radius,
                                                  angle)
            label_center = label_radius * np.array(
        ...
        self.play(
            Rotate(self.radius,
                   new_angle - self.circle_arc.angle,
                   about_point=ORIGIN),
...
def get_circle_drawing_terms(radius=1, positioning_func=lambda m: m.center()):
    ...
    def reposition_decimal(decimal):
        ...
        interp_length = interpolate(decimal.get_width(), decimal.get_height(),
                                    alpha)
    ...
    changing_decimal = ChangingDecimal(decimal,
                                       lambda a: a * TAU,
...
class PiTauDebate(PiCreatureScene):
    def construct(self):
        ...
        terms, generate_anims1, generate_anims2 = get_circle_drawing_terms(
            radius=1.25,
            positioning_func=lambda m: m.to_edge(RIGHT, buff=2).to_edge(
                UP, buff=1))
        circle, radius, one, decimal = terms

        self.play(RemovePiCreatureBubble(pi), RemovePiCreatureBubble(tau),
                  *generate_anims1())
        ...
        decimal = DecimalNumber(np.pi,
                                num_decimal_places=4,
                                show_ellipsis=True)
        decimal.scale(0.75)
        decimal.next_to(brace, DOWN, SMALL_BUFF)

        self.play(FadeIn(VGroup(circle, diameter, one)), tau.change,
                  "confused", pi.change, "hooray")
...
class HartlAndPalais(Scene):
    def construct(self):
        ...
        self.play(
            Homotopy(
                lambda x, y, z, t: (x + 0.1 * np.sin(2 * TAU * t) * np.exp(
                    -10 * (t - 0.5 - 0.5 * (y - 1.85))**2), y, z),
...
class HistoryOfOurPeople(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("Today: The history \\\\ of our people.",
                          bubble_kwargs={
                              "width": 4,
                              "height": 3
                          })
...
class WhatIsRight(PiCreatureScene):
    ...
    def construct(self):
        ...
        literal_derivative = TexMobject("f'(x) = \\frac{f(x+dx) - f(x)}{dx}")
        limit_derivative = TexMobject(
            "f'(x) = \\lim_{h \\to 0}\\frac{f(x+h) - f(x)}{h}")

        divide_by_zero_definable = TexMobject("\\frac{1}{0}",
                                              "\\text{ has meaning}")
        divide_by_zero_undefinable = TexMobject("\\frac{1}{0}",
                                                "\\text{ is undefined}")

        left_mobs = VGroup(sum_over_N_converges, literal_derivative,
                           divide_by_zero_definable)
        right_mobs = VGroup(sum_over_N_diverges, limit_derivative,
        ...
        for left, right in zip(left_mobs, right_mobs)[:-1]:
            right.align_to(left)
            self.play(randy.change, "raise_right_hand", FadeInFromDown(left),
...
class Thumbnail(Scene):
    def construct(self):
        pi, eq, num = formula = TexMobject("\\pi", "=", "6.283185\\dots")
        formula.scale(2)
