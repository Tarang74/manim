        ...
        while continue_rolling_over:
            ...
            if run_all_at_once:
                ...
                def get_modified_rate_func(anim):
                    return lambda t: anim.original_rate_func(start_t + t /
                                                             denom)
            ...
            self.play(MoveToTarget(moving_dot),
                      *added_anims,
...
class TowersOfHanoiScene(Scene):
    ...
    def add_pegs(self):
        ...
        self.pegs = VGroup(*[
            peg.copy().shift(vect)
            for vect in (self.peg_spacing * LEFT, ORIGIN,
                         self.peg_spacing * RIGHT)
        ])
        self.add(self.pegs)
    ...
    def add_disks(self):
        self.disks = VGroup(*[
            Rectangle(
                ...
                np.linspace(self.disk_min_width, self.disk_max_width,
                            self.num_disks),
    ...
    def move_disks_to_peg(self,
        ...
        if stay_on_peg:
            ...
            self.play(ApplyMethod(disks.move_to, target_bottom_point, DOWN),
                      *added_anims,
                      path_arc=path_arc * 2,
                      run_time=run_time,
                      rate_func=lambda t: smooth(t, 2))
...
class IntroduceKeith(Scene):
    def construct(self):
        ...
        self.play(keith.change_mode, "hooray",
                  Transform(original_content, bubble.content))
        self.play(Blink(morty))
        self.wait()
        self.play(FadeIn(randy))
        self.play(randy.change_mode, "confused", randy.look_at, keith.eyes,
                  keith.change_mode, "plain", keith.look_at, randy.eyes,
                  morty.change_mode, "plain", morty.look_at, randy.eyes,
                  FadeOut(bubble), FadeOut(original_content),
                  ShowCreation(randy.bubble), Write(randy.bubble.content))
        self.play(Blink(keith))
...
class IntroduceTowersOfHanoi(TowersOfHanoiScene):
    ...
    def show_setup(self):
        ...
        self.play(ApplyMethod(self.pegs.restore, lag_ratio=0.5, run_time=2),
    ...
    def note_disk_labels(self):
        ...
        for label in labels:
            label.save_state()
            self.play(label.scale_in_place,
                      2,
                      label.set_color,
                      YELLOW,
                      last.restore,
    ...
    def cannot_move_disk_onto_smaller_disk(self):
        ...
        disk = self.disks[2]
        disk.save_state()
        self.move_disks_to_peg(
            [2],
            2,
            added_anims=[Transform(self.title, also_not_allowed, run_time=1)])
        cross.replace(disk)
...
class ExampleFirstMoves(TowersOfHanoiScene):
    def construct(self):
        ...
        self.wait()
        self.play(self.disks[0].set_fill, YELLOW,
                  self.disks[0].label.set_color, BLACK)
        self.wait()
        self.move_disk(0)
        self.wait()
        self.play(self.disks[1].set_fill, YELLOW_D,
...
class KeithShowingBinary(Scene):
    def construct(self):
        ...
        def get_bit_flip():
            return Transform(bits,
                             bits_list.pop(0),
                             rate_func=squish_rate_func(smooth, 0, 0.7))

        self.play(keith.change_mode, "wave_1", keith.look_at, bits,
                  morty.look_at, bits, Write(bits))
        for x in range(2):
            self.play(get_bit_flip())
        self.play(morty.change_mode, "pondering", morty.look_at, bits,
                  get_bit_flip())
        while bits_list:
            added_anims = []
        ...
        self.play(randy.change_mode, "confused", ShowCreation(randy.bubble),
...
class IntroduceBase10(Scene):
    ...
    def expand_example_number(self):
        ...
        expansion = TexMobject("1(100) + ", "3(10) + ", "7")
...
class RhythmOfDecimalCounting(CountingScene):
    ...
    def construct(self):
        ...
        self.wait(2)
        self.play(Transform(brace, one_brace),
                  Transform(two_digits, tens_place))
        self.wait()
        ten_group.save_state()
        self.play(
            ten_group.scale_in_place,
            7,
            ten_group.shift,
            2 * (DOWN + LEFT),
        )
        ...
        self.play(hundred_group.scale, 14, hundred_group.to_corner,
                  DOWN + LEFT)
        self.wait()
        ...
        self.play(groups[2].to_edge, RIGHT,
...
class DecimalCountingAtHundredsScale(CountingScene):
    CONFIG = {
        ...
        "counting_dot_starting_position":
        (FRAME_X_RADIUS + 1) * RIGHT + (FRAME_Y_RADIUS - 2) * UP,
        "ones_configuration_location":
        2 * UP + 5.7 * RIGHT,
        "num_start_location":
        DOWN + 3 * RIGHT
    ...
    def construct(self):
        ...
        added_zeros.next_to(self.number_mob,
                            RIGHT,
                            SMALL_BUFF,
                            aligned_edge=DOWN)
        ...
        Transform(
            ...
            Arrow(self.number_mob,
                  self.dot_templates[0],
                  color=self.power_colors[0])).update(1)
...
class IntroduceBinaryCounting(BinaryCountingScene):
    ...
    def initial_counting(self):
        ...
        self.play(randy.change_mode, "hesitant", randy.look_at,
                  self.number_mob, Write(bubble.content))
        self.wait()
        curr_content = bubble.content
        bubble.write("$1 \\! \\cdot \\! 2+$", "$0$")
        bubble.content[0][0].set_color(self.power_colors[1])
        self.play(Transform(curr_content, bubble.content), randy.change_mode,
                  "pondering", randy.look_at, self.number_mob)
        ...
        self.play(randy.change_mode, "happy", randy.look_at, self.number_mob,
...
class BinaryCountingAtEveryScale(Scene):
    ...
    def construct(self):
        title = TextMobject(
            "Count to %d (which is %s in binary)" %
        ...
        self.wait()
        self.play(get_run_through(bit_mobs[2**(self.num_bits - 1) + 1:]),
                  Write(again))
        self.wait()
...
class IntroduceSolveByCounting(TowersOfHanoiScene):
    ...
    def construct(self):
        ...
        self.play(last_bit.restore, disk.set_fill, disk.original_fill_color,
        ...
        self.play(disk.set_fill, RED, disk[1].set_fill, BLACK,
                  fours_bit.set_fill, RED)
        self.move_disk(2, run_time=2)
        self.play(disk.set_fill, disk.original_fill_color, disk[1].set_fill,
                  BLACK, fours_bit.restore, FadeOut(brace), FadeOut(words))
        self.wait()
        ...
        brace = braces[3]
        words = word_list[3]
        self.play(self.get_increment_animation(), GrowFromCenter(brace),
                  Write(words, run_time=1))
        disk = self.disks[3]
        eights_bit = self.curr_bit_mob[3]
        eights_bit.save_state()
        self.play(disk.set_fill, GREEN, disk[1].set_fill, BLACK,
    ...
    def get_increment_animation(self):
        return Succession(
            Transform(self.curr_bit_mob,
                      next(self.bit_mobs_iter),
                      lag_ratio=0.5,
                      path_arc=-np.pi / 3), Animation(self.curr_bit_mob))
...
class RecursionTime(Scene):
    def construct(self):
        ...
        self.play(Transform(keith, keith_kick), morty.change_mode, "happy",
                  ShowCreation(bubble), Write(bubble.content, run_time=1))
        self.play(morty.change_mode, "hooray", Transform(keith, keith_hooray),
...
class RecursiveSolution(TowersOfHanoiScene):
    ...
    def construct(self):
        ...
        steps = VGroup(title, sub_step_brace, sub_steps, sub_sub_steps_brace,
                       sub_sub_steps)
        steps.arrange(DOWN)
        steps.scale(0.7)
        steps.to_edge(UP)
        ...
        self.play(ShowCreation(arc), Write(q_mark), sub_steps[-1].set_color,
                  YELLOW)
        self.wait()
        self.play(GrowFromCenter(sub_sub_steps_brace),
                  *list(map(FadeOut, [arc, q_mark])))

        #Disk 2 frustration
        big_disk = self.disks[2]
        self.eyes.move_to(big_disk.get_top(), DOWN)
        self.play(FadeIn(self.eyes), big_disk.set_fill, RED,
        ...
        #Move sub-sub-tower
        self.move_subtower_to_peg(2,
                                  0,
                                  run_time=2,
                                  added_anims=[
                                      self.eyes.look_at_anim(
                                          self.pegs[0].get_bottom()),
                                      Write(sub_sub_steps[0])
                                  ])
        self.blink()
        self.move_disk_to_peg(2,
                              2,
                              run_time=2,
                              added_anims=[Write(sub_sub_steps[1])])
        self.look_at(self.disks[0])
        big_disk.remove(self.eyes)
        self.move_subtower_to_peg(2,
                                  2,
        ...
        self.move_subtower_to_peg(
            ...
            1,
            run_time=2,
            added_anims=[self.eyes.look_at_anim(self.pegs[0])])
...
class KeithSaysBigToSmall(Scene):
    def construct(self):
        ...
        self.add(keith)
        self.play(Blink(keith))
        self.play(keith.change_mode, "speaking", ShowCreation(bubble),
...
class NoRoomForInefficiency(Scene):
    def construct(self):
        ...
        self.add(morty)
        self.play(morty.change_mode, "speaking", ShowCreation(bubble),
                  Write(bubble.content))
        self.play(Blink(morty))
        self.wait()
...
class WhyDoesBinaryAchieveThis(Scene):
    def construct(self):
        ...
        self.play(morty.change_mode, "confused", morty.look_at, keith.eyes,
...
class BothAreSelfSimilar(Scene):
    def construct(self):
        ...
        self.add(morty)
        self.play(morty.change_mode, "hooray", ShowCreation(bubble),
                  Write(bubble.content))
        self.play(Blink(morty))
        self.wait()
...
class ShowFourDiskFourBitsParallel(IntroduceSolveByCounting):
    ...
    def counting_subtask(self):
        ...
        #     )
        self.play(Succession(
            *[self.get_increment_animation() for x in range(num_tasks)]),
                  rate_func=linear,
...
class CoolRight(Scene):
    def construct(self):
        ...
        self.play(morty.change_mode, "surprised", morty.look, OUT,
                  ShowCreation(bubble), Write(bubble.content))
        self.play(Blink(morty))
        self.wait()
        curr_content = bubble.content
...
class MentionLastVideo(Scene):
    def construct(self):
        ...
        self.add(morty, keith)
        self.play(keith.change_mode, "raise_right_hand", keith.look_at, point,
                  morty.change_mode, "pondering", morty.look_at, point)
        self.play(Blink(keith))
...
class RecursiveSolutionToConstrained(RecursiveSolution):
    ...
    def construct(self):
        ...
        steps_to_fade = VGroup(title, sub_steps_brace,
                               *list(sub_steps[:2]) + list(sub_steps[3:]))
        self.add(title)

        #Initially move big disk
        self.play(FadeIn(self.eyes), big_disk.set_fill, GREEN,
                  big_disk.label.set_fill, BLACK)
        big_disk.add(self.eyes)
        big_disk.save_state()
        self.blink()
        ...
        self.move_subtower_to_peg(self.num_disks - 1,
                                  ...
                                  added_anims=[
                                      FadeOut(blocking),
                                      FadeOut(arrow),
                                      self.eyes.change_mode_anim(
                                          "plain",
                                          thing_to_look_at=self.pegs[2]),
                                      Write(sub_steps[0], run_time=1),
                                  ])
        self.blink()

        #Proceed through actual process
        self.move_disk_to_peg(self.num_disks - 1,
                              1,
                              added_anims=[
        ...
        #Solve subproblem
        self.move_subtower_to_peg(num_disks - 1,
                                  2,
                                  added_anims=[
        ...
        self.move_disk_to_peg(
            num_disks - 1,
            1,
            added_anims=[Write(sub_sub_steps[1], run_time=1)])
        self.wait()
        self.move_subtower_to_peg(num_disks - 1,
                                  0,
                                  added_anims=[
        ...
        self.move_disk_to_peg(
            num_disks - 1,
            2,
            added_anims=[Write(sub_sub_steps[3], run_time=1)])
        self.wait()
        big_disk.remove(self.eyes)
        self.move_subtower_to_peg(num_disks - 1,
                                  2,
        ...
        self.play(
            steps.shift,
            (FRAME_Y_RADIUS - sub_sub_steps.get_top()[1] - MED_SMALL_BUFF) *
            UP, self.eyes.look_at_anim(steps))
        self.play(
            ApplyMethod(
        ...
        for mob in last_subdivisions:
            self.play(ApplyMethod(mob.set_fill, None, 1),
...
class SolveConstrainedByCounting(ConstrainedTowersOfHanoiScene):
    ...
    def construct(self):
        ...
        for index in get_ternary_ruler_sequence(self.num_disks - 1):
            self.move_disk(index,
                           stay_on_peg=False,
                           added_anims=[self.increment_animation()])
...
class CompareNumberSystems(Scene):
    def construct(self):
        ...
        self.play(Blink(morty))
        self.wait()
        self.play(morty.shrug, morty.look_at, keith.eyes, keith.shrug,
                  keith.look_at, morty.eyes)
        self.wait()
        self.play(morty.change_mode, "hesitant", morty.look_at, zero_one.name,
                  keith.change_mode, "erm", keith.look_at, zero_one.name)
        self.play(Blink(morty))
...
class TernaryCountingSelfSimilarPattern(Scene):
    ...
    def construct(self):
        ...
        def get_increment():
            return Transform(curr_ternary_mob,
        ...
        for i, step in enumerate(steps):
            ...
            if i % 2 == 0:
                self.play(Succession(*[
                    get_increment() for x in range(3**(self.num_trits - 1) - 1)
                ]),
                          run_time=1)
            else:
...
class TernaryCountingSelfSimilarPatternFiveTrits(
        TernaryCountingSelfSimilarPattern):
    CONFIG = {
        "num_trits": 5,
...
class SolveConstrainedWithTernaryCounting(ConstrainedTowersOfHanoiScene):
    ...
    def setup(self):
        ...
        ternary_mobs = VGroup(
            *[get_ternary_tex_mob(x) for x in range(3**self.num_disks)])
        ternary_mobs.scale(2)
        ternary_mobs.next_to(self.peg_labels, DOWN)
    ...
    def move_next_disk(self, run_time=None, stay_on_peg=False):
        self.move_disk(next(self.disk_index_iter),
                       run_time=run_time,
                       stay_on_peg=stay_on_peg)


class DescribeSolutionByCountingToConstrained(
        SolveConstrainedWithTernaryCounting):
    def construct(self):
        braces = [
        ...
        self.play(FadeIn(brace), Write(word, run_time=1),
        ...
        self.increment_number()
        self.play(Transform(brace, braces[1]), Transform(word, words[1]),
...
class AnswerConfigurationsCount(TowersOfHanoiScene):
    ...
    def construct(self):
        ...
        for parens, disk in zip(parentheticals, reversed(list(self.disks))):
            VGroup(parens, parens.brace,
                   parens.three).set_color(disk.get_color())
            self.play(Write(parens, run_time=1), FadeIn(disk))
            self.play(ApplyMethod(disk.next_to, self.pegs[2], UP, run_time=2),
        ...
        parentheticals = VGroup(*reversed([
            TexMobject("""
                \\left(
                    ...
                    \\end{array}
                \\right)
            """ % d) for d in range(self.num_disks)
        ]))
...
class RepeatingConfiguraiton(Scene):
    def construct(self):
        ...
        special_arrows = VGroup(*[
            Arrow(title.get_bottom(), dot, color=RED) for dot in special_dots
        ])
...
class SierpinskiGraphScene(Scene):
    ...
    def get_node_circles(self, order=3):
        ...
        for island, angle in (islands[0], np.pi / 6), (islands[2],
                                                       5 * np.pi / 6):
            island.rotate(np.pi,
...
class IntroduceGraphStructure(SierpinskiGraphScene):
    ...
    def introduce_nodes(self):
        ...
        for index in 3, 21, 8, 17, 10, 13:
            ...
            node.save_state()
            self.play(node.set_height, FRAME_HEIGHT - 2, node.next_to, ORIGIN,
                      vect)
            self.wait()
...
class DescribeTriforcePattern(SierpinskiGraphScene):
    ...
    def construct(self):
        ...
        bold_edges = [
            self.node_indices_to_edge(*pair).copy().set_stroke(
                self.disk_color, 6) for pair in self.index_pairs
        ]
        ...
        def wiggle_island(island):
            return ApplyMethod(island.rotate_in_place,
                               np.pi / 12,
                               run_time=1,
                               rate_func=wiggle)
...
class MentionFinalAnimation(Scene):
    def construct(self):
        ...
        self.add(morty)
        self.wait()
        self.play(morty.change_mode, "speaking", morty.look_at, bubble.content,
                  ShowCreation(bubble), Write(bubble.content))
        self.play(Blink(morty))
        self.wait(2)
        self.play(Blink(morty))
...
class PatreonThanks(Scene):
    ...
    def construct(self):
        ...
        left_patrons = VGroup(
            *list(map(TextMobject, self.specific_patrons[:n_patrons / 2])))
...
class ShowSierpinskiCurvesOfIncreasingOrder(Scene):
    ...
    def construct(self):
        graph_scenes = [
            SierpinskiGraphScene(num_disks=order,
