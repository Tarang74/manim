...
class CircleScene(PiCreatureScene):
    ...
    def setup(self):
        ...
        self.radius_line = Line(self.circle.get_center(),
        ...
        self.add(self.circle, self.radius_line, self.radius_brace,
                 self.radius_label)
    ...
    def increase_radius(self, numerical_dr=True, run_time=2):
        radius_mobs = VGroup(self.radius_line, self.radius_brace,
                             self.radius_label)
        nudge_line = Line(self.radius_line.get_right(),
    ...
    def get_outer_ring(self):
        return self.get_ring(radius=self.radius,
                             dR=self.dR,
    ...
    def unwrap_rings(self, *rings, **kwargs):
        ...
        rings = VGroup(*rings)
        unwrapped = VGroup(
            *[self.get_unwrapped(ring, **kwargs) for ring in rings])
        self.play(rings.rotate,
                  np.pi / 2,
                  rings.next_to,
    ...
    def get_unwrapped(self, ring, to_edge=LEFT, **kwargs):
        ...
        result.set_points_as_corners([
            ...
            interpolate(np.pi * R * RIGHT + ring.dR * UP, np.pi * R * LEFT +
                        ring.dR * UP, a)
            for a in np.linspace(0, 1, n_anchors / 2)
        ])
...
class PatronsOnly(Scene):
    def construct(self):
        ...
        self.add(morty)
        self.play(Write(title), morty.change_mode, "wave_1")
        self.play(Blink(morty))
        self.play(morty.change_mode, "pondering", morty.look_at, title)
        self.play(Blink(morty))
        self.wait()
...
class Introduction(TeacherStudentsScene):
    ...
    def show_series(self):
        ...
        words = TextMobject("Welcome to \\\\", "Essence of calculus")
        ...
        self.play(
            ApplyMethod(this_video.restore, run_time=3), *[
                ApplyFunction(
                    lambda p: p.change_mode("hooray").look_at(series[1]), pi)
        ...
        self.play(Homotopy(
            ...
            apply_function_kwargs={"maintain_smoothness": False},
                  ...
                  *[
                  ...
                  run_time=5)
        self.play(
            FadeOut(self.teacher.bubble), FadeOut(self.teacher.bubble.content),
            *[
    ...
    def look_to_center(self):
        anims = []
        for pi in self.get_pi_creatures():
            anims += [pi.change_mode, "pondering", pi.look_at, 2 * UP]
        self.play(*anims)
        self.random_blink(6)
        self.play(*[
            ApplyMethod(pi.change_mode, "happy")
    ...
    def go_through_students(self):
        ...
        self.play(
            ApplyMethod(remembered_symbols.fade,
                        0.7,
                        lag_ratio=0.5,
                        run_time=3))
        self.play(
        ...
        self.play(Blink(pi2))
        self.play(pi2.restore, pi2.fade,
                  pi3.restore, pi3.change_mode, "pleading", pi3.look_at,
                  bubble.get_right(), bubble.pin_to, pi3, FadeOut(cant_wait))
    ...
    def zoom_in_on_first(self):
        ...
        self.play(FadeOut(everything), MoveToTarget(this_video, run_time=2))
...
class IntroduceCircle(Scene):
    def construct(self):
        ...
        area, circumference = words = VGroup(
            *list(map(TextMobject, ["Area =", "Circumference ="])))
        area.set_color(BLUE)
        ...
        derivative = TexMobject("\\frac{d}{dR}", "\\pi R^2", "=", "2\\pi R")
        integral = TexMobject("\\int_0^R", "2\\pi r", "\\, dR = ", "\\pi R^2")
        ...
        self.play(Rotate(radius, 2 * np.pi, about_point=circle.get_center()),
                  ShowCreation(circle))
        self.play(
            FadeIn(area),
            Write(area_form),
        ...
        self.play(
            circle.set_stroke,
            circumference.get_color(),
            FadeIn(circumference),
            Animation(radius),
            Animation(R),
        )
        self.play(
            Transform(area_form.copy(),
                      circum_form,
                      path_arc=-np.pi / 2,
                      run_time=3))
        self.wait()
        self.play(area_form.copy().replace,
                  derivative[1],
                  circum_form.copy().replace,
                  derivative[3],
                  Write(derivative[0]),
                  Write(derivative[2]),
                  run_time=1)
        ...
        self.wait()
        self.play(GrowFromCenter(brace), Write(to_be_explained))
        self.wait()
...
class HeartOfCalculus(GraphScene):
    ...
    def construct(self):
        ...
        rects.shift((self.graph_origin[1] - rects.get_center()[1]) * UP)
        self.play(rects.restore, lag_ratio=0.5, run_time=3)
        while rect_sets:
            self.play(Transform(rects, rect_sets.pop(0)), run_time=2)
...
class PragmatismToArt(Scene):
    def construct(self):
        ...
        circle2 = Circle(radius=2, stroke_color=YELLOW)
        ...
        self.play(Blink(morty))
        self.play(
            morty.change_mode, "raise_left_hand", morty.look_at, art,
            Transform(VectorizedPoint(morty.get_corner(UP + RIGHT)),
                      art), pragmatism.fade, 0.7, pragmatism.rotate_in_place,
            np.pi / 4, pragmatism.shift, DOWN + LEFT)
        self.play(Blink(morty))
        self.play(GrowFromCenter(circle1), morty.look_at, circle1)
        self.play(ShowCreation(circle2))
        self.play(ShowCreation(arrow), Write(q_marks), circle2.restore)
        self.play(Blink(morty))
...
class IntroduceTinyChangeInArea(CircleScene):
    ...
    def construct(self):
        ...
        group = VGroup(expression, expression_brace, change_in_area,
        ...
        self.play(Write(new_area_word), GrowFromCenter(new_area_brace))
        self.wait(2)
        self.play(group.fade, 0.7, self.pi_creature.change_mode, "happy")
        self.wait()
        self.play(outer_ring.set_color,
                  YELLOW,
        ...
        self.play(FadeIn(big_rect), area_form.set_color, BLUE,
                  self.two_pi_R.set_color, GREEN, self.pi_creature.change_mode,
                  "happy")
    ...
    def show_unwrapping(self, outer_ring):
        ...
        dR.generate_target()
        lp, rp = TexMobject("()")
        change_in_area = TextMobject("Change in area $\\approx$")
        final_area = VGroup(change_in_area, two_pi_R.target, lp,
                            dR.target.scale(1. / 0.7), rp)
        ...
        self.play(GrowFromCenter(circum_brace), Write(two_pi_R))
        self.wait()
        self.play(GrowFromCenter(dR_brace), Write(dR))
        self.wait()
        self.play(
        ...
        self.wait()
        group = VGroup(almost_rect, final_area, two_pi_R, dR, circum_brace,
                       dR_brace)
    ...
    def work_out_expression(self, expression_group):
        ...
        expanded.next_to(new_area_form,
                         DOWN,
                         aligned_edge=LEFT,
                         buff=MED_SMALL_BUFF)
        ...
        for subset, target, writer in trips:
            starter = VGroup(*np.array(list(new_area_form.copy()))[subset])
            self.play(Transform(starter, target, run_time=2), Write(writer))
        ...
        self.play(GrowFromCenter(approx_brace), self.pi_creature.change_mode,
                  "hooray")
        self.wait()
        self.play(GrowFromCenter(error_brace), Write(error_words),
    ...
    def second_unwrapping(self, outer_ring):
        ...
        self.play(almost_rect.set_color, YELLOW, self.pi_creature.change_mode,
                  "pondering")
...
class BuildToDADR(CircleScene):
    ...
    def write_initial_terms(self):
        ...
        self.wait()
        self.play(*it.chain(list(map(Write, [equals, two_pi_R, dR])),
                            list(map(FadeIn, [circum_text, circum_brace]))))
        self.wait()
        self.play(*it.chain(list(map(Write, [plus, pi, dR2, squared])),
    ...
    def show_fractions(self):
        ...
        self.play(*it.chain([Write(term.frac_line) for term in terms],
                            list(map(MoveToTarget, [self.equals, self.plus]))))
        self.play(*[term.denom.restore for term in terms])
        self.wait(2)
        self.play(self.outer_ring.set_color, YELLOW, rate_func=there_and_back)
        self.play(self.nudge_label.scale_in_place, 2, rate_func=there_and_back)
        self.wait(2)
        ...
        self.play(
            ...
            MaintainPositionRelativeTo(self.circum_term.label,
                                       self.circum_term[0]))
        self.circum_term = self.circum_term[0]
        self.wait(2)
        self.play(FadeOut(self.error_term[-1]), FadeOut(self.error_term.denom))
        ...
        self.play(
            ...
            FadeOut(self.error_term.frac_line), self.plus.shift,
            0.7 * LEFT + 0.1 * UP,
            MaintainPositionRelativeTo(self.error_term.label, self.error_term))
        self.wait()
    ...
    def transition_to_dR(self):
        ...
        for s, width in ("(0.01)", 0.05), ("(0.001)", 0.03), ("dR", 0.03):
            new_dRs = VGroup(
                *[TexMobject(s).move_to(mob, LEFT) for mob in dRs])
        ...
        difference_text = TextMobject("``Tiny ",
                                      "d",
                                      "ifference in ",
                                      "$R$",
                                      "''",
        ...
        self.play(Write(difference_text, run_time=2),
                  ShowCreation(difference_arrow), self.pi_creature.change_mode,
                  "speaking")
        self.wait()
        ...
        self.change.denom.target.next_to(frac_line.target, DOWN,
                                         2 * SMALL_BUFF)
    ...
    def elaborate_on_d(self):
        ...
        arc.set_height(self.change.get_center()[1] -
                       self.change.denom.get_center()[1])
        arc.next_to(self.change.frac_line, LEFT)
        arc.add_tip()

        self.play(ShowCreation(arc), self.pi_creature.change_mode, "sassy")
        self.wait()
        ...
        d_group.arrange()
        d_group.next_to(self.pi_creature.get_corner(UP + RIGHT),
                        buff=LARGE_BUFF)
        error_group = VGroup(self.plus, self.error_term, self.error_term.label)

        self.play(Write(d), self.pi_creature.change_mode, "speaking")
        ...
        self.play(GrowFromCenter(equality_brace), Write(equal_word,
                                                        run_time=1))
        self.wait(2)
        ...
        new_nudge_arrow = Arrow(self.nudge_label,
                                big_nudge_line,
                                buff=SMALL_BUFF)
        ...
        self.less_wrong_philosophy = VGroup(d, arrow, ignore_error)
    ...
    def not_infinitely_small(self):
        ...
        self.play(randy.change_mode, "speaking", ShowCreation(bubble),
                  Write(bubble.content), self.pi_creature.change_mode,
                  "confused")
        self.wait()
        ...
        for mob in to_infs:
            ...
            mob.inf.move_to(mob)
        self.play(*[Transform(mob, mob.inf) for mob in to_infs])
        self.wait()
        ...
        for n in range(7):
            ...
            self.play(Transform(self.nudge_label, target),
        ...
        self.play(FadeOut(bubble), FadeOut(bubble.content),
                  ShowCreation(new_bubble), Write(new_bubble.content),
                  randy.change_mode, "erm", randy.set_color, BLUE_E,
...
class NameDerivative(IntroduceTinyChangeInArea):
    ...
    def name_derivative_for_cricle(self):
        ...
        words = TextMobject("``Derivative'' of $A$\\\\", "with respect to $R$")
        ...
        self.play(Transform(self.nudge_line.copy(), dR, run_time=2),
                  Write(frac_line))
        self.wait()
        self.play(ShowCreation(dA_dR_circle), ShowCreation(arrow),
                  Write(words))
        self.wait()
    ...
    def interpret_geometrically(self):
        target_formula = TexMobject("\\frac{d \\quad}{dR} = ")
        ...
        self.play(
            Write(target_formula), MoveToTarget(area_circle),
            MoveToTarget(circum_circle,
                         run_time=2,
    ...
    def show_limiting_process(self):
        ...
        for frac in fracs[:len(sample_dRs)] + [fracs[-1]]:
            ...
            if frac is fracs[-1]:
                ...
                arrow.add(cross)
            arrows.add(arrow)
        self.play(Transform(ring_group, big_group),
    ...
    def emphasize_equality(self):
        ...
        self.play(equals.scale_in_place,
                  1.5,
                  equals.set_color,
                  GREEN,
                  rate_func=there_and_back,
                  run_time=2)
        self.play(self.two_pi_R.set_stroke,
                  YELLOW,
                  3,
                  rate_func=there_and_back,
                  run_time=2)
        ...
        new_words = TextMobject("Systematically\\\\", "ignore error")
...
class DerivativeAsTangentLine(ZoomedScene):
    ...
    def setup_axes(self):
        ...
        graph_label.next_to(graph.point_from_proportion(2), LEFT)
    ...
    def show_zoomed_in_steps(self):
        ...
        self.play(
            *list(map(FadeIn, [self.little_rectangle, self.big_rectangle])))
        self.play(self.little_rectangle.move_to,
        ...
        deriv_is_slope = TexMobject("\\frac{dA}{dR} =", "\\text{Slope}")
    ...
    def show_tangent_lines(self):
        ...
        def line_update_func(line):
            ...
            line.rotate(self.angle_of_tangent(R) - line.get_angle())
        ...
        self.last_alpha = alphas[np.argmin(
            [get_norm(point - curr_graph_point) for point in graph_points])]

        def shift_everything_to_alpha(alpha, run_time=3):
            self.play(MoveAlongPath(self.dot,
                                    self.graph,
    ...
    def note_R_value_of_point(self):
        ...
        arrow = Arrow(self.x_axis_label.get_left(), dot, buff=SMALL_BUFF)
    ...
    def state_commonality(self):
        ...
        self.play(bubble.stretch_to_fit_width, 5, bubble.shift, RIGHT,
                  Transform(bubble.content, new_words), morty.change_mode,
                  "hesitant")
        self.play(Blink(morty))
        self.wait()
...
class IntroduceConcentricRings(CircleScene):
    ...
    def build_up_rings(self):
        ...
        dr_line = Line(rings[-2].get_top(), rings[-1].get_top(), color=YELLOW)
        ...
        foreground_group = VGroup(self.radius_brace, self.radius_label,
                                  self.radius_line)
        self.play(FadeIn(outermost_ring), Animation(foreground_group))
        self.play(Write(dr_text), ShowCreation(dr_arrow),
                  ShowCreation(dr_line))
        foreground_group.add(dr_line, dr_arrow, dr_text)
        self.change_mode("pondering")
        self.wait()
        self.play(FadeIn(VGroup(*rings[:-1]), lag_ratio=1, run_time=5),
    ...
    def add_up_areas(self):
        ...
        area_sum = VGroup(*it.chain(
            *list(zip([ring for ring in moving_rings.target], plusses))))
        ...
        self.play(
            ...
            Write(VGroup(plusses, dots_equals_area),
                  rate_func=squish_rate_func(smooth, 0.5, 1)),
    ...
    def unwrap_special_ring(self):
        ...
        self.play(MoveToTarget(rings), Animation(foreground_group))
        ...
        self.play(GrowFromCenter(brace), Write(two_pi_r))
        ...
        self.play(Write(think_concrete), ShowCreation(arrow),
        ...
        self.to_fade = VGroup(brace, left_brace, two_pi_r, dr, think_concrete,
                              arrow, less_wrong)
    ...
    def write_integral(self):
        ...
        integral_expression = VGroup(def_int, self.two_pi_r.target,
        ...
        self.play(GrowFromCenter(brace), Write(formula_q),
        ...
        self.play(MoveToTarget(self.rings, rate_func=there_and_back),
                  Animation(self.foreground_group))
        self.wait()
        self.grow_and_shrink_r_line(zero, R)
        self.wait()
        self.play(MoveToTarget(self.two_pi_r),
        ...
        self.integral_terms = VGroup(self.integral_expression[1],
    ...
    def grow_and_shrink_r_line(self, zero_target, R_target):
        ...
        self.radial_line.target.scale_in_place(0.1 /
                                               self.radial_line.get_length())
        ...
        self.play(MoveToTarget(self.radial_line), MoveToTarget(self.r_label),
        ...
        self.wait()
        self.radial_line.target.scale_in_place(self.radius /
                                               self.radial_line.get_length())
        equals_0.target = TexMobject("=R")
        equals_0.target.next_to(self.radial_line.target.get_center_of_mass(),
                                UP + LEFT,
                                buff=SMALL_BUFF)
        ...
        self.add(R_target)
        self.wait()
        self.play(self.radial_line.restore, self.r_label.restore,
                  FadeOut(equals_0))
    ...
    def ask_about_approx(self):
        ...
        question = TextMobject("Should this be\\\\", "an approximation?")
        ...
        self.play(Transform(self.equals, approx), Write(question),
                  ShowCreation(arrow), self.pi_creature.change_mode,
                  "confused")
        self.wait(2)
        self.play(*[
            ...
            for ring in self.rings
             ...
             Animation(self.foreground_group)])
        self.wait()
        self.play(Transform(question, approach_words),
                  Transform(arrow, Arrow(approach_words, approx)),
                  self.equals.restore, self.pi_creature.change_mode, "happy")
        self.wait(2)
        self.play(self.integral_expression.set_color_by_gradient, BLUE, GREEN,
                  GrowFromCenter(int_brace), Write(integral_word))
        self.wait()
...
class AskAboutGeneralCircles(TeacherStudentsScene):
    def construct(self):
        ...
        self.random_blink(2)
        self.teacher_says("All in due time", )
        self.change_student_modes(*["happy"] * 3)
        self.random_blink(2)
...
class GraphIntegral(GraphScene):
    ...
    def show_horizontal_axis(self):
        ...
        self.play(ShowCreation(moving_arrow), self.little_r.set_color, YELLOW)
        for arrow in reversed(arrows):
            self.play(Transform(moving_arrow, arrow, run_time=4))
        self.play(FadeOut(moving_arrow), self.little_r.set_color, WHITE)
    ...
    def add_rectangles(self):
        ...
        arrows = VGroup(*[
            Arrow(
                values_words.get_bottom(), tick.get_center(), tip_length=0.15)
            for tick in ticks
        ])

        dr_brace = Brace(VGroup(*ticks[special_tick_index:special_tick_index +
                                       2]),
                         buff=SMALL_BUFF)
        ...
        self.play(ShowCreation(ticks, lag_ratio=0.5), Write(R_label))
        self.play(Write(values_words), ShowCreation(arrows))
        self.wait()
        self.play(GrowFromCenter(dr_brace), Write(dr_text))
        self.wait()
        ...
        self.play(rectangles.restore, Animation(ticks), run_time=2)
        self.wait()
        self.play(*[
            ApplyMethod(rect.fade, 0.7)
            for rect in rectangles if rect is not special_rect
        ] + [Animation(ticks)])
        self.play(GrowFromCenter(left_brace), Write(height_label))
        self.wait()

        graph = self.graph_function(lambda r: 2 * np.pi * r, animate=False)
        graph_label = self.label_graph(self.graph,
                                       "f(r) = 2\\pi r",
                                       proportion=0.5,
                                       direction=LEFT,
                                       animate=False)
        self.play(rectangles.restore, Animation(ticks), FadeOut(left_brace),
                  Transform(height_label, graph_label), ShowCreation(graph))
        self.wait(3)
    ...
    def thinner_rectangles(self):
        for x in range(2, 8):
            new_rects = self.get_rectangles(dr=self.dr / x,
                                            stroke_width=1. / x)
    ...
    def ask_about_area(self):
        ...
        arrow = Arrow(question.get_bottom(), self.rectangles, buff=SMALL_BUFF)
        self.play(Write(question), ShowCreation(arrow))
        self.wait()

    def get_rectangles(self, dr, stroke_width=1):
        return self.get_riemann_rectangles(0,
                                           self.R,
                                           dr,
...
class FundamentalTheorem(CircleScene):
    ...
    def setup(self):
        ...
        group = VGroup(self.circle, self.radius_line, self.radius_brace,
                       self.radius_label)
        self.remove(*group)
        group.shift(DOWN)
    ...
    def add_derivative_terms(self):
        symbolic = TexMobject("\\frac{d(\\pi R^2)}{dR} =", "2\\pi R")
        ...
        area_circle = Circle(stroke_width=0,
                             fill_color=self.area_color,
                             fill_opacity=0.5,
                             radius=radius)
        area_circle.next_to(geometric[0], buff=SMALL_BUFF)
        circum_circle = Circle(color=self.circum_color, radius=radius)
        ...
        self.derivative_terms.arrange(DOWN, buff=LARGE_BUFF, aligned_edge=LEFT)
        self.derivative_terms.next_to(ORIGIN, LEFT, buff=LARGE_BUFF)

        self.play(Write(self.derivative_terms), self.pi_creature.change_mode,
                  "hooray")
        self.wait()

    def add_integral_terms(self):
        symbolic = TexMobject("\\int_0^R", "2\\pi r", "\\cdot", "dr", "=",
                              "\\pi R^2")
        ...
        area_circle = Circle(radius=geometric[-1].get_width() / 2,
                             stroke_width=0,
                             fill_color=self.area_color,
                             fill_opacity=0.5)
        area_circle.move_to(geometric[-1])
        circum_circle = Circle(radius=geometric[1].get_width() / 2,
        ...
        self.integral_terms.arrange(DOWN, buff=LARGE_BUFF, aligned_edge=LEFT)
    ...
    def bring_in_circle(self):
        self.play(FadeOut(self.derivative_terms[0]),
                  FadeOut(self.integral_terms[0]),
                  self.derivative_terms[1].to_corner, UP + LEFT,
                  MED_LARGE_BUFF, self.integral_terms[1].to_corner, UP + RIGHT,
                  MED_LARGE_BUFF, self.pi_creature.change_mode, "speaking")
        self.introduce_circle()
    ...
    def show_outer_ring(self):
        ...
        self.play(GrowFromCenter(brace), Write(deriv),
                  self.pi_creature.change_mode, "happy")
    ...
    def show_all_rings(self):
        ...
        self.play(
            ApplyMethod(rings.restore, lag_ratio=0.5, run_time=5),
            Animation(self.foreground_group),
        )
    ...
    def emphasize_oposites(self):
        ...
        self.play(ShowCreation(arrow), Write(opposites))
        self.wait()


class NameTheFundamentalTheorem(TeacherStudentsScene):
    def construct(self):
        symbols = TexMobject("\\frac{d}{dx} \\int_0^x f(t)dt = f(x)", )
        symbols.to_corner(UP + LEFT)
        brace = Brace(symbols)
        abstract = brace.get_text("Abstract version")
        self.add(symbols)
        self.play(
            GrowFromCenter(brace), Write(abstract), *[
...
class CalculusInANutshell(CircleScene):
    ...
    def morph_word(self):
        ...
        self.wait()
        self.play(Transform(calculus, rings, lag_ratio=0.5, run_time=5))
        self.wait()
    ...
    def show_remainder_of_series(self):
        ...
        self.play(MoveToTarget(everything), first.restore, run_time=2)
        self.play(FadeIn(
            VGroup(*series[1:]),
            lag_ratio=0.5,
            run_time=2,
        ...
        self.wait()
        self.play(GrowFromCenter(brace), Write(derivatives))
        self.wait()


class Thumbnail(CircleScene):
    CONFIG = {"radius": 2, "circle_corner": ORIGIN}
    ...
    def construct(self):
        ...
        circum_circle = Circle(color=YELLOW)
