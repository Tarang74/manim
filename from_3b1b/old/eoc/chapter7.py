...
class ThisVideo(TeacherStudentsScene):
    def construct(self):
        ...
        for index in 0, 2:
            ...
            self.play(GrowFromCenter(brace), Write(tex_mob, run_time=2))
        self.play(this_video.set_color, YELLOW, GrowFromCenter(this_brace),
...
class LimitJustMeansApproach(PiCreatureScene):
    ...
    def construct(self):
        ...
        expression = VGroup(limit_expression[1].copy(), point, point.copy())
        ...
        for next_expression in evaluated_expressions:
            ...
            if brace not in self.get_mobjects():
                self.play(GrowFromCenter(brace), Write(question))
            self.wait(0.5)
        self.wait(2)
    ...
    def get_expression(self, dx):
        result = TexMobject("{(2 + ", str(dx), ")^3 - 2^3 \\over", str(dx))
...
class Goals(Scene):
    def construct(self):
        goals = [
            TextMobject("Goal %d:" % d, s) for d, s in [
...
class RefreshOnDerivativeDefinition(GraphScene):
    ...
    def construct(self):
        ...
        start_x_v_line, nudged_x_v_line = [
            self.get_vertical_line_to_graph(self.start_x + nudge,
                                            graph,
        ...
        derivative = TexMobject("{df", "\\over \\,", "dx}", "(",
                                str(self.start_x), ")")
        ...
        self.play(input_copy.next_to, self.coords_to_point(self.start_x, 0),
                  DOWN)
        ...
        self.wait(2)
        self.play(ReplacementTransform(ss_group.dx_label.copy(), dx),
        ...
        new_deriv = TexMobject("{f", "(", str(self.start_x), "+", "dx", ")",
                               "-", "f", "(", str(self.start_x), ")",
                               "\\over \\,", "dx")
        ...
        self.play(*[
            ...
            Transform(deriv_brace, new_brace), dx_to_0.next_to, new_brace, DOWN
        ])
        ...
        self.play(ReplacementTransform(deriv_brace, lim),
...
class RantOpenAndClose(Scene):
    def construct(self):
        opening, closing = [
            TextMobject(start,
                        "Rant on infinitesimals",
                        "$>$",
                        arg_separator="") for start in ("$<$", "$<$/")
        ]
...
class DiscussLowercaseDs(RefreshOnDerivativeDefinition, PiCreatureScene,
                         ZoomedScene):
    CONFIG = {"zoomed_canvas_corner": UP + LEFT}
    ...
    def skip_superclass_anims(self):
        ...
        self.play(*[ApplyMethod(mob.shift, 2.5 * LEFT)
                    for mob in everything] + [FadeIn(self.pi_creature)])
    ...
    def replace_dx_terms(self):
        ...
        for tex in "\\Delta x", "h":
            dx_list_replacement = [
                TexMobject(tex).set_color(self.dx_color).move_to(dx, DOWN)
                for dx in dx_list
            ]
            self.play(
                Transform(
    ...
    def compare_rhs_and_lhs(self):
        ...
        lhs_rect, rhs_rect = rects = [
            Rectangle(color=GREEN_B).replace(mob, stretch=True)
        ...
        self.play(ShowCreation(d_circles, run_time=2),
        ...
        self.wait(2)
        self.play(ReplacementTransform(lhs_rect, rhs_rect),
                  self.pi_creature.change_mode, "raise_right_hand")
        self.wait(2)
        self.play(
            ReplacementTransform(df.copy(),
                                 df_target,
                                 path_arc=-np.pi / 2,
                                 run_time=2))
        self.wait(2)
        self.play(Indicate(VGroup(*rhs[:2]), run_time=2))
        self.wait()
        ...
        self.definition_group = VGroup(lhs, equals, rhs_rect, rhs,
                                       formal_definition_words)
        self.lhs, self.rhs, self.rhs_rect = lhs, rhs, rhs_rect

    def h_is_finite(self):
        self.play(FadeOut(self.graph_label), self.definition_group.center,
        ...
        h_group = VGroup(self.rhs[1].get_part_by_tex("dx"),
        ...
        self.play(moving_h.move_to, moving_h.original_center,
...
class OtherViewsOfDx(TeacherStudentsScene):
    def construct(self):
        definition = TexMobject("{df", "\\over \\,", "dx}", "(", "2", ")", "=",
                                "\\lim", "_{h", "\\to", "0}", "{f", "(", "2",
                                "+", "h", ")", "-", "f", "(", "2", ")",
                                "\\over \\,", "h}")
        tex_to_color = {
        ...
        for statement in statements:
            statement.h, statement.dx = [
                VGroup(*statement.get_parts_by_tex(
                    tex, substring=False)).set_color(GREEN)
                for tex in ("$h$", "$dx$")
            ]

        #Question
        self.student_says(statements[0],
                          student_index=1,
        ...
        dx_movement = Transform(moving_dx, statements[2].dx, run_time=2)
        ...
        bubble_intro.update(1)
        dx_movement = Transform(moving_dx, statements[3].dx, run_time=2)
        bubble_intro.update(0)
        last_bubble = self.get_students()[2].bubble
        self.play(
            bubble_intro, dx_movement, FadeOut(last_bubble),
            FadeOut(last_bubble.content),
            *it.chain(*[[
                pi.change_mode, "pondering", pi.look_at, bubble_intro.mobject
...
class GoalsListed(Scene):
    def construct(self):
        ...
        goals.arrange(DOWN, buff=LARGE_BUFF, aligned_edge=LEFT)
...
class GraphLimitExpression(GraphScene):
    ...
    def introduce_function(self):
        ...
        derivative = TexMobject("{d(x^3)", "\\over \\,", "dx}", "(", "2", ")")
        tex_to_color = {
            "h": self.h_color,
            "dx": self.h_color,
            "2": self.two_color
        ...
        indices = [0, 6, 11, 13]
        funcs = [lambda h: (2 + h)**3, lambda h: (2 + h)**3 - 2**3, self.func]
        graph = None
        for i, j, func in zip(indices, indices[1:], funcs):
            ...
            self.wait()
        self.wait()
        self.play(MoveToTarget(expression), FadeIn(limit, lag_ratio=0.5),
                  GrowFromCenter(brace))
        self.play(Write(derivative))
        self.wait(2)
        self.play(expression.restore,
    ...
    def emphasize_non_definedness_at_0(self):
        ...
        self.wait()
        self.play(ReplacementTransform(expression.copy(), new_expression))
        self.wait()
        self.play(Write(rhs))
        self.wait()
        self.play(GrowFromCenter(ud_brace), Write(undefined))
        self.wait(2)

        self.point_to_zero_group = VGroup(h_equals_0, arrow, dot)
        self.plug_in_zero_group = VGroup(new_expression, rhs, ud_brace,
                                         undefined)
    ...
    def draw_limit_point_hole(self):
        ...
        self.play(ShowCreation(circle.copy().set_fill(opacity=0),
                               remover=True))
        self.remove(colored_circle)
        self.play(circle.scale_in_place, 0.3, run_time=2, rate_func=wiggle)
        self.wait()
    ...
    def show_limit(self):
        ...
        self.wait()
        self.play(
            ReplacementTransform(ed_group.input_range.copy(),
        ...
        for v_line, h_line in (right_v_line, top_h_line), (left_v_line,
                                                           bottom_h_line):
            self.play(ShowCreation(v_line), ShowCreation(h_line))
            self.wait()
            self.play(v_line.move_to,
                      ...
                      run_time=3)
            self.play(
                VGroup(h_line, v_line).set_stroke,
                GREY,
                2,
            )
        self.wait()
    ...
    def show_epsilon_delta_intuition(self):
        self.play(FadeOut(self.ed_group.epsilon_lines),
                  FadeIn(self.input_range_brace_group))
        self.ed_group.epsilon_lines.restore()
        self.wait()
        self.play(self.ed_group.delta_lines.restore,
                  Animation(self.input_range_brace_group),
                  run_time=2)
        self.play(FadeOut(self.input_range_brace_group))
        self.play(
            ReplacementTransform(self.ed_group.input_range.copy(),
        ...
        self.animate_epsilon_delta_group_change(
            ...
            added_anims=[
                ApplyMethod(hole.scale_in_place, 0.5, run_time=8)
                for hole in holes
            ])
        self.wait()
    ...
    def get_epsilon_delta_group(
        ...
        dashed_line = DashedLine(ORIGIN,
                                 dashed_line_length * RIGHT,
        ...
        result.delta_lines = VGroup(*[
            dashed_line.copy().rotate(np.pi / 2).move_to(point, DOWN)
            for point in x_axis_points
        ])
        if self.func(limit_x) < 0:
            result.delta_lines.rotate(
                np.pi, RIGHT, about_point=result.delta_lines.get_bottom())
    ...
    def animate_epsilon_delta_group_change(self, epsilon_delta_group,
                                           target_delta, **kwargs):
        ...
        def update_ed_group(ed_group, alpha):
            ...
            new_group = self.get_epsilon_delta_group(delta,
                                                     limit_x=limit_x,
                                                     **ed_group_kwargs)
        ...
        self.play(
            UpdateFromAlphaFunc(epsilon_delta_group, update_ed_group,
                                **kwargs), *added_anims)
...
class LimitCounterExample(GraphLimitExpression):
    ...
    def draw_graph(self):
        ...
        graph = VGroup(left_graph, left_hole, right_hole, right_graph)

        self.play(ShowCreation(graph, run_time=5))
        self.wait()
        self.play(
            ReplacementTransform(left_hole.copy().set_stroke(YELLOW),
                                 right_hole))
        self.wait()
    ...
    def approach_zero(self):
        ...
        def update_lines(lines, alpha):
            ...
            x_val = interpolate(sign * self.big_delta, sign * basically_zero,
                                alpha)
        ...
        for lines in right_lines, left_lines:
            ...
            self.play(UpdateFromAlphaFunc(lines, update_lines, run_time=3))
    ...
    def write_limit_not_defined(self):
        limit = TexMobject("\\lim", "_{h", "\\to 0}", "f(", "h", ")")
    ...
    def show_epsilon_delta_intuition(self):
        ...
        self.wait()
        self.animate_epsilon_delta_group_change(ed_group,
                                                target_delta=self.small_delta,
                                                run_time=6)
    ...
    def get_epsilon_delta_group(self, delta, **kwargs):
        ...
        for mob, coords in pairs:
            mob.add(self.get_hole(*coords, color=color, radius=radius))
        return ed_group
...
class PrefaceToEpsilonDeltaDefinition(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.student_says("Isn't that pretty \\\\ technical?",
                          target_mode="guilty",
                          added_anims=[
                              title.to_edge, UP,
        ...
        self.wait(3)
        words = TextMobject("It's a glimpse of\\\\", "real analysis")
        words.set_color_by_tex("real", YELLOW)
        self.teacher_says(words, bubble_kwargs={"height": 3, "width": 6})
...
class EpsilonDeltaExample(GraphLimitExpression, ZoomedScene):
    ...
    def construct(self):
        self.delta_list = [epsilon / 6.0 for epsilon in self.epsilon_list]
        self.skip_superclass_anims()
    ...
    def introduce_epsilon(self):
        epsilon_group, small_epsilon_group = list(
            map(self.get_epsilon_group, self.epsilon_list[:2]))
        ...
        self.play(Write(distance), ShowCreation(arrows))
        self.wait()
        self.play(*list(map(FadeOut, [distance, arrows])))
        self.play(Transform(epsilon_group, small_epsilon_group, run_time=2))
        self.wait()
    ...
    def match_epsilon(self):
        self.animate_epsilon_delta_group_change(
            self.ed_group,
            target_delta=self.delta_list[1],
            run_time=2,
            added_anims=[
                ApplyMethod(hole.scale_in_place, 0.25, run_time=2)
    ...
    def introduce_delta(self):
        ...
        self.play(
            ApplyWave(VGroup(self.ed_group.output_range, self.holes[1]),
                      direction=RIGHT))
        self.wait(2)
    ...
    def smaller_epsilon(self):
        ...
        self.play(
            Transform(self.epsilon_group, self.get_epsilon_group(new_epsilon)))
        self.wait()
        self.animate_epsilon_delta_group_change(
            self.ed_group,
            target_delta=new_delta,
            added_anims=[
                Transform(self.delta_group, self.get_delta_group(new_delta))
            ] + [ApplyMethod(hole.scale_in_place, 0.5) for hole in self.holes])
        self.ed_group.input_range.make_jagged()
        self.wait(2)
    ...
    def get_epsilon_group(self, epsilon, limit_value=12):
        ...
        result.braces = VGroup(*[
            brace.copy().set_height(group.get_height()).next_to(
                group, RIGHT, SMALL_BUFF) for i in (1, 2)
        ...
        for label, brace in zip(result.labels, result.braces):
            label.set_height(min(label.get_height(), 0.8 * brace.get_height()))
    ...
    def get_delta_group(self, delta):
        ...
        result.labels = VGroup(*[
            TexMobject("\\delta").scale(1. / self.zoom_factor)
            for brace in result.braces
        ])
        for label, brace in zip(result.labels, result.braces):
            label.next_to(brace, DOWN, buff=SMALL_BUFF / self.zoom_factor)
...
class EpsilonDeltaCounterExample(LimitCounterExample, EpsilonDeltaExample):
    ...
    def introduce_epsilon(self):
        ...
        self.play(*it.chain([
            ReplacementTransform(epsilon_group.limit_line.copy(), line)
            for line in epsilon_group.epsilon_lines
        ], list(map(GrowFromCenter, epsilon_group.braces))))
        self.play(*list(map(Write, epsilon_group.labels)))
        self.play(Write(rhs))
...
class TheoryHeavy(TeacherStudentsScene):
    def construct(self):
        lhs = TexMobject("{df", "\\over \\,", "dx}", "(", "x", ")")
        equals = TexMobject("=")
        rhs = TexMobject("\\lim", "_{h", "\\to 0}", "{f", "(", "x", "+", "h",
                         ")", "-", "f", "(", "x", ")", "\\over \\,", "h}")
        ...
        Scene.play(
            self, Write(lhs), FadeOut(student.bubble),
            FadeOut(student.bubble.content), *[
                ApplyFunction(
                    lambda pi: pi.change_mode("pondering").look_at(
                        epsilon_delta), pi) for pi in self.get_pi_creatures()
            ])
        student.bubble = None
        part_tex_pairs = [
            ("df", "f"),
        ...
        self.play(
            Write(equals), Write(lim), *[
                ReplacementTransform(VGroup(*lhs.get_parts_by_tex(t1)).copy(),
                                     VGroup(*rhs.get_parts_by_tex(t2)),
                                     run_time=2,
                                     rate_func=squish_rate_func(
                                         smooth, alpha, alpha + 0.5))
                for (t1, t2), alpha in zip(
                    part_tex_pairs, np.linspace(0, 0.5, len(part_tex_pairs)))
            ])
        self.wait(2)
        self.play(Write(epsilon_delta), ShowCreation(arrow))
        self.wait(3)
        derivative.add(epsilon_delta, arrow)
        self.student_says("How do you \\\\ compute limits?",
                          student_index=2,
                          added_anims=[
                              derivative.scale, 0.8, derivative.to_corner,
                              UP + LEFT
                          ])
...
class LHopitalExample(LimitCounterExample, PiCreatureScene, ZoomedScene,
                      ReconfigurableScene):
    CONFIG = {
        "graph_origin": ORIGIN,
    ...
    def introduce_function(self):
        ...
        self.play(
            Write(func_label),
            Transform(x_copy,
                      VGroup(*func_label.get_parts_by_tex("x")),
                      remover=True))
    ...
    def show_non_definedness_at_one(self):
        ...
        v_line, alt_v_line = [
            self.get_vertical_line_to_graph(x,
                                            self.graph,
                                            line_class=DashedLine,
                                            color=self.x_color)
            for x in (1, -1)
        ]
        hole, alt_hole = [self.get_hole(x, self.func(x)) for x in (1, -1)]
        ...
        self.play(Blink(morty), ShowCreation(v_line))
        self.play(
            RemovePiCreatureBubble(morty,
                                   target_mode="pondering",
                                   look_at_arg=func_1),
            ReplacementTransform(self.func_label.copy(), func_1))
        self.wait(2)
        ...
        self.animate_epsilon_delta_group_change(ed_group,
                                                ...
                                                run_time=4)
        self.wait()
        self.play(Write(lim), MoveToTarget(self.func_label), Write(equals_q),
                  morty.change_mode, "confused", morty.look_at, lim)
        self.wait(2)
        self.play(func_1_group.to_corner, UP + LEFT,
    ...
    def plug_in_value_close_to_one(self):
        ...
        self.play(ReplacementTransform(self.func_label.copy(), label))
        self.wait()
        self.play(Write(rhs))
        self.wait()
    ...
    def ask_about_systematic_process(self):
        ...
        to_fade = VGroup(*self.x_axis.numbers[:len(self.x_axis.numbers) / 2])

        self.play(FadeIn(morty), FadeOut(to_fade))
        ...
        self.play(
            ...
            self.func_1_group.scale,
            self.tex_scale_value,
            self.func_1_group.move_to,
            morty.get_corner(UP + LEFT),
            DOWN + LEFT,
            self.func_1_group.shift,
            MED_LARGE_BUFF * UP,
        )
        self.wait(2)
        self.play(
        ...
        self.play(
            ...
            self.lim_group.scale,
            self.tex_scale_value,
            self.lim_group.to_corner,
            UP + LEFT,
    ...
    def show_graph_of_numerator_and_denominator(self):
        sine_graph = self.get_graph(lambda x: np.sin(np.pi * x),
                                    color=self.sin_color)
        sine_label = self.get_graph_label(sine_graph,
                                          "\\sin(\\pi x)",
                                          x_val=4.5,
                                          direction=UP)
        parabola = self.get_graph(lambda x: x**2 - 1,
                                  color=self.parabola_color)
        parabola_label = self.get_graph_label(parabola, "x^2 - 1")
        fader = VGroup(*[
            Rectangle(
                width=FRAME_WIDTH,
                height=FRAME_HEIGHT,
        ...
        self.play(ShowCreation(sine_graph, run_time=2),
    ...
    def zoom_in_to_trouble_point(self):
        ...
        lil_rect.move_to(self.coords_to_point(*self.zoomed_rect_center_coords))
        self.wait()
        self.play(lil_rect.scale_in_place, 1. / self.zoom_factor)
        self.wait()
    ...
    def talk_through_sizes_of_nudges(self):
        ...
        zoom_tex_scale_factor = min(0.75 / self.zoom_factor, 1.5 * self.dx)
        ...
        d_sine_arrow, d_parabola_arrow = [
            Arrow(self.coords_to_point(1 + self.dx, 0),
                  self.coords_to_point(1 + self.dx,
        ...
        self.wait()
        self.play(
            ReplacementTransform(VGroup(dx_label[1].copy()),
                                 d_sine_label,
                                 run_time=2))
        self.wait(2)
        self.play(d_sine_label.shift, d_sine_label.get_height() * UP)
        tex_pair_lists = [
            [
                ("sin", "cos"),
                ("pi", "pi"),
        ...
        for tex_pairs in tex_pair_lists:
            self.play(*[
                ReplacementTransform(
                    ...
                    cos_dx.get_part_by_tex(t2)) for t1, t2 in tex_pairs
            ])
            self.wait()
        ...
        self.wait(2)
        self.play(brace.set_width, cos.get_width(), brace.next_to, cos, UP,
                  SMALL_BUFF / self.zoom_factor, FadeOut(brace_text))
        ...
        self.play(neg_pi_dx.next_to, d_sine_arrow, RIGHT,
                  SMALL_BUFF / self.zoom_factor)
        self.wait()
        ...
        self.play(
            ReplacementTransform(VGroup(dx_label[1].copy()),
                                 ...
                                 run_time=2))
        self.wait(2)
        self.play(d_parabola_label.shift, d_parabola_label.get_height() * UP)
        tex_pair_lists = [[
            ("2", "2"),
            ("x", "x"),
        ], [
        ...
        for tex_pairs in tex_pair_lists:
            self.play(*[
                ReplacementTransform(
                    ...
                    two_x_dx.get_part_by_tex(t2)) for t1, t2 in tex_pairs
            ])
        self.wait()
    ...
    def substitute_x_equals_1(self, tex_mob, zoom_tex_scale_factor):
        ...
        self.wait()
        self.play(ReplacementTransform(x.copy(), eq_x), Transform(x, one),
                  Write(equals))
        self.wait()
    ...
    def show_final_ratio(self):
        ...
        rhs = TexMobject("\\approx", "{-\\pi", "\\, dx", "\\over \\,", "2",
                         "\\, dx}")
        ...
        circles = VGroup(*[
            Circle(color=GREEN).replace(dx).scale_in_place(1.3) for dx in dxs
        ])

        #Show numerator and denominator
        self.play(FocusOn(ratio))
        for mob in numerator, denominator:
            self.play(ApplyWave(mob, direction=UP + RIGHT, amplitude=0.1))
            self.wait()
        ...
        #Shrink dx
        self.transition_to_alt_config(transformation_kwargs={"run_time": 2},
                                      dx=self.dx / 10)
        self.wait()
    ...
    def get_func_label(self, argument="x"):
        ...
        result = TexMobject("{\\sin(\\pi ", in_tex, ")", " \\over \\,", in_tex,
                            "^2 - 1}")
...
class GeneralLHoptial(LHopitalExample):
    ...
    def add_graphs(self):
        ...
        f_label = self.get_graph_label(f_graph,
                                       "f(x)",
                                       x_val=3,
                                       direction=RIGHT)
        ...
        g_label = self.get_graph_label(g_graph, "g(x)", x_val=4, direction=UP)
    ...
    def zoom_in(self):
        ...
        self.wait()
        self.play(
            lil_rect.scale_in_place,
            1. / self.zoom_factor,
            self.a_dot.scale_in_place,
            1. / self.zoom_factor,
            run_time=3,
        )
        self.wait()
    ...
    def show_limit_in_symbols(self):
        ...
        equals_zero_over_zero = TexMobject("=", "{\\, 0 \\,", "\\over \\,",
                                           "0 \\,}")
        equals_q = TexMobject(*"=???")
        ...
        self.play(ReplacementTransform(
            VGroup(*self.f_label).copy(), VGroup(frac_a.numerator)),
                  ReplacementTransform(
        ...
        self.wait(2)
        self.play(
            ReplacementTransform(VGroup(*frac_a.get_parts_by_tex("a")),
        ...
        self.play(ReplacementTransform(equals_zero_over_zero, equals_q))
        self.wait()
    ...
    def show_tiny_nudge(self):
        ...
        df_arrow, dg_arrow = [
            Arrow(self.coords_to_point(self.a_value + self.dx, 0),
        ...
        for char, arrow in ("f", df_arrow), ("g", dg_arrow):
            label = TexMobject("\\frac{d%s}{dx}" % char, "(", "a", ")",
                               "\\,dx")
            label.scale(zoom_tex_scale_factor)
    ...
    def show_example(self):
        ...
        self.play(
            Write(rhs[0]),
            ReplacementTransform(VGroup(*lhs[3:6]).copy(), VGroup(*rhs[1:4])))
        self.wait()
        self.play(
            ReplacementTransform(
                VGroup(*lhs[6:8]).copy(),
    ...
    def show_bernoulli_and_lHopital(self):
        ...
        strike = Line(lhopital_name.get_left(),
        ...
        self.play(Write(bernoulli_name), FadeIn(bernoulli_image))
        self.wait()
        self.play(FadeIn(lhopital_image), bernoulli_image.next_to, arrow, DOWN,
                  SMALL_BUFF, ShowCreation(arrow), FadeIn(dollars))

        for x in range(10):
            dollars.restore()
            self.play(
                *[Transform(*pair) for pair in zip(dollars, dollars[1:])] +
                [FadeOut(dollars[-1])])
    ...
    def get_frac(self, input_tex, color=WHITE):
        result = TexMobject("{f", "(", input_tex, ")", "\\over \\,", "g", "(",
                            input_tex, ")}")
...
class CannotUseLHopital(TeacherStudentsScene):
    def construct(self):
        deriv = TexMobject("{d(e^x)", "\\over \\,", "dx}", "(", "x", ")", "=",
                           "\\lim", "_{h", "\\to 0}", "{e^{", "x", "+", "h}",
                           "-", "e^", "x", "\\over \\,", "h}")
        deriv.to_edge(UP)
        ...
        self.play(
            Write(deriv),
            *it.chain(*[[pi.change_mode, "pondering", pi.look_at, deriv]
                        for pi in self.get_pi_creatures()]))
        self.wait()
        self.student_says("Use L'Hôpital's rule!", target_mode="hooray")
        self.wait()
        answer = TexMobject("\\text{That requires knowing }", "{d(e^x)",
                            "\\over \\,", "dx}")
...
class NextVideo(TeacherStudentsScene):
    def construct(self):
        ...
        ftc = TexMobject("F(b)", "-", "F(a)", "=", "\\int_a^b", "{dF",
                         "\\over \\,", "dx}", "(x)", "dx")
        ...
        self.add(series)
        self.play(GrowFromCenter(brace), next_video.set_color, YELLOW,
                  self.get_teacher().change_mode, "raise_right_hand",
                  self.get_teacher().look_at, next_video)
        self.play(Write(integral))
        self.wait(2)
        self.play(*[
            ReplacementTransform(VGroup(*integral.get_parts_by_tex(p1)),
