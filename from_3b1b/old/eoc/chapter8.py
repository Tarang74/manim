...
class Chapter8OpeningQuote(OpeningQuote, PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(author.next_to, self.pi_creature.get_corner(UP + LEFT), UP,
                  self.pi_creature.change_mode, "raise_right_hand")
        self.wait(3)
        self.play(author.restore, self.pi_creature.change_mode, "plain")
        self.play(words_copy.next_to, self.pi_creature, LEFT, MED_SMALL_BUFF,
                  UP, self.pi_creature.change_mode, "thinking")
        self.wait(2)
        self.play(Write(formula), self.pi_creature.change_mode, "confused")
        self.wait()

    def get_formula(self):
        result = TexMobject("{d(\\sin(\\theta)) \\over \\,", "d\\theta}", "=",
                            "\\lim_{", "h", " \\to 0}", "{\\sin(\\theta+", "h",
                            ") - \\sin(\\theta) \\over", " h}", "=", "\\lim_{",
                            "h", " \\to 0}", "{\\big[ \\sin(\\theta)\\cos(",
                            "h", ") + ", "\\sin(", "h",
...
class ThisVideo(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(3)
        self.play(
            this_video.restore, next_video.shift,
            next_video.get_height() * DOWN / 2, next_video.set_color, YELLOW,
            integral[0].copy().next_to, next_video, DOWN, MED_LARGE_BUFF,
            FadeOut(formula),
            *it.chain(*[[pi.change_mode, "plain", pi.look_at, next_video]
...
class InCarRestrictedView(ShowSpeedometer):
    ...
    def construct(self):
        ...
        self.play(MoveCar(car, self.point_B, rate_func=s_rate_func),
                  IncrementNumber(time_label[1], run_time=8),
                  MaintainPositionRelativeTo(window, car),
                  *self.get_added_movement_anims(rate_func=v_rate_func,
                                                 radians=-(16.0 / 70) * 4 *
                                                 np.pi / 3),
                  run_time=8)
        eight = TexMobject("8").move_to(time_label[1])
        self.play(
            Transform(time_label[1],
                      eight,
...
class PlotVelocity(GraphScene):
    ...
    def plot_points(self):
        ...
        self.play(Succession(*dot_intro_anims, rate_func=linear),
                  ApplyMethod(self.speedometer.move_needle_to_velocity,
                              v_func(4),
    ...
    def get_v_graph_and_label(self):
        ...
        graph_label.next_to(graph.point_from_proportion(7. / 8.), UP + RIGHT)
        self.v_graph = graph
...
class AskAboutAntiderivative(PlotVelocity):
    ...
    def write_antiderivative(self):
        ...
        randy.shift(2 * RIGHT)
        words = TexMobject("{d(", "???", ") \\over \\,", "dt}", "=", "t(8-t)")
...
class Antiderivative(PiCreatureScene):
    def construct(self):
        ...
        self.wait()
        self.play(ShowCreation(bottom_arc), Write(antiderivative),
                  self.pi_creature.change_mode, "raise_right_hand")
        self.wait(2)
        for pair in reversed(list(zip(functions, alt_functions))):
            self.play(Transform(*pair), self.pi_creature.change_mode,
                      "pondering")
        self.wait(2)
    ...
    def get_arcs(self, functions):
        ...
        bottom_line = Line(f1.get_corner(DOWN + RIGHT),
                           f2.get_corner(DOWN + LEFT))
...
class AreaUnderVGraph(PlotVelocity):
    ...
    def show_rects(self):
        ...
        rects = self.get_riemann_rectangles(flat_graph,
                                            x_min=0,
                                            x_max=8,
                                            dx=1.0)
...
class ConstantVelocityPlot(PlotVelocity):
    ...
    def draw_graph(self):
        graph = self.get_graph(lambda t: 10,
                               x_min=0,
                               x_max=8,
    ...
    def show_product(self):
        rect = Rectangle(stroke_width=0,
                         ...
                         fill_opacity=0.5)
        rect.replace(VGroup(self.graph, VectorizedPoint(self.graph_origin)),
                     stretch=True)
    ...
    def comment_on_area_wierdness(self):
        ...
        self.play(self.area_rect.set_color,
                  YELLOW,
...
class PiecewiseConstantPlot(PlotVelocity):
    ...
    def always_changing(self):
        ...
        def update_group(group, alpha):
            ...
            prop = interpolate(self.min_graph_proportion,
                               self.max_graph_proportion, alpha)
    ...
    def show_piecewise_constant_graph(self):
        ...
        alt_lines = [
            line.copy().set_color(YELLOW) for line in pw_constant_graph[:4]
        ]
        ...
        for new_line in alt_lines[1:]:
            for mob in line.end_dot, new_line.start_dot, new_line:
                self.play(Transform(line, mob, run_time=1. / 3))
            self.remove(line)
            self.add(new_line)
            self.wait(2)
            line = new_line
    ...
    def compute_distance_on_each_interval(self):
        ...
        flat_rects = self.get_riemann_rectangles(self.get_graph(lambda t: 0),
                                                 x_min=0,
                                                 x_max=8,
                                                 dx=1)
        rects = rect_list[0]
    ...
    def revert_to_specific_approximation(self):
        ...
        example_text = TextMobject("For example, \\\\", "$dt$", "$=0.25$")
        ...
        self.play(
            ...
            ReplacementTransform(dt_label.copy(),
    ...
    def show_specific_rectangle(self):
        ...
        for t in t_vals:
            v_line = self.get_vertical_line_to_graph(t,
                                                     self.v_graph,
                                                     color=YELLOW)
        ...
        self.play(t_label_copy.scale, 1. / 0.7, t_label_copy.next_to,
                  self.v_graph_label, DOWN + LEFT, 0)
        self.wait()
        ...
        self.wait()
        for x in range(2):
            self.play(rect.stretch_to_fit_height,
        ...
        self.play(*list(
            map(FadeOut,
                [group[1] for group in (v_lines, h_lines, height_labels)])))
        self.play(
            v_lines[0].set_color,
            RED,
            rate_func=there_and_back,
        ...
        area = TextMobject("7$\\frac{\\text{m}}{\\text{s}}$", "$\\times$",
                           "0.25s", "=", "1.75m")
        ...
        self.play(*list(
            map(FadeOut, [
                area, arrow, v_lines[0], h_lines[0], height_labels[0], rect,
                t_labels
            ])))
    ...
    def show_v_dt_for_all_rectangles(self):
        ...
        for rect in rects_subset:
            ...
            anims = [
                rect.set_fill, None, 1, dt_brace_group.next_to, rect, DOWN,
                SMALL_BUFF
            ]
    ...
    def write_integral_symbol(self):
        integral = TexMobject("\\int", "^8", "_0", "v(t)", "\\,dt")
        ...
        for bound, num in zip(bounds, [0, 8]):
            ...
            self.play(bound_copy.scale, 1.5, bound_copy.next_to, point, DOWN,
                      MED_LARGE_BUFF)
        ...
        for mob, tex in (self.v_t, "v(t)"), (self.dt_label, "dt"):
            self.play(
                ReplacementTransform(mob.copy().set_color(YELLOW),
    ...
    def roles_of_dt(self):
        ...
        self.play(morty.change_mode, "raise_right_hand",
                  morty.look, UP + RIGHT, dt_copy.next_to,
                  morty.get_corner(UP + RIGHT), UP, dt_copy.set_color, YELLOW)
        self.play(Blink(morty))
        self.play(ReplacementTransform(dt_copy.copy(), int_dt, run_time=2),
                  morty.look_at, int_dt)
        self.wait(2)
        self.play(ReplacementTransform(dt_copy.copy(), self.dt_label),
                  morty.look_at, self.dt_label)
        self.play(*[
            ApplyMethod(tick.shift,
                        ...
                        rate_func=squish_rate_func(
                            ...
                            self.ticks, np.linspace(0, 0.8, len(self.ticks)))
        ])
        self.wait()

        #Shrink dt just a bit
        self.play(morty.change_mode, "pondering", rects.set_fill, None, 0.75,
        ...
        self.play(Blink(morty))
        self.play(*[
            ApplyFunction(lambda r: r.shift(0.2 * UP).set_fill(None, 1),
                          rect,
                          run_time=2,
                          rate_func=squish_rate_func(
                              there_and_back,
    ...
    def what_does_sum_approach(self):
        ...
        distance_words = TextMobject("Area", "= Distance traveled")
        distance_words.next_to(rects, UP)
        arrow = Arrow(distance_words[0].get_bottom(),
                      rects.get_center(),
                      color=WHITE)

        self.play(
            PiCreatureSays(morty, "Why not $\\Sigma$?", target_mode="sassy"))
        self.play(Blink(morty))
        self.wait()
    ...
    def get_ticks(self, rects):
        ticks = VGroup(*[
            Line(point + self.tick_size * UP / 2, point +
                 self.tick_size * DOWN / 2)
            for t in np.linspace(0, 8,
                                 len(rects) + 1)
...
class TellMeThatsNotSurprising(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("Tell me that's \\\\ not surprising!",
                          target_mode="hooray",
                          run_time=1)
        self.wait(3)
...
class HowDoesThisHelp(TeacherStudentsScene):
    def construct(self):
        self.student_says("How does this help\\textinterrobang",
                          target_mode="angry",
                          run_time=1)
...
class AreaUnderACurve(GraphScene):
    ...
    def construct(self):
        ...
        rect_list = self.get_riemann_rectangles_list(graph,
                                                     self.num_iterations)
...
class AreaIsDerivative(PlotVelocity, ReconfigurableScene):
    ...
    def introduce_variable_area(self):
        ...
        self.add_T_label(6)
        self.change_area_bounds(new_t_max=4,
                                rate_func=there_and_back,
                                run_time=2)
        self.wait()
    ...
    def write_integral(self):
        ...
        self.change_area_bounds(new_t_max=3,
                                rate_func=there_and_back,
                                run_time=3)
        self.wait()
    ...
    def show_rectangle_approximation(self):
        ...
        self.play(*[
            ReplacementTransform(
                mob, formula1.get_part_by_tex(tex), run_time=2)
            for mob, tex in [
                (self.ds_label, "ds"),
                (self.ds_arrow, "="),
    ...
    def change_area_bounds(self, new_t_min=None, new_t_max=None, **kwargs):
        ...
        self.play(UpdateFromAlphaFunc(group, update_group),
                  *list(map(Animation, self.foreground_mobjects)), **kwargs)
...
class FindAntiderivative(Antiderivative):
    ...
    def first_part(self):
        ...
        self.play(*[
            ReplacementTransform(t_squared[i].copy(),
                                 two_t[1 - i],
                                 run_time=2,
                                 path_arc=-np.pi / 6.) for i in (0, 1)
        ])
    ...
    def second_part(self):
        ...
        part1_group = VGroup(self.s_part1, self.v_part1, self.arcs_copy,
                             self.words_copy)
        ...
        self.play(part1_group.scale, 0.5, part1_group.to_corner, UP + LEFT,
                  self.pi_creature.change_mode, "plain")
        self.play(self.v_part2.restore, self.v_part2.shift, LEFT)
        self.play(FadeIn(self.q_marks))
        self.wait()

        self.play(FadeOut(self.q_marks), FadeIn(t_cubed), self.v_part2.shift,
                  DOWN + RIGHT)
        self.play(*[
            ReplacementTransform(
                t_cubed[i].copy(),
    ...
    def add_plus_C(self):
        ...
        plus_Cs = [TexMobject("+%d" % d) for d in range(1, 8)]
...
class GraphSPlusC(GraphDistanceVsTime):
    ...
    def construct(self):
        ...
        v_line = self.get_vertical_line_to_graph(6,
                                                 graph,
...
class LowerBound(AreaIsDerivative):
    ...
    def mention_lower_bound(self):
        ...
        self.play(zero_label.scale, 1.5, zero_label.next_to, self.graph_origin,
                  DOWN, MED_LARGE_BUFF, FadeOut(circle))
        self.wait()
    ...
    def show_alternate_antiderivative_difference(self):
        ...
        numbers = [
            TexMobject("%d" % d).next_to(self.coords_to_point(d, 0), DOWN,
                                         MED_LARGE_BUFF) for d in (1, 7)
        ]
        ...
        self.play(self.T_label_group[0].set_fill, None, 0,
    ...
    def add_constant_to_antiderivative(self):
        ...
        self.wait(2)
        self.play(group.arrange, group.next_to, antideriv_diff, DOWN,
                  MED_LARGE_BUFF)
        self.wait()
    ...
    def get_integral(self, lower_bound, upper_bound):
        result = TexMobject("\\int", "^" + upper_bound, "_" + lower_bound,
                            "t(8-t)", "\\,dt")
    ...
    def get_antiderivative_difference(self, lower_bound, upper_bound):
        ...
        parts = []
        for s in strings:
            part = TexMobject("\\left(", "4", s, "^2", "-", "\\frac{1}{3}", s,
                              "^3"
                              "\\right))")
...
class FundamentalTheorem(GraphScene):
    ...
    def add_graph_and_integral(self):
        ...
        for bound, tex in (self.lower_bound, "a"), (self.upper_bound, "b"):
            ...
            v_line = self.get_vertical_line_to_graph(bound,
                                                     graph,
    ...
    def show_f_dx_sum(self):
        ...
        self.play(*[
            ApplyMethod(rect.set_fill, None,
        ...
        for i in range(start_rect_index + 1, last_rect_index):
            self.play(
                rects[i - 1].set_fill, None, low_opacity, rects[i].set_fill,
                None, 1, f_brace.set_height, rects[i].get_height(),
                f_brace.next_to, rects[i], LEFT, 0, dx_brace.next_to, rects[i],
                DOWN, 0, *[
        ...
        self.play(*it.chain(
            list(
                map(FadeOut,
                    [f_brace, dx_brace, f_brace.label, dx_brace.label])),
    ...
    def write_antiderivative(self):
        deriv = TexMobject("{d", "F", "\\over\\,", "dx}", "(x)", "=", "f(x)")
        ...
        for x in range(2):
            self.play(*[
                ApplyFunction(
                    ...
                    rect,
                    run_time=3,
                    rate_func=squish_rate_func(there_and_back, alpha, alpha +
                                               0.2))
                for rect, alpha in zip(self.rects,
    ...
    def show_antiderivative_considering_bounds(self):
        ...
        for label, line in reversed(list(zip(self.bound_labels,
                                             self.v_lines))):
...
class NegativeArea(GraphScene):
    ...
    def add_graph_and_area(self):
        graph = self.get_graph(lambda x: -0.02 * (x + 1) * (x - 3) * (x - 7) *
                               (x - 10),
                               x_min=0,
                               x_max=8,
                               color=VELOCITY_COLOR)
        ...
        self.play(ShowCreation(graph), FadeIn(
            area,
            run_time=2,
            lag_ratio=0.5,
        ))
    ...
    def show_negative_point(self):
        v_line = self.get_vertical_line_to_graph(self.sample_input,
                                                 self.graph,
                                                 color=RED)
        self.play(ShowCreation(v_line))
        self.wait()
...
class NextVideo(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(next_video.set_color, YELLOW, next_video.shift,
                  next_video.get_height() * DOWN / 2, self.teacher.change_mode,
                  "raise_right_hand")
        self.play(Write(integral))
        self.wait(5)
