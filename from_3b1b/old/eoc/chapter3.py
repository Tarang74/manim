...
class PoseAbstractDerivative(TeacherStudentsScene):
    def construct(self):
        ...
        self.student_says("Why?",
                          target_mode="sassy",
                          added_anims=[
                              content_copy.scale, 0.8, content_copy.to_corner,
                              UP + LEFT
                          ])
...
class ContrastAbstractAndConcrete(Scene):
    def construct(self):
        ...
        functions = VGroup(*list(
            map(TexMobject, [
                "f(x) = 2x^2 - x^3", "f(x) = \\sin(x)", "f(x) = e^x",
                "\\v_dots"
            ])))
    ...
    def get_spring_anim(self, alignement_mob):
        ...
        return Transform(compact_spring,
                         extended_spring,
...
class DerivativeOfXSquaredAsGraph(GraphScene, ZoomedScene, PiCreatureScene):
    ...
    def ask_about_df_dx(self):
        ...
        df_dx.next_to(self.input_to_graph_point(self.start_x, self.graph),
                      DOWN + RIGHT,
                      buff=MED_SMALL_BUFF)
        ...
        self.play(Write(derivative_q, run_time=1),
        ...
        self.play(
            ApplyFunction(
                ...
                self.little_rectangle), self.pi_creature.change_mode,
            "pondering", self.pi_creature.look_at, ss_group)
        self.play(
...
class NudgeSideLengthOfSquare(PiCreatureScene):
    ...
    def introduce_square(self):
        ...
        self.play(DrawBorderThenFill(square), self.pi_creature.change_mode,
                  "plain")
    ...
    def increase_area(self):
        ...
        right_rect = Rectangle(width=self.dx,
        ...
        right_line = Line(self.square.get_corner(UP + RIGHT),
                          self.square.get_corner(DOWN + RIGHT),
                          stroke_width=0)
        bottom_line = Line(self.square.get_corner(DOWN + RIGHT),
        ...
        self.play(*it.chain(
            *[[mob.shift, vect * SMALL_BUFF]
              for mob, vect in [(right_rect, RIGHT), (
                  bottom_rect,
                  DOWN), (corner_square, DOWN +
                          RIGHT), (right_brace,
                                   RIGHT), (bottom_brace,
                                            DOWN), (little_braces,
                                                    DOWN + RIGHT)]]))
    ...
    def write_df_equation(self):
        ...
        df_equation = VGroup(
            ...
            TexMobject("="), right_rect.copy(), TextMobject("+"),
            right_rect.copy(), TexMobject("+"), corner_square.copy())
        df_equation.arrange()
        df_equation.next_to(self.function_label,
                            DOWN,
                            aligned_edge=LEFT,
                            buff=SMALL_BUFF)
        ...
        self.wait()
        self.play(*[mob.restore for mob in (r1, r2, s)] +
    ...
    def set_color_shapes(self):
        ...
        tups = [(self.right_rect, self.bottom_rect, r1, r2),
                (self.corner_square, s)]
        for tup in tups:
            self.play(*it.chain(*[[m.scale_in_place, 1.2, m.set_color, RED]
                                  for m in tup]),
    ...
    def examine_thin_rectangles(self):
        ...
        for group, tex, color in (xs, "3", self.three_color), (dxs, "0.01",
                                                               self.dx_color):
            group.save_state()
        ...
        self.wait()
        self.play(FadeOut(example_value),
                  *[mob.restore for mob in (xs, dxs, text)])
        self.remove(text)
        text.restore()
        self.add(text)
    ...
    def examine_tiny_square(self):
        ...
        self.wait()
        self.play(MoveToTarget(self.dxs), self.pi_creature.change_mode,
                  "plain")
        ...
        self.play(Transform(text, alt_text), self.pi_creature.change_mode,
                  "raise_right_hand")
        self.wait(2)
        self.play(*[mob.restore for mob in (self.dxs, text)] +
    ...
    def show_smaller_dx(self):
        ...
        for mob in self.get_mobjects():
            mob.save_state()
        self.play(*[
            Transform(*pair) for pair in zip(
                self.get_mobjects(),
    ...
    def rule_of_thumb(self):
        ...
        self.play(Write(safe_to_ignore, run_time=2),
                  self.pi_creature.change_mode, "raise_right_hand")
        self.play(FadeOut(circle), FadeOut(safe_to_ignore),
                  dx_squared_group.fade, 0.5, dx_squared_group.to_corner,
                  UP + RIGHT, self.pi_creature.change_mode, "plain")
        self.wait()
    ...
    def write_out_derivative(self):
        ...
        self.play(Transform(r1, self.right_rect),
                  Transform(r2, self.bottom_rect), FadeOut(plus1),
                  FadeOut(self.thin_rect_brace))
        self.play(self.thin_rect_area.next_to, VGroup(df, equals), RIGHT,
                  MED_SMALL_BUFF, UP, self.pi_creature.change_mode, "thinking")
        self.wait(2)
        self.play(ApplyMethod(df.next_to, frac_line, UP, SMALL_BUFF),
        ...
        self.play(*[
            Transform(*pair)
            for pair in zip(self.mobjects_at_end_of_write_out_derivative,
...
class NudgeSideLengthOfCube(Scene):
    ...
    def introduce_cube(self):
        ...
        self.play(*[
            ApplyMethod(piece.shift, 0.5 *
                        (piece.get_center() - cube.get_center()))
            for piece in dv_pieces
        ...
        self.faces, self.bars, self.corner_cube = [
            VGroup(
                *[piece for piece in dv_pieces if piece.type == target_type])
            for target_type in ("face", "bar", "corner_cube")
        ]

    def write_df_equation(self):
        df_equation = VGroup(TexMobject("df"), TexMobject("="),
                             self.organize_faces(self.faces.copy()),
                             TexMobject("+"),
                             self.organize_bars(self.bars.copy()),
                             TexMobject("+"), self.corner_cube.copy())
        ...
        self.wait()
        self.play(*[
            ApplyMethod(mob.fade, 0.7) for mob in
            [plus1, bars, plus2, corner_cube, extras_brace, ignore_text]
        ])
        self.wait()
    ...
    def write_derivative(self):
        ...
        self.play(MoveToTarget(derivative), df_equals.next_to,
                  derivative.target[0], LEFT, df_equals.shift, 0.07 * DOWN)
        ...
        self.play(FadeOut(frac_line2), FadeOut(dx_below_dx_squared),
                  dx_squared[-1].set_color, BLACK, inner_dx.next_to,
                  extra_stuff[0], RIGHT, SMALL_BUFF)
        self.wait()
        self.shrink_dx("Derivative is written", restore=False)
        self.play(
            *[ApplyMethod(mob.fade, 0.7) for mob in (extra_stuff, inner_dx)])
        self.wait(2)

        anims = []
        for mob in list(self.faces) + list(self.bars) + list(self.corner_cube):
            vect = mob.get_center() - self.cube.get_center()
            anims += [mob.shift, -(1. / 3) * vect]
    ...
    def grab_pieces(self, start_pieces, end_pices, to_write=None):
        for piece in start_pieces:
            ...
            piece.target.rotate_in_place(
                np.pi / 12,
                piece.get_center() - self.cube.get_center())
...
class GraphOfXCubed(GraphScene):
    ...
    def construct(self):
        ...
        deriv_label = self.get_graph_label(deriv_graph,
                                           "\\frac{df}{dx}(x) = 3x^2",
                                           x_val=-3,
                                           direction=LEFT)
...
class PatternForPowerRule(PiCreatureScene):
    ...
    def introduce_pattern(self):
        ...
        for exponent, color in zip(exp_range, colors):
            derivative = TexMobject("\\frac{d(x^%d)}{dx} = " % exponent,
    ...
    def generalize_pattern(self):
        ...
        power_rule = TexMobject("\\frac{d (x^n)}{dx} = ", "nx^{n-1}")
        title = TextMobject("``Power rule''")
        title.next_to(power_rule, UP, MED_LARGE_BUFF)
        lines = VGroup(*[
            Line(deriv.get_right(),
                 power_rule.get_left(),
                 buff=MED_SMALL_BUFF,
                 color=deriv[0][2].get_color()) for deriv in derivatives
        ])

        self.play(
            Transform(VGroup(*[d[0].copy() for d in derivatives]),
                      VGroup(power_rule[0]),
                      replace_mobject_with_target_in_scene=True),
            ShowCreation(lines),
            lag_ratio=0.5,
    ...
    def show_hopping(self):
        ...
        for exp, color in zip(exp_range, self.colors[1:]):
            form = TexMobject("x^", str(exp), "\\rightarrow", str(exp), "x^",
                              str(exp - 1))
            ...
            self.wait()
            self.play(self.pi_creature.change_mode, "hesitant",
...
class PowerRuleAlgebra(Scene):
    ...
    def construct(self):
        ...
        equals = TexMobject("=")
        equals2 = TexMobject("=")
        full_product = TexMobject(*paren_strings * 3 + ["\\cdots"] +
                                  paren_strings)
        ...
        self.play(FadeIn(x_dx_to_n), GrowFromCenter(nudge_brace),
                  GrowFromCenter(nudged_output))
        self.wait()
        self.play(Write(VGroup(equals, full_product)),
                  GrowFromCenter(product_brace,
    ...
    def workout_product(self, equals, full_product):
        ...
        xs, dxs = [
            VGroup(*[
                submob for submob, tex in product_part_tex_pairs
                if tex == target_tex
    ...
    def pull_out_linear_terms(self, x_to_n, product_part_tex_pairs, xs, dxs):
        ...
        for dx_index, dx in enumerate(dxs):
            ...
            self.play(MoveToTarget(dx_copy), MoveToTarget(xs_copy),
                      Write(dots))
...
class OneOverX(PiCreatureScene, GraphScene):
    ...
    def introduce_puddle(self):
        ...
        self.play(DrawBorderThenFill(rect_group.rectangle),
                  Write(rect_group.area_label), self.pi_creature.change_mode,
                  "thinking")
        self.play(
        ...
        for d in 2, 3:
            self.change_rectangle_group(rect_group,
                                        d,
                                        target_group_kwargs={
                                            "x_label":
                                            str(d),
    ...
    def perform_nudge(self):
        ...
        h_lines = VGroup(*[
            DashedLine(ORIGIN, (rect_copy.get_width() + LARGE_BUFF) * RIGHT,
                       color=self.df_color,
                       stroke_width=2).move_to(rect.get_corner(UP +
                                                               LEFT), LEFT)
            for rect in (rect_group.rectangle, new_rect)
        ])

        v_lines = VGroup(*[
            DashedLine(ORIGIN, (rect_copy.get_height() + MED_LARGE_BUFF) * UP,
                       color=self.dx_color,
                       stroke_width=2).move_to(rect.get_corner(DOWN +
                                                               RIGHT), DOWN)
            for rect in (rect_group.rectangle, new_rect)
        ])
        ...
        negative_arrow = Arrow(negative.get_left(),
                               df_label.get_corner(UP + RIGHT),
                               color=RED)

        area_changes = VGroup()
        point_pairs = [(new_rect.get_corner(UP + RIGHT),
                        rect_copy.get_corner(DOWN + RIGHT)),
                       (new_rect.get_corner(UP + LEFT),
                        rect_copy.get_corner(UP + RIGHT))]
        for color, point_pair in zip([self.dx_color, self.df_color],
                                     point_pairs):
            area_change_rect = Rectangle(fill_opacity=1,
                                         fill_color=color,
                                         stroke_width=0)
        ...
        area_gained_label.next_to(rect_copy.get_corner(DOWN + RIGHT),
                                  UP + LEFT,
                                  buff=SMALL_BUFF)
        area_gained_arrow = Arrow(area_gained_label.get_top(),
        ...
        area_lost_arrow = Arrow(area_lost_label.get_top(),
                                area_lost.get_center(),
                                buff=0,
                                color=WHITE)

        question = TexMobject("\\frac{d(1/x)}{dx} = ???")
        question.next_to(
        ...
        self.play(FadeOut(rect_group.area_label),
                  ReplacementTransform(rect_copy, new_rect),
                  MoveToTarget(recip_brace), MoveToTarget(recip_label),
        ...
        self.play(FadeIn(area_gained), Write(area_gained_label, run_time=2),
                  ShowCreation(area_gained_arrow))
        self.wait()
        self.play(FadeIn(area_lost), Write(area_lost_label, run_time=2),
                  ShowCreation(area_lost_arrow))
        self.wait()
        self.revert_to_original_skipping_status()  ###
        self.play(Write(negative), ShowCreation(negative_arrow))
        self.wait()
        self.play(Write(question), self.pi_creature.change_mode,
                  "raise_right_hand")
        self.wait(2)
    ...
    def create_pi_creature(self):
        ...
        morty.scale(self.morty_scale_val,
    ...
    def change_rectangle_group(self,
        ...
        target_group = self.get_rectangle_group(target_x,
                                                **target_group_kwargs)
        ...
        self.play(UpdateFromAlphaFunc(rect_group, update_rect_group),
                  *added_anims, **anim_kwargs)
        rect_group.x_val = target_x


class AskRecipriocalQuestion(Scene):
    def construct(self):
        tex = TexMobject("(\\text{What number?})", "\\cdot x = 1")
...
class SquareRootOfX(Scene):
    ...
    def introduce_square(self):
        square = Square(side_length=self.square_width,
    ...
    def nudge_square(self):
        ...
        bigger_square = Square(side_length=self.square_width + self.d_sqrt_x,
        ...
        lines = VGroup(*[
            DashedLine(ORIGIN, (self.square_width + MED_LARGE_BUFF) * vect,
                       color=WHITE,
                       stroke_width=3).shift(s.get_corner(corner))
        ...
        question.next_to(area_increase,
                         DOWN,
                         aligned_edge=LEFT,
                         buff=LARGE_BUFF)
        ...
        self.play(*it.chain(list(map(ShowCreation, lines)),
...
class DerivativeOfSineIsSlope(Scene):
    def construct(self):
        tex = TexMobject("\\frac{d(\\sin(\\theta))}{d\\theta} = ",
...
class DerivativeIntuitionFromSineGraph(GraphScene):
    ...
    def draw_sine_graph(self):
        ...
        self.play(ShowCreation(graph),
                  UpdateFromFunc(v_line,
                                 lambda v: self.v_line_update(v, graph)),
                  run_time=2. / rps,
                  rate_func=linear)
        self.wait()
        self.graph = graph
    ...
    def draw_derivative_from_slopes(self):
        ...
        v_line = DashedLine(self.graph_origin,
                ...
                added_anims=[
                    ShowCreation(
                        ...
                        rate_func=lambda t: interpolate(
                            last_theta / self.x_max, next_theta / self.x_max,
                            smooth(t)),
                        run_time=3,
                    ),
                    UpdateFromFunc(v_line,
    ...
    def setup_axes(self):
        ...
        for x in range(1, 4):
            ...
            label.next_to(self.coords_to_point(x * np.pi, 0), DOWN,
                          MED_LARGE_BUFF)
            self.add(label)
...
class DerivativeFromZoomingInOnSine(IntroduceUnitCircleWithSine, ZoomedScene):
    ...
    def zoom_in(self):
        self.activate_zooming()
        self.little_rectangle.next_to(self.radial_line.get_end(), UP,
                                      LARGE_BUFF)
        self.play(
            *list(map(FadeIn, [self.little_rectangle, self.big_rectangle])))
        self.play(self.little_rectangle.move_to, self.radial_line.get_end(),
                  DOWN + RIGHT, self.little_rectangle.shift,
                  SMALL_BUFF * (DOWN + RIGHT))
        self.wait()

    def perform_nudge(self):
        d_theta_arc = Arc(start_angle=self.example_radians,
                          angle=self.d_theta,
                          radius=self.unit_length,
                          color=MAROON_B,
                          stroke_width=6)
        d_theta_arc.scale(self.zoom_factor)
        d_theta_brace = Brace(d_theta_arc,
                              rotate_vector(RIGHT, self.example_radians))
        d_theta_label = TexMobject("d\\theta")
        d_theta_label.next_to(d_theta_brace.get_center(),
                              d_theta_brace.direction, MED_LARGE_BUFF)
        ...
        d_sine_brace.next_to(d_sine_line,
                             LEFT,
    ...
    def show_similar_triangles(self):
        ...
        new_angle_label.scale(little_triangle.get_width() /
                              big_triangle.get_height())
        ...
        self.play(little_triangle.scale,
                  2,
        ...
        self.play(
            ReplacementTransform(little_triangle.copy().set_fill(opacity=0),
                                 big_triangle_copy,
                                 path_arc=np.pi / 2,
                                 run_time=2))
        self.wait()
        self.play(ReplacementTransform(big_triangle_copy, big_triangle),
                  Animation(self.angle_label))
        self.wait()
        self.play(
            self.radial_line.rotate_in_place,
            np.pi / 12,
        ...
        self.play(new_angle_label.scale_in_place,
                  2,
...
class NextVideo(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(next_video.set_color, YELLOW, next_video.shift,
                  MED_LARGE_BUFF * DOWN)
        self.wait()
        for mob in group:
            self.play(
...
class Thumbnail(NudgeSideLengthOfCube):
    def construct(self):
        ...
        formula = TexMobject("\\frac{d(x^3)}{dx} = 3x^2")
