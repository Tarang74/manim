...
class LastVideo(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(self.teacher.change_mode, "raise_right_hand", e_to_x.scale,
                  1.5, e_to_x.set_color, YELLOW, e_to_x.next_to,
...
class DoublingPopulation(PiCreatureScene):
    ...
    def introduce_expression(self):
        ...
        pop_brace, mass_brace = [
            Brace(function[0], DOWN) for function in (P_t, M_t)
        ]
        ...
        self.play(Write(f_x))
        self.wait()
        self.play(Transform(f_x, f_t),
                  FadeIn(t_expression, run_time=2, lag_ratio=0.5))
    ...
    def ask_about_dM_dt(self):
        ...
        self.play(FadeOut(self.pop_brace), FadeOut(self.pop_brace.text),
    ...
    def growth_per_day(self):
        ...
        self.play(FadeOut(self.dM_dt_question),
                  FadeOut(self.population_size_descriptor), FadeIn(day_to_day))
    ...
    def relate_growth_rate_to_pop_size(self):
        ...
        rect.stretch_to_fit_height(rect.get_height() + MED_SMALL_BUFF)
        self.play(
            *list(map(FadeOut, [false_deriv.brace, false_deriv.brace_text])))
    ...
    def create_pi_creatures(self):
        ...
        creature_array = VGroup(*[
            VGroup(*[PiCreature(mode="plain")
                     for y in range(height)]).arrange(UP, buff=MED_LARGE_BUFF)
    ...
    def get_from_day_to_day_label(self):
        ...
        top_words = TextMobject("From day", str(curr_day), "to",
                                str(curr_day + 1), ":")
        ...
        bottom_words = TexMobject(str(2**curr_day), "\\text{ creatures}",
                                  "\\over {1 \\text{ day}}")
...
class GraphOfTwoToT(GraphScene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            ReplacementTransform(height_label.copy(),
        ...
        self.play(
            UpdateFromAlphaFunc(label_group,
                                update_label_group,
                                run_time=3,
...
class FakeDiagram(TeacherStudentsScene):
    def construct(self):
        ...
        corner_line = Line(
            *[graph.get_corner(DOWN + RIGHT) for graph in graphs])
        ...
        diagram = VGroup(graphs, two_to_t, dt_brace, dt, side_brace, deriv,
                         circle, words, arrow)

        self.play(self.teacher.change_mode, "raise_right_hand")
        self.play(Animation(VectorizedPoint(graphs.get_right())),
                  DrawBorderThenFill(foreground_graph), Write(two_to_t))
        self.wait()
        self.play(
            ReplacementTransform(
                foreground_graph.copy(),
                background_graph,
            ), Animation(foreground_graph), Animation(two_to_t),
            GrowFromCenter(dt_brace), Write(dt))
        ...
        self.play(Write(words), ShowCreation(arrow), self.teacher.change_mode,
                  "shruggie")
        self.wait(3)
        self.play(
            FadeOut(diagram), *[
...
class CompareTwoConstantToEightConstant(PiCreatureScene):
    def construct(self):
        ...
        self.play(Animation(derivs), Write(why), self.pi_creature.change,
                  "confused", derivs)
        self.wait()
        for deriv in derivs:
            for index in -5, -2:
    ...
    def get_derivative_expression(self, base):
        ...
        result = TexMobject("{d(", base_str, "^t", ")", "\\over", "dt}", "=",
...
class AskAboutConstantOne(TeacherStudentsScene):
    def construct(self):
        note = TexMobject("{ d(a^", "t", ")", "\\over \\,", "dt}", "=", "a^",
                          "t", "(\\text{Some constant})")
...
class GraphOfExp(GraphScene):
    ...
    def show_slopes(self):
        ...
        slope_labels = [
            TextMobject("Slope = $e^%d$" % x).next_to(
                vl.get_top(), UP + RIGHT).shift(0.7 * RIGHT / x)
            for vl, x in zip(v_lines, self.example_inputs)
        ]
        ...
        self.animate_secant_slope_group_change(
            ...
            run_time=2,
            added_anims=[
                Transform(*pair, path_arc=np.pi / 6, run_time=2) for pair in [
                    slope_labels,
                    v_lines,
                    height_labels,
                ]
...
class ApplyChainRule(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(2)
        self.play(*[
            Transform(*deriv_equation.get_parts_by_tex(
                tex, substring=False).copy()[:2],
                      path_arc=-np.pi,
                      run_time=2) for tex in ("e^", "{3", "t}")
        ] + [Write(deriv_equation.get_part_by_tex("="))])
        self.play(self.teacher.change_mode, "happy")
        self.wait()
        self.play(ShowCreation(circle))
        self.play(
            Transform(*deriv_equation.get_parts_by_tex("3").copy()[-1:-3:-1]))
...
class WhyNaturalLogOf2ShowsUp(TeacherStudentsScene):
    ...
    def add_e_to_the_three_t(self):
        ...
        self.play(FadeIn(exp_c, run_time=2, lag_ratio=0.5),
    ...
    def show_e_to_log_2(self):
        equation = TexMobject("2", "^t", "= e^", "{\\ln(2)", "t}")
        ...
        derivative = TexMobject("\\ln(2)", "2", "^t", "=", "\\ln(2)", "e^",
                                "{\\ln(2)", "t}")
        ...
        self.play(
            Write(log_words),
            ShowCreation(log_words_arrow,
                         run_time=2,
                         rate_func=squish_rate_func(smooth, 0.5, 1)))
        self.change_student_modes(*["pondering"] * 3, look_at_arg=log_words)
        self.wait(2)
        ...
        self.play(ShowCreation(derivative_arrow), Write(derivative_symbol),
                  Write(derivative))
        self.wait(3)
        self.play(self.teacher.change_mode, "happy")
        self.wait(2)
    ...
    def get_a_to_t(self, a):
        ...
        result = TexMobject("{d(", a_str, "^t", ")", "\\over", "dt}", "=",
                            log_str, a_str, "^t")
...
class CompareWaysToWriteExponentials(GraphScene):
    ...
    def construct(self):
        ...
        graphs = [
            self.get_graph(lambda t: base**t, color=GREEN) for base in bases
        ]
        graph = graphs[0]
    ...
    def construct(self):
        ...
        for part in group:
            ...
            if const:
                ...
                log = brace.get_text("$\\log_{%s}(2)$" %
                                     part[1].get_tex_string()[:-1])
                log.set_color(BLUE)
                part.add(brace, log)
        ...
        self.play(*[
            ApplyFunction(lambda m: m.shift(SMALL_BUFF * UP).set_color(RED),
                          part,
                          run_time=2,
        ...
        self.change_student_modes(*["pondering"] * 3, look_at_arg=exp)
        self.wait(3)
...
class TemperatureOverTimeOfWarmWater(GraphScene):
    ...
    def construct(self):
        self.setup_axes()
        graph = self.get_graph(lambda t: 3 * np.exp(-0.3 * t) + self.T_room,
                               color=RED)
        h_line = DashedLine(*[
        ...
        ode = TexMobject("\\frac{d\\Delta T}{dt} = -k \\Delta T")
        ode.to_corner(UP + RIGHT)

        solution = TexMobject("\\Delta T(", "t", ") = e", "^{-k", "t}")
        ...
        delta_T_group_update = UpdateFromFunc(delta_T_group,
                                              update_delta_T_group)
        self.play(
...
class InvestedMoney(Scene):
    def construct(self):
        # cash_str = "\\$\\$\\$"
        cash_str = "M"
        equation = TexMobject("{d", cash_str, "\\over", "dt}", "=", "(1 + r)",
                              cash_str)
        ...
        solution = TexMobject(cash_str, "(", "t", ")", "=", "e^", "{(1+r)",
                              "t}")
        solution.set_color_by_tex("t", YELLOW)
...
class NaturalLog(Scene):
    ...
    def show_natural_loggedness(self, expression):
        ...
        self.play(FadeOut(expression[-2]), FadeOut(constant), ln_a.move_to,
                  constant, LEFT, ln_a.set_color, BLUE)
        self.wait()
...
class NextVideo(TeacherStudentsScene):
    def construct(self):
        ...
        this_tex = TexMobject("{d(", "a^t", ") \\over dt} = ", "a^t",
                              "\\ln(a)")
        this_tex[1][1].set_color(YELLOW)
        this_tex[3][1].set_color(YELLOW)
        this_tex.next_to(brace, DOWN)

        next_tex = VGroup(*list(
            map(TextMobject, ["Chain rule", "Product rule", "$\\vdots$"])))
        ...
        self.play(
            Write(this_tex[4]),
            ReplacementTransform(this_tex[3][0].copy(),
                                 this_tex[4][3],
                                 path_arc=np.pi,
                                 remover=True))
        self.wait(2)
        self.play(this_tex.replace, this_video)
        self.play(brace.next_to, next_video, DOWN, this_video.restore,
                  Animation(this_tex), next_video.set_color, YELLOW,
                  Write(next_tex),
                  self.get_teacher().change_mode, "raise_right_hand")
        self.change_student_modes(*["pondering"] * 3, look_at_arg=next_tex)
        self.wait(3)
