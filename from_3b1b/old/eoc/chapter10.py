...
def taylor_approximation(func, highest_term, center_point=0):
    derivatives = [
        derivative(func, center_point, n=n) for n in range(highest_term + 1)
    ]
...
class Chapter10OpeningQuote(OpeningQuote):
    CONFIG = {
        "quote": [
            "For me, mathematics is a collection of ", "examples", "; a ",
            "theorem", " is a statement about a collection of ", "examples",
...
class Pendulum(ReconfigurableScene):
    ...
    def show_height(self):
        ...
        self.cosine = VGroup(
            *
            [height_tex.get_part_by_tex(tex) for tex in ("cos", "theta", ")")])
        self.one_minus = VGroup(*[
            height_tex.get_part_by_tex(tex) for tex in ("\\big(1-", "\\big)")
        ])
    ...
    def substitute_approximation(self):
        ...
        cosine.target.next_to(cosine_approx,
                              LEFT,
                              align_using_submobjects=True)
        kwargs = self.get_swing_kwargs()

        self.play(FadeIn(cosine_approx, run_time=2, lag_ratio=0.5),
                  MoveToTarget(cosine), morty.change, "pondering",
                  cosine_approx)
        self.wait()
    ...
    def get_trajectory_dots(self, n_dots=40, color=YELLOW):
        ...
        dots = VGroup(*[
            ...
            Dot(angle_to_point(angle), radius=0.005) for angle in angles
        ])
    ...
    def get_v_line(self):
        return DashedLine(self.anchor_point,
                          self.anchor_point + self.radius * DOWN,
                          color=WHITE)
    ...
    def get_height_brace_tex(self, brace):
        tex_mob = TexMobject("R", "\\big(1-", "\\cos(", "\\theta", ")",
                             "\\big)")
...
class PendulumWithBetterApprox(Pendulum):
    CONFIG = {
        "approx_tex": [
            "\\approx 1 - ", "{\\theta", "^2", "\\over", "2}", "+", "{\\theta",
            "^4", "\\over", "24}"
        ],
        "leave_original_cosine":
        True,
        ...
        v_lines, alt_v_lines = [
            VGroup(*[
                self.get_vertical_line_to_graph(
                    u * dx, high_graph, line_class=DashedLine, color=YELLOW)
                for u in (-1, 1)
            ]) for dx in (0.01, 0.7)
        ]
...
class ConstructQuadraticApproximation(ExampleApproximationWithCos):
    ...
    def add_cosine_graph(self):
        ...
        self.play(ShowCreation(cosine_graph),
    ...
    def add_quadratic_graph(self):
        ...
        self.play(
            ReplacementTransform(self.cosine_graph.copy(),
                                 quadratic_graph,
                                 run_time=3))
    ...
    def introduce_quadratic_constants(self):
        ...
        arrows = VGroup(*[
            Arrow(free_to_change.get_top(),
                  const.get_bottom(),
                  tip_length=0.75 * Arrow.CONFIG["tip_length"],
                  color=const.get_color()) for const in const_terms
        ])
        alt_consts_list = [
            (0, -1, -0.25),
            (1, -1, -0.25),
        ...
        self.play(*[
            ApplyMethod(const.scale_in_place,
                        0.8,
                        run_time=2,
                        rate_func=squish_rate_func(there_and_back, a, a +
                                                   0.75))
            for const, a in zip(const_terms,
                                np.linspace(0, 0.25, len(const_terms)))
        ])
    ...
    def show_value_at_zero(self):
        ...
        ax0_group.next_to(self.cosine_label,
                          RIGHT,
                          align_using_submobjects=True)
        one = TexMobject("1")
        ...
        v_line = self.get_vertical_line_to_graph(0,
                                                 self.cosine_graph,
    ...
    def set_c0_to_one(self):
        ...
        equals_c0.next_to(poly_at_zero.get_part_by_tex("="),
                          DOWN,
                          buff=MED_LARGE_BUFF,
                          aligned_edge=LEFT)
        poly_group = VGroup(
        ...
        self.play(ReplacementTransform(self.quadratic_tex.copy(),
                                       poly_at_zero))
        self.wait(2)
        self.play(FadeIn(equals_c0))
        self.wait(2)
        self.play(
            Transform(poly_group, poly_group_target, run_time=2,
                      lag_ratio=0.5))
        self.wait(2)
    ...
    def compute_cosine_derivative(self):
        ...
        self.play(
            FadeIn(VGroup(derivative, *rhs[:2]), run_time=2, lag_ratio=0.5))
        self.wait(2)
        self.play(Write(VGroup(*rhs[2:])), run_time=2)
        self.wait()
        self.play(
            Rotate(self.tangent_line,
                   np.pi / 12,
                   in_place=True,
                   run_time=3,
                   rate_func=wiggle))
    ...
    def compute_polynomial_derivative(self):
        ...
        derivative_at_zero = self.get_quadratic_derivative("c_1",
                                                           "c_2",
                                                           arg="0")
        ...
        poly_group_target = VGroup(
            TexMobject("=", "0", "+0").set_color_by_tex("0",
                                                        self.colors[1],
                                                        substring=False),
        ...
        self.wait(2)
        self.play(
            Transform(poly_group, poly_group_target, run_time=3,
                      lag_ratio=0.5))
        self.wait(2)
    ...
    def point_out_negative_concavity(self):
        partial_cosine_graph = self.get_graph(np.cos,
                                              x_min=-1,
                                              x_max=1,
                                              color=PINK)
        ...
        for x, run_time in (-1, 2), (1, 4):
            self.play(
                self.get_tangent_line_change_anim(self.tangent_line,
                                                  x,
                                                  self.cosine_graph,
                                                  run_time=run_time))
            self.wait()
        self.play(
            *list(map(FadeOut, [partial_cosine_graph, self.tangent_line])))

    def compute_cosine_second_derivative(self):
        second_deriv, rhs = self.get_cosine_second_derivative()

        self.play(
            FadeIn(VGroup(second_deriv, *rhs[1][:2]),
                   run_time=2,
                   lag_ratio=0.5))
        self.wait(3)
    ...
    def compute_polynomial_second_derivative(self):
        ...
        second_derivs = [
            TexMobject("{d^2 P \\over dx^2}", "(x)", "=", "2", c2)
            for c2 in c2s
        ]
        for deriv, second_deriv in zip(derivs, second_derivs):
            ...
            second_deriv.next_to(deriv,
                                 DOWN,
                                 buff=MED_LARGE_BUFF,
                                 aligned_edge=LEFT)

        poly_group = VGroup(second_derivs[0], derivs[0], self.quadratic_tex)
        poly_group_target = VGroup(second_derivs[1], derivs[1],
    ...
    def box_final_answer(self):
        ...
        box.stretch_to_fit_width(self.quadratic_tex.get_width() +
                                 MED_LARGE_BUFF)
        box.stretch_to_fit_height(self.quadratic_tex.get_height() +
                                  MED_LARGE_BUFF)
    ...
    def get_quadratic_graph(self, c0=1, c1=0, c2=-0.5):
        return self.get_graph(lambda x: c0 + c1 * x + c2 * x**2,
                              color=self.colors[2])

    def get_quadratic_tex(self, c0, c1, c2, arg="x"):
        tex_mob = TexMobject("P(", arg, ")", "=", c0, "+", c1, arg, "+", c2,
                             arg, "^2")
    ...
    def get_quadratic_derivative(self, c1, c2, arg="x"):
        result = TexMobject("{dP \\over dx}", "(", arg, ")", "=", c1, "+", "2",
                            c2, arg)
        ...
        if hasattr(self, "quadratic_tex"):
            result.next_to(self.quadratic_tex,
                           DOWN,
                           buff=MED_LARGE_BUFF,
                           aligned_edge=LEFT)
        return result
    ...
    def get_tangent_line_change_anim(self, tangent_line, new_x, graph,
                                     **kwargs):
        ...
        def update(tangent_line, alpha):
            ...
            new_line = self.get_tangent_line(x,
                                             graph,
                                             color=tangent_line.get_color())
    ...
    def get_cosine_derivative(self):
        ...
        derivative.next_to(self.cosine_label,
                           DOWN,
                           buff=MED_LARGE_BUFF,
                           aligned_edge=LEFT)
    ...
    def get_cosine_second_derivative(self):
        ...
        second_deriv.next_to(self.cosine_derivative,
                             DOWN,
                             buff=MED_LARGE_BUFF,
                             aligned_edge=LEFT)
...
class ReflectOnQuadraticApproximation(TeacherStudentsScene):
    ...
    def show_example_approximation(self):
        approx_at_x, approx_at_point = [
            TexMobject("\\cos(", s, ")", "\\approx", "1 - \\frac{1}{2}", "(",
                       s, ")", "^2").next_to(self.get_students(), UP, 2)
            for s in (
                "x",
                "0.1",
            )
        ...
        real_result = TexMobject("\\cos(", "0.1", ")", "=",
                                 "%.7f\\dots" % np.cos(0.1))
        real_result.shift(
        ...
        self.play(Write(approx_at_x, run_time=2), self.teacher.change_mode,
                  "raise_right_hand")
        self.wait(2)
        ...
        self.change_student_modes(
            *["plain"] * 3,
            added_anims=list(
                map(FadeOut, [approx_at_point, approx_rhs, real_result])),
    ...
    def add_polynomial(self):
        ...
        self.wait(2)
        self.play(*[
            ApplyMethod(const.shift,
                        MED_LARGE_BUFF * UP,
                        run_time=2,
                        rate_func=squish_rate_func(there_and_back, a, a + 0.7))
            for const, a in zip(const_terms,
                                np.linspace(0, 0.3, len(const_terms)))
        ])
        self.wait()
    ...
    def show_c1(self):
        ...
        c1.save_state()
        equation = TexMobject("\\frac{dP}{dx}(0) = \\frac{d(\\cos)}{dx}(0)")
        equation.to_corner(UP + RIGHT)
    ...
    def get_polynomial(self, c0="c_0", c1="c_1", c2="c_2"):
        polynomial = TexMobject("P(x) = ", c0, "+", c1, "x", "+", c2, "x^2")
...
class SimilarityOfChangeBehavior(ConstructQuadraticApproximation):
    def construct(self):
        ...
        dot_anims = [
            UpdateFromAlphaFunc(dot, update_dot, run_time=3) for dot in dots
        ]
...
class CubicAndQuarticApproximations(ConstructQuadraticApproximation):
    ...
    def add_background(self):
        ...
        self.polynomial = TexMobject("P(x)=", "1", "-\\frac{1}{2}", "x^2")
    ...
    def take_third_derivative_of_cubic(self):
        ...
        third_derivative = TexMobject("\\frac{d^3 P}{dx^3}(x) = ", "0")
        ...
        self.play(
            ReplacementTransform(polynomial.quadratic_part.copy(),
    ...
    def show_third_derivative_of_cosine(self):
        ...
        self.play(
            ReplacementTransform(self.cosine_second_derivative.copy(),
    ...
    def set_c3_to_zero(self):
        c3s = VGroup(self.polynomial_third_derivative[-1][-1],
                     self.plus_cubic_term.get_part_by_tex("c_3"))
        zeros = VGroup(*[TexMobject("0").move_to(c3) for c3 in c3s])
    ...
    def show_cubic_curves(self):
        ...
        graph.save_state()
        alt_graphs = [
            self.get_graph(func, color=real_graph.get_color()) for func in [
                lambda x: x * (x - 1) * (x + 1), lambda x: 1 - 0.5 *
                (x**2) + 0.2 * (x**3)
            ]
        ]
    ...
    def show_fourth_derivative_of_cosine(self):
        ...
        self.play(
            ReplacementTransform(self.cosine_third_derivative.copy(),
    ...
    def take_fourth_derivative_of_quartic(self):
        ...
        fourth_deriv_lhs.next_to(self.polynomial,
                                 DOWN,
                                 buff=MED_LARGE_BUFF,
                                 aligned_edge=LEFT)
        alt_rhs = TexMobject("=", "24 \\cdot", "c_4")
        alt_rhs.next_to(fourth_deriv_lhs.get_part_by_tex("="),
                        DOWN,
                        buff=LARGE_BUFF,
                        aligned_edge=LEFT)
    ...
    def solve_for_c4(self):
        c4s = VGroup(self.fourth_deriv_alt_rhs.get_part_by_tex("c_4"),
    ...
    def get_cosine_fourth_derivative(self):
        ...
        fourth_deriv.next_to(self.cosine_third_derivative,
                             DOWN,
                             buff=MED_LARGE_BUFF,
                             aligned_edge=LEFT)
        rhs = TexMobject("=", "\\cos(0)", "=", "1")
...
class FactorialTerms(CubicAndQuarticApproximations):
    def construct(self):
        lhs_list = [
            TexMobject("{d%s" % s, "\\over", "dx%s}" % s, "(", "c_8", "x^8",
                       ")=") for i in range(9)
        ...
        added_anims = [
            ReplacementTransform(start_lhs,
                                 target_lhs,
...
class HigherTermsDontMessUpLowerTerms(Scene):
    ...
    def add_polynomial(self):
        ...
        c0, c2, c4 = [
            polynomial.get_part_by_tex(tex) for tex in (c0_tex, c2_tex, c4_tex)
        ]
        ...
        no_affect_words = TextMobject("Doesn't affect \\\\ previous terms")
    ...
    def show_second_derivative(self):
        second_deriv = TexMobject("{d^2 P \\over dx^2}(", "0", ")", "=", "2",
                                  self.c2_tex, "+", "3 \\cdot 4", self.c4_tex,
                                  "(", "0", ")", "^2")
        ...
        second_deriv.next_to(self.polynomial,
                             DOWN,
                             buff=MED_LARGE_BUFF,
                             aligned_edge=LEFT)
...
class EachTermControlsOneDerivative(Scene):
    def construct(self):
        ...
        deriv_words = VGroup(*[
            TextMobject("Controls \\\\ $%s(0)$" % tex) for tex in [
                "P",
                "\\frac{dP}{dx}",
...
class ApproximateNearNewPoint(CubicAndQuarticApproximations):
    ...
    def shift_approximation_center(self):
        ...
        for target, rt in zip(self.target_approx_centers, [3, 4, 4]):
            self.change_approximation_center(quartic_graph,
                                             dot,
                                             target,
                                             run_time=rt)
    ...
    def change_approximation_center(self, graph, dot, target, **kwargs):
        ...
        self.play(UpdateFromAlphaFunc(graph, update_quartic),
                  UpdateFromAlphaFunc(dot, update_dot), **kwargs)
    ...
    def get_polynomial(self, arg, center_tex):
        result = TexMobject(
            "P_{%s}(x)" % center_tex, "=", "c_0",
            *it.chain(*[["+", "c_%d" % d, "%s^%d" % (arg, d)]
                        for d in range(1, 5)]))
    ...
    def get_quartic_approximation(self, a):
        ...
        func = lambda x: sum([(c / math.factorial(n)) * (x - a)**n
...
class TranslationOfInformation(CubicAndQuarticApproximations):
    ...
    def mention_information_exchange(self):
        deriv_info = TextMobject("Derivative \\\\ information \\\\ at a point")
        ...
        center_v_line = self.get_vertical_line_to_graph(0,
                                                        self.cosine_graph,
                                                        line_class=DashedLine,
                                                        color=YELLOW)
        outer_v_lines = VGroup(
            *[center_v_line.copy().shift(vect) for vect in (LEFT, RIGHT)])
        ...
        self.play(
            ReplacementTransform(VGroup(center_v_line).copy(), outer_v_lines))
        self.play(
        ...
        for x in -1, 1, 0:
            ...
            self.play(
                UpdateFromAlphaFunc(
                    dot,
                    lambda d, a: d.move_to(
                        self.input_to_graph_point(interpolate(start_x, x, a),
                                                  self.cosine_graph)),
                    run_time=2))
        self.wait()
        self.play(*list(
            map(FadeOut, [deriv_info, arrow, output_info, outer_v_lines])))
    ...
    def show_derivative_pattern(self):
        derivs_at_x, derivs_at_zero = [
            VGroup(*[
                TexMobject(tex, "(", arg, ")")
                for tex in ["\\cos", "-\\sin", "-\\cos", "\\sin", "\\cos"]
            ]) for arg in ("x", "0")
        ]
        arrows = VGroup(*[
            Arrow(UP, ORIGIN, color=WHITE, buff=0, tip_length=MED_SMALL_BUFF)
        ...
        zeros = VGroup(
            *[deriv.get_part_by_tex("0") for deriv in derivs_at_zero])

        self.play(FadeIn(derivs_at_x[0]))
        self.wait()
        for start_d, arrow, target_d in zip(group[::2], group[1::2],
                                            group[2::2]):
            self.play(ReplacementTransform(start_d.copy(), target_d),
                      ShowCreation(arrow))
            self.wait()
        self.wait()
        ...
        self.wait()
        self.play(*[
            ReplacementTransform(zero.copy(),
                                 self.dot,
                                 run_time=3,
        ...
        self.cosine_derivative_group = VGroup(derivs_at_zero, arrows,
                                              group[-1], rhs_group)
    ...
    def show_polynomial(self):
        ...
        self.play(*[
            ...
            for dv, pc, a in zip(derivative_values, polynomial.coefficients,
    ...
    def name_taylor_polynomial(self):
        brace = Brace(
            VGroup(self.polynomial.coefficients, self.polynomial.factorials),
            DOWN)
        ...
        self.wait()
        self.play(Transform(self.quadratic_graph, quartic_graph, run_time=2),
                  Animation(self.dot))
        self.wait(2)
    ...
    def draw_new_function_graph(self):
        ...
        self.play(*list(
            map(FadeOut, [
                self.cosine_derivative_group, self.cosine_graph,
                self.quadratic_graph, self.v_line, self.dot
            ])))
    ...
    def write_general_function_derivative(self):
        derivs_at_x, derivs_at_zero, derivs_at_a = deriv_lists = [
            VGroup(*[
                TexMobject("\\text{$%s$}" % args[0], *args[1:]) for args in [
                    ("f", "(", arg, ")"),
                    ("\\frac{df}{dx}", "(", arg, ")"),
                    ("\\frac{d^2 f}{dx^2}", "(", arg, ")"),
                    ("\\frac{d^3 f}{dx^3}", "(", arg, ")"),
        ...
        self.v_line.put_start_and_end_on(self.graph_origin,
                                         self.dot.get_center())
        ...
        self.play(
            ReplacementTransform(zeros.copy(),
                                 self.dot,
                                 run_time=2,
                                 lag_ratio=0.5))
    ...
    def replace_coefficients_in_generality(self):
        ...
        self.play(
            ReplacementTransform(self.polynomial,
                                 new_polynomial,
                                 run_time=2,
                                 lag_ratio=0.5),
    ...
    def walk_through_terms(self):
        ...
        approx_graphs = [
            self.get_graph(taylor_approximation(func, n), color=WHITE)
            for n in range(7)
        ]
        ...
        braces = [
            Brace(VGroup(left_mob, *right_mobs[:n]), DOWN)
    ...
    def show_polynomial_around_a(self):
        new_polynomial = self.get_polynomial(
            "(x-a)",
            *[tex_mob.get_tex_string() for tex_mob in self.derivs_at_a[:-2]])
        new_polynomial.to_corner(UP + RIGHT)
        ...
        def get_update_function(target_x):
            def update(group, alpha):
                ...
                new_approx_graph = self.get_graph(taylor_approximation(
        ...
        self.play(
            ...
            Animation(self.polynomial), Animation(polynomial_third_term))
        self.wait()
...
class ThisIsAStandardFormula(TeacherStudentsScene):
    def construct(self):
        ...
        self.change_student_modes(*["sad"] * 3,
...
class ExpPolynomial(TranslationOfInformation, ExampleApproximationWithExp):
    ...
    def show_derivatives(self):
        ...
        derivs_at_x, derivs_at_zero = [
            VGroup(*[TexMobject("e^%s" % s).set_color(c) for c in self.colors])
            for s in ("x", "0")
        ]
...
class ShowSecondTerm(TeacherStudentsScene):
    def construct(self):
        colors = CubicAndQuarticApproximations.CONFIG["colors"]
        polynomial = TexMobject("f(a)", "+", "\\frac{df}{dx}(a)", "(x - a)",
...
class SecondTermIntuition(AreaIsDerivative):
    ...
    def nudge_end_point(self):
        ...
        self.play(FadeIn(dark_area),
                  *list(map(Animation, self.foreground_mobjects)))
        self.play(FadeOut(self.T_label_group), FadeIn(dx_label_group),
                  FadeIn(rect), FadeIn(height_brace))
        self.wait()
    ...
    def point_out_triangle(self):
        ...
        triangle.replace(Line(self.rect.get_corner(UP + LEFT),
    ...
    def relabel_start_and_end(self):
        ...
        for vect, tex in (LEFT, "a"), (RIGHT, "x"):
            ...
            arrow = Arrow(label.get_corner(UP - vect),
                          point,
                          buff=SMALL_BUFF,
                          tip_length=0.2,
                          color=WHITE)
    ...
    def compute_triangle_area(self):
        ...
        base_line = Line(
            *[triangle.get_corner(DOWN + vect) for vect in (LEFT, RIGHT)])
        ...
        base_arrow = Arrow(base_label.get_left(),
        ...
        height_labels = [
            TexMobject("\\text{Height} = ", s, "(x-a)") for s in
            ["(\\text{Slope})", "\\frac{d^2 f_{\\text{area}}}{dx^2}(a)"]
        ]
        ...
        self.play(FadeIn(base_label), ShowCreation(base_arrow),
        ...
        group.target[-1].next_to(group.target[-2],
                                 RIGHT,
                                 buff=SMALL_BUFF,
                                 align_using_submobjects=True)
    ...
    def walk_through_taylor_terms(self):
        mini_area, mini_rect, mini_triangle = [
            mob.copy() for mob in (self.dark_area, self.rect, self.triangle)
        ]
        ...
        self.wait()
        self.play(FadeIn(VGroup(*analytic_taylor[:3])),
                  self.dark_area.set_fill, BLUE_E, 1,
                  Transform(self.__name__, new_func_name))
        self.wait()
        self.play(self.rect.scale_in_place, 0.5, rate_func=there_and_back)
        self.play(FadeIn(VGroup(*analytic_taylor[3:5])))
        self.wait(2)
        self.play(self.triangle.scale_in_place, 0.5, rate_func=there_and_back)
...
class EachTermHasMeaning(TeacherStudentsScene):
    def construct(self):
        ...
        self.teacher_says("Each term \\\\ has meaning!",
                          target_mode="hooray",
                          bubble_kwargs={
                              "height": 3,
                              "width": 4
                          })
        self.change_student_modes(*["thinking"] * 3, look_at_arg=4 * UP)
        self.wait(3)
...
class AskAboutInfiniteSum(TeacherStudentsScene):
    ...
    def ask_question(self):
        ...
        series = TexMobject("\\cos(x)", "\\approx",
        ...
        self.play(RemovePiCreatureBubble(self.teacher),
                  ...
                  FadeIn(big_rect), randy.change_mode, "pondering")
    ...
    def name_taylor_series(self):
        series_def = TextMobject("Infinite sum $\\Leftrightarrow$ series")
        ...
        self.wait(2)
        self.play(series_def.scale, 0.7, series_def.to_corner, UP + RIGHT,
...
class ConvergenceExample(Scene):
    def construct(self):
        ...
        series = TexMobject(*list(
            it.chain(*[["\\frac{1}{%d}" % (3**n), "+"]
                       for n in range(1, max_shown_power)])) + ["\\cdots"])
    ...
    def get_partial_sum_braces(self, series, partial_sums):
        braces = [
            Brace(VGroup(*series[:n])) for n in range(1,
                                                      len(series) - 1, 2)
        ]
        last_brace = braces[-1]
        braces += [
...
class ExpConvergenceExample(ConvergenceExample):
    ...
    def get_series(self, arg, n_terms=5):
        series = TexMobject(
            "1", "+",
            *list(
                it.chain(*[["\\frac{%s^%d}{%d!}" % (arg, n, n), "+"]
                           for n in range(1, n_terms + 1)])) + ["\\cdots"])
...
class ExpGraphConvergence(ExpPolynomial, ExpConvergenceExample):
    ...
    def construct(self):
        ...
        v_line = self.get_vertical_line_to_graph(self.example_input,
                                                 graph,
                                                 line_class=DashedLine,
                                                 color=YELLOW)
        dot = Dot(color=YELLOW)
        ...
        self.play(dot.move_to, self.input_to_graph_point(x, graph),
                  ShowCreation(v_line))
        self.wait()
    ...
    def add_series(self):
        ...
        braces = self.get_partial_sum_braces(series,
                                             np.zeros(self.n_iterations))
        for brace in braces:
    ...
    def get_approx_graphs(self):
        def get_nth_approximation(n):
            return lambda x: sum(
                [float(x**k) / math.factorial(k) for k in range(n + 1)])
        ...
        for approx_graph, color in zip(approx_graphs, colors):
            ...
            dot = Dot(color=WHITE)
            dot.move_to(
                self.input_to_graph_point(self.example_input, approx_graph))
...
class BoundedRadiusOfConvergence(CubicAndQuarticApproximations):
    ...
    def construct(self):
        self.setup_axes()
        func = lambda x: (np.sin(x**2 + x) + 0.5) * (np.log(x + 1.01) + 1
                                                     ) * np.exp(-x)
        ...
        outer_v_lines = VGroup(*[
            DashedLine(self.coords_to_point(x, -2),
                       self.coords_to_point(x, 2),
                       color=WHITE) for x in (-1, 1)
        ])

        colors = list(self.colors) + [GREEN, MAROON_B, PINK]
        approx_graphs = [
            self.get_graph(taylor_approximation(func, n), color=color)
...
class RadiusOfConvergenceForLnX(ExpGraphConvergence):
    ...
    def add_series(self):
        series = TexMobject("\\ln(x) \\rightarrow", "(x-1)", "-",
                            "\\frac{(x-1)^2}{2}", "+", "\\frac{(x-1)^3}{3}",
                            "-", "\\frac{(x-1)^4}{4}", "+", "\\cdots")
        lhs = VGroup(*series[1:])
        series.add_background_rectangle()
        series.scale(0.8)
        ...
        self.braces = self.get_partial_sum_braces(lhs,
                                                  np.zeros(self.n_iterations))
    ...
    def show_bounds(self):
        ...
        v_lines = [
            DashedLine(*[self.coords_to_point(x, y) for y in (-2, 2)])
            for x in (0, 1, 2)
        ]
        outer_v_lines = VGroup(*v_lines[::2])
        center_v_line = VGroup(v_lines[1])
        input_v_line = Line(*[
            self.coords_to_point(self.convergent_example, y) for y in (-4, 3)
        ])
    ...
    def show_converging_point(self):
        approx_graphs = [
            self.get_graph(taylor_approximation(self.func, n, 1), color=WHITE)
        ...
        for graph in approx_graphs:
            dot = Dot(color=WHITE)
            dot.move_to(
                self.input_to_graph_point(self.convergent_example, graph))
            graph.dot = dot
            graph.add(dot)
        ...
        self.play(*it.chain(list(map(FadeIn, [approx_graph, brace])),
                            list(map(Animation, self.foreground_mobjects))))
        self.wait()
        new_graphs = approx_graphs[1:self.initial_n_iterations]
        for new_graph, new_brace in zip(new_graphs, self.braces[1:]):
            self.play(Transform(approx_graph, new_graph),
                      Transform(brace, new_brace),
                      *list(map(Animation, self.foreground_mobjects)))
            self.wait()
        approx_graph.remove(approx_dot)
        self.play(
            approx_dot.move_to, self.coords_to_point(self.divergent_example,
                                                     0),
            *it.chain(list(map(FadeOut, [approx_graph, brace])),
                      list(map(Animation, self.foreground_mobjects))))
        self.wait()
    ...
    def show_diverging_point(self):
        for graph in self.approx_graphs:
            graph.dot.move_to(
                self.input_to_graph_point(self.divergent_example, graph))

        approx_graph = self.approx_graphs[0].deepcopy()
        brace = self.braces[0].copy()

        self.play(ReplacementTransform(self.approx_dot, approx_graph.dot),
                  FadeIn(approx_graph[0]), FadeIn(brace),
                  *list(map(Animation, self.foreground_mobjects)))

        new_graphs = self.approx_graphs[1:self.initial_n_iterations]
        for new_graph, new_brace in zip(self.approx_graphs[1:],
                                        self.braces[1:]):
            self.play(Transform(approx_graph, new_graph),
                      Transform(brace, new_brace),
                      *list(map(Animation, self.foreground_mobjects)))
    ...
    def write_divergence(self):
        ...
        for new_graph in new_graphs:
            self.play(Transform(self.approx_graph, new_graph),
                      *list(map(Animation, self.foreground_mobjects)))
            self.wait()
...
class MoreToBeSaid(TeacherStudentsScene):
    ...
    def construct(self):
        words = TextMobject("Lagrange error bounds, ", "convergence tests, ",
                            "$\\dots$")
        ...
        self.teacher_says("There's still \\\\ more to learn!",
                          target_mode="surprised",
                          bubble_kwargs={
                              "height": 3,
                              "width": 4
        ...
        for word in words:
            ...
            self.wait()
        self.teacher_says("About everything", )
        self.change_student_modes(*["pondering"] * 3)
        self.wait()
        self.remove()
        self.pi_creatures = []  ##Hack
        self.play(RemovePiCreatureBubble(self.teacher), FadeOut(words),
                  FadeIn(fade_rect), randy.change, "happy", rect)
