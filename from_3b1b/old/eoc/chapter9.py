...
class AverageOfContinuousVariable(GraphScene):
    ...
    def construct(self):
        self.setup_axes()
        graph = self.get_graph(lambda x: 0.1 * x * (x - 3) * (x - 6) + 4)
        graph_label = self.get_graph_label(graph, "f(x)")
        boundary_lines = self.get_vertical_lines_to_graph(
            graph, *self.bounds, num_lines=2, line_class=DashedLine)
        ...
        question = TextMobject("What is the average \\\\ value of $f(x)$?")
...
class ThisVideo(TeacherStudentsScene):
    def construct(self):
        ...
        self.teacher_says("A new view of \\\\ the fundamental theorem",
...
class AverageOfSineStart(AverageOfContinuousVariable):
    ...
    def add_graph(self, run_time=1):
        ...
        graph_label = self.get_graph_label(graph, "\\sin(x)", direction=UP)
        self.play(ShowCreation(graph), Write(graph_label), run_time=run_time)
    ...
    def ask_about_average(self):
        ...
        v_line = self.get_vertical_line_to_graph(midpoint,
                                                 self.graph,
                                                 line_class=DashedLine,
                                                 color=WHITE)

        self.play(FadeIn(half_period_graph))
        self.play(Write(question, run_time=2), ShowCreation(arrow))
        self.play(ShowCreation(v_line))
        for bound in self.bounds + [midpoint]:
            self.play(
                self.get_v_line_change_anim(v_line,
                                            self.graph,
                                            bound,
                                            run_time=3))
...
class LengthOfDayGraph(GraphScene):
    ...
    def add_graph(self):
        ...
        for label in y_label, x_label:
            self.play(FadeIn(label, run_time=2, lag_ratio=0.5))
        self.play(
    ...
    def show_solar_pannel(self):
        ...
        self.play(randy.change, "thinking", panels.get_right(),
                  FadeIn(panels, run_time=2, lag_ratio=0.5))
    ...
    def set_color_summer_months(self):
        ...
        summer_rect.replace(Line(self.graph_origin,
        ...
        summer_words, winter_words = [
            TextMobject("%s \\\\ months" % s).move_to(rect) for s, rect in [
                ("Summer", summer_rect),
                ("Winter", winter_rect),
            ]
        ]

        for rect, words in (summer_rect, summer_words), (winter_rect,
                                                         winter_words):
            self.play(FadeIn(rect), Write(words, run_time=2))
        self.wait()
    ...
    def mention_constants(self):
        ...
        self.play(*[
            ApplyFunction(lambda c: c.scale_in_place(0.9).shift(
                SMALL_BUFF * DOWN).set_color(RED),
                          constant,
                          run_time=3,
                          rate_func=squish_rate_func(there_and_back, a, a +
                                                     0.7))
            for constant, a in zip(constants,
...
class AverageOfFiniteSet(Scene):
    CONFIG = {"lengths": [1, 4, 2, 5]}

    def construct(self):
        lengths = self.lengths
        lines = VGroup(*[Line(ORIGIN, length * RIGHT) for length in lengths])
        ...
        symbols = VGroup(*plusses + [TexMobject("=")])
        ...
        dividing_lines = VGroup(*[
            ...
            for alpha in np.linspace(0, 1,
                                     len(lengths) + 1)
            for p in [
                interpolate(lines.target.get_left(), lines.target.get_right(),
                            alpha)
            ]
        ])
        ...
        self.add(lines)
        self.play(FadeIn(labels, lag_ratio=0.5, run_time=3))
        self.wait()
        ...
        self.play(*it.chain(list(map(Write, averages)),
...
class TryToAddInfinitelyManyPoints(AverageOfSineStart):
    ...
    def try_to_add_infinitely_many_values(self):
        v_lines = VGroup(*[
            self.get_vertical_line_to_graph(numerator * np.pi / denominator,
                                            self.graph,
                                            color=YELLOW,
        ...
        self.wait(2)
        self.play(MoveToTarget(v_lines, run_time=3, lag_ratio=0.5),
                  Write(plusses))
    ...
    def show_continuum(self):
        ...
        input_range = Line(
            *[self.coords_to_point(bound, 0) for bound in self.bounds])
        VGroup(arrow, input_range).set_color(RED)

        self.play(FadeIn(arrow))
        self.play(arrow.next_to, input_range.get_start(), DOWN + RIGHT,
                  SMALL_BUFF)
        self.play(
            arrow.next_to,
            input_range.copy().get_end(),
            DOWN + RIGHT,
            SMALL_BUFF,
            ShowCreation(input_range),
            run_time=3,
        )
        self.play(arrow.next_to,
                  input_range.get_start(),
                  DOWN + RIGHT,
                  SMALL_BUFF,
                  run_time=3)
...
class FiniteSample(TryToAddInfinitelyManyPoints):
    ...
    def show_finite_sample(self):
        ...
        plusses = VGroup(*[TexMobject("+").scale(0.75) for l in v_lines])
        numerator = VGroup(*it.chain(*list(zip(summed_v_lines, plusses))))
        for group in numerator, plusses:
            group.remove(plusses[-1])
        numerator.arrange(RIGHT, buff=SMALL_BUFF, aligned_edge=DOWN)
        ...
        self.wait()
        self.play(
            ReplacementTransform(v_lines.copy(),
                                 summed_v_lines,
                                 run_time=3,
                                 lag_ratio=0.5),
            Write(plusses, rate_func=squish_rate_func(smooth, 0.3, 1)))
    ...
    def get_sample_lines(self, dx, color=YELLOW, stroke_width=2):
        return VGroup(*[
            ...
            ) for x in np.arange(self.bounds[0] + dx, self.bounds[1], dx)
        ])
...
class FeelsRelatedToAnIntegral(TeacherStudentsScene):
    def construct(self):
        self.student_says("Seems integral-ish...", target_mode="maybe")
        self.play(self.teacher.change_mode, "happy")
        self.wait(2)
...
class IntegralOfSine(FiniteSample):
    ...
    def show_riemann_rectangles(self):
        ...
        flat_rects = self.get_riemann_rectangles(self.get_graph(lambda x: 0),
                                                 **kwargs)
        thin_kwargs = dict(kwargs)
        thin_kwargs["dx"] = self.thin_dx
        thin_kwargs["stroke_width"] = 0
        self.thin_rects = self.get_riemann_rectangles(self.graph,
                                                      **thin_kwargs)
    ...
    def let_dx_approach_zero(self, restore=True):
        ...
        self.transform_between_riemann_rects(self.rects,
                                             self.thin_rects,
                                             run_time=3)
        self.wait(2)
        if restore:
    ...
    def bring_back_average(self):
        ...
        #Bring back average
        self.play(self.average.restore,
                  self.average.center,
                  self.average.to_edge,
                  UP,
                  self.integral.to_edge,
                  DOWN,
                  run_time=2)
        self.wait()
        self.play(
        ...
        self.wait()
        self.play(FadeOut(self.dx_label), FadeIn(example_dx))
        ...
        self.play(*[
            ApplyFunction(
                ...
                run_time=3,
            ) for line, a in zip(self.v_lines,
        ...
        self.play(ShowCreation(input_range))
        self.play(
            ReplacementTransform(self.x_axis_labels[1].copy(),
                                 rhs.get_part_by_tex("pi")))
        self.play(FadeOut(input_range))
        self.play(
            ReplacementTransform(example_dx.copy(),
                                 rhs.get_part_by_tex("0.1")),
            Write(rhs.get_part_by_tex("/")))
        self.wait(2)

        #Substitute number of samples
        self.play(ReplacementTransform(example_dx, self.dx_label))
        dx = rhs.get_part_by_tex("0.1")
        self.play(Transform(dx, TexMobject("dx").move_to(dx)))
        self.wait(2)
        approx = rhs.get_part_by_tex("approx")
        rhs.remove(approx)
        self.play(FadeOut(num_samples), FadeOut(num_samples_copy),
                  FadeOut(approx), rhs.next_to, self.average[1], DOWN)
        self.wait()
    ...
    def distribute_dx(self):
        ...
        self.play(MoveToTarget(dx, path_arc=np.pi / 2), Write(parens),
                  frac_line.stretch_to_fit_width,
                  parens.get_width() + dx.get_width(), frac_line.shift,
                  dx.get_width() * RIGHT / 2, FadeOut(over))
        self.wait(2)
        ...
        self.play(average.scale, 0.9, average.next_to, arrow,
                  LEFT, average.shift_onto_screen, ShowCreation(arrow),
                  Write(over_pi), MoveToTarget(integral, run_time=2))
        self.wait(2)
...
class LetsSolveThis(TeacherStudentsScene):
    def construct(self):
        expression = TexMobject("{\\int_0^\\pi ", " \\sin(x)",
                                "\\,dx \\over \\pi}")
        expression.to_corner(UP + LEFT)
        question = TextMobject("What's the antiderivative \\\\ of",
                               "$\\sin(x)$", "?")
...
class Antiderivative(AverageOfSineStart):
    ...
    def negate_derivative_of_cosine(self):
        cos, neg_cos, sin, neg_sin = graphs = [
            self.get_graph(func) for func in [
                np.cos,
                lambda x: -np.cos(x),
                np.sin,
                lambda x: -np.sin(x),
        ...
        for graph, label, x_val, vect in zip(graphs, labels, x_vals, vects):
            graph.label = self.get_graph_label(graph,
                                               label,
                                               x_val=x_val,
                                               direction=vect,
                                               buff=SMALL_BUFF)
        ...
        for F, f in ("\\cos", "-\\sin"), ("-\\cos", "\\sin"):
            deriv = TexMobject("{d(", F, ")", "\\over\\,", "dx}", "(x)", "=",
                               f, "(x)")
        ...
        for graph in cos, neg_sin:
            self.play(ShowCreation(graph, rate_func=smooth),
                      Write(graph.label,
        ...
        self.wait()
        self.play(*[
            ReplacementTransform(*pair) for pair in [
                (derivs),
                (cos, neg_cos),
    ...
    def walk_through_slopes(self):
        ...
        ss_group = self.get_secant_slope_group(0,
                                               neg_cos,
                                               dx=0.001,
        ...
        self.play(*[ApplyMethod(m.fade, 0.6) for m in faders])
        self.wait()
        ...
        v_line_anim = self.get_v_line_change_anim(v_line, sin_copy, 2 * np.pi,
                                                  **kwargs)
        self.animate_secant_slope_group_change(
            ss_group,
            target_x=2 * np.pi,
            added_anims=[ShowCreation(sin_copy, **kwargs), v_line_anim],
            **kwargs)
        self.play(*list(map(FadeOut, [ss_group, v_line, sin_copy])))
        self.wait()
    ...
    def apply_ftoc(self):
        deriv = self.deriv
        integral = TexMobject("\\int", "^\\pi", "_0", "\\sin(x)", "\\,dx")
        rhs = TexMobject(
            "=",
            "\\big(",
            "-\\cos",
        ...
        (start_pi, end_pi), (start_zero, end_zero) = start_end_pairs = [[
            m.get_part_by_tex(tex) for m in (integral, rhs)
        ] for tex in ("\\pi", "0")]
        ...
        self.play(
            FadeIn(
                VGroup(
                    *[part for part in rhs if part not in [end_pi, end_zero]]),
                lag_ratio=0.5,
                run_time=2,
            ))
        self.wait()
        for start, end in start_end_pairs:
            self.play(
                ReplacementTransform(start.copy(),
                                     end,
                                     path_arc=np.pi / 6,
                                     run_time=2))
            self.wait()
    ...
    def show_difference_in_antiderivative(self):
        ...
        self.play(ReplacementTransform(cos_tex.copy(), pi_dot))
        self.wait()
    ...
    def comment_on_area(self):
        ...
        area_two = TexMobject("2").replace(self.two_height_label)
    ...
    def divide_by_pi(self):
        ...
        frac_lines = VGroup(*[
            TexMobject("\\over\\,").stretch_to_fit_width(
                mob.get_width()).move_to(mob)
            for mob in (integral, rhs_without_eq)
        ])
        frac_lines.shift((integral.get_height() / 2 + SMALL_BUFF) * DOWN)
        ...
        answer.next_to(frac_lines, RIGHT, align_using_submobjects=True)
        answer.shift_onto_screen()

        self.play(equals.next_to, frac_lines[0].copy(), RIGHT,
        ...
        self.play(*[
            ReplacementTransform(
                rhs.get_part_by_tex(tex, substring=False).copy(),
                pi_minus_zeros[1].get_part_by_tex(tex))
            for tex in ("\\pi", "-", "0")
        ])
        self.wait(2)

        full_equation = VGroup(integral, frac_lines, rhs, pi_minus_zeros)
        ...
        self.antiderivative_fraction = VGroup(rhs_without_eq, frac_lines[1],
                                              pi_minus_zeros[1])
        self.integral_fraction = VGroup(integral, frac_lines[0],
                                        pi_minus_zeros[0], equals)
    ...
    def set_color_antiderivative_fraction(self):
        ...
        self.play(FadeIn(big_rect), FadeIn(morty), Animation(fraction))
        self.play(morty.change, "raise_right_hand", fraction)
        self.play(Blink(morty))
        self.wait(2)
        self.play(FadeOut(big_rect), FadeOut(morty), Animation(fraction))
    ...
    def show_slope(self):
        ...
        self.play(FadeOut(self.rects), FadeOut(self.area_two))
        self.play(ShowCreation(new_v_line))
        self.play(ShowCreation(new_h_line), Write(pi))
        self.wait()
...
class GeneralAverage(AverageOfContinuousVariable):
    ...
    def add_graph(self):
        ...
        graph_label = self.get_graph_label(graph, "f(x)")
        v_lines = VGroup(*[
            self.get_vertical_line_to_graph(x, graph, line_class=DashedLine)
        ...
        self.play(ShowCreation(graph),
                  ...
                  run_time=2)
        for line, label in zip(v_lines, labels):
            self.play(Write(label), ShowCreation(line))
        self.wait()
    ...
    def ask_about_average(self):
        v_line = self.get_vertical_line_to_graph(self.bounds[0],
                                                 self.graph,
                                                 line_class=DashedLine,
                                                 color=WHITE)
        average = TextMobject("Average = ")
        fraction = TexMobject("{\\displaystyle \\int", "^b", "_a", "f(x)",
                              "\\,dx", "\\over", "b", "-", "a}")
        ...
        self.play(
            ...
            Write(VGroup(*[
                ...
                for tex in ("int", "f(x)", "dx", "over")
                  ...
                  run_time=4),
            *[
                ReplacementTransform(label.copy(),
                                     fraction.get_part_by_tex(tex,
                                                              substring=False),
                                     run_time=2)
                for label, tex in zip(self.bounds_labels, ["_a", "^b"])
            ])
        self.play(
            next_v_line_anim(),
            Write(
                ...
                ReplacementTransform(label.copy(),
                                     ...
                                     rate_func=squish_rate_func(
                                         smooth, 0.25, 0.75))
                for label, tex in zip(self.bounds_labels, ["a}", "b"])
            ])
    ...
    def show_signed_area(self):
        ...
        self.play(FadeIn(rects, run_time=2, lag_ratio=0.5))
    ...
    def put_area_away(self):
        self.play(
            FadeOut(self.area),
            self.average_expression.scale,
            0.75,
            self.average_expression.to_corner,
            DOWN + RIGHT,
        )
        self.wait()

    def show_finite_sample(self):
        v_lines = self.get_vertical_lines_to_graph(self.graph,
                                                   x_min=self.bounds[0],
                                                   x_max=self.bounds[1],
                                                   color=GREEN)
        for line in v_lines:
        ...
        self.play(ReplacementTransform(dx.copy(), rhs.get_part_by_tex("dx")))
        self.wait(2)

        self.play(
            FadeIn(add_up_f_over), *[
                ApplyFunction(lambda m: m.fade().set_stroke(width=2), v_line)
                for v_line in v_lines
            ])
        self.play(*[
            ApplyFunction(lambda m: m.restore().set_stroke(width=5),
                          v_line,
                          run_time=3,
                          rate_func=squish_rate_func(there_and_back, a, a +
                                                     0.2))
        ...
        self.play(
            ...
            RIGHT,
            SMALL_BUFF,
            frac_line.scale_about_point,
            1.2,
            frac_line.get_left(),
            frac_line.stretch_to_fit_height,
            frac_line.get_height(),
        )
        ...
        double_arrow.next_to(int_fraction.target.get_part_by_tex("over"), LEFT)

        self.play(MoveToTarget(int_fraction),
                  VGroup(add_up_f_over, rhs_copy).shift, 0.4 * DOWN)
        self.play(Write(double_arrow))
        self.play(*list(map(FadeOut, [dx, brace, num_samples, approx, rhs])))
        self.wait()
    ...
    def show_improving_samples(self):
        ...
        new_v_lines_list = [
            self.get_vertical_lines_to_graph(
                ...
                color=GREEN,
                stroke_width=float(stroke_width) / n) for n in range(1, 4)
        ]
        ...
        for new_v_lines in new_v_lines_list:
            self.play(
                Transform(self.v_lines, new_v_lines, run_time=2,
                          lag_ratio=0.5))
        self.wait()
...
class GeneralAntiderivative(GeneralAverage):
    ...
    def add_antiderivative_graph(self):
        ...
        antideriv_graph = self.get_graph(
            lambda x: scipy.integrate.quad(self.graph.underlying_function, 1, x
                                           )[0],
            color=YELLOW,
            x_max=x_max,
        )
        antideriv_graph_label = self.get_graph_label(antideriv_graph,
                                                     "F(x)",
                                                     x_val=x_max)

        deriv = TexMobject("{dF", "\\over", "dx}", "(x)", "=", "f(x)")
        deriv.set_color_by_tex("dF", antideriv_graph.get_color())
        deriv.set_color_by_tex("f(x)", BLUE)
        deriv.next_to(antideriv_graph_label, DOWN, MED_LARGE_BUFF, LEFT)

        self.play(
            ShowCreation(antideriv_graph),
            Write(antideriv_graph_label,
                  rate_func=squish_rate_func(smooth, 0.5, 1)),
            run_time=2,
        )
        self.wait()
    ...
    def show_average_in_terms_of_F(self):
        ...
        self.play(*[
            ReplacementTransform(label.copy(),
                                 ...
                                 rate_func=squish_rate_func(
                                     smooth, a, a + 0.7))
            for label, tex, a in zip(self.bounds_labels, "ab", [0, 0.3])
        ])
        self.wait()
        ...
        self.play(
            ...
            a_label.shift,
            0.7 * a_label.get_width() * LEFT,
            a_label.shift,
            2.2 * a_label.get_height() * UP,
        )
        self.play(denominator_copy.next_to, h_line_brace, DOWN, SMALL_BUFF)
        self.wait(3)
    ...
    def show_change_in_height(self, numerator):
        ...
        self.play(self.antideriv_graph.fade, 0.7,
                  Animation(graph_within_bounds))
        self.play(
            ShowCreation(v_line),
            b_label.shift,
            b_label.get_width() * RIGHT,
            b_label.shift,
            1.75 * b_label.get_height() * DOWN,
        )
        self.play(ShowCreation(h_line))
        self.play(
            numerator.scale,
        ...
        self.antideriv_graph.add(graph_within_bounds, v_line, h_line,
                                 numerator, brace)
        self.h_line = h_line
    ...
    def shift_antideriv_graph_up_and_down(self):
        for vect in 2 * UP, 3 * DOWN, UP:
            self.play(self.antideriv_graph.shift, vect, run_time=2)
        self.wait()
...
class ASecondIntegralSensation(TeacherStudentsScene):
    def construct(self):
        ...
        continuum = UnitInterval(color=GREY, unit_size=6)
        ...
        sigma_to_integral = TexMobject("\\sum \\Leftrightarrow \\int")
        sigma_to_integral.next_to(self.teacher.get_corner(UP + LEFT), UP,
                                  MED_LARGE_BUFF)

        self.teacher_says("A second integral \\\\ sensation")
        ...
        self.change_student_modes(*["pondering"] * 3, look_at_arg=numbers)
...
class Thumbnail(GraphScene):
    ...
    def construct(self):
        ...
        line = self.get_secant_slope_group(0.75 * np.pi, cosine,
                                           dx=0.01).secant_line
        ...
        double_arrow = DoubleArrow(area.get_bottom(),
                                   slope.get_left(),
                                   color=YELLOW,
                                   tip_length=0.75,
                                   buff=MED_LARGE_BUFF)
