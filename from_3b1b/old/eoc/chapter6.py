...
class Chapter6OpeningQuote(OpeningQuote):
    CONFIG = {
        "quote": [
            "Do not ask whether a ", "statement is true until",
...
class ThisWasConfusing(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(self.get_teacher().change_mode, "raise_right_hand",
                  words.set_fill, None, 1, words.shift, 0.5 * UP)
...
class SlopeOfCircleExample(ZoomedScene):
    ...
    def introduce_circle(self):
        ...
        equation.next_to(circle.point_from_proportion(1. / 8), UP + RIGHT)
    ...
    def talk_through_pythagorean_theorem(self):
        ...
        for line, tex in zip(lines, "5xy"):
            ...
            label.next_to(line.get_center(),
                          rotate_vector(UP, line.get_angle()),
                          buff=SMALL_BUFF)
            self.play(ShowCreation(line), Write(label))
            labels.add(label)
        ...
        movers = labels.copy()
        pairs = list(
            zip([movers[1], movers[2], movers[0]],
                self.circle_equation[1][0:-1:3]))
        self.play(*[ApplyMethod(m1.replace, m2) for m1, m2 in pairs])
        self.wait()
    ...
    def show_dx_and_dy(self):
        ...
        for line, tex in (dx_line, "dx"), (dy_line, "dy"):
            ...
            next_to_vect = np.round(rotate_vector(DOWN, line.get_angle()))
            label.next_to(line,
                          next_to_vect,
        ...
        self.wait()
        self.play(self.little_rectangle.restore,
                  dot.scale_in_place,
                  1. / self.zoom_factor,
                  run_time=2)
        self.wait()
    ...
    def point_out_this_is_not_a_graph(self):
        ...
        brace_text = brace.get_text("Not $y = f(x)$", buff=SMALL_BUFF)
        ...
        self.wait()
        self.play(ShowCreation(new_circle, run_time=2), Animation(brace_text))
    ...
    def perform_implicit_derivative(self):
        ...
        self.play(
            ...
            MED_LARGE_BUFF,
            equation.shift,
            FRAME_X_RADIUS * RIGHT / 2,
        )
        self.play(morty.change_mode, "confused", morty.look_at, equation)
        self.play(Blink(morty))
        ...
        self.wait()
        self.play(morty.change_mode, "shruggie", FadeOut(q_marks))
        self.play(Blink(morty))
        self.play(
            morty.change_mode,
            "pondering",
        ...
        final_form = TexMobject("\\frac{dy}{dx} = \\frac{-x}{y}")
        ...
        self.play(*[
            ReplacementTransform(
                ...
                run_time=2,
                path_arc=np.pi / 2,
            ) for mover, target in [(dy, new_dy), (dx, new_dx), (eq, new_eq),
                                    (x, new_x), (y, new_y)]
...
class RelatedRatesExample(ThreeDScene):
    ...
    def introduce_ladder(self):
        ...
        self.play(ShowCreation(ladder, run_time=2))
        self.wait()

        self.play(DrawBorderThenFill(wall), MoveToTarget(ladder), run_time=2)
        self.wait()
    ...
    def slide_ladder(self):
        ...
        down_arrow, left_arrow = [
            Arrow(ORIGIN, vect, color=YELLOW, buff=0) for vect in (DOWN, LEFT)
        ]
    ...
    def write_equation(self):
        ...
        self.wait()
        self.let_ladder_fall(self.ladder,
                             y_label.shift,
                             self.start_y * DOWN / 2,
        ...
        self.wait(2)
        self.play(ReplacementTransform(x_label.copy(), equation[0]),
    ...
    def isolate_x_of_t(self):
        ...
        self.play(FadeIn(randy))
        self.play(
            randy.change_mode, "raise_right_hand", randy.look_at, alt_equation,
            *[
                ReplacementTransform(self.equation[i].copy(),
                                     alt_equation[j],
                                     path_arc=np.pi / 2,
                                     run_time=3,
        ...
        self.let_ladder_fall(self.ladder,
                             *self.get_added_anims_for_ladder_fall(),
                             run_time=8,
                             rate_func=there_and_back)
        self.play(*list(map(FadeOut, [randy, find_dx_dt, alt_equation])))
        self.wait()
    ...
    def discuss_lhs_as_function(self):
        ...
        function_of_time = brace.get_text("Function of time")
        constant_words = TextMobject("""that happens to
            be constant""")
        constant_words.set_color(YELLOW)
        constant_words.next_to(function_of_time, DOWN)

        derivative = TexMobject("\\frac{d\\left(x(t)^2 + y(t)^2 \\right)}{dt}")
        ...
        derivative_scaffold = VGroup(*list(derivative[:2]) +
                                     list(derivative[13:]))

        self.play(GrowFromCenter(brace), Write(function_of_time))
        self.wait()
        ...
        self.wait()
        self.play(
            *list(map(FadeOut, [brace, constant_words, function_of_time])))
        self.play(
        ...
        self.derivative = VGroup(derivative_scaffold, derivative_interior)
    ...
    def let_dt_pass(self):
        ...
        self.play(GrowFromCenter(dt_brace), Write(dt_brace_text, run_time=2))
        self.play(*list(
            map(FadeOut, [
                self.dy_arrow,
                self.dy_label,
        ...
        for line, brace, label in (dy_line, dy_brace,
                                   dy_label), (dx_line, dx_brace, dx_label):
            self.play(ShowCreation(line),
    ...
    def take_derivative_of_lhs(self):
        ...
        lhs_derivative.next_to(derivative_scaffold,
                               DOWN,
                               aligned_edge=RIGHT,
                               buff=MED_LARGE_BUFF)
        ...
        def perform_replacement(index_pairs):
            self.play(*[
                ReplacementTransform(equation[i].copy(),
                                     lhs_derivative[j],
                                     path_arc=np.pi / 2,
                                     run_time=2) for i, j in index_pairs
            ])
        ...
        self.play(
            ReplacementTransform(self.equals_zero.copy(), equals_zero_copy))
        self.wait(2)
    ...
    def replace_terms_in_final_form(self):
        ...
        new_lhs_derivative = TexMobject("2", "(%d)" % int(self.start_x),
                                        "\\frac{dx}{dt}", "+", "2",
                                        "(%d)" % int(self.start_y), "(-1)",
                                        "= 0")
        new_lhs_derivative[1].set_color(GREEN)
        VGroup(*new_lhs_derivative[2][:2]).set_color(GREEN)
        new_lhs_derivative[5].set_color(RED)
        new_lhs_derivative.next_to(self.lhs_derivative,
                                   DOWN,
                                   buff=MED_LARGE_BUFF,
                                   aligned_edge=RIGHT)
        ...
        def fill_in_equation_part(*indices):
            self.play(*[
                ReplacementTransform(self.lhs_derivative[i].copy(),
                                     new_lhs_derivative[i],
                                     run_time=2) for i in indices
            ])
    ...
    def write_final_solution(self):
        solution = TexMobject("\\frac{dx}{dt} = \\frac{4}{3}")
        for i in 0, 1, -1:
            solution[i].set_color(GREEN)
        solution[-3].set_color(RED)
        solution.next_to(self.new_lhs_derivative,
                         DOWN,
                         buff=MED_LARGE_BUFF,
                         aligned_edge=RIGHT)
    ...
    def let_ladder_fall(self, ladder, *added_anims, **kwargs):
        ...
        self.play(Transform(ladder, ladder.fallen), *added_anims, **kwargs)

    def reset_ladder(self, ladder, *added_anims, **kwargs):
        kwargs["run_time"] = kwargs.get("run_time", 2)
        self.play(Transform(ladder, ladder.target), *added_anims, **kwargs)

    def update_brace(self, brace):
        Transform(brace, self.get_ladder_brace(self.ladder)).update(1)
        return brace

    def update_x_and_y_lines(self, x_and_y_lines):
        Transform(x_and_y_lines, self.get_x_and_y_lines(self.ladder)).update(1)
        return x_and_y_lines

    def update_x_and_y_labels(self, x_and_y_labels):
        Transform(x_and_y_labels, self.get_x_and_y_labels()).update(1)
    ...
    def get_ladder_brace(self, ladder):
        ...
        length_label = brace.get_text(length_string, use_next_to=False)
...
class LightweightCircleExample(SlopeOfCircleExample):
    ...
    def construct(self):
        ...
        self.example_point_coords_mob.scale(
            1.5,
            about_point=self.example_point_coords_mob.get_corner(UP + RIGHT))
...
class CompareLadderAndCircle(PiCreatureScene, ThreeDScene):
    ...
    def introduce_both_scenes(self):
        ...
        for mobs, vect in (ladder_mobs, LEFT), (circle_mobs, RIGHT):
            ...
            mobs.next_to(self.pi_creature.get_corner(UP + vect),
                         UP,
                         buff=SMALL_BUFF,
                         aligned_edge=-vect)
        ...
        self.play(self.pi_creature.change_mode, "raise_right_hand",
                  self.pi_creature.look_at, ladder_mobs,
                  ApplyMethod(ladder_mobs.restore, run_time=2))
        self.play(self.pi_creature.change_mode, "raise_left_hand",
    ...
    def show_derivatives(self):
        equation = TexMobject("x", "^2", "+", "y", "^2", "= 5^2")
        derivative = TexMobject("2", "x", "dx", "+", "2", "y", "dy", "=0")
        self.color_equations(equation, derivative)
        equation.to_edge(UP)
        equation.shift(MED_LARGE_BUFF * LEFT)
        derivative.next_to(equation, DOWN, buff=MED_LARGE_BUFF)

        self.play(Write(equation), self.pi_creature.change_mode, "plain",
                  self.pi_creature.look_at, equation)
        self.wait()
        self.play(*[
            ReplacementTransform(
                equation[i].copy(), derivative[j], path_arc=np.pi / 2)
            for i, j in enumerate([1, 0, 3, 5, 4, 7])
        ] + [Write(derivative[j]) for j in (2, 6)])
    ...
    def comment_on_ladder_derivative(self):
        ...
        time_equation = TexMobject("x(t)", "^2", "+", "y(t)", "^2", "= 5^2")
        time_derivative = TexMobject("2", "x(t)", "\\frac{dx}{dt}", "+", "2",
                                     "y(t)", "\\frac{dy}{dt}", "=0")
        ...
        self.wait(2)
        self.play(equation.restore, derivative.restore, FadeOut(brace),
                  FadeOut(brace_text), self.pi_creature.change_mode,
                  "confused")
        self.wait()
...
class TwoVariableFunctionAndDerivative(SlopeOfCircleExample):
    ...
    def write_equation(self):
        ...
        self.play(Write(s))
        self.wait()
        self.play(FadeIn(s_rect), s.restore, GrowFromCenter(xy))
        self.wait()
    ...
    def show_example_point(self):
        ...
        equals_25.next_to(new_s_expression,
                          RIGHT,
        ...
        self.play(
            ReplacementTransform(self.s_expression.copy(), new_s_expression),
            ShowCreation(dot))
        ...
        self.example_point_dot = dot
        self.example_point_label = VGroup(new_s_expression, equals_25)
    ...
    def shift_example_point(self, coords):
        ...
        self.play(self.example_point_dot.move_to, point,
    ...
    def take_derivative_symbolically(self):
        ...
        self.play(*[FadeIn(derivative[0])] + [
            ReplacementTransform(
                self.s_expression[1][0].copy(),
                derivative[1][0],
            )
        ] + [Write(derivative[1][j]) for j in (3, 7)])
        self.play(*[
            ReplacementTransform(equation[1][i].copy(),
                                 derivative[1][j],
                                 path_arc=np.pi / 2,
                                 run_time=2,
                                 rate_func=squish_rate_func(
                                     smooth, (j - 1) / 12., (j + 6) / 12.))
    ...
    def show_dx_dy_step(self):
        ...
        self.tiny_step_group = VGroup(dx_line, dx_line.label, dy_line,
                                      dy_line.label, s_label, new_s_label,
                                      new_dot)
    ...
    def plug_in_example_values(self):
        ...
        def add_example_parts(*indices):
            self.play(*[
                ReplacementTransform(self.derivative[1][i].copy(),
                                     deriv_example[1][i],
                                     run_time=2) for i in indices
            ])
        ...
        self.play(FadeIn(randy))
        self.play(randy.change_mode, "pondering", randy.look_at,
                  deriv_example.get_left())
        self.play(Blink(randy))
        self.play(randy.look_at, deriv_example.get_right())
        self.wait()
        self.play(Indicate(self.equation), randy.look_at, self.equation)
        self.play(Blink(randy))
        self.play(randy.change_mode, "thinking", randy.look_at,
                  self.big_rectangle)
        self.wait(2)
        self.play(PiCreatureSays(randy, "Approximately", target_mode="sassy"))
        self.wait()
    ...
    def ask_about_equalling_zero(self):
        ...
        self.play(Write(equals_zero), randy.change_mode, "confused",
    ...
    def show_tangent_step(self):
        ...
        s_label.next_to(point, DOWN, buff=MED_LARGE_BUFF, aligned_edge=RIGHT)
        s_label.scale(1. / self.zoom_factor, about_point=point)
        arrow1, arrow2 = [
            Arrow(s_label.get_top(),
                  mob,
                  preserve_tip_size_when_scaling=False,
                  color=self.example_color,
                  buff=SMALL_BUFF / self.zoom_factor,
                  tip_length=0.15 / self.zoom_factor) for mob in (dot, new_dot)
        ]
    ...
    def point_out_equalling_zero(self):
        ...
        self.play(FocusOn(equals_zero), self.randy.look_at, equals_zero)
    ...
    def show_tangent_line(self):
        ...
        self.play(Blink(randy))
        self.play(RemovePiCreatureBubble(randy))
        self.play(GrowFromCenter(line), randy.look_at, line)
        self.wait(2)
        self.play(self.little_rectangle.scale_in_place,
                  self.zoom_factor / 2,
                  run_time=4,
                  rate_func=there_and_back)
        self.wait(2)
...
class TryOtherExamples(TeacherStudentsScene):
    def construct(self):
        ...
        formula.next_to(self.get_teacher().get_corner(UP + LEFT),
                        UP,
                        buff=MED_LARGE_BUFF,
                        aligned_edge=RIGHT)

        self.teacher_says("""Nothing special
            about $x^2 + y^2 = 25$""")
        self.wait()
        self.play(
...
class AlternateExample(ZoomedScene):
    ...
    def draw_graph(self):
        graphs = VGroup(*[
            FunctionGraph(
                ...
                x_min=x_min + 0.1,
                x_max=x_max - 0.1,
            ) for u in [-1, 1] for x_min, x_max in [
                (-4 * np.pi, -2 * np.pi),
                (-np.pi, np.pi),
                (2 * np.pi, 4 * np.pi),
            ]
        ...
        self.play(ShowCreation(graphs, run_time=3, lag_ratio=0),
    ...
    def emphasize_meaning_of_points(self):
        ...
        def update_label(label):
            ...
            vect[0] *= 2
            vect[1] *= -1
            label.move_to(point + vect * 0.4 * label.get_width())

        update_dot(dot, 0)
        update_label(label)

        self.play(ShowCreation(dot), Write(label), run_time=1)
        self.play(
        ...
        self.wait()
        self.play(*[
            ApplyMethod(label[1][i].copy().move_to,
                        self.formula[1][j],
                        run_time=3,
                        rate_func=squish_rate_func(smooth, count /
                                                   6., count / 6. + 2. / 3))
    ...
    def show_tiny_step(self):
        ...
        self.step_group = VGroup(arrow, dx_line, dx_line.label, dy_line,
                                 dy_line.label)
    ...
    def ask_about_derivatives(self):
        ...
        arrows = VGroup(*[Arrow(word, part) for part in (lhs, rhs)])

        self.play(FocusOn(formula))
        self.play(Write(word), ShowCreation(arrows))
        self.wait()
    ...
    def differentiate_lhs(self):
        ...
        derivative.next_to(brace, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        derivative_rects = [
    ...
    def put_step_on_curve(self):
        ...
        distances = np.apply_along_axis(lambda p: np.abs(p[0] - arrow_end[0]),
                                        1, graph_points)
        index = np.argmin(distances)
        new_end_point = graph_points[index]

        lil_rect = self.little_rectangle
        self.play(arrow.put_start_and_end_on,
                  point,
                  new_end_point,
    ...
    def manipulate_to_find_dy_dx(self):
        full_derivative = VGroup(self.lhs_derivative, self.equals_dx)
        brace = Brace(full_derivative, DOWN, buff=SMALL_BUFF)
        words = brace.get_text("Commonly, solve for",
                               "$dy/dx$",
                               buff=SMALL_BUFF)
...
class AskAboutNaturalLog(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.play(
            Write(log_deriv), exp_deriv.next_to, log_deriv, UP, LARGE_BUFF, *[
...
class DerivativeOfNaturalLog(ZoomedScene):
    ...
    def describe_as_implicit_curve(self):
        ...
        def update_label(label):
            ...
            label.move_to(point + vect * 0.5 * label.get_width())
    ...
    def slope_gives_derivative(self):
        ...
        self.play(GrowFromCenter(line), Animation(dot))
    ...
    def rearrange_equation(self):
        ...
        self.play(FadeIn(rect), Animation(VGroup(e, new_y, new_eq)),
    ...
    def take_derivative(self):
        ...
        pairs = [(VGroup(e, y), VGroup(new_e, new_y)), (new_y, dy),
                 (eq, new_eq), (x, dx)]
    ...
    def solve_for_dy_dx(self):
        ...
        rearranged = TexMobject("{dy \\over ", " dx}", "=", "{1 \\over ", "e",
                                "^y}")
        ...
        self.play(*[
            ...
            for m1, m2 in [(original_rect, new_rect), (dx, new_dx), (
                dy, new_dy), (eq, new_eq), (ey_group,
                                            new_ey_group), (e.copy(),
                                                            one_over)]
        ])
        self.wait()
...
class FinalWords(TeacherStudentsScene):
    def construct(self):
        words = TextMobject("This is a peek into \\\\", "Multivariable",
                            "calculus")
        mvc = VGroup(*words[1:])
        ...
        self.play(video.set_color, YELLOW, video.shift,
                  video.get_height() * DOWN / 2)
        lim.next_to(video, DOWN)
        self.play(
            Write(lim),
            *it.chain(*[[pi.change_mode, "pondering", pi.look_at, lim]
                        for pi in self.get_pi_creatures()]))
        self.wait(3)
...
class Thumbnail(AlternateExample):
    def construct(self):
        title = VGroup(
            *list(map(TextMobject, ["Implicit", "Differentiation"])))
        title.arrange(DOWN)
        title.scale(3)
