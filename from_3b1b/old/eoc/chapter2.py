...
class Introduction(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.teacher_says("It's actually a \\\\",
                          "very subtle idea",
                          target_mode="well")
        ...
        self.play(
            FadeOut(bubble), FadeOut(bubble.content), FadeOut(goals),
            phrase.center, phrase.scale, 1.5, phrase.to_edge, UP,
            *it.chain(
        ...
        self.play(change.set_color_by_gradient, BLUE, YELLOW,
                  GrowFromCenter(change_brace), Write(change_description),
                  get_clock_anim())
        self.play(get_clock_anim(1))
        stopped_clock = clock.copy()
        stopped_clock.next_to(instantaneous_description, DOWN)
        self.play(
            instantaneous.set_color, BLUE, GrowFromCenter(instantaneous_brace),
...
class IntroduceCar(Scene):
    ...
    def construct(self):
        ...
        self.play(MoveCar(car, point_B, run_time=10),
...
class GraphCarTrajectory(GraphScene):
    ...
    def graph_sigmoid_trajectory_function(self, **kwargs):
        graph = self.get_graph(lambda t: 100 * smooth(t / 10.), **kwargs)
        self.s_graph = graph
        return graph
    ...
    def comment_on_slope(self, graph, origin):
        ...
        for x in range(1, 10):
            ...
            self.play(rect.move_to, self.coords_to_point(curr_time, 0),
                      DOWN + LEFT, Transform(change_lines, new_change_lines))
            if curr_time == 5:
    ...
    def show_velocity_graph(self):
        ...
        for time, show_slope in (4.5, True), (9, False):
            self.play(self.rect.move_to, self.coords_to_point(time, 0),
                      DOWN + LEFT)
            if show_slope:
    ...
    def ask_critically_about_velocity(self):
        ...
        self.play(
            PiCreatureSays(morty, "Think critically about \\\\",
                           "what velocity means."))
...
class ShowSpeedometer(IntroduceCar):
    ...
    def setup(self):
        start_angle = -np.pi / 6
        end_angle = 7 * np.pi / 6
        speedometer = Arc(start_angle=start_angle,
...
class SnapshotOfACar(Scene):
    def construct(self):
        ...
        speed_lines = VGroup(*[
            Line(point, point + 0.5 * LEFT) for point in
            [0.5 * UP + 0.25 * RIGHT, ORIGIN, 0.5 * DOWN + 0.25 * RIGHT]
        ])
...
class CompareTwoTimes(Scene):
    ...
    def show_equation(self):
        ...
        VGroup(VGroup(*formula[1:1 + ed_len]),
               VGroup(*formula[2 + ed_len:2 + ed_len +
                               sd_len])).set_color(DISTANCE_COLOR)
        VGroup(
            VGroup(*formula[-2 - seconds_len - et_len - st_len:-2 -
                            seconds_len - st_len]),
    ...
    def get_car_state(self, distance, time):
        ...
        time_label = TextMobject("Time:", str(time), "seconds")
...
class VelocityAtIndividualPointsVsPairs(GraphCarTrajectory):
    ...
    def construct(self):
        ...
        distance_label = self.label_graph(distance_graph,
                                          label="s(t)",
                                          proportion=1,
                                          direction=RIGHT,
                                          buff=SMALL_BUFF)
        ...
        self.play(ShowCreation(velocity_graph))
        velocity_label = self.label_graph(velocity_graph,
                                          label="v(t)",
    ...
    def show_two_times_on_distance(self):
        line1 = self.get_vertical_line_to_graph(self.start_time -
                                                self.dt / 2.0)
        line2 = self.get_vertical_line_to_graph(self.start_time +
                                                self.dt / 2.0)
        p1 = line1.get_end()
        p2 = line2.get_end()
        interim_point = p2[0] * RIGHT + p1[1] * UP
        ...
        for line, brace, text in (dt_line, dt_brace,
                                  dt_text), (ds_line, ds_brace, ds_text):
...
class FirstRealWorld(TeacherStudentsScene):
    def construct(self):
        ...
        self.change_student_modes("happy", "hooray", "happy")
        self.wait(3)
...
class SidestepParadox(Scene):
    def construct(self):
        ...
        title = TextMobject("Instantaneous", "rate of change")
        title.to_edge(UP)
...
class CompareTwoVerySimilarTimes(CompareTwoTimes):
    ...
    def construct(self):
        ...
        ds_symbols, dt_symbols = [
            VGroup(
                *[mob for mob in formula if mob.get_color() == Color(color)])
        ...
        VGroup(ds_brace, ds_text, formula.target).move_to(formula,
                                                          UP).shift(0.5 * UP)
        self.play(MoveToTarget(formula), GrowFromCenter(ds_brace),
                  Write(ds_text))
        self.wait(2)
...
class DsOverDtGraphically(GraphCarTrajectory, ZoomedScene):
    ...
    def construct(self):
        ...
        def get_ds_dt_group(time):
            ...
            result = VGroup()
            for line, char, vect in (dt_line, "t", DOWN), (ds_line, "s",
                                                           RIGHT):
        ...
        self.play(
            ApplyFunction(align_little_rectangle_on_ds_dt_group,
        ...
        rise_over_run = TexMobject("=\\frac{\\text{rise}}{\\text{run}}")
        ...
        full_formula = TexMobject("=\\frac{s(t+dt) - s(t)}{dt}")
        ...
        v_line_at_t, v_line_at_t_plus_dt = [
            self.get_vertical_line_to_graph(time,
                                            line_class=Line,
        ...
        def sin_wiggle(t):
            return (10 / (2 * np.pi / 10.)) * (np.sin(2 * np.pi * t / 10.) +
                                               2 * np.pi * t / 10.)
        ...
        for graph_func in graph_funcs:
            new_graph = self.graph_function(graph_func,
                                            color=DISTANCE_COLOR,
                                            is_main_graph=False)
            self.remove(new_graph)
            new_velocity_graph = self.get_derivative_graph(graph=new_graph, )
...
class DefineTrueDerivative(Scene):
    def construct(self):
        ...
        numerical_rhs_list = [
            self.get_fraction("0.%s1" % ("0" * x)) for x in range(7)
        ]
    ...
    def get_fraction(self, dt_string):
        tex_mob = TexMobject("\\frac{s(t + %s) - s(t)}{%s}" %
                             (dt_string, dt_string))
        part_lengths = [
            0,
            len("s(t+"),
...
class SecantLineToTangentLine(GraphCarTrajectory, DefineTrueDerivative):
    ...
    def get_ds_dt_group(self, dt, animate=False):
        ...
        secant_line.scale_in_place(self.secant_line_length /
                                   secant_line.get_length())
        ...
        if dt.get_width() > dt_line.get_width():
            dt.scale(dt_line.get_width() / dt.get_width(),
        ...
        if ds.get_height() > ds_line.get_height():
            ds.scale(ds_line.get_height() / ds.get_height(),
    ...
    def draw_axes(self):
        ...
        self.play(
            Write(VGroup(self.x_axis, self.y_axis, self.graph,
                         self.graph_label),
                  run_time=4))
        self.wait()
    ...
    def show_tangent_line(self):
        ...
        self.play(UpdateFromAlphaFunc(ds_dt_group, update_ds_dt_group),
                  run_time=15)
        self.wait()
        ...
        self.play(UpdateFromAlphaFunc(ds_dt_group, update_as_tangent_line),
                  run_time=8,
                  rate_func=there_and_back)
        self.wait()
        ...
        v_line = self.get_vertical_line_to_graph(self.curr_time,
                                                 self.graph,
                                                 line_class=Line,
                                                 line_kwargs={
                                                     "color": MAROON_B,
...
class LeadIntoASpecificExample(TeacherStudentsScene, SecantLineToTangentLine):
    ...
    def construct(self):
        ...
        self.play(
            Write(deriv_def),
            *it.chain(*[[pi.change_mode, "thinking", pi.look_at, dot]
...
class TCubedExample(SecantLineToTangentLine):
    ...
    def add_ds_dt_group(self):
        ...
        rhs = TexMobject("\\frac{s(2+dt) - s(2)}{dt}")
        rhs.next_to(lhs[-1])
    ...
    def show_expansion(self):
        ...
        terms = [
            ...
            for i1, i2 in zip([0] +
                              list(np.cumsum(term_lens)), np.cumsum(term_lens))
        ]

        dts = [
            VGroup(*terms[1][-2:]),
            VGroup(*terms[2][6:8]),
            VGroup(*terms[3][1:3]), terms[-1]
        ]
        ...
        self.play(*it.chain(
            *[[tc.scale, 1.3,
               tc.get_corner(vect), tc.set_color, RED]
              for tc, vect in zip(two_cubed_terms, [DOWN + RIGHT, DOWN +
                                                    LEFT])]))
        self.play(*list(map(FadeOut, two_cubed_terms)))
        numerator = VGroup(*terms[1:4])
        self.play(numerator.scale, 1.4, numerator.get_bottom(),
        ...
        arrow = Arrow(self.lhs[4].get_bottom(),
                      terms[1][2].get_top(),
                      color=WHITE,
                      buff=MED_SMALL_BUFF)
        ...
        self.play(
            Transform(self.ds_dt_group, self.get_ds_dt_group(self.start_dt)))
        self.play(UpdateFromAlphaFunc(self.ds_dt_group, ds_dt_group_update),
                  faders.fade,
                  0.7,
                  run_time=5)
        self.wait(2)
        ...
        arrow = Arrow(self.rhs.get_bottom(),
        ...
        self.play(self.ds_dt_group.rotate_in_place,
                  np.pi / 24,
                  rate_func=wiggle)
        self.play(ShowCreation(v_line))
        self.wait()
        ...
        twos = [
            self.lhs[6], self.rhs[2], self.rhs[10], lhs_copy[6], deriv_term[2]
        ]
        for two in twos:
        ...
        self.play(UpdateFromAlphaFunc(self.ds_dt_group,
                                      update_as_tangent_line),
                  run_time=5,
                  rate_func=there_and_back)
        self.wait(2)
...
class YouWouldntDoThisEveryTime(TeacherStudentsScene):
    def construct(self):
        self.change_student_modes("pleading", "guilty", "hesitant", run_time=0)
        self.teacher_says("You wouldn't do this \\\\ every time")
        self.change_student_modes(*["happy"] * 3)
        self.wait(2)
        self.student_thinks("$\\frac{d(t^3)}{dt} = 3t^2$", )
        self.wait(3)
        ...
        self.play(this_video.restore, next_video.set_color, YELLOW,
...
class ContrastConcreteDtWithLimit(Scene):
    def construct(self):
        ...
        r_formula = TexMobject("\\frac{d(t^3)}{dt} = 3t^2")
...
class TimeForAnActualParadox(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(teacher.change_mode, "raise_right_hand", teacher.look_at,
                  words, Write(words))
        self.play(*list(map(Write, [arrow, paradoxes])))
        self.play(*it.chain(
            *[[pi.change_mode, mode, pi.look_at, words] for pi, mode in zip(
...
class ParadoxAtTEquals0(TCubedExample):
    ...
    def ask_question(self):
        question = TextMobject("At time $t=0$,", "is \\\\ the car moving?")
    ...
    def show_tangent_line(self):
        ...
        self.play(UpdateFromAlphaFunc(line, line_update),
    ...
    def single_out_question(self):
        ...
        self.play(everything.fade, 0.8, question.center, morty.change_mode,
                  "confused", morty.look_at, ORIGIN)
        self.play(Blink(morty))
        for word in change_word, moment_word:
            self.play(word.scale_in_place,
                      1.2,
                      word.set_color,
                      YELLOW,
                      rate_func=there_and_back,
                      run_time=1.5)
        self.wait(2)
        self.play(everything.restore, FadeOut(question), morty.change_mode,
                  "raise_right_hand", morty.look_at, self.derivative)
        self.play(GrowFromCenter(brace), FadeIn(brace_text))
        self.wait()
        self.play(self.tangent_line.rotate_in_place,
                  np.pi / 24,
                  rate_func=wiggle,
                  run_time=1)
        self.play(Blink(morty))
...
class TinyMovement(ZoomedScene):
    ...
    def show_initial_motion(self):
        ...
        self.play(zoom_rect.scale_in_place,
    ...
    def get_ratio(self, power=1):
        ...
        result = VGroup(*[
            VGroup(*expression[i1:i2]) for i1, i2 in zip(
                np.cumsum(lengths),
                np.cumsum(lengths)[1:],
            )
        ])
...
class Promotion(PiCreatureScene):
    ...
    def construct(self):
        ...
        url = TextMobject("AoPS.com/", "3blue1brown", arg_separator="")
