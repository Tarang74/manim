...
class Introduce(TeacherStudentsScene):
    def construct(self):
        ...
        derivs = VGroup(*[
            TexMobject("{d", "^%d" % n, "f \\over dx", "^%d}" %
                       n).set_color_by_tex(str(n), YELLOW)
            for n in range(2, 5)
        ])
    ...
    def show_curvature(self):
        positive_curve, negative_curve = [
            self.get_graph(
                ...
                x_max=x_max,
                color=color,
            ).set_stroke(width=6) for x_min, x_max, color in [
        ...
        self.play(ShowCreation(positive_curve, run_time=3),
        ...
        self.play(ShowCreation(negative_curve, run_time=3),
        ...
        self.play(*list(
            map(FadeOut, [
                self.graph, self.ss_group, negative_curve,
...
class IntroduceNotation(TeacherStudentsScene):
    def construct(self):
        clunky_deriv = TexMobject("{d", "\\big(", "{df", "\\over", "dx}",
                                  "\\big)", "\\over", "dx }")
        ...
        dfs, overs, dxs = list(
            map(clunky_deriv.get_parts_by_tex, ["df", "over", "dx"]))
        ...
        self.student_says("What's that notation?",
                          target_mode="raise_left_hand")
        self.change_student_modes("confused", "raise_left_hand", "confused")
        self.play(FadeIn(clunky_deriv, run_time=2, lag_ratio=0.5),
        ...
        self.wait()
        self.play(FadeOut(denominator.circle), Write(dx_to_zero),
...
class HowToReadNotation(GraphScene, ReconfigurableScene):
    ...
    def take_two_steps(self):
        v_lines = [
            self.get_vertical_line_to_graph(self.x + i * self.dx,
                                            self.graph,
    ...
    def show_dfs(self):
        ...
        df_dx_groups = VGroup()
        df_labels = VGroup()
        for i, v_line1, v_line2 in zip(it.count(1), self.v_lines,
                                       self.v_lines[1:]):
            dx_line = Line(v_line1.get_bottom(),
                           v_line2.get_bottom(),
                           color=GREEN)
            dx_line.move_to(v_line1.get_top(), LEFT)
            dx_lines.add(dx_line)

            df_line = Line(dx_line.get_right(),
                           v_line2.get_top(),
                           color=YELLOW)
    ...
    def show_ddf(self):
        ...
        df_dx_groups.target.arrange(RIGHT,
                                    buff=MED_LARGE_BUFF,
                                    aligned_edge=DOWN)
        df_dx_groups.target.next_to(self.df_dx_groups,
                                    RIGHT,
                                    buff=3,
                                    aligned_edge=DOWN)
        ...
        ddf.scale(df_labels[0].get_height() / ddf.get_height(),
                  about_point=ddf.get_right())
        ddf.set_color(MAROON_B)

        self.play(*list(map(MoveToTarget, [df_dx_groups, df_labels])),
                  run_time=2)
    ...
    def show_proportionality_to_dx_squared(self):
        ...
        rhs = TexMobject("\\approx", "(\\text{Some constant})", "(dx)^2")
        rhs.scale(0.8)
        rhs.next_to(ddf.target, RIGHT)

        example_dx = TexMobject("dx = 0.01 \\Rightarrow (dx)^2 = 0.0001")
...
class SecondDerivativeAsAcceleration(Scene):
    ...
    def introduce_acceleration(self):
        a_words = TexMobject("{d^2 s \\over dt^2}(t)", "\\Leftrightarrow",
                             "\\text{Acceleration}")
    ...
    def show_functions(self):
        ...
        s_words = TexMobject("s(t)", "\\Leftrightarrow",
                             "\\text{Displacement}")
        s_words.set_color_by_tex("s(t)", s_scene.graph.get_color())
        v_words = TexMobject("\\frac{ds}{dt}(t)", "\\Leftrightarrow",
                             "\\text{Velocity}")
        v_words.set_color_by_tex("ds", v_scene.graph.get_color())
        j_words = TexMobject("\\frac{d^3 s}{dt^3}(t)", "\\Leftrightarrow",
                             "\\text{Jerk}")
        ...
        for x in range(2):
            self.show_car_movement(run_time=3,
        ...
        self.wait()
        self.play(a_graph.scale, 0.3, a_graph.next_to, self.a_words, LEFT,
    ...
    def show_car_movement(self, *added_anims, **kwargs):
        distance = get_norm(self.car.get_center() -
