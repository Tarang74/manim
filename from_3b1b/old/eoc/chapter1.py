...
class CircleScene(PiCreatureScene):
    ...
    def setup(self):
        ...
        self.radius_group = VGroup(self.radius_line, self.radius_brace,
                                   self.radius_label)
...
class Introduction(TeacherStudentsScene):
    ...
    def show_series(self):
        ...
        self.change_student_modes(*["hooray"] * 3,
                                  ...
                                  added_anims=[
                                      ApplyMethod(this_video.restore,
                                                  run_time=3),
                                  ])
        self.play(*[
            ApplyMethod(video.shift,
                        0.5 * video.get_height() * DOWN,
                        run_time=3,
                        rate_func=squish_rate_func(there_and_back, alpha,
                                                   alpha + 0.3))
    ...
    def show_many_facts(self):
        ...
        for last_rule, rule, video_index in zip(rules, alt_rules_list,
                                                video_indices):
            ...
            self.wait()
        self.play(self.teacher.change_mode, "happy", self.teacher.look_at,
                  student.eyes)
    ...
    def invent_calculus(self):
        ...
        self.play(Write(invent), MoveToTarget(calculus), student.change_mode,
                  "erm", student.look_at, calculus)
...
class PreviewFrame(Scene):
    def construct(self):
        ...
        titles = [
            TextMobject("Chapter %d:" % d,
                        s).to_edge(UP).set_color(next(colors)) for d, s in [
...
class ProductRuleDiagram(Scene):
    def construct(self):
        ...
        for mob in df_brace, df_label, dg_brace, dg_label:
            mob.save_state()
            mob.scale(0.01,
                      about_point=rect.get_corner(mob.get_center() -
                                                  rect.get_center()))
        ...
        deriv = TexMobject("d(", "fg", ")", "=", "f", "\\cdot", "dg", "+", "g",
                           "\\cdot", "df")
        deriv.to_edge(UP)
        alpha_iter = iter(np.linspace(0, 0.5, 5))
        self.play(*[
            ApplyMethod(mob.copy().move_to,
...
class IntroduceCircle(CircleScene):
    ...
    def question_area(self):
        ...
        unwrapped_rings = VGroup(
            *[self.get_unwrapped(ring, to_edge=None) for ring in rings])
        ...
        self.wait()
        self.play(FadeIn(rings, **ring_anim_kwargs),
                  Animation(self.radius_group), FadeOut(q_marks),
        ...
        self.play(rings.rotate,
                  ...
                  path_arc=np.pi / 2,
                  **ring_anim_kwargs)
        self.play(Transform(rings, unwrapped_rings, **ring_anim_kwargs), )
        self.wait()

    def show_calculus_symbols(self):
        ftc = TexMobject("\\int_0^R", "\\frac{dA}{dr}", "\\,dr", "=", "A(R)")
        ftc.shift(2 * UP)

        self.play(
            ReplacementTransform(
                self.area.get_part_by_tex("R").copy(),
                ftc.get_part_by_tex("int")), self.pi_creature.change_mode,
            "plain")
        self.wait()
        self.play(
            ReplacementTransform(
...
class ApproximateOneRing(CircleScene, ReconfigurableScene):
    ...
    def try_to_understand_area(self):
        ...
        self.play(ShowCreation(lines, run_time=2, lag_ratio=0.5),
                  Animation(self.radius_group), self.pi_creature.change_mode,
                  "maybe")
        self.wait(2)
    ...
    def slice_into_rings(self):
        ...
        self.play(FadeIn(rings, lag_ratio=0.5, run_time=3),
                  Animation(self.radius_group), self.pi_creature.change_mode,
                  "pondering", self.pi_creature.look_at, self.circle)
        self.wait(2)
        for x in range(2):
            self.play(Rotate(rings, np.pi, in_place=True, run_time=2),
                      Animation(self.radius_group),
    ...
    def straighten_ring_out(self):
        ...
        for text in trapezoid, rectangle_ish:
            text.next_to(self.pi_creature.get_corner(UP + RIGHT),
                         DOWN + RIGHT,
                         buff=MED_LARGE_BUFF)
        ...
        strike = Line(trapezoid.get_left(),
                      trapezoid.get_right(),
                      stroke_color=RED,
                      stroke_width=8)
        self.play(Write(rectangle_ish), ShowCreation(strike),
                  self.pi_creature.change_mode, "happy")
        self.wait()
    ...
    def approximate_as_rectangle(self):
        ...
        self.play(
            ...
            ReplacementTransform(self.ring_radius_group[1].copy(),
        ...
        self.play(ReplacementTransform(side_brace.copy(), alt_side_brace),
                  ReplacementTransform(dr_label.copy(), alt_dr_label),
                  run_time=2)
        self.wait()
        self.play(dr_label.next_to, concrete_dr.copy(), LEFT, SMALL_BUFF, DOWN,
                  Write(concrete_dr, run_time=2), self.pi_creature.change_mode,
                  "pondering")
        self.wait(2)
        self.play(
        ...
        self.wait(2)
        self.look_at(self.circle)
        self.play(ApplyWave(self.rings, amplitude=0.1),
                  Animation(self.radius_group),
...
class MoveForwardWithApproximation(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        words = TextMobject("It gets better", "\\\\ for smaller ", "$dr$")
...
class GraphRectangles(CircleScene, GraphScene):
    ...
    def draw_ring_sum(self):
        ...
        area_label = TexMobject("\\text{Area}", "\\approx", "2\\pi", "r",
                                "\\,dr")
        ...
        arrows = VGroup(*[
            Arrow(arrow_start,
                  ring.target.get_boundary_point(arrow_start -
                                                 ring.target.get_center()),
                  color=ring.get_color()) for ring in rings
    ...
    def draw_r_values(self):
        ...
        values_of_r.next_to(self.x_axis,
                            UP,
                            buff=2 * LARGE_BUFF,
                            aligned_edge=LEFT)

        r_ticks = VGroup(*[
            Line(self.coords_to_point(r, -self.tick_height),
                 self.coords_to_point(r, self.tick_height),
                 color=YELLOW) for r in np.arange(0, 3, 0.1)
        ])
        arrows = VGroup(*[
            Arrow(values_of_r.get_part_by_tex("r").get_bottom(),
                  tick.get_top(),
                  buff=SMALL_BUFF,
                  color=YELLOW,
                  tip_length=0.15) for tick in (r_ticks[0], r_ticks[-1])
        ])
        first_tick = r_ticks[0].copy()
        moving_arrow = arrows[0].copy()

        index = 2
        dr_brace = Brace(VGroup(*r_ticks[index:index + 2]),
                         DOWN,
                         buff=SMALL_BUFF)
        dr_label = TexMobject("dr")
        ...
        self.wait()
        self.play(Transform(moving_arrow, arrows[-1]),
    ...
    def unwrap_rings_onto_graph(self):
        ...
        self.play(*[
            MoveToTarget(ring,
                         path_arc=-np.pi / 2,
                         run_time=4,
                         rate_func=squish_rate_func(smooth, alpha, alpha +
                                                    0.25)) for ring, alpha
            in zip(transformed_rings,
        ...
        self.play(transformed_rings.set_fill, None, 0.2,
                  Animation(highlighted_ring), *foreground_animations)
        self.play(self.dr_group.arrange, DOWN, self.dr_group.next_to,
                  highlighted_ring, DOWN, SMALL_BUFF)
        self.wait()
        ...
        self.wait()
        self.play(transformed_rings.set_fill, None, 1, FadeOut(side_brace),
                  FadeOut(height_label), *foreground_animations)
        ...
        self.play(*[
            ApplyMethod(
                ring.replace, ring.rect, method_kwargs={"stretch": True})
    ...
    def draw_graph(self):
        graph_label = self.get_graph_label(self.graph,
                                           "2\\pi r",
                                           direction=UP + LEFT,
                                           x_val=2.5,
                                           buff=SMALL_BUFF)
        ...
        self.play(*[
            Transform(rect,
                      ...
                      rate_func=squish_rate_func(
                          lambda t: 0.1 * there_and_back(t), alpha, alpha +
                          0.5),
                      lag_ratio=0.5) for rect, flat_rect, alpha in zip(
    ...
    def compute_area_under_graph(self):
        formula, formula_with_R = formulas = [
            self.get_area_formula(R) for R in ("3", "R")
        ]
        for mob in formulas:
        ...
        base_line = Line(self.coords_to_point(0, 0),
        ...
        VGroup(fresh_rings, self.radius_group).to_corner(UP + LEFT,
                                                         buff=SMALL_BUFF)
        ...
        self.play(
            *list(map(FadeOut, [self.ghost_rings, self.ring_sum.tex_mobs])))
    ...
    def get_ring_sum(self, rings):
        ...
        for ring, alpha in zip(middle_rings, alphas):
            ...
            ring.target.move_to(
                interpolate(dots.get_left(), last_ring.target.get_center(),
                            alpha))
        ...
        draw_ring_sum_anims += [
            MoveToTarget(ring,
                         run_time=3,
                         path_arc=-np.pi / 3,
                         rate_func=squish_rate_func(smooth, alpha,
                                                    alpha + 0.8))
    ...
    def get_area_formula(self, R):
        formula = TexMobject("\\text{Area}", "&= \\frac{1}{2}", "b", "h",
                             "\\\\ &=", "\\frac{1}{2}", "(%s)" % R,
                             "(2\\pi \\cdot %s)" % R, "\\\\ &=", "\\pi ",
                             "%s" % R, "^2")
...
class RecapCircleSolution(GraphRectangles, ReconfigurableScene):
    ...
    def show_sum(self):
        ...
        for ring, radius in zip(visible_rings, radii):
            label = TexMobject("\\approx", "2\\pi", "(%s)" % str(radius),
                               "(%s)" % str(self.dR))
        ...
        arrows = VGroup(*[
            Arrow(
                area_label.get_left(), approximation.get_right(), color=WHITE)
            for approximation in approximations
        ])

        self.play(Write(area_label))
        self.play(
            ShowCreation(arrows, lag_ratio=0), FadeIn(radial_lines), *[
                ReplacementTransform(area_label.copy(),
    ...
    def dr_indicates_spacing(self):
        r_ticks = VGroup(*[
            Line(self.coords_to_point(r, -self.tick_height),
                 self.coords_to_point(r, self.tick_height),
                 color=YELLOW) for r in np.arange(0, 3, self.dR)
        ])
        ...
        self.wait()
        self.play(GrowFromCenter(dr_brace), dr_copy.next_to, dr_brace.copy(),
                  UP)
        self.wait()
    ...
    def show_riemann_sum(self):
        ...
        self.play(
            ReplacementTransform(self.rings.copy(),
                                 rects,
                                 run_time=2,
                                 lag_ratio=0.5),
            Animation(self.x_axis),
...
class ExampleIntegralProblems(PiCreatureScene, GraphScene):
    ...
    def write_integral_condition(self):
        ...
        self.play(Write(words), self.pi_creature.change_mode,
                  "raise_right_hand")
        self.wait()
    ...
    def show_car(self):
        ...
        ticks = VGroup(*[
            Line(p + tick_height * UP / 2,
                 p + tick_height * DOWN / 2,
                 color=YELLOW,
                 stroke_width=2) for t in np.arange(0, self.t_max, self.dt)
    ...
    def show_graph(self):
        ...
        rects = self.get_riemann_rectangles(v_graph,
                                            x_min=0,
                                            x_max=self.t_max,
                                            dx=self.dt)
        ...
        self.play(FadeIn(pre_rects, run_time=2, lag_ratio=0.5),
                  Animation(self.ticks))
        self.play(
            ReplacementTransform(pre_rects, rects, run_time=3, lag_ratio=0.5),
            Animation(self.ticks), Write(self.axes, run_time=1))
    ...
    def let_dt_approach_zero(self):
        thinner_rects_list = [
            self.get_riemann_rectangles(self.v_graph,
                                        x_min=0,
                                        x_max=self.t_max,
                                        dx=self.dt / (2**n),
                                        stroke_width=1. / (2**n))
        ...
        for thinner_rects in thinner_rects_list:
            self.play(
                Transform(self.rects, thinner_rects, run_time=2,
                          lag_ratio=0.5))
            self.wait()
    ...
    def show_confusion(self):
        ...
        self.play(self.pi_creature.change_mode, "confused",
...
class AreaUnderParabola(GraphScene):
    ...
    def ask_about_area(self):
        ...
        v_lines = VGroup(*[
            DashedLine(FRAME_HEIGHT * UP, ORIGIN, color=RED).move_to(
                self.coords_to_point(x, 0), DOWN)
    ...
    def move_right_point_to(self, target_x, **kwargs):
        ...
        self.play(UpdateFromAlphaFunc(group, update_group, **kwargs),
...
class PlayingTowardsDADX(AreaUnderParabola, ReconfigurableScene):
    ...
    def describe_sliver(self):
        ...
        dA_arrow = Arrow(dA_label.get_bottom() + MED_SMALL_BUFF * DOWN,
                         dA_rect.get_center(),
                         buff=0,
                         color=WHITE)

        difference_in_area = TextMobject("d",
                                         "ifference in ",
                                         "A",
                                         "rea",
                                         arg_separator="")
        ...
        self.play(graph_label_copy.set_color, WHITE, graph_label_copy.next_to,
                  side_brace, LEFT, SMALL_BUFF)
        self.wait()
    ...
    def shrink_dx(self, **kwargs):
        self.transition_to_alt_config(deriv_dx=0.05,
    ...
    def write_dA_dx(self):
        ...
        deriv_equation = TexMobject("{dA", "\\over \\,", "dx}", "\\approx",
                                    f_tex)
        ...
        self.play(*[
            ReplacementTransform(
                mob.copy(), equation.get_part_by_tex(tex), run_time=2)
        ...
        for tex, mob in (f_tex, x_squared), ("dx", dx):
            self.play(
                *list(map(Indicate, [equation.get_part_by_tex(tex), mob])))
            self.wait(2)
        self.play(*[
            ReplacementTransform(
    ...
    def dA_remains_a_mystery(self):
        ...
        A_circle, dA_circle = [
            Circle(color=color).replace(mob, stretch=True).scale_in_place(1.5)
            for mob, color in [(self.A_func, RED), (self.deriv_equation,
                                                    GREEN)]
        ]
        q_marks = TexMobject("???")
        ...
        self.play(Blink(randy))
        self.wait()
        self.play(randy.change_mode, "surprised", randy.look_at, dA_circle,
    ...
    def write_example_inputs(self):
        ...
        labels_lines_vects = list(
            zip([three, three_plus_dx], self.right_v_lines, [LEFT, RIGHT]))
        ...
        for label, line, vect in labels_lines_vects:
            ...
            self.play(FadeIn(label), FadeIn(label.arrow),
    ...
    def show_dA_dx_in_detail(self):
        d = self.default_right_x
        expression = TexMobject("{A(", "%d.001" % d, ") ", "-A(", "%d" % d,
                                ")", "\\over \\,", "0.001}", "\\approx",
                                "%d" % d, "^2")
        ...
        self.play(
            ...
            ReplacementTransform(self.graph_label_copy.copy(),
                                 VGroup(*expression[-2:]),
                                 run_time=2))
        self.wait()

    def show_smaller_x(self):
        self.transition_to_alt_config(default_right_x=2,
                                      deriv_dx=0.04,
                                      transformation_kwargs={"run_time": 2})
...
class AlternateAreaUnderCurve(PlayingTowardsDADX):
    ...
    def approximation_improves_for_smaller_dx(self):
        ...
        dx_to_zero_words = TextMobject("Gets better \\\\ as", "$dx \\to 0$")
        ...
        self.wait()
        self.transition_to_alt_config(deriv_dx=self.deriv_dx / 4.0,
                                      transformation_kwargs={"run_time": 2})
...
class NextVideoWrapper(Scene):
    def construct(self):
        ...
        titles = [
            TextMobject("Chapter %d:" % d, s) for d, s in [
...
class FundamentalTheorem(Scene):
    def construct(self):
        ...
        deriv = TexMobject("{dA", "\\over \\,", "dx}", "=", "x^2")
...
class NextVideos(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            *it.chain(*[[pi.change_mode, "pondering", pi.look_at, this_video]
                        for pi in self.get_students()]))
        self.play(*[
            ApplyMethod(pi.look_at, series) for pi in self.get_pi_creatures()
        ])
        self.play(*[
            ApplyMethod(video.shift,
                        0.5 * video.get_height() * DOWN,
