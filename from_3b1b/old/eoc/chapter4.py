...
class TransitionFromLastVideo(TeacherStudentsScene):
    def construct(self):
        ...
        combination_rules = VGroup(*[
            TexMobject("\\frac{d}{dx}\\left(%s\\right)" % tex) for tex in [
                "\\sin(x) + x^2",
                "\\sin(x)(x^2)",
                "\\sin(x^2)",
            ]
        ...
        #Simple rules
        self.add(series)
        self.play(FadeIn(brace), last_video.set_color, YELLOW)
        ...
        self.play(last_video.restore, Animation(simple_rules), brace.next_to,
                  this_video, DOWN, this_video.set_color, YELLOW)
        ...
        for rule in combination_rules:
            ...
            if rule is combination_rules[-1]:
                ...
                added_anims = [
                    inner_func.shift, 0.5 * DOWN, inner_func.set_color, YELLOW
                ]
            self.play(interior.set_color, YELLOW, *added_anims, lag_ratio=0.5)
            self.wait()
        self.wait()
        ...
        for mob in subtraction, decomposed_subtraction, division, decomposed_division:
            mob.next_to(VGroup(self.get_teacher(),
                               self.get_students()[-1]),
                        UP,
                        buff=MED_LARGE_BUFF)
        ...
        self.play(
            ...
            FRAME_Y_RADIUS * UP,
            UP,
            combination_rules.to_edge,
            UP,
        )
        pairs = [(subtraction, decomposed_subtraction),
                 (division, decomposed_division)]
        for question, answer in pairs:
            self.play(
                Write(question),
                combination_rules.fade,
                0.2,
        ...
        monster = TexMobject("\\Big(", "e^{\\sin(x)} \\cdot", "\\cos\\Big(",
                             "\\frac{1}{x^3}", " + x^3", "\\Big)", "\\Big)^4")
        monster.next_to(self.get_pi_creatures(), UP)
        parts = [
            VGroup(*monster[3][2:]),
            VGroup(*monster[3][:2]), monster[4],
            VGroup(monster[2], monster[5]), monster[1],
        ...
        words = list(
            map(TextMobject, [
                "composition", "product", "composition", "sum", "composition"
            ]))

        for word, part in zip(words, reversed(parts)):
            word.set_color(YELLOW)
            word.next_to(monster, UP)
            self.play(FadeIn(word), part.scale_in_place, 1.2, part.set_color,
                      YELLOW)
            self.wait()
...
class DampenedSpring(Scene):
    def construct(self):
        compact_spring, extended_spring = [
            ParametricFunction(
                lambda t:
                (t / denom) * RIGHT + np.sin(t) * UP + np.cos(t) * OUT,
                t_max=12 * np.pi,
                color=GREY,
            ).shift(3 * LEFT) for denom in (12.0, 2.0)
        ]
        ...
        def update_spring(spring, a):
            spring.interpolate(compact_spring, extended_spring,
                               0.5 * (np.exp(-4 * a) * np.cos(40 * a) + 1))

        equation = TexMobject("\\text{Length} = 2 + e^{-4t}\\cos(20t)")
        equation.to_edge(UP)

        self.add(moving_spring, equation)
        self.play(
            UpdateFromAlphaFunc(moving_spring,
                                update_spring,
                                run_time=10,
                                rate_func=linear))
        self.wait()
...
class PreSumRuleDiscussion(Scene):
    def construct(self):
        ...
        specific = TexMobject("\\frac{d}{dx}(", "\\sin(x)", "+", "x^2", ")",
                              "=", "\\cos(x)", "+", "2x")
        general = TexMobject("\\frac{d}{dx}(", "g(x)", "+", "h(x)", ")", "=",
        ...
        for indices in [(1, 2, 3), (6, ), (7, 8)]:
            self.play(*[
                ReplacementTransform(specific[i].copy(), general[i])
                for i in indices
            ])
            self.wait()
...
class SumRule(GraphScene):
    ...
    def add_graphs(self):
        ...
        sum_graph = self.get_graph(lambda x: np.sin(x) + x**2, color=SUM_COLOR)
        ...
        sine_v_lines, parabox_v_lines = v_line_sets = [
            self.get_vertical_lines_to_graph(graph,
        ...
        ], [graph.fade for graph in graphs], [Animation(labels)]))
        self.wait()
    ...
    def zoom_in_on_graph(self):
        graph_parts = VGroup(self.axes, self.sine_graph, self.parabola,
                             self.sum_graph, *self.v_line_sets)
        ...
        for mob in self.graph_labels.target:
            mob.scale(1. / self.graph_scale_factor,
                      about_point=mob.get_bottom())
            mob.shift_onto_screen()
        self.play(*list(map(MoveToTarget, [graph_parts, self.graph_labels])))
        self.wait()
    ...
    def show_example_stacking(self):
        ...
        sine_lines, parabola_lines = [
            VGroup(example_v_lines[i], nudged_v_lines[i]) for i in (0, 1)
        ]
        faders = VGroup(*[
            line for line in it.chain(*v_line_sets)
            if line not in example_v_lines
        ])
        label_groups = []
        for line, tex, vect in zip(sine_lines, ["", "+dx"], [LEFT, RIGHT]):
            dot = Dot(line.get_bottom(), radius=0.03, color=YELLOW)
            label = TexMobject("x=" + str(self.example_input) + tex)
        ...
        self.wait()
        self.play(sine_lines[0].shift, UP + 4 * LEFT, sine_lines[0].brace_anim,
        ...
        self.wait()
        self.play(*it.chain(*[[line.restore, line.brace_anim]
                              for line in example_v_lines]))

        ## Nudged_lines
        self.play(
            Write(label_groups[1]),
            *it.chain(*[[line.restore, line.set_stroke, None, 4]
                        for line in nudged_v_lines]))
        self.wait()
        for line in nudged_v_lines:
            self.play(GrowFromCenter(line.brace), Write(line.brace_text))
        self.wait()
    ...
    def show_df(self):
        ...
        deriv_mob.next_to(self.func_mob,
                          DOWN,
                          buff=MED_LARGE_BUFF,
                          aligned_edge=LEFT)
        ...
        self.play(parabola_lines[0].shift,
        ...
        d_x_squared_lines = self.show_difference(parabola_lines, d_x_squared,
                                                 VGroup())
        self.wait()
    ...
    def show_difference(self, v_lines, target_tex, added_tex):
        ...
        h_lines = VGroup(
            *[DashedLine(ORIGIN, 2 * RIGHT, stroke_width=3) for x in range(2)])
        ...
        self.wait()
        self.play(ReplacementTransform(brace_text.copy(), target_tex),
                  Write(added_tex))
        return VGroup(h_lines, brace, brace_text)

    def expand_derivative(self):
        expanded_deriv = TexMobject("df", "=", "\\cos(x)", "\\,dx", "+", "2x",
                                    "\\,dx")
        ...
        expanded_deriv.next_to(self.deriv_mob,
                               DOWN,
                               buff=MED_LARGE_BUFF,
                               aligned_edge=LEFT)
        background_rect = BackgroundRectangle(expanded_deriv)

        rearranged_deriv = TexMobject("{df \\over", "dx}", "=", "\\cos(x)",
                                      "+", "2x")
        ...
        self.play(
            FadeIn(background_rect, rate_func=squish_rate_func(smooth, 0.6,
                                                               1)),
        ...
        self.play(*[
            Transform(
                expanded_deriv[i], rearranged_deriv[j], path_arc=-np.pi / 2)
...
class DiscussProducts(TeacherStudentsScene):
    def construct(self):
        ...
        wrong_product_rule.next_to(self.get_teacher().get_corner(UP + LEFT),
                                   UP,
                                   buff=MED_LARGE_BUFF).shift_onto_screen()

        self.teacher_says("Products are a bit different", target_mode="sassy")
        self.wait(2)
        self.play(
        ...
        self.wait()
        self.teacher_says("Think about the \\\\ underlying meaning",
...
class NotGraphsForProducts(GraphScene):
    ...
    def construct(self):
        self.setup_axes()
        sine_graph = self.get_graph(np.sin, color=SINE_COLOR)
        sine_graph.label = self.get_graph_label(sine_graph,
                                                "\\sin(x)",
                                                x_val=3 * np.pi / 2,
                                                direction=DOWN)
        parabola = self.get_graph(lambda x: x**2, color=X_SQUARED_COLOR)
        ...
        product_graph = self.get_graph(lambda x: np.sin(x) * (x**2),
                                       color=PRODUCT_COLOR)
        product_graph.label = self.get_graph_label(product_graph,
                                                   "\\sin(x)x^2",
                                                   x_val=2.8,
                                                   direction=UP + RIGHT,
                                                   buff=0)
        ...
        words.set_color(RED)
        self.play(everything.fade, Write(words))
        self.wait()
...
class IntroduceProductAsArea(ReconfigurableScene):
    ...
    def introduce_box(self):
        ...
        for new_x in 0.5, 2, self.default_x:
            self.animate_x_change(new_x, run_time=2)
        self.wait()
    ...
    def talk_though_sine(self):
        ...
        for x, rt in zip([0.25, np.pi / 2, 3, self.default_x], [2, 4, 4, 2]):
            ...
            self.wait()
        self.play(*it.chain(list(map(FadeOut, [y_axis, graph, label, v_line])),
                            [Animation(x_axis)]))
        self.wait()
    ...
    def define_f_of_x(self):
        f_def = TexMobject("f(x)", "=", self.top_func_label,
                           self.side_func_label, "=", "\\text{Area}")
    ...
    def nudge_x(self):
        ...
        self.animate_x_change(self.default_x + self.dx,
                              rate_func=there_and_back,
                              run_time=2,
        ...
        self.play(
            ...
            VGroup(*labels[1]).shift,
            right_box.get_width() * RIGHT,
        )
        self.play(
            df_boxes.space_out_submobjects,
            1.1,
            df_boxes.move_to,
            box,
            UP + LEFT,
    ...
    def get_nudge_label_group(self):
        ...
        dx_line = Line(*[
            line.number_to_point(self.x_slider.x_val + num) for num in (
                0,
                self.dx,
            )
        ])
        dx_line.set_stroke(self.df_box_kwargs["fill_color"], width=6)
    ...
    def get_df_boxes(self):
        ...
        df_boxes = VGroup(*[
            Rectangle(height=height, width=width, **
                      self.df_box_kwargs).next_to(box, vect, buff=0)
            for (height, width), vect in zip(heights_and_widths, vects)
        ])
        return df_boxes
    ...
    def write_df(self):
        ...
        box, labels = self.box_label_group
        self.full_box_parts = VGroup(
            *it.chain([box], self.df_boxes, labels, self.df_box_labels))

        self.play(Write(VGroup(*deriv[:2])))
        self.play(df_boxes_copy.arrange, df_boxes_copy.set_fill, None,
                  self.df_box_kwargs["fill_opacity"], df_boxes_copy.next_to,
                  deriv[1])
    ...
    def set_color_boxes_and_label(self, boxes, label):
        ...
        self.play(label[1].scale_in_place, 1.1, rate_func=there_and_back)
        self.play(boxes.restore, label.restore)
        self.wait()

    def set_color_right_boxes(self):
        self.set_color_boxes_and_label(VGroup(*self.df_boxes[1:]),
    ...
    def describe_bottom_box(self, bottom_box_area):
        ...
        faders = VGroup(*[
            m for m in self.full_box_parts
            if m not in [bottom_box, top_label, right_label]
        ])
        faders.save_state()
        ...
        self.play(ReplacementTransform(bottom_box_copy, bottom_box_area),
    ...
    def describe_right_box(self, right_box_area):
        ...
        faders = VGroup(*[
            m for m in self.full_box_parts
            if m not in [right_box, top_label, right_label]
        ])
        faders.save_state()
    ...
    def expand_derivative(self):
        ...
        expanded_deriv = TexMobject("df", "=", self.top_func_label,
                                    self.side_func_derivative, "\\,dx", "+",
                                    self.side_func_label,
                                    self.top_func_derivative, "\\,dx")
        ...
        for index in 6, 3:
            self.deriv.submobjects.insert(index + 1, self.deriv[index].copy())
        ...
        self.play(*[
            ReplacementTransform(
                expanded_deriv[i], final_deriv[j], path_arc=-np.pi / 2)
            for i, j in [
                (0, 0),
                (1, 2),
                (2, 3),
    ...
    def write_mneumonic(self):
        ...
        self.play(morty.change_mode, "raise_right_hand", Write(words))
        self.wait()

    ###############

    def animate_x_change(self,
                         target_x,
        ...
        self.play(UpdateFromAlphaFunc(box_label_group, update_box_label_group),
                  UpdateFromAlphaFunc(x_slider, update_x_slider), *added_anims,
                  **kwargs)
    ...
    def get_box(self, x):
        box = Rectangle(width=self.x_unit_to_space_unit * self.top_func(x),
    ...
    def get_box_labels(self, box):
        result = VGroup()
        for label_tex, vect in (self.top_func_label,
                                UP), (self.side_func_label, RIGHT):
...
class MneumonicExample(TeacherStudentsScene):
    def construct(self):
        d, left, right, rp = deriv_q = TexMobject("\\frac{d}{dx}(", "\\sin(x)",
                                                  "x^2", ")")
        deriv_q.to_edge(UP)
        ...
        self.play(left_words.next_to, left_terms, DOWN, MED_LARGE_BUFF, RIGHT)
        self.play(ReplacementTransform(left_words.copy(), left_terms))
        self.wait()
        ...
        self.play(right_words.next_to, right_terms, DOWN, MED_LARGE_BUFF, LEFT)
        self.play(ReplacementTransform(right_words.copy(), right_terms))
        self.wait()
        self.play(
...
class ThreeLinesChainRule(ReconfigurableScene):
    ...
    def introduce_line_group(self):
        ...
        for label in labels:
            self.play(ReplacementTransform(last_label.copy(), label))
            self.wait()
            last_label = label
    ...
    def talk_through_movement(self):
        ...
        self.play(FadeIn(sine_text), ShowCreation(sine_arrow))
        self.wait(2)
    ...
    def nudge_x(self):
        ...
        braces = VGroup()
        numbers = VGroup()
        for start, end, line, label, config in zip(starts, ends, lines, labels,
                                                   self.line_configs):
        ...
        dx_squared_value = TexMobject("= 2x\\,dx ", "\\\\ = 2(%s)dx" % x_value)
        ...
        self.animate_x_change(self.example_x + self.dx,
                              run_time=1,
        ...
        self.play(two_x_dx_copy.move_to,
                  dx_squared,
                  LEFT,
                  dx_squared.next_to,
                  dx_squared,
    ...
    def replace_x_squared_with_h(self):
        ...
        self.transition_to_alt_config(return_to_original_configuration=False,
                                      **new_config)
    ...
    def animate_x_change(self, target_x, **kwargs):
        ...
        def update(line_group, alpha):
            ...
            for line, label, config in zip(lines, labels, self.line_configs):
                new_label = self.get_line_label(line, new_x, **config)
                Transform(label, new_label).update(1)
            line_group.x_val = new_x

        self.play(UpdateFromAlphaFunc(self.line_group, update), *added_anims,
                  **kwargs)
    ...
    def get_number_line(self, center_y, **number_line_config):
    ...
    def get_line_label(self, number_line, x, func, func_label, triangle_color,
                       **spillover_kwargs):
        triangle = RegularPolygon(
            n=3,
            start_angle=-np.pi / 2,
        ...
        triangle.move_to(number_line.number_to_point(func(x)), DOWN)
...
class GeneralizeChainRule(Scene):
    def construct(self):
        ...
        general = TexMobject("\\frac{d}{dx}", "g(", "h(x)", ")", "=",
                             "{dg \\over ", " dh}", "(", "h(x)", ")",
                             "{dh \\over", " dx}", "(x)")
        ...
        self.add(example)
        braces = VGroup(example_outer_brace, example_inner_brace,
        ...
        for brace in braces:
            ...
            self.wait()
        self.wait()
        self.play(*it.chain(*[[mob.scale_in_place, 1.2, mob.set_color, YELLOW]
                              for mob in (example_inner, d_example_inner)]),
                  rate_func=there_and_back)
        self.play(
            Transform(example_inner.copy(),
                      d_example_inner,
                      path_arc=-np.pi / 2,
                      remover=True))
        self.wait()
        self.play(GrowFromCenter(d_example_d_inner_brace),
                  Write(d_example_d_inner_brace.text))
        self.play(
            Transform(VGroup(*reversed(example_inner.copy())),
                      d_example_d_inner,
                      path_arc=-np.pi / 2,
                      run_time=2,
                      remover=True))
        ...
        self.play(morty.set_fill, None, 1, morty.change_mode,
                  "raise_left_hand", morty.look,
                  UP + LEFT, d_general_outer_copy.next_to,
        ...
        self.play(morty.change_mode, "hooray", morty.look_at, inner,
                  inner.shift, UP)
        ...
        self.play(Blink(morty))
        self.wait()
        self.play(*list(map(FadeOut, [d_general_outer_copy, inner, circle])))
        ...
        points = VGroup(
            *list(map(VectorizedPoint, [m.get_left() for m in to_collapse])))
        self.play(
        ...
        self.play(
            PiCreatureSays(
                ...
                notational trick
            """),
            VGroup(dg_dh, dh_dx, equals_dg_dx, strikes, *general[:5]).shift,
            DOWN, FadeOut(example))
        self.wait()
        self.play(Blink(morty))
        self.wait()
...
class WatchingVideo(PiCreatureScene):
    def construct(self):
        ...
        formulas = VGroup(*[
            TexMobject("\\frac{d}{dx}\\left( %s \\right)" % s) for s in [
                "e^x \\sin(x)",
        ...
        for formula in formulas:
            self.play(Write(formula, run_time=2), randy.change_mode,
                      "thinking")
            self.wait()
...
class NextVideo(TeacherStudentsScene):
    def construct(self):
        ...
        pre_expression = TexMobject("x", "^2", "+", "y", "^2", "=", "1")
        d_expression = TexMobject("2", "x", "\\,dx", "+", "2", "y", "\\,dy",
                                  "=", "0")
        expression_to_d_expression_indices = [1, 0, 0, 2, 4, 3, 3, 5, 6]
        expression = VGroup()
        ...
        for mob in expression, d_expression:
            mob.scale(1.2)
            mob.next_to(self.get_teacher().get_corner(UP + LEFT),
                        UP,
                        buff=MED_LARGE_BUFF)
            mob.shift_onto_screen()
        ...
        self.add(series)
        self.play(next_video.shift, 0.5 * DOWN, next_video.set_color, YELLOW,
                  self.get_teacher().change_mode, "raise_right_hand")
        self.wait()
        self.play(
            Write(expression), *[
...
class Thumbnail(IntroduceProductAsArea):
    CONFIG = {"default_x": 0.8, "dx": 0.05}

    def construct(self):
        self.x_slider = self.get_x_slider(self.default_x)
        blg = self.box_label_group = self.get_box_label_group(self.default_x)
        df_boxes = self.get_df_boxes()
