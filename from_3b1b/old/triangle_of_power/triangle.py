...
class TOP(VMobject):
    ...
    def generate_points(self):
        vertices = [
            self.radius *
            rotate_vector(RIGHT, 7 * np.pi / 6 - i * 2 * np.pi / 3)
            for i in range(3)
        ]
        self.triangle = Polygon(*vertices, color=WHITE, stroke_width=5)
    ...
    def put_in_vertex(self, index, mobject):
        ...
        mobject.center()
        mobject.shift(
            interpolate(self.get_center(),
                        self.get_vertices()[index], 0.7))
        return mobject

    def get_surrounding_circle(self, color=YELLOW):
        return Circle(radius=1.7 * self.radius, color=color).shift(
    ...
    def get_value_height(self):
        return self.triangle.get_height(
        ) / self.triangle_height_to_number_height
...
class IntroduceNotation(Scene):
    def construct(self):
        ...
        two, three, eight = [
            top.put_on_vertex(i, num) for i, num in enumerate([2, 3, 8])
        ]
...
class SixDifferentInverses(Scene):
    def construct(self):
        ...
        vects = it.starmap(
            op.add,
            it.product([3 * UP, 0.5 * UP, 2 * DOWN], [2 * LEFT, 2 * RIGHT]))
        ...
        self.play(ApplyMethod(VMobject(*rules[::2]).to_edge, LEFT),
    ...
    def animate_top_rule(self, top_rule):
        ...
        self.play(Transform(*cancel_parts, rate_func=rush_into))
        self.play(FadeOut(VMobject(*cancel_parts)),
    ...
    def construct(self):
        ...
        for i, rule in enumerate(rules):
            if i % 2 == 0:
                ...
                anim = Blink(randy)
            self.play(ApplyMethod(bubble.add_content, rule), anim)
            self.wait()
        self.wait()
        ...
        self.play(ApplyMethod(randy.change_mode, "angry"),
                  Transform(bubble, speech_bubble), Transform(equation, words),
...
class AdditiveProperty(Scene):
    def construct(self):
        ...
        self.play(ApplyMethod(exp_rule.to_edge, UP),
        ...
        self.wait()
        all_tops = [
            m for m in t_exp_rule.split() + t_log_rule.split()
            if isinstance(m, TOP)
        ]
    ...
    def write_old_style_rules(self):
        ...
        a4, x3, y3, eq2, a5, x4, p2, a6, y4 = np.array(
            end.split())[[3, 5, 6, 8, 12, 14, 16, 20, 22]]
        start_copy = start.copy()
        self.play(Write(start_copy))
        self.wait()
        self.play(
            Transform(VMobject(a1, x1, a2, y1, eq1, a3, p1, x2, a3.copy(), y2),
                      VMobject(a4, x3, a4.copy(), y3, eq2, a5, p2, x4, a6,
                               y4)))
        self.play(Write(end))
        self.clear()
    ...
    def put_in_circles(self, tops):
        ...
        for top in tops:
            ...
            anims.append(Transform(top.copy().set_color(YELLOW), circle))
    ...
    def set_color_appropriate_parts(self, t_exp_rule, t_log_rule):
        #Horribly hacky
        circle1 = t_exp_rule.split()[0].put_on_vertex(2, Circle())
        top_dot = t_exp_rule.split()[1]
        circle2 = t_exp_rule.split()[2].put_on_vertex(2, Circle())
        top_plus = t_exp_rule.split()[4].values[1]

        bottom_times = t_log_rule.split()[0].values[2]
        circle3 = t_log_rule.split()[2].put_on_vertex(1, Circle())
        bottom_plus = t_log_rule.split()[3]
        circle4 = t_log_rule.split()[4].put_on_vertex(1, Circle())

        mob_lists = [[circle1, top_dot, circle2], [top_plus], [bottom_times],
                     [circle3, bottom_plus, circle4]]
        for mobs in mob_lists:
            copies = VMobject(*mobs).copy()
            self.play(ApplyMethod(copies.set_color, YELLOW, run_time=0.5))
            self.play(
                ApplyMethod(copies.scale_in_place,
                            1.2,
...
class ConstantOnTop(Scene):
    def construct(self):
        ...
        self.play(FadeIn(lower_left_x), Write(x_cubed), run_time=1)
        self.wait()
        self.play(*[
            Transform(*pair, path_arc=np.pi) for pair in [
...
class MultiplyWithConstantTop(Scene):
    def construct(self):
        ...
        equals = TexMobject("=")
        top_exp_equation = VMobject(top1, times, top2, equals, top3)
...
class RightStaysConstantQ(Scene):
    def construct(self):
        top1, top2, top3 = old_tops = [
            TOP(None, s, "8") for s in ("x", "y", TexMobject("x?y"))
        ]
        q_mark = TexMobject("?").scale(2)
        equation = VMobject(top1, q_mark, top2, TexMobject("="), top3)
        equation.arrange(buff=0.7)
        symbols_at_top = VMobject(
            *[top.values[1] for top in (top1, top2, top3)])
        ...
        bubble.clear()
        self.play(Transform(triangle, equation),
                  FadeOut(eight),
...
class AOplusB(Scene):
    def construct(self):
        self.add(
            TexMobject("a \\oplus b = \\dfrac{1}{\\frac{1}{a} + \\frac{1}{b}}"
                       ).scale(2))
        self.wait()
...
class GoExplore(Scene):
    def construct(self):
        ...
        self.wait(4)
        self.play(ApplyMethod(VMobject(explore, by_the_way).shift, 20 * LEFT))
        self.wait(3)
