...
class DontLearnFromSymbols(Scene):
    def construct(self):
        ...
        equation = VMobject(TOP(2, "x"), TexMobject("\\times"), TOP(2, "y"),
                            TexMobject("="), TOP(2, "x+y"))
        equation.arrange()
        q_marks = TextMobject("???")
...
class NotationReflectsMath(Scene):
    def construct(self):
        ...
        self.play(ShowCreationPerSubmobject(
            arrow, rate_func=lambda t: min(smooth(3 * t), 1)),
...
class AsymmetriesInTheMath(Scene):
    def construct(self):
        assyms_of_top = VMobject(
            ...
            TOP("a", "b", "c", radius=0.75).set_color(BLUE)).arrange()
...
class NotSymbolicPatterns(Scene):
    def construct(self):
        ...
        self.play(Write(substantive_reasoning),
                  ApplyMethod(randy.change_mode, "pondering_looking_left"),
                  run_time=1)
        self.wait(2)
        self.play(ApplyMethod(line.shift, 10 * DOWN),
...
class TriangleOfPowerIsBetter(Scene):
    def construct(self):
        ...
        top.set_color(BLUE)
        alts = VMobject(
            *list(map(TexMobject, ["x^y", "\\log_x(z)", "\\sqrt[y]{z}"])))
