    ...
    def interpolate_mobject(self, alpha):
        ...
        points = [
            ...
            (np.sign(np.cos(theta)) *
             np.sqrt(np.tan(theta)**2 - np.sin(theta)**2) + np.cos(theta)) *
            RIGHT + self.center,
        ]
...
class Notation(Scene):
    ...
    def shift_to_good_and_back(self):
        ...
        new_sin, new_cos, new_tan = [
            VMobject().set_points_as_corners(corners).replace(
...
class TwoThreeEightExample(Scene):
    def construct(self):
        ...
        self.play(
            Transform(VMobject(two1, dot1, two2, dot2, brace, three),
                      exp_three,
                      path_arc=-np.pi / 2), Transform(two3, base_two))
        self.clear()
        ...
        self.play(*[
            ...
            for pair in [(exp_three, rad_three), (
                VMobject(),
                rad1), (VMobject(),
                        rad2), (eight,
                                rad_eight), (eq, rad_eq), (base_two, rad_two)]
        ])
        self.wait()
        self.play(
            ApplyMethod(VMobject(rad1, rad2).set_color,
                        RED,
        ...
        self.play(*[
            Transform(*pair, path_arc=np.pi / 2)
            for pair in [(rad1, l), (rad2, o), (
                rad2.copy(),
                g), (rad_two, log_two), (VMobject(), p1), (
                    rad_eight,
                    log_eight), (VMobject(),
                                 p2), (rad_eq, log_eq), (rad_three, log_three)]
        ])
        self.wait()
        self.play(
            ApplyMethod(VMobject(l, o, g).set_color,
                        RED,
                        rate_func=there_and_back,
                        run_time=2))
        self.wait()
...
class PascalsCollision(Scene):
    def construct(self):
        ...
        equation = TexMobject(["{n \\choose k}", " = \\dfrac{n!}{(n-k)!k!}"])
        equation.scale(0.5)
        ...
        self.play(Write(words))
        self.wait()
        self.play(Transform(pascals_triangle, final_triangle),
                  Transform(n_choose_k, final_triangle), FadeOut(formula),
...
class LogarithmProperties(Scene):
    def construct(self):
        ...
        self.play(ApplyMethod(randy.change_mode, "confused"),
                  ShowCreation(bubble), Write(words))
        self.show_frame()
...
class HaveToShare(Scene):
    def construct(self):
        words = list(
            map(TextMobject, ["Lovely", "Symmetrical", "Utterly Reasonable"]))
