...
class FromPuzzleToSolution(MovingCameraScene):
    def construct(self):
        ...
        self.play(frame.replace, images[0], run_time=3)
        self.wait()
...
class AskAboutFindingNewVelocities(Scene):
    ...
    def add_velocity_phase_space_point(self):
        self.vps_point = VectorizedPoint([
            ...
            np.sqrt(self.block2.mass) * self.block2.velocity, 0
        ])
    ...
    def ask_about_transfer(self):
        ...
        self.wait()
        self.play(FadeInFrom(momentum_words, RIGHT),
    ...
    def show_ms_and_vs(self):
        ...
        for group in label_groups:
            group.rects = VGroup(*map(SurroundingRectangle, group))
    ...
    def show_value_on_equations(self):
        ...
        v_terms, momentum_v_terms = [
            VGroup(*[expr.get_part_by_tex("v_{}".format(d)) for d in [1, 2]])
            for expr in [energy_expression, momentum_expression]
        ]
        v_braces = VGroup(
            *[Brace(term, UP, buff=SMALL_BUFF) for term in v_terms])
        ...
        energy_decimal.add_updater(lambda m: m.set_value(get_energy()))
        ...
        momentum_decimal.add_updater(lambda m: m.set_value(get_momentum()))
    ...
    def get_energy_and_momentum_expressions(self):
        ...
        momentum_expression = TexMobject("m_1 v_1 + m_2 v_2 =",
                                         "\\text{const.}",
                                         tex_to_color_map=tex_to_color_map)
        return VGroup(
            energy_expression,
    ...
    def go_through_next_collision(self,
    ...
    def get_next_velocity_labels_animation(self):
        return FadeInFrom(self.get_next_velocity_labels(), LEFT, run_time=0.5)
...
class IntroduceVelocityPhaseSpace(AskAboutFindingNewVelocities):
    ...
    def show_two_equations(self):
        ...
        v1_terms, v2_terms = v_terms = VGroup(*[
            VGroup(*[expr.get_parts_by_tex(tex) for expr in equations])
            for tex in ("v_1", "v_2")
        ])
        for eq in equations:
    ...
    def draw_axes(self):
        ...
        vps_dot.add_updater(
            lambda m: m.move_to(axes.coords_to_point(*self.get_velocities())))
    ...
    def rescale_axes(self):
        ...
        vps_dot.add_updater(lambda m: m.move_to(ellipse.get_left()))
        ...
        xy_equation = self.xy_equation = TexMobject("\\frac{1}{2}", "\\left(",
                                                    "x^2", "+", "y^2",
                                                    "\\right)", "=",
                                                    "\\text{const.}")
        ...
        vps_dot.clear_updaters()
        vps_dot.add_updater(lambda m: m.move_to(
            self.axes.coords_to_point(*self.vps_point.get_location()[:2])))

        self.play(FadeInFrom(xy_equation, UP), FadeOut(equations[1]))
        self.wait()
        ...
        for x in [0.5 * curr_x, 2 * curr_x, curr_x]:
            ...
            self.play(
                ...
                x * RIGHT,
                UpdateFromFunc(
                    ellipse, lambda m: m.set_width(
                        2 * get_norm(vps_dot.get_center() - axes_center, ), ).
                    move_to(axes_center)),
                run_time=2,
            )
        self.wait()
    ...
    def show_conservation_of_momentum_equation(self):
        ...
        self.play(FadeOut(xy_equation), energy_expression.set_fill,
                  {"opacity": 0.2}, FadeInFromDown(momentum_expression))
    ...
    def show_momentum_line(self):
        ...
        slope_label = TexMobject("\\text{Slope =}",
                                 "-\\sqrt{\\frac{m_1}{m_2}}")
        ...
        foreground_mobs = VGroup(self.equations[1], self.momentum_xy_equation,
    ...
    def reiterate_meaning_of_line_and_circle(self):
        ...
        for x in [0.25, -0.5, 0.25]:
            self.play(vps_point.shift, x * line_vect, run_time=2)
        self.wait()
        self.play(
            Rotating(
                vps_point,
    ...
    def reshow_first_jump(self):
        ...
        for n in range(8):
            vps_point.move_to([start_point, curr_point][n % 2])
            self.wait(0.5)
        self.wait()
    ...
    def show_reflection_about_x(self):
        ...
        # )
        for n in range(4):
            self.play(vps_point.move_to, [old_location, curr_location][n % 2])
        self.wait()

        group = VGroup(self.ellipse, self.lines[-1],
    ...
    def show_remaining_collisions(self):
        ...
        end_region = Polygon(axes.coords_to_point(0, 0),
...
class HuntForPi(TeacherStudentsScene):
    def construct(self):
        self.student_says("Hunt for $\\pi$!",
                          bubble_kwargs={"direction": LEFT},
                          target_mode="hooray")
        self.change_all_student_modes(
            "hooray", added_anims=[self.teacher.change, "happy"])
        self.wait()
...
class YCoordZero(Scene):
    def construct(self):
        ...
        rect.save_state()
        self.play(rect.stretch, 0.01, 1, rect.set_fill, {"opacity": 1})
        self.wait()
...
class CircleDiagramFromSlidingBlocks(Scene):
    ...
    def construct(self):
        ...
        self.show_circle_lines(times=times,
    ...
    def show_circle_lines(self, times, slope):
        ...
        vector.add_updater(
            lambda v: v.put_start_and_end_on(ORIGIN, dot.get_center()))
    ...
    def get_lines(self, radius, slope):
        ...
        for n in range(n_clacks + 1):
            ...
            if n % 2 == 0:
                angle *= -1
            new_point = radius * np.array([-np.cos(angle), -np.sin(angle), 0])
            points.append(new_point)

        lines = VGroup(*[Line(p1, p2) for p1, p2 in zip(points, points[1:])])
...
class CircleDiagramFromSlidingBlocksSameMass(CircleDiagramFromSlidingBlocks):
    CONFIG = {"BlocksAndWallSceneClass": BlocksAndWallExampleSameMass}


class CircleDiagramFromSlidingBlocksSameMass1e1(CircleDiagramFromSlidingBlocks
                                                ):
    CONFIG = {"BlocksAndWallSceneClass": BlocksAndWallExampleMass1e1}


class CircleDiagramFromSlidingBlocks1e2(CircleDiagramFromSlidingBlocks):
    CONFIG = {"BlocksAndWallSceneClass": BlocksAndWallExampleMass1e2}


class CircleDiagramFromSlidingBlocks1e4(CircleDiagramFromSlidingBlocks):
    CONFIG = {"BlocksAndWallSceneClass": BlocksAndWallExampleMass1e4}
...
class AnalyzeCircleGeometry(CircleDiagramFromSlidingBlocks, MovingCameraScene):
    ...
    def add_mass_ratio_label(self, mass_ratio=None):
        ...
        mass_ratio_label = TextMobject("Mass ratio =",
                                       "{:,} : 1".format(mass_ratio))
    ...
    def show_equal_arc_lengths(self):
        ...
        lower_arcs = VGroup(*[
            Arc(start_angle=(PI + n * 2 * theta),
                angle=(2 * theta),
                radius=radius) for n in range(n_arcs + 1)
        ])
        ...
        movers = VGroup(*it.chain(*zip(arc_copies, equal_signs)))
    ...
    def use_arc_lengths_to_count(self):
        ...
        self.play(ShowIncreasingSubsets(all_arcs),
    ...
    def focus_on_three_points(self):
        ...
        theta_arc = Arc(radius=1, start_angle=-90 * DEGREES, angle=theta)
        ...
        two_theta_label.next_to(arc_center, normalize(arc_center), SMALL_BUFF)
        ...
        self.play(
            LaggedStartMap(FadeOut,
                           VGroup(*to_fade.family_members_with_points())))
        ...
        self.wait()
        self.play(ShowCreation(theta_arc), FadeInFrom(theta_label, UP))
        self.wait()
        self.play(
            line_pair.set_stroke,
            WHITE,
            1,
    ...
    def show_likewise_for_all_jumps(self):
        ...
        self.play(
            Restore(lines, lag_ratio=0.5, run_time=2),
            FadeOut(self.center_lines_group),
            FadeOut(self.three_dots),
        )
        self.play(
            LaggedStartMap(
                ApplyFunction, every_other_line, lambda line:
    ...
    def drop_arc_for_each_hop(self):
        ...
        self.remove(lines)
        for line, arc, label, wedge in zip(lines, arcs, two_theta_labels,
                                           wedges):
            self.play(
                ShowCreation(line),
    ...
    def zoom_out(self):
        frame = self.camera_frame
        self.play(frame.scale,
                  2, {"about_point": (TOP + RIGHT_SIDE)},
                  run_time=3)
        self.wait()
...
class InscribedAngleTheorem(Scene):
    ...
    def show_circle(self):
        ...
        def get_point(angle):
            return circle.point_from_proportion((angle % TAU) / TAU)
        ...
        def get_dots():
            return VGroup(*[get_dot(at.get_value()) for at in angle_trackers])

        def update_labels(labels):
            center = circle.get_center()
            for dot, label in zip(dots, labels):
                label.move_to(center + 1.2 * (dot.get_center() - center))
        ...
        def get_center_lines():
            ...
            lines = VGroup(
                *[Line(p1, p2) for p1, p2 in zip(points, points[1:])])
        ...
        labels = VGroup(*[TexMobject("P_{}".format(n + 1)) for n in range(3)])
        ...
        # Further animations
        self.play(
            angle_trackers[0].set_value,
            TAU / 8,
        )
        self.play(
            angle_trackers[2].set_value,
            -TAU / 8,
        )
        self.wait()
        center_lines.update()
        ...
        self.play(
            ...
            TransformFromCopy(theta_label.copy().clear_updaters(),
                              two_theta_label),
        )
        self.wait()
    ...
    def let_point_vary(self):
        ...
        for angle in [TAU / 4, 3 * TAU / 4]:
            self.play(p2_tracker.set_value, angle, **kwargs)
            self.wait()
        self.play(p1_tracker.set_value, PI, **kwargs)
        self.wait()
        self.play(p3_tracker.set_value, TAU / 3, **kwargs)
        self.wait()
        self.play(p2_tracker.set_value, 7 * TAU / 8, **kwargs)
        self.wait()


class SimpleSlopeLabel(Scene):
    def construct(self):
        label = TexMobject("\\text{Slope}", "=",
...
class AddTwoThetaManyTimes(Scene):
    def construct(self):
        ...
        for n in range(5):
            ...
            self.wait(0.25)
        self.play(FadeInFrom(expression[-2:], LEFT), GrowFromCenter(brace),
    ...
    def get_central_question(self, brace_vect=DOWN):
        expression = TexMobject("N", "\\cdot", "\\theta", "<", "\\pi")
        N = expression[0]
        N.set_color(BLUE)
        brace = Brace(N, brace_vect, buff=SMALL_BUFF)
        question = brace.get_text("Maximal integer?", )
        question.set_color(YELLOW)
    ...
    def show_example(self):
        ...
        self.wait()
        self.play(ChangeDecimalToValue(N_mob, 314, run_time=5))
        self.wait()
    ...
    #
    def get_changable_equation(self,
                               value,
                               tex_string=None,
                               n_decimal_places=10):
        int_mob = Integer(1)
        ...
        int_mob.add_updater(lambda m: m.next_to(tex_mob, LEFT, SMALL_BUFF))
        int_mob.add_updater(align_number)
        rhs.add_updater(lambda m: m.set_value(value * int_mob.get_value()))
        rhs.add_updater(lambda m: m.next_to(tex_mob, RIGHT, SMALL_BUFF))
...
class ComputeThetaFor1e4(AnalyzeCircleGeometry):
    ...
    def add_circle_with_three_lines(self):
        ...
        self.play(lines_to_fade.set_stroke, WHITE, 1, 0.3, ShowCreation(arc),
    ...
    def write_slope(self):
        ...
        self.play(FadeInFrom(slope_label[3:5], LEFT))
        self.wait()
        self.play(
            Write(slope_label[5]),
            TransformFromCopy(self.mass_ratio_label[1][:3], slope_label[6]))
        self.wait()
    ...
    def show_tangent(self):
        ...
        arctan_equation = TexMobject("\\theta", "=", "\\arctan", "(", "1 / 10",
                                     ")")
        arctan_equation.next_to(tan_equation, DOWN, MED_LARGE_BUFF)

        self.play(FadeInFromDown(tan_equation[:8]), )
        self.play(TransformFromCopy(theta_label_group[1], opposite),
                  run_word.set_color, opposite.get_color())
        self.play(WiggleOutThenIn(run_word))
        self.play(TransformFromCopy(opposite, adjacent), rise_word.set_color,
                  adjacent.get_color())
...
class ThetaChart(Scene):
    ...
    def populate_columns(self):
        ...
        entries = [
            ("$m_1$ : $m_2$", "$\\arctan(\\sqrt{m_2} / \\sqrt{m_1})$", "")
        ] + [(
            "{:,} : 1".format(10**(2 * exp)),
    ...
    def show_values(self):
        ...
        for value in values:
            self.play(
                LaggedStartMap(FadeIn, value, lag_ratio=0.1, run_time=0.5))
            self.wait(0.5)
    ...
    def highlight_example(self, exp):
        ...
        self.play(other_entries.set_fill, {"opacity": 0.25}, example.set_fill,
                  {"opacity": 1}, ShowCreationThenFadeAround(example))
        self.wait()
        self.play(
            MoveToTarget(value),
            Write(rhs),
...
class CentralQuestionFor1e2(AddTwoThetaManyTimes):
    ...
    def construct(self):
        ...
        b10_equation = self.get_changable_equation(10**(-exp),
                                                   n_decimal_places=exp)
        ...
        eq_centers = [
            eq[1][2].get_center() for eq in [b10_equation, arctan_equation]
        ]
...
class AskAboutArctanOfSmallValues(TeacherStudentsScene):
    def construct(self):
        ...
        equation1 = TexMobject("\\arctan", "(", "x", ")", "\\approx", "x")
        ...
        self.play(FadeInFromDown(equation1), self.teacher.change,
                  "raise_right_hand",
...
class ActanAndTanGraphs(GraphScene):
    ...
    def construct(self):
        ...
        arctan_v_line = always_redraw(lambda: self.get_vertical_line_to_graph(
        ...
        tan_v_line = always_redraw(lambda: self.get_vertical_line_to_graph(
        ...
        self.add(axes)
        self.play(ShowCreation(id_graph), Write(id_label))
        self.play(ShowCreation(arctan_graph), Write(arctan_label))
...
class UnitCircleIntuition(Scene):
    ...
    def show_fraction(self):
        ...
        rhs = TexMobject("{\\text{height}", "\\over", "\\text{width}}")
        ...
        self.play(
            ...
            self.r_label.fade, 1, Write(rhs[2]))
...
class TangentTaylorSeries(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(ShowCreation(series_error_rect),
        ...
        self.look_at(series)
        self.play(TransformFromCopy(series[:8], example[:8]),
...
class SumUpWrapper(Scene):
    def construct(self):
        ...
        self.add(
            FullScreenFadeRectangle(fill_color=DARK_GREY, fill_opacity=0.5))
        self.play(
...
class ConservationLawSummary(Scene):
    def construct(self):
        ...
        for color in [RED, BLUE, PINK, YELLOW]:
            self.play(ShowCreation(circle.copy().set_color(color), ))
...
class FinalCommentsOnPhaseSpace(Scene):
    ...
    def state_to_point(self):
        ...
        self.play(FadeOutAndShift(self.to_fade, UP), FadeInFrom(state, UP))
        self.play(GrowArrow(arrow), FadeInFrom(point, LEFT))
        self.wait(2)
