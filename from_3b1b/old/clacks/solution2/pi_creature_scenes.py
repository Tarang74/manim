...
class OnAnsweringTwice(TeacherStudentsScene):
    def construct(self):
        ...
        self.student_says("But we already \\\\ solved it",
        ...
        for oq in other_questions:
            self.play(
                shown_questions.shift, 0.85 * UP, FadeInFromDown(oq),
                self.get_student_changes(*["pondering"] * 3, look_at_arg=oq))
...
class NowForTheGoodPart(TeacherStudentsScene):
    def construct(self):
        self.teacher_says(
            ...
            target_mode="hooray",
            added_anims=[
                self.get_student_changes("hooray", "surprised", "happy")
            ],
        )
        self.wait(2)
