...
class AboutSpaceFillingCurves(TransformOverIncreasingOrders):
    ...
    def show_infinite_objects(self):
        ...
        rationals, other_equals, naturals = TexMobject(
            ["|\\mathds{Q}|", "=", "|\\mathds{N}|"]).scale(2).split()
        ...
        self.wait()
        self.play(ShimmerIn(summand), ShimmerIn(equals), ShimmerIn(result),
...
class PostponePhilosophizing(Scene):
    def construct(self):
        abstract, arrow, concrete = TextMobject(
            ["Abstract", " $\\rightarrow$ ", "Concrete"]).scale(2).split()

        self.add(abstract, arrow, concrete)
        self.wait()
        self.play(*[
            ApplyMethod(
                word1.replace, word2, path_func=path_along_arc(np.pi / 2))
...
class ImageToSound(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(
            Transform(picture, string.mobject, run_time=3,
                      rate_func=rush_into))
        self.remove(picture)
        self.play(string)
    ...
    def construct(self):
        ...
        component_strings = [
            Vibrate(num_periods=k + 1,
                    ...
                    color=color,
                    center=2 * DOWN + UP * k)
            for k, color in zip(list(range(overtones)),
...
class ListenToAllPixels(Scene):
    def construct(self):
        ...
        colors = [
            Color(
                rgb=interpolate(np.array(red.rgb), np.array(blue.rgb), alpha))
        ...
        sub_vibrations = [
            Vibrate(string.copy().shift((n - 1) * UP).set_color(colors[n]),
                    overtones=1,
                    spatial_period=6. / (n + 1),
                    temporal_period=1. / (n + 1),
                    amplitude=0.5 / (n + 1)) for n in range(4)
        ]
        words = TexMobject(
        ...
        self.wait()
        self.play(
            DelayByOrder(ApplyMethod(grid.set_color_by_gradient, red, blue)))
        self.play(Transform(grid, freq_line))
        self.wait()
        self.play(ShimmerIn(words, rate_func=squish_rate_func(smooth, 0, 0.2)),
                  *sub_vibrations,
                  run_time=5)
        self.play(*[
            TransformAnimations(sub_vib, vibration)
...
class ThinkInTermsOfReverseMapping(Scene):
    def construct(self):
        ...
        dot3, arrow3 = [
            mob.copy().shift(5 * LEFT + UP) for mob in (dot1, arrow1)
        ]
        dot4, arrow4 = [
            mob.copy().shift(5 * LEFT + 0.9 * UP) for mob in (dot2, arrow2)
        ]
...
class WeaveLineThroughPixels(Scene):
    ...
    def construct(self, order):
        ...
        squares = Mobject(*[
            Square(side_length=unit, color=WHITE).shift(x * right + y * up) for
            x, y in it.product(list(range(2**order)), list(range(2**order)))
        ])
        squares.center()
        targets = Mobject()
        for square in squares.submobjects:
            center = square.get_center()
            distances = np.apply_along_axis(lambda p: get_norm(p - center), 1,
                                            curve.points)
...
class TellMathematicianFriend(Scene):
    def construct(self):
        ...
        squiggle_mouth.apply_function(lambda x_y_z: (x_y_z[0], x_y_z[
            1] + 0.02 * np.sin(50 * x_y_z[0]), x_y_z[2]))
        ...
        self.play(Transform(point, bubble), )
        self.remove(point)
        self.add(bubble)
        ...
        self.add(words3)
        self.wait(2)
        self.play(ApplyPointwiseFunction(lambda p: 15 * p / get_norm(p),
                                         bubble),
...
class Order1PseudoHilbertCurve(Scene):
    def construct(self):
        ...
        self.add(words, s)
        self.wait()
        self.play(Transform(s, pre_words,
...
class IncreaseResolution(Scene):
    def construct(self):
        ...
        self.add(grid)
        self.play(GrowFromCenter(side_brace), GrowFromCenter(top_brace),
                  ShimmerIn(top_words), ShimmerIn(side_words))
        self.wait()
        self.play(DelayByOrder(Transform(*grids)),
...
class IncreasingResolutionWithSnakeCurve(Scene):
    def construct(self):
        ...
        start_dots, end_dots = [
            Mobject(*[
                Dot(curve.points[int(x * curve.get_num_points())], color=color)
                for x, color in [(0.202, GREEN), (0.48, BLUE), (0.7, RED)]
            ]) for curve in (start_curve, end_curve)
        ]
        self.add(start_curve)
        self.wait()
        self.play(ShowCreation(start_dots, run_time=2),
                  ApplyMethod(start_curve.fade))
        end_curve.fade()
        self.play(Transform(start_curve, end_curve),
