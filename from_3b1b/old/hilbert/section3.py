...
class InfiniteObjectsEncapsulateFiniteObjects(Scene):
    ...
    def construct(self):
        words = [
            TextMobject(text, size="\\large") for text in [
                "Truths about infinite objects", " encapsulate ",
        ...
        infinite_objects = [
            TexMobject("\\sum_{n=0}^\\infty",
                       size="\\normal").set_color(RED_E),
            Sierpinski(order=8).scale(0.3),
            TextMobject("$\\exists$ something infinite $\\dots$").set_color(
                RED_B)
        ]
        finite_objects = [
            TexMobject("\\sum_{n=0}^N", size="\\normal").set_color(GREEN_E),
            self.get_triangles(),
            TextMobject("$\\forall$ finite somethings $\\dots$").set_color(
                GREEN_B)
        ]
        for infinite, finite, n in zip(infinite_objects, finite_objects,
                                       it.count(1, 2)):
