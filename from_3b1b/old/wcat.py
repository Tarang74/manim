...
class ClosedLoopScene(Scene):
    CONFIG = {
        ...
        "square_vertices":
        [2 * RIGHT + UP, 2 * UP + LEFT, 2 * LEFT + DOWN, 2 * DOWN + RIGHT],
        "rect_vertices": [
            0 * RIGHT + 1 * UP,
            -1 * RIGHT + 2 * UP,
            -3 * RIGHT + 0 * UP,
        ...
        for d1, d2 in pairs:
            ...
            line.update_anim = UpdateFromFunc(
                line, lambda l: l.put_start_and_end_on(
                    l.start_dot.get_center(), l.end_dot.get_center()))
    ...
    def square_scores(self, all_quads):
        midpoint_diffs = np.apply_along_axis(
            get_norm, 1, 0.5 * (all_quads[:, 0] + all_quads[:, 2]) - 0.5 *
...
class WhenIWasAKid(TeacherStudentsScene):
    ...
    def state_excitement(self, children, speaker):
        ...
        me = children[-1]
        self.play(FadeOut(self.get_students()),
    ...
    def run_class(self):
        ...
        self.random_blink()
        self.student_says("How is this math?",
                          student_index=-1,
...
class DefineInscribedSquareProblem(ClosedLoopScene):
    ...
    def draw_loop(self):
        ...
        self.play(ShowCreation(self.connecting_lines, lag_ratio=0, run_time=2),
                  Animation(self.dots))
        self.wait(2)
...
class PairOfPairBecomeRectangle(Scene):
    def construct(self):
        dots = VGroup(Dot(4 * RIGHT + 0.5 * DOWN, color=MAROON_B),
...
class PairOfRealsToPlane(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(pair_label.next_to, two_d_point, UP + LEFT, SMALL_BUFF,
                  Rotate(y_axis, np.pi / 2), Rotate(dot2, np.pi / 2),
                  FadeOut(arrows))
        lines = VGroup(*[
...
class RepresentPairInUnitSquare(ClosedLoopScene):
    def construct(self):
        ...
        for mob in movers:
            mob.generate_target()
        shift_vals = [
            RIGHT + DOWN, LEFT + DOWN, LEFT + 2 * UP, 3 * DOWN, 2 * RIGHT + UP,
            RIGHT + UP, 3 * LEFT + 3 * DOWN
        ]
...
class EdgesOfSquare(Scene):
    ...
    def get_coordinate_labels(self, square):
        ...
        dot_groups = [
            VGroup(*[
                Dot(
                    interpolate(square.get_corner(DOWN + vect),
                                square.get_corner(UP + vect), alpha))
        ...
        label_groups = [
            VGroup(*[
                TexMobject("(%s, %s)" % (a, b)).scale(0.7) for b in alpha_range
            ]) for a in (0, 1)
        ]
    ...
    def get_arrows(self, x_edges, y_edges):
        ...
        return [
            VGroup(*[
                VGroup(*[
                    Arrow(edge.point_from_proportion(a1),
...
class EndpointsGluedTogether(ClosedLoopScene):
    def construct(self):
        ...
        self.loop = line
        dots = VGroup(
            *[Dot(line.get_critical_point(vect)) for vect in (LEFT, RIGHT)])
        dots.set_color(BLUE)

        self.add(interval, dots)
        self.play(dots.rotate_in_place, np.pi / 20, rate_func=wiggle)
        self.wait()
        self.transform_loop(original_loop,
                            added_anims=[
                                ApplyMethod(dot.move_to,
                                            original_loop.points[0])
                                for dot in dots
                            ],
                            run_time=3)
        self.wait()
...
class NotHelpful(Scene):
    def construct(self):
        ...
        self.add(morty)
        self.play(FadeIn(bubble), FadeIn(bubble.content), morty.change_mode,
                  "angry", morty.look, OUT)
        self.play(Blink(morty))
        self.wait()
...
class FoldUnitSquare(EdgesOfSquare):
        ...
        for a in 0, 1:
            for color in colors:
                ...
                for b1, b2 in zip(b_range, b_range[1:]):
                    arrow = Arrow(self.get_point_from_coords(a, b1),
    ...
    def perform_fold(self):
        diag_line = DashedLine(self.square.get_corner(DOWN + LEFT),
    ...
    def ask_about_gluing(self):
        ...
        self.play(keepers.scale, 0.6, keepers.shift, 4 * RIGHT + UP,
                  FadeIn(randy))
        self.play(randy.change_mode, "pondering", randy.look_at, keepers,
...
class PrepareForMobiusStrip(Scene):
    ...
    def add_triangles(self):
        ...
        for tri in triangles:
            ...
            alpha_range = np.linspace(0, 1, 4)
            arrows = VGroup(*[
                Arrow(interpolate(anchors[0], anchors[1], a),
