...
class CircleScene(Scene):
    ...
    def __init__(self, radians, radius=RADIUS, *args, **kwargs):
        ...
        self.points = [(self.radius * np.cos(angle),
                        self.radius * np.sin(angle), 0) for angle in radians]
        self.dots = [Dot(point) for point in self.points]
        self.lines = [
            Line(p1, p2) for p1, p2 in it.combinations(self.points, 2)
        ]
        self.n_equals = TexMobject("n=%d" % len(radians), ).shift(
            (-FRAME_X_RADIUS + 1, FRAME_Y_RADIUS - 1.5, 0))
...
class CountSections(CircleScene):
    def __init__(self, *args, **kwargs):
        ...
        self.play(*[
            ...
            for points in it.combinations(self.points, 2) for i in (0, 1)
        ],
                  run_time=2.0)
...
class MoserPattern(CircleScene):
    ...
    def __init__(self, radians, *args, **kwargs):
        ...
        for n in range(1, len(radians) + 1):
            ...
            self.add(*[
                Line(p[0], p[1]) for p in it.combinations(self.points[:n], 2)
            ])
            tex_stuffs = [
...
class HardProblemsSimplerQuestions(Scene):
    def __init__(self, *args, **kwargs):
        ...
        for mob in [
                fermat2, fermat3, fermat["2"], fermat["3"], fermat2_jargon,
                fermat3_jargon
        ]:
        ...
        self.add(fermat["n"])
        self.play(
            *[Transform(deepcopy(fermat["n"]), f_copy) for f_copy in copies])
        ...
        circle_grid = Mobject(Circle(), Grid(radius=2),
        ...
        other_grid = Mobject(Grid(radius=2),
        ...
        self.play(FadeIn(fermat2_jargon), FadeIn(fermat3_jargon),
        ...
        self.play(
            ...
            Transform(Point((-FRAME_X_RADIUS, 0, 0)),
...
class CountLines(CircleScene):
    def __init__(self, radians, *args, **kwargs):
        ...
        new_lines = [
            ...
            for y in np.arange(-(self.radius - 1), self.radius -
                               1, (2 * self.radius - 2) / len(self.lines))
        ]
        self.add(text)
        self.wait()
        self.play(*[
...
class GeneralPositionRule(Scene):
    def __init__(self, *args, **kwargs):
        Scene.__init__(self, *args, **kwargs)
        tuples = [
            (np.arange(0, 2 * np.pi, np.pi / 3), "Not okay",
            ...
            (np.arange(0, 2 * np.pi, np.pi / 4), "Not okay",
        ...
        for radians, words, pairs in tuples:
            ...
            if words == "Okay":
                ...
                words_mob.set_color()
                intersecting_lines = [
                    line.scale_in_place(0.3).set_color() for i, j in pairs
                ...
                if first_time:
                    self.play(
                        Transform(Mobject(*intersecting_lines), words_mob))
                    first_time = False
                self.wait()
            self.remove(*self.mobjects)
...
class LineCorrespondsWithPair(CircleScene):
    ...
    def __init__(self, radians, dot0_index, dot1_index, *args, **kwargs):
        ...
        line, dot0, dot1 = self.lines[line_index], self.dots[
            dot0_index], self.dots[dot1_index]
        self.lines.remove(line)
        self.dots.remove(dot0)
        self.dots.remove(dot1)
        self.wait()
        self.play(
            *[ApplyMethod(mob.fade, 0.2) for mob in self.lines + self.dots])
        self.play(*[
            Transform(dot,
                      Dot(dot.get_center(), 3 * dot.radius),
                      rate_func=there_and_back) for dot in (dot0, dot1)
        ])
...
class IllustrateNChooseK(Scene):
    ...
    def __init__(self, n, k, *args, **kwargs):
        ...
        pronunciation = TextMobject("(pronounced ``%d choose %d\'\')" % (n, k))
...
class LinesIntersectOutside(CircleScene):
    ...
    def __init__(self, radians, indices, *args, **kwargs):
        ...
        self.play(*[
            FadeOut(mob, rate_func=not_quite_there) for mob in self.mobjects
            if mob not in lines_to_save
        ])
        self.play(*[
            Transform(Line(self.points[indices[p0]], self.points[indices[p1]]),
        ...
        for quad in quadruplets:
            intersection_dot = Dot(
                intersection(
                    ...
                    (self.points[quad[1]], self.points[quad[3]]))).repeat(3)
...
class GraphsAndEulersFormulaJoke(Scene):
    def __init__(self, *args, **kwargs):
        ...
        graph = ParametricFunction(lambda t: (10 * t,
                                              ((10 * t)**3 - 10 * t), 0),
        ...
        self.play(
            CounterclockwiseTransform(
                eulers, Point((FRAME_X_RADIUS, FRAME_Y_RADIUS, 0))))
...
class DoubledEdges(GraphScene):
    def __init__(self, *args, **kwargs):
        ...
        crazy_lines = [(line, Line(line.end, line.start),
                        CurvedLine(line.start,
                                   line.end), CurvedLine(line.end, line.start))
...
class EulersFormula(GraphScene):
    def __init__(self, *args, **kwargs):
        ...
        self.add(formula)
        colored_dots = [
            deepcopy(d).scale_in_place(1.5).set_color("red") for d in self.dots
        ]
        colored_edges = [
            Mobject(
                Line(midpoint, start),
                Line(midpoint, end),
            ).set_color("red") for e in self.edges
            for start, end, midpoint in [(e.start, e.end,
                                          (e.start + e.end) / 2.0)]
        ]
        frame_time = 0.3

        self.generate_regions()
        parameters = [(colored_dots, "V", "mobject", "-", "show"),
...
class ShowMoserGraphLines(CircleScene):
    def __init__(self, radians, *args, **kwargs):
        ...
        n_choose_2, plus_2_n_choose_4, plus_n = TexMobject(
            [r"{n \choose 2}", r"&+2{n \choose 4}\\", r"&+n"]).split()
        ...
        small_lines = [
            deepcopy(line).scale_in_place(0.5) for line in self.lines
        ]

        for mobs, symbol in [(self.dots, n),
...
class HowIntersectionChopsLine(CircleScene):
    ...
    def __init__(self, radians, indices, *args, **kwargs):
        ...
        new_lines = [
            Line(line.start, intersection_point) for line in lines
        ] + [Line(intersection_point, line.end) for line in lines]
        self.play(*[
            Transform(line,
                      Line((-3, h, 0), (3, h, 0)),
                      rate_func=there_and_back,
                      run_time=3.0) for line, h in zip(lines, (-1, 1))
        ])
        self.remove(*lines)
        self.play(*[
            Transform(line,
                      deepcopy(line).scale(1.1).scale_in_place(1 / 1.1),
                      run_time=1.5) for line in new_lines
        ])
...
class ApplyEulerToMoser(CircleScene):
    def __init__(self, *args, **kwargs):
        ...
        nc41 = {}
        dicts = [
            V, minus, minus1, E, plus, plus1, plus2, F, equals, two, two1, n,
            n1, nc2, nc4, nc41
        ]
        ...
        for d in dicts:
            ...
            for key in list(d.keys()):
                d[key] = deepcopy(d[main_key]).shift(d[key].get_center() -
                                                     main_center)
        ...
        self.remove(*self.mobjects)
        self.play(Transform(
            E[3], Mobject(nc2[4], plus1[4], two1[4], nc41[4], plus2[4],
                          n1[4])),
                  *[
                      Transform(d[3], d[4])
                      ...
                      Transform(deepcopy(cp), scp) for cp, scp in zip(
                          self.circle_pieces, self.smaller_circle_pieces)
                  ],
                  run_time=2.0)
        self.wait()
        self.remove(*self.mobjects)
        self.play(
            Transform(Mobject(plus2[4], n1[4], minus[4], n[4]),
                      Point((FRAME_X_RADIUS, FRAME_Y_RADIUS, 0))),
            *[
                Transform(d[4], d[5]) for d in
                [F, equals, nc2, plus1, two1, nc41, minus1, nc4, plus, two]
            ])
        self.wait()
        self.remove(*self.mobjects)
        self.play(
...
class FormulaRelatesToPowersOfTwo(Scene):
    def __init__(self, *args, **kwargs):
        ...
        self.wait()
        self.play(*[
            Transform(deepcopy(s), result) for s, result in zip(sums, results)
        ])
        powers_of_two = [
            TexMobject("2^{%d}" % (i - 1)).scale(scale_factor).shift(
                result.get_center()).set_color()
...
class DrawPascalsTriangle(PascalsTriangleScene):
    def __init__(self, *args, **kwargs):
        ...
        for n in range(1, nrows):
            ...
            starts += [
                Mobject(self.coords_to_mobs[n - 1][k - 1],
                        self.coords_to_mobs[n - 1][k]) for k in range(1, n)
            ]
            ...
            self.play(*[
                Transform(starts[i],
                          ...
                          run_time=1.5,
                          black_out_extra_points=False) for i in range(n + 1)
            ])
...
class PascalRuleExample(PascalsTriangleScene):
    def __init__(self, nrows, *args, **kwargs):
        ...
        nums_above = [
            self.coords_to_mobs[n - 1][k - 1], self.coords_to_mobs[n - 1][k]
        ]
...
class PascalsTriangleWithNChooseK(PascalsTriangleScene):
    def __init__(self, *args, **kwargs):
        ...
        for i in [0, 1]:
            ...
            self.remove(*self.mobjects)
            self.play(*[
                CounterclockwiseTransform(deepcopy(mob_dicts[i][n][k]),
                                          mob_dicts[1 - i][n][k])
    ...
    def __init__(self, nrows, n, k, *args, **kwargs):
        ...
        for b in range(k + 1):
            ...
            if b < k:
                ...
                self.remove(b_mob)
        self.play(*[
            ApplyMethod(mob.fade, 0.2) for mob in triangle_terms
...
class PascalsTriangleSumRows(PascalsTriangleScene):
    def __init__(self, *args, **kwargs):
        ...
        for n in range(self.nrows):
            ...
            desired_center = np.array(
                (self.diagram_width / 2.0,
                 self.coords_to_mobs[n][0].get_center()[1], 0))
        ...
        for n in range(self.nrows):
            ...
            self.play(Transform(start, powers_of_two[n]),
                      FadeIn(equalses[n]),
                      run_time=run_time)
        self.wait()
        ...
        for n in range(self.nrows):
            self.play(
                CounterclockwiseTransform(powers_of_two[n],
...
class MoserSolutionInPascal(PascalsTriangleScene):
    args_list = [(N_PASCAL_ROWS, n)
                 for n in range(3, 8)] + [(BIG_N_PASCAL_ROWS, 10)]
    ...
    def __init__(self, nrows, n, *args, **kwargs):
        ...
        self.wait()
        self.play(*[
            FadeIn(self.coords_to_n_choose_k[n0][k0]) for n0, k0 in self.coords
        ...
        self.play(*[
            CounterclockwiseTransform(self.coords_to_n_choose_k[n0][k0],
                                      self.coords_to_mobs[n0][k0])
            for n0, k0 in self.coords if (n0, k0) not in [(n, k)
                                                          for k in term_range]
        ] + [
...
class RotatingPolyhedra(Scene):
    ...
    def __init__(self, polyhedra_classes, *args, **kwargs):
        ...
        polyhedra = [
            Class().scale(1.5).shift((1, 0, 0)) for Class in polyhedra_classes
        ]
        curr_mob = polyhedra.pop()
        for mob in polyhedra:
            self.play(
                TransformAnimations(Rotating(curr_mob, **rot_kwargs),
...
class ExplainNChoose2Formula(Scene):
    ...
    def __init__(self, n, a, b, *args, **kwargs):
        ...
        self.play(*[
            CounterclockwiseTransform(
                mob,
                Point(mob.get_center()).set_color("black")) for mob in nums
        ])
        self.play(*[
        ...
        self.play(CounterclockwiseTransform(a_mob, a_copy),
...
class ExplainNChoose4Formula(Scene):
    ...
    def __init__(self, n, *args, **kwargs):
        ...
        quad = (4, 2, 5, 1)
        tuple_mobs = TexMobjects(("( %d , %d , %d , %d )" % quad).split(" "))
        ...
        form_mobs = TexMobject([
            ...
            "(%d-1)" % n,
            "(%d-2)" % n,
            "(%d-3)" % n, r"\over {4 \cdot 3 \cdot 2 \cdot 1}"
        ]).split()
        ...
        for i in range(4):
            ...
            self.play(
                ApplyMethod(nums_compound.shift,
                            (curr_num - quad[i]) * up_unit))
            self.remove(nums_compound)
            nums = nums_compound.split()
            ...
            self.add(chosen)
            if i < 3:
                right_shift = quad_mobs[i +
                                        1].get_center() - chosen.get_center()
                right_shift[1] = 0
                self.play(ApplyMethod(nums_compound.shift, right_shift))
            else:
                self.play(*[
        ...
        for count in range(6):
            ...
            self.play(
                CounterclockwiseTransform(compound_quad,
...
class IntersectionChoppingExamples(Scene):
    def __init__(self, *args, **kwargs):
        ...
        for pairs, exp in [(pairs1, "3 + 2(2) = 7"),
            ...
            self.play(*[
                Transform(line,
                          deepcopy(line).scale(1.2).scale_in_place(1 / 1.2))
                for line in lines
            ])
