...
def get_fourier_graph(
    ...
    time_samples = np.vectorize(time_func)(np.linspace(t_min, t_max,
                                                       n_samples))
    fft_output = np.fft.fft(time_samples)
    frequencies = np.linspace(0.0, n_samples / (2.0 * time_range),
                              n_samples // 2)
    ...
    f_min, f_max = [
        axes.x_axis.point_to_number(graph.points[i]) for i in (0, -1)
    ]
    ...
    **kwargs  ##Just eats these
    ...
    def fourier_transform(f):
        z = scalar * scipy.integrate.quad(
            lambda t: func(t) * np.exp(complex(0, -TAU * f * t)), t_min,
            t_max)[0]
...
class Introduction(TeacherStudentsScene):
    def construct(self):
        ...
        fourier_graph = FunctionGraph(get_fourier_transform(func, 0, 5),
                                      x_min=0,
                                      x_max=5)
...
class TODOInsertCosineWrappingAroundCircle(TODOStub):
    CONFIG = {
        "message":
        "Give a picture-in-picture \\\\ of cosine wrapping around circle",
    }
...
class AddingPureFrequencies(PiCreatureScene):
    ...
    def play_A440(self):
        ...
        self.broadcast(
            FadeIn(A_label),
            Succession(ApplyMethod, randy.change, "pondering", Animation,
                       randy, Blink, randy))
    ...
    def measure_air_pressure(self):
        ...
        equilibrium_line = DashedLine(axes.coords_to_point(0, eh),
        ...
        brace = Brace(
            Line(
                ...
                axes.coords_to_point((n + 1) / frequency,
                                     func((n + 1) / frequency)),
            ), UP)
        ...
        self.play(randy.change, "erm", graph, GrowFromCenter(brace),
                  Write(words))
        self.wait()
        graph.save_state()
        self.play(
    ...
    def play_mix(self):
        ...
        self.broadcast(
            self.get_broadcast_animation(n_circles=6), self.pi_creature.change,
            "thinking", *[
                ShowCreation(graph, run_time=4, rate_func=linear)
    ...
    def separate_out_parts(self):
        ...
        self.play(*it.chain(list(map(MoveToTarget, movers)), [
    ...
    def draw_full_sum(self):
        ...
        rect = Rectangle(height=2.5 * FRAME_Y_RADIUS,
        ...
        self.play(self.get_graph_line_animation(self.A_axes, self.A_graph),
    ...
    def show_initial_sound(self):
        def func(x):
            return self.equilibrium_height + 0.2 * np.sum(
                [np.cos(2 * np.pi * f * x) for f in self.frequencies])
    ...
    def ponder_question(self):
        ...
        self.play(
            ...
            LaggedStartMap(
                ...
                pure_graphs,
                lambda g:
                (lambda m: m.shift(SMALL_BUFF * UP).set_color(YELLOW), g),
                rate_func=wiggle))
        self.play(FadeOut(rect))
        self.play(Blink(randy))
        self.wait()
...
class Quadrant(VMobject):
    ...
    def generate_points(self):
        points = [
            r * RIGHT for r in np.arange(0, self.radius, 1. / self.density)
        ]
        points += [
            self.radius * (np.cos(theta) * RIGHT + np.sin(theta) * UP)
            for theta in np.arange(0, TAU / 4, 1. /
                                   (self.radius * self.density))
        ]
        points += [
            r * UP for r in np.arange(self.radius, 0, -1. / self.density)
        ]
...
class MachineThatTreatsOneFrequencyDifferently(Scene):
    def construct(self):
        ...
        arrow1, q_marks, arrow2 = group = VGroup(Vector(DOWN),
                                                 TextMobject("???").scale(1.5),
                                                 Vector(DOWN))
...
class FourierMachineScene(Scene):
    ...
    def get_time_axes(self):
        ...
        time_label.next_to(time_axes.coords_to_point(self.time_label_t, 0),
                           DOWN)
    ...
    def get_fourier_transform_graph(self, time_graph, **kwargs):
        ...
        # return result
        return self.frequency_axes.get_graph(get_fourier_transform(
            func, t_min, t_max, **kwargs),
    ...
    def get_polarized_animation(self, mobject, freq=1.0):
        ...
        def update_p_mob(p_mob):
            Transform(p_mob, self.get_polarized_mobject(mobject,
                                                        freq=freq)).update(1)
    ...
    def get_vector_animations(self,
                              graph,
                              draw_polarized_graph=True,
                              **kwargs):
        config = dict(self.default_y_vector_animation_config)
        config.update(kwargs)
        anims = [
            self.get_time_graph_y_vector_animation(graph, **config),
            self.get_polarized_vector_animation(graph.polarized_mobject,
                                                **config),
        ]
    ...
    def animate_time_sweep(self, freq, n_repeats=1, t_max=None, **kwargs):
        ...
        for x in range(n_repeats):
            ...
            self.play(ApplyMethod(v_line.move_to, ctp(t_max, 0), DOWN),
    ...
    def get_v_lines_indicating_periods(self, freq, n_lines=None):
        ...
        v_lines = VGroup(*[
            DashedLine(ORIGIN, 1.5 * UP).move_to(
                self.time_axes.coords_to_point(n * period, 0), DOWN)
    ...
    def get_period_v_lines_update_anim(self):
        def update_v_lines(v_lines):
            ...
            Transform(v_lines,
                      self.get_v_lines_indicating_periods(freq)).update(1)
...
class WrapCosineGraphAroundCircle(FourierMachineScene):
    ...
    def show_initial_signal(self):
        ...
        v_lines = VGroup(*[
            DashedLine(ORIGIN, 2 *
                       UP, color=RED).move_to(axes.coords_to_point(x, 0), DOWN)
            for x in (1, 2)
        ])
        ...
        self.play(FadeIn(words), LaggedStartMap(FadeIn, braces),
    ...
    def wrap_around_circle(self):
        ...
        self.play(
            ReplacementTransform(moving_graph,
                                 polarized_graph,
                                 run_time=3,
                                 path_arc=-TAU / 2))
    ...
    def show_time_sweeps(self):
        ...
        self.animate_time_sweep(freq=freq,
                                t_max=4,
                                run_time=6,
                                added_anims=[FadeIn(v_lines)])
        self.play(FadeIn(winding_freq_label),
            ...
            if count == 2:
                self.play(
                    LaggedStartMap(
                        ...
                        v_lines,
                        lambda mob:
                        (lambda m: m.shift(0.25 * UP).set_color(YELLOW), mob),
    ...
    def get_winding_frequency_label(self):
        freq = self.initial_winding_frequency
        winding_freq_label = VGroup(DecimalNumber(freq, num_decimal_places=2),
...
class DrawFrequencyPlot(WrapCosineGraphAroundCircle, PiCreatureScene):
    ...
    def setup_graph(self):
        ...
        self.add(self.graph)
        self.add(
            self.get_polarized_mobject(self.graph,
    ...
    def introduce_frequency_plot(self):
        ...
        x_coord_label.next_to(frequency_axes.y_axis.get_top(),
                              RIGHT,
                              aligned_edge=UP,
                              buff=LARGE_BUFF)
    ...
    def draw_full_frequency_plot(self):
        ...
        fourier_graph_update = self.get_fourier_graph_drawing_update_anim(
            fourier_graph)
        v_line = DashedLine(self.frequency_axes.coords_to_point(0, 0),
    ...
    def label_as_almost_fourier(self):
        ...
        almost_fourier_label = TextMobject("``Almost Fourier Transform''", )
    ...
    def generate_fourier_dot_transform(self, fourier_graph):
        ...
        def update_dot(dot):
            ...
            dot.move_to(
                self.frequency_axes.input_to_graph_point(f, fourier_graph))

        self.fourier_graph_dot_anim = UpdateFromFunc(self.fourier_graph_dot,
                                                     update_dot)
...
class StudentsHorrifiedAtScene(TeacherStudentsScene):
    def construct(self):
        self.change_student_modes(*3 * ["horrified"],
...
class ShowLowerFrequency(DrawFrequencyPlot):
    ...
    def show_lower_frequency_signal(self):
        ...
        v_lines = VGroup(*[
            DashedLine(ORIGIN, 1.5 *
                       UP).move_to(axes.coords_to_point(x, 0), DOWN)
            for x in (1, 2)
        ])
    ...
    def play_with_lower_frequency_signal(self):
        ...
        fourier_graph_update = self.get_fourier_graph_drawing_update_anim(
            fourier_graph)
        x_coord_label = TextMobject("x-coordinate of center of mass")
        x_coord_label.scale(self.text_scale_val)
        x_coord_label.next_to(
            self.frequency_axes.input_to_graph_point(self.signal_frequency,
                                                     fourier_graph), UP)
        x_coord_label.set_color(self.center_of_mass_color)
        self.generate_fourier_dot_transform(fourier_graph)
...
class ShowLinearity(DrawFrequencyPlot):
    ...
    def show_sum_of_signals(self):
        ...
        sum_graph = self.get_time_graph(lambda t: sum([
        ...
        sum_label = TextMobject("%d Hz" % int(low_freq), "+",
                                "%d Hz" % int(high_freq))
        trips = [
        ...
        self.play(
            LaggedStartMap(FadeIn, VGroup(high_freq_label, low_freq_label)))
        self.wait()
        self.play(
            ...
            ReplacementTransform(VGroup(low_freq_label, high_freq_label),
                                 sum_label))
        self.wait()
        self.graph = graph
    ...
    def show_winding_with_sum_graph(self):
        ...
        self.play(ReplacementTransform(graph.copy(), pol_graph),
                  GrowFromCenter(dot))
...
class ShowCommutativeDiagram(ShowLinearity):
    ...
    def show_diagram(self):
        ...
        fa_group = VGroup(frequency_axes, frequency_axes.deepcopy(),
                          frequency_axes.deepcopy())
        ...
        for func, color, label, ta, fa in zip(funcs, colors, labels, ta_group,
                                              fa_group):
        ...
        def apply_transform(index):
            ...
            fa = fa_group[index]
            anims = [
                ReplacementTransform(getattr(ta, attr), getattr(fa, attr))
        ...
        self.wait()
        self.play(
            GrowArrow(sum_arrows[1]), Write(sum_arrows[1].words), *[
                ReplacementTransform(fa.copy(), fa_group[2])
                for fa in fa_group[:2]
            ])
        self.wait(2)
        self.play(
            GrowArrow(sum_arrows[0]), Write(sum_arrows[0].words), *[
                ReplacementTransform(mob.copy(), ta_group[2], run_time=1)
    ...
    def point_out_spikes(self):
        ...
        sum_spike_rects = VGroup(
            *[self.get_spike_rect(fa_group[2], freq) for freq in freqs])
...
class BeforeGettingToTheFullMath(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(Write(formula), self.teacher.change, "raise_right_hand",
...
class FilterOutHighPitch(AddingPureFrequencies, ShowCommutativeDiagram):
    ...
    def show_intensity_vs_time_graph(self):
        ...
        labels = VGroup(time_label, intensity_label)
        labels.scale(0.75)
        time_label.next_to(axes.x_axis,
                           DOWN,
                           aligned_edge=RIGHT,
                           buff=SMALL_BUFF)
        intensity_label.next_to(axes.y_axis,
                                RIGHT,
                                aligned_edge=UP,
                                buff=SMALL_BUFF)
        axes.labels = labels
        ...
        self.play(
            FadeIn(axes), FadeIn(axes.labels), randy.change, "pondering", axes,
    ...
    def filter_out_high_pitch(self):
        ...
        new_graph = frequency_axes.get_graph(filtered_func,
                                             num_graph_points=300)
        new_graph.set_color(RED)

        self.play(spike_rect.stretch, 4, 0)
        self.play(Transform(fourier_graph, new_graph),
                  spike_rect.stretch,
                  0.01,
                  1, {"about_point": frequency_axes.coords_to_point(0, 0)},
                  run_time=2)
        self.wait()
    ...
    def get_broadcast_anims(self, run_time=7, **kwargs):
        return [
            self.get_broadcast_animation(n_circles=n,
                                         run_time=run_time,
                                         big_radius=7,
                                         start_stroke_width=5,
                                         **kwargs) for n in (5, 7, 10, 12)
        ]
...
class ApplyFourierToFourier(DrawFrequencyPlot):
    ...
    def swap_graphs(self):
        ...
        self.play(wound_up_graph.next_to,
                  FRAME_X_RADIUS * LEFT,
                  LEFT,
                  remover=True)
        self.wait()
        ...
        double_fourier_graph = frequency_axes.get_graph(
            lambda t: 0.25 * np.cos(TAU * 2 * t)).set_color(PINK)
...
class WriteComplexExponentialExpression(DrawFrequencyPlot):
    ...
    def setup_plane(self):
        ...
        circle.apply_complex_function(
            lambda z: R3_to_complex(circle_plane.number_to_point(np.exp(z))))
        circle_plane.add(circle)
    ...
    def show_plane_as_complex_plane(self):
        ...
        number_label_update_anim = ContinualChangingDecimal(
            ...
            position_update_func=lambda l: l.next_to(
                dot, DOWN + RIGHT, buff=SMALL_BUFF),
        )
        ...
        self.wait()
        self.play(
            FadeOut(VGroup(dot, self.dot_component_anim.mobject,
                           number_label)))
    ...
    def show_eulers_formula(self):
        ...
        def get_circle_point(scalar=1, t_shift=0):
            return plane.number_to_point(scalar *
                                         np.exp(complex(0,
                                                        get_t() + t_shift)))
        ...
        exp_base_update = Mobject.add_updater(
            exp_base, lambda e: e.move_to(
                get_circle_point(scalar=1.1, t_shift=0.01 * TAU)))
        vector_update = Mobject.add_updater(
            vector, lambda v: v.put_start_and_end_on(plane.number_to_point(0),
                                                     get_circle_point()))
        ...
        for mob in arc, circle:
            ...
            mob.apply_function(
                lambda p: plane.number_to_point(np.exp(R3_to_complex(p))))
        distance_label = DecimalNumber(exp_decimal.number,
                                       unit="\\text{units}")
        ...
        distance_label.move_to(
            plane.number_to_point(1.1 * np.exp(complex(0, 0.4 * get_t())), ),
            DOWN + LEFT)
        ...
        self.play(
            ReplacementTransform(
                VGroup(exp_base[1], exp_decimal[1]).copy(), exp_expression))
        ...
        self.play(
            FadeOut(time_label), FadeIn(frequency_label), t.restore,
        ...
        ghost_dot.clear_updaters()
        always_shift(ghost_dot, rate=-0.1 * TAU)
        self.play(FadeOut(example_frequency), FadeOut(frequency_label.arrow),
                  GrowFromCenter(minus), two_pi_i_f_t_group.restore)
        self.wait(4)
        ...
        self.play(*list(
            map(FadeOut, [
                update.mobject
                for update in updates if update.mobject is not vector
            ])))
        ...
        self.set_variables_as_attrs(ambient_ghost_dot_movement, ghost_dot,
    ...
    def find_center_of_mass(self):
        ...
        integral = TexMobject("{1", "\\over", "t_2 - t_1}", "\\int", "_{t_1}",
                              "^{t_2}")
        ...
        randy.to_edge(RIGHT)
        full_expression_rect = SurroundingRectangle(VGroup(integral, dt_mob),
                                                    color=RED)
        com_dot = self.center_of_mass_dot
        self.center_of_mass_dot_anim.update(0)
        com_arrow = Arrow(full_expression_rect.get_left(),
                          com_dot,
                          buff=SMALL_BUFF)
    ...
    def show_center_of_mass_sampling(self, n_dots):
        ...
        self.play(
            ReplacementTransform(
                ...
                lag_ratio=0.5,
                run_time=2,
            ), MoveToTarget(time_graph), *time_graph.dot_fade_anims)
        self.play(ReplacementTransform(time_graph.copy(), pol_graph.target),
...
class BuildUpExpressionStepByStep(TeacherStudentsScene):
    def construct(self):
        expression = TexMobject("\\frac{1}{t_2 - t_1}", "\\int_{t_1}^{t_2}",
                                "g(t)", "e", "^{-2\\pi i", "f", "t}", "dt")
        ...
        self.wait(3)
        self.teacher_says("Just one final \\\\ distinction.",
                          bubble_kwargs={
                              "height": 2.5,
                              "width": 3.5
                          },
...
class ScaleUpCenterOfMass(WriteComplexExponentialExpression):
    ...
    def scale_up_center_of_mass(self):
        ...
        time_span = Rectangle(stroke_width=0,
                              fill_color=TEAL,
                              fill_opacity=0.4)
        axes = self.time_axes
        time_span.replace(Line(axes.coords_to_point(0, 0),
        ...
        short_graph, long_graph = [
            axes.get_graph(
                ...
                x_min=0,
                x_max=t_max,
            ).match_style(graph) for t_max in (3, 6)
        ]
        ...
        self.play(*[
            ReplacementTransform(com_vector.copy(), cvc, path_arc=-TAU / 10)
        ...
        pairs = list(
            zip(to_squish.family_members_with_points(),
        ...
        self.play(com_dot.move_to,
                  ...
                  *[
                      ShowCreation(
                          mob,
                          rate_func=lambda a: interpolate(0.5, 1, smooth(a)))
        ...
        to_fade = VGroup(self.integral_brace, self.integral_label, self.frac,
                         self.frac_cross)
        self.play(
            to_shift.shift,
            2 * DOWN,
    ...
    def comment_on_current_signal(self):
        ...
        new_com_vector_update = UpdateFromFunc(
            com_vector, lambda v: v.put_start_and_end_on(
                origin, com_dot.get_center()).scale(12, about_point=origin))

        v_lines = self.get_v_lines_indicating_periods(freq=1.0, n_lines=3)[:2]
        graph_portion = axes.get_graph(graph.underlying_function,
                                       x_min=1,
                                       x_max=2)
        ...
        self.wait()
        self.play(com_vector.scale,
                  0.5, {"about_point": origin},
                  rate_func=there_and_back,
                  run_time=2)
        self.wait(2)
        self.change_frequency(
...
class TakeAStepBack(TeacherStudentsScene):
    def construct(self):
        self.student_says("Hang on, go over \\\\ that again?",
                          target_mode="confused"),
...
class SummarizeTheFullTransform(DrawFrequencyPlot):
    ...
    def show_transform_function(self):
        ...
        g_hat_f.next_to(frequency_axes.input_to_graph_point(2, fourier_graph),
                        UP)
    ...
    def show_winding(self):
        ...
        g_hat_f_indication = Succession(
            ...
            Mobject(), {"run_time": 5},
            Write,
            real_part, {"run_time": 2},
            Animation,
            Mobject(), {"run_time": 3},
            ShowCreation,
            imaginary_fourier_graph, {"run_time": 3},
            rate_func=squish_rate_func(lambda x: x, 0, 31. / winding_run_time),
...
class SummarizeFormula(Scene):
    ...
    def get_expression(self):
        expression = TexMobject("\\hat g(", "f", ")", "=", "\\int", "_{t_1}",
                                "^{t_2}", "g({}", "t", ")", "e", "^{-2\\pi i",
                                "f", "t}", "dt")
...
class OneSmallNote(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("Just one \\\\ small note...",
                          # target_mode =
                          )
...
class BoundsAtInfinity(SummarizeFormula):
    def construct(self):
        ...
        bound_rects = VGroup(
            *[SurroundingRectangle(b, buff=0.5 * SMALL_BUFF) for b in bounds])
        bound_rects.set_color(TEAL)
        inf_bounds = VGroup(
            *[VGroup(TexMobject(s + "\\infty")) for s in ("-", "+")])
        decimal_bounds = VGroup(*[DecimalNumber(0) for x in range(2)])
        for bound, inf_bound, d_bound in zip(bounds, inf_bounds,
                                             decimal_bounds):
        ...
        self.wait()
        self.play(
            ReplacementTransform(self.get_time_interval(0, 0.01),
                                 time_interval))
        ...
        self.play(VGroup(axes, graph).stretch,
                  0.05,
                  0,
                  Transform(time_interval, wide_interval),
                  UpdateFromAlphaFunc(axes.x_axis.numbers,
...
class ShowUncertaintyPrinciple(Scene):
    def construct(self):
        ...
        self.add(title)
        top_axes = Axes(x_min=-FRAME_X_RADIUS,
        ...
        self.add(*[
            ...
            for graph, axes in [(top_graph,
                                 top_axes), (bottom_graph, bottom_axes)]
        ])
...
class SubscribeOrBinge(PiCreatureScene):
    def construct(self):
        ...
        video_anim = LaggedStartMap(Succession,
                                    videos,
                                    lambda v: (
                                        FadeIn,
                                        v,
                                    ...
                                    run_time=10)
        sub_arrow = Arrow(subscribe.get_bottom(),
        ...
        self.play(
            ...
            Succession(
                ...
                ), Blink, morty, ApplyMethod, morty.change, "shruggie", videos,
                Animation, Mobject(), {"run_time": 2}, Blink, morty, Animation,
                Mobject(), {"run_time": 4}))
...
class PuzzleDescription(Scene):
    def construct(self):
        lines = VGroup(TextMobject("Convex set", "$C$", "in $\\mathds{R}^3$"),
    ...
    def construct(self):
        ...
        for f in np.linspace(1.98, 2.02, 7):
            pol_graph = ParametricFunction(
                lambda t: complex_to_R3((2 + np.cos(2 * TAU * t) + np.cos(
                    3 * TAU * t)) * np.exp(-complex(0, TAU * f * t))),
                t_min=-5,
                t_max=5,
