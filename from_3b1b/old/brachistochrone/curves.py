...
class SlideWordDownCycloid(Animation):
    ...
    def interpolate_mobject(self, alpha):
        ...
        for letter, time, end_letter in zip(self.mobject.split(),
                                            virtual_times, self.end_letters):
            time = max(time, 0)
            time = min(time, 1)
            if time < cut_offs[0]:
...
class TryManyPaths(PathSlidingScene):
    def construct(self):
        ...
        curr_path = None
        for path in paths:
            new_slider = self.adjust_mobject_to_index(randy.copy(), 1,
                                                      path.points)
    ...
    def get_paths(self):
        sharp_corner = Mobject(
            Line(3 * UP + LEFT, LEFT), Arc(angle=np.pi / 2, start_angle=np.pi),
            Line(DOWN, DOWN + 3 * RIGHT)).ingest_submobjects().set_color(GREEN)
        paths = [
            Arc(angle=np.pi / 2, radius=3, start_angle=4),
            LoopTheLoop(),
            Line(7 * LEFT, 7 * RIGHT, color=RED_D), sharp_corner,
            FunctionGraph(lambda x: 0.05 * (x**2) + 0.1 * np.sin(2 * x)),
            FunctionGraph(lambda x: x**2,
                          x_min=-3,
                          x_max=2,
...
class NotTheCircle(PathSlidingScene):
    def construct(self):
        ...
        self.play(ApplyMethod(path.put_start_and_end_on,
                              start,
                              end,
