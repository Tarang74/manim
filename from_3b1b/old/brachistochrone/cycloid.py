...
class CycloidScene(Scene):
    ...
    def generate_cycloid(self):
        self.cycloid = Cycloid(point_a=self.point_a,
                               radius=self.radius,
    ...
    def draw_cycloid(self, run_time=3, *anims, **kwargs):
        ...
        self.play(
            RollAlongVector(self.circle,
                            self.cycloid.points[-1] - self.cycloid.points[0],
                            **kwargs), ShowCreation(self.cycloid, **kwargs),
            *anims)
...
class LeviSolution(CycloidScene):
    ...
    def roll_into_position(self):
        self.play(
            RollAlongVector(self.circle, (1 - self.cycloid_fraction) *
                            self.radius * 2 * np.pi * LEFT,
                            rotation_vector=IN,
                            run_time=2))
    ...
    def show_pendulum(self, arc_angle=np.pi, arc_color=GREEN):
        ...
        right_angle_symbol = Mobject(Line(UP, UP + RIGHT),
                                     Line(UP + RIGHT, RIGHT))
        ...
        for start, angle in pairs:
            ...
            self.play(ShowCreation(arc),
                      ApplyMethod(line.rotate_in_place,
                                  angle,
    ...
    def show_theta(self, radius=1):
        arc = Arc(angle=self.tangent_line.get_angle() - np.pi / 2,
                  radius=radius,
                  start_angle=np.pi / 2)
    ...
    def show_similar_triangles(self):
        ...
        new_arc = Arc(angle=self.tangent_line.get_angle() - np.pi / 2,
                      radius=0.5,
                      start_angle=np.pi)
        ...
        triangle1 = Polygon(self.p_point,
                            self.c_point,
                            self.bottom_point,
                            color=MAROON,
                            **kwargs)
        triangle2 = Polygon(y_point,
                            self.p_point,
                            self.c_point,
                            color=WHITE,
                            **kwargs)
        ...
        self.play(Transform(self.arc.copy(), new_arc),
    ...
    def show_y(self):
        ...
        y_equals.shift(2 * UP)
        y_expression = TexMobject(["D ", "\\sin", "^2", "(\\theta)"])
        ...
        self.play(Transform(self.d_sin_squared_theta, temp_expr),
                  ShimmerIn(y_mob), ShowCreation(y_equals))
    ...
    def rearrange(self):
        ...
        self.play(ShimmerIn(y_sqrt), ShimmerIn(d_sqrt),
        ...
        self.play(Transform(equals, new_eq),
                  ShimmerIn(sin_over),
                  ShimmerIn(one_over),
                  ApplyMethod(d_sqrt.next_to,
                              one_over,
                              DOWN,
                              path_func=path_along_arc(-np.pi)),
                  ApplyMethod(y_sqrt.next_to,
                              sin_over,
                              DOWN,
                              path_func=path_along_arc(-np.pi)),
                  run_time=2)
...
class SlidingObject(CycloidScene, PathSlidingScene):
    ...
    def construct(self, with_words):
        ...
        start_randy = self.adjust_mobject_to_index(randy.copy(), 1,
                                                   self.cycloid.points)
...
class RotateWheel(CycloidScene):
    def construct(self):
        ...
        self.play(Rotating(self.circle, axis=OUT, run_time=5,
                           rate_func=smooth))
