...
class TwoToMany(MultilayeredScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(*[
            FadeIn(layer, rate_func=squish_rate_func(smooth, x, 1))
            for layer, x in zip(layers[1:], it.count(0, 0.2))
        ] + [Transform(glass, layers[0])])
        self.wait()

    def get_glass(self):
        return self.RectClass(height=FRAME_Y_RADIUS,
                              width=FRAME_WIDTH,
                              color=BLUE_E).shift(FRAME_Y_RADIUS * DOWN / 2)
...
class RaceLightInLayers(MultilayeredScene, PhotonScene):
    ...
    def construct(self):
        ...
        lines = [
            line.copy().shift(layer.get_center()) for layer in self.layers
        ]
        ...
        self.play(*[
            self.photon_run_along_path(
                line, rate_func=rate_maker(rate), run_time=2)
...
class ShowDiscretePath(MultilayeredScene, PhotonScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(
            self.photon_run_along_path(self.discrete_path,
                                       rate_func=rush_into,
                                       run_time=3))
        self.wait()
...
class ShowLayerVariables(MultilayeredScene, PhotonScene):
    ...
    def construct(self):
        ...
        for layer, x in zip(self.layers[:3], it.count(1)):
            ...
            brace_endpoints = Mobject(Point(self.top),
...
class LimitingProcess(MultilayeredScene):
    ...
    def construct(self):
        ...
        glass_sets = [
            Mobject(*[
                Mobject(*layer_sets[x][(2**x) * index:(2**x) * (index + 1)])
                for index in range(self.n_layers)
            ]).ingest_submobjects() for x in range(num_iterations)
        ]
