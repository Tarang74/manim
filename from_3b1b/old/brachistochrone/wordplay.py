    ...
    def construct(self):
        ...
        self.play(*[
            Transform(word,
                      ...
                      rate_func=linear,
                      run_time=2) for word in displayed_words
...
class FermatsPrincipleStatement(Scene):
    def construct(self):
        ...
        self.play(DelayByOrder(FadeIn(everything, run_time=3)),
                  Animation(words))
        self.play(ApplyMethod(everything.set_color, WHITE), )
        self.wait()
...
class VideoProgression(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(FadeOut(light_in_two), GrowFromCenter(snells),
                  DelayByOrder(ApplyMethod(arrow2.set_color, BLUE_D)))
        self.wait()
        self.play(FadeOut(optics), GrowFromCenter(multi),
...
class Section2(Scene):
    def construct(self):
        self.add(
            TextMobject("Section 2: Mark Levi's insight, and a challenge",
                        size="\\large"))
        self.wait()
