...
class MultilayeredGlass(PhotonScene, ZoomedScene):
    ...
    def construct(self):
        ...
        photon_run = self.photon_run_along_path(self.discrete_path,
                                                run_time=1,
                                                rate_func=rush_into)
    ...
    def generate_layer_info(self):
        self.layer_thickness = float(self.top -
                                     self.bottom) / self.num_discrete_layers
        self.layer_tops = np.arange(self.top, self.bottom,
    ...
    def generate_layers(self):
        ...
        def create_region(top, color):
            return Region(lambda x, y:
                          (y < top) & (y > top - self.layer_thickness),
                          color=color)
    ...
    def generate_discrete_path(self):
        ...
        self.discrete_path.add_line(
            points[end],
            FRAME_X_RADIUS * RIGHT + (self.layer_tops[-1] - 1) * UP)
    ...
    def show_layer_variables(self):
        ...
        for (top1, top2), x in zip(layer_top_pairs, it.count(1)):
            eq_mob = TexMobject(["v_%d" % x, "=", "\sqrt{\phantom{y_1}}"],
                                size="\\Large")
            midpoint = UP * (top1 + top2) / 2
            eq_mob.shift(midpoint)
            v_eq = eq_mob.split()
            center_paths.append(
                Line(midpoint + FRAME_X_RADIUS * LEFT,
                     midpoint + FRAME_X_RADIUS * RIGHT))
            brace_endpoints = Mobject(Point(self.top * UP + x * RIGHT),
        ...
        for v_eq, start_y, end_y, brace in quads:
            self.remove(brace)
            self.play(ShowCreation(v_eq[1]), ShowCreation(v_eq[2]),
    ...
    def isolate_bend_points(self):
        ...
        for index in range(3):
            ...
            self.play(
                ApplyMethod(little_square.shift,
    ...
    def show_snells(self, index, frame):
        ...
        left, equals, right = TexMobject([left_text, "=", right_text]).split()
        vs = []
        sqrt_ys = []
        for x, numerator in [(index, left), (index + 1, right)]:
            v, sqrt_y = [
                TexMobject(text, size="\\Large").next_to(numerator, DOWN)
                for text in ("v_%d" % x, "\\sqrt{y_%d}" % x)
            ]
            vs.append(v)
            sqrt_ys.append(sqrt_y)
        start, end = [
            Mobject(left.copy(), mobs[0], equals.copy(), right.copy(),
                    mobs[1]).replace(frame) for mobs in (vs, sqrt_ys)
        ]
