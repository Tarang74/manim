...
class PhysicalIntuition(Scene):
    def construct(self):
        ...
        self.play(*[
            Transform(*pair) for pair in zip(terms.split(), all_arrows.split())
        ])
...
class TimeLine(Scene):
    def construct(self):
        ...
        timeline = NumberLine(numerical_radius=300,
                              ...
                              unit_length_to_spatial_width=FRAME_X_RADIUS /
                              100,
                              tick_frequency=10,
...
class SolutionsToTheBrachistochrone(Scene):
    def construct(self):
        ...
        cycloids = Mobject(
            *[Cycloid(radius=r, end_theta=2 * np.pi) for r in r_range])
        lower_left = 2 * DOWN + 6 * LEFT
        lines = Mobject(*[
            Line(
                lower_left, lower_left + 5 * r * np.cos(np.arctan(r)) * RIGHT +
                2 * r * np.sin(np.arctan(r)) * UP) for r in r_range
        ])
        ...
        self.wait()
        self.play(Transform(cycloids, lines),
...
class VideoLayout(Scene):
    def construct(self):
        ...
        left_brace = Brace(
            Mobject(Point(left), Point(interpolate(left, right, 2. / 3))),
            DOWN)
        right_brace = Brace(
            Mobject(Point(interpolate(left, right, 2. / 3)), Point(right)), UP)
