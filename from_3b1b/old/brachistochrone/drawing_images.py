...
def sort_by_color(mob):
    indices = np.argsort(
        np.apply_along_axis(lambda p: -get_norm(p), 1, mob.rgbas))
...
class TracePicture(Scene):
    args_list = [("Newton", ), ("Mark_Levi", ), ("Steven_Strogatz", ),
                 ("Pierre_de_Fermat", ), ("Galileo_Galilei", ),
                 ("Jacob_Bernoulli", ), ("Johann_Bernoulli2", ),
                 ("Old_Newton", )]
    ...
    def construct(self, name):
        ...
        self.play(
            ...
            ShowCreation(edge_mobject, run_time=run_time, rate_func=linear))
