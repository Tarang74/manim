...
class PhotonScene(Scene):
    def wavify(self, mobject):
        ...
        thick_nudge_sizes = nudge_sizes.repeat(3).reshape(
            (len(nudge_sizes), 3))
...
class PhotonThroughLens(MultipathPhotonScene):
    ...
    def get_paths(self):
        ...
        focal_point = 2 * RIGHT
        return [
            Mobject(Line(FRAME_X_RADIUS * LEFT + fc[1] * UP, fc), Line(fc, sc),
                    Line(sc, focal_point),
                    Line(focal_point,
                         6 * focal_point - 5 * sc)).ingest_submobjects()
...
class PhotonOffMirror(MultipathPhotonScene):
    ...
    def get_paths(self):
        ...
        return [
            Mobject(Line(start_point, anchor_point),
                    Line(anchor_point, end_point)).ingest_submobjects()
    ...
    def get_paths(self):
        ...
        return [
            Mobject(
                ...
                Line([midpoint, 0, 0],
                     [end_point, -FRAME_Y_RADIUS, 0])).ingest_submobjects()
...
class ShowMultiplePathsScene(PhotonScene):
    def construct(self):
        ...
        self.play(self.photon_run_along_path(curr_path),
...
class ShowMultiplePathsThroughLens(ShowMultiplePathsScene):
    ...
    def get_paths(self):
        ...
        return [
            Mobject(Line(self.start_point, a), Line(a, b),
                    Line(b, self.end_point)).set_color(color)
    ...
    def get_paths(self):
        return [
            Mobject(Line(self.start_point, midpoint),
                    Line(midpoint, self.end_point)).set_color(color)
...
class StraightLinesFastestInConstantMedium(PhotonScene):
    def construct(self):
        ...
        self.play(
            self.photon_run_along_path(squaggle, run_time=2, rate_func=linear))
...
class LightIsFasterInAirThanWater(ShowMultiplePathsInWater):
    def construct(self):
        ...
        photon_runs = []
        photon_runs.append(
            self.photon_run_along_path(path1,
                                       rate_func=lambda t: min(1, 1.2 * t)))
...
class GeometryOfGlassSituation(ShowMultiplePathsInWater):
    def construct(self):
        ...
        pairs_for_end_transform = [(mob, mob.copy())
                                   for mob in (top_line, bottom_line,
                                               left_brace, x_mob)]
        ...
        self.wait(3)
        self.clear()
        self.add(glass, point_a, point_b, A, B, top_line, bottom_line,
                 left_brace, x_mob)
        ...
        self.play(*[
            ...
            for pair in [(top_line,
                          end_top_line), (bottom_line, end_bottom_line),
                         (left_brace, end_brace), (x_mob, end_x_mob)]
...
class Spring(Line):
    ...
    def generate_points(self):
        ...
        def loop(t):
            return micro_radius * (RIGHT + np.cos(m * t) * LEFT +
                                   np.sin(m * t) * UP)

        new_epsilon = self.epsilon / (m * micro_radius) / length

        self.add_points(
            [t * RIGHT + loop(t) for t in np.arange(0, 1, new_epsilon)])
...
class SpringSetup(ShowMultiplePathsInWater):
    ...
    def generate_springs(self, ring):
        self.start_springs, self.end_springs = [
            Mobject(Spring(self.start_point, r.get_top()),
    ...
    def slide_ring(self, ring):
        self.play(
            ApplyMethod(ring.shift, self.ring_shift_val, **self.slide_kwargs))
