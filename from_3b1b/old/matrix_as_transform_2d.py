...
class ShowMultiplication(NumberLineScene):
    ...
    def copy_original_line(self):
        ...
        self.play(
            ApplyFunction(lambda m: m.shift(DOWN).set_color("lightgreen"),
                          copied_line),
            *[ApplyMethod(mob.shift, DOWN) for mob in copied_num_mobs])
        self.wait()
...
class ExamplesOfNonlinearOneDimensionalTransforms(NumberLineScene):
    def construct(self):
        ...
        pairs = [(sinx_plux_x, "numbers don't remain evenly spaced"),
...
class TransformScene2D(Scene):
    ...
    def add_x_y_arrows(self):
        self.x_arrow = Arrow(ORIGIN,
                             self.number_plane.num_pair_to_point((1, 0)),
                             color="lightgreen",
                             **ARROW_CONFIG)
        self.y_arrow = Arrow(ORIGIN,
                             self.number_plane.num_pair_to_point((0, 1)),
                             color=LIGHT_RED,
                             **ARROW_CONFIG)
        self.add(self.x_arrow, self.y_arrow)
        self.number_plane.filter_out(lambda x_y_z: (0 < x_y_z[0]) and
                                     (x_y_z[0] < 1) and (abs(x_y_z[1]) < 0.1))
        self.number_plane.filter_out(lambda x_y_z1: (0 < x_y_z1[1]) and (
...
class ShowMatrixTransform(TransformScene2D):
    ...
    def construct(self, matrix, with_background, show_matrix):
        ...
        def func(mobject):
            mobject.points[:, :2] = np.dot(mobject.points[:, :2],
                                           np.transpose(matrix))
            return mobject

        self.wait()
        kwargs = {"run_time": 2.0, "path_func": self.get_path_func(matrix)}
        ...
        if hasattr(self, "x_arrow") and hasattr(self, "y_arrow"):
            for arrow, index in (self.x_arrow, 0), (self.y_arrow, 1):
                new_arrow = Arrow(ORIGIN,
                                  self.number_plane.num_pair_to_point(
                                      matrix[:, index]),
    ...
    def get_density_factor(self, matrix):
        max_norm = max(
            [abs(get_norm(column)) for column in np.transpose(matrix)])
...
class ExamplesOfNonlinearTwoDimensionalTransformations(Scene):
    def construct(self):
        ...
        pairs = [(squiggle, "lines do not remain straight"),
...
class TrickyExamplesOfNonlinearTwoDimensionalTransformations(Scene):
    def construct(self):
        ...
        diagonal = Line(DOWN * FRAME_Y_RADIUS + LEFT * FRAME_X_RADIUS,
...
class ShowMatrixTransformWithDot(TransformScene2D):
    ...
    def construct(self, matrix, with_background, show_matrix):
        ...
        anims = [
            ...
            ApplyMethod(dot.shift,
                        func(deepcopy(dot)).get_center() - dot.get_center(),
                        **kwargs),
        ]
        if hasattr(self, "x_arrow") and hasattr(self, "y_arrow"):
            for arrow, index in (self.x_arrow, 0), (self.y_arrow, 1):
    ...
    def get_path_func(self, matrix):
        rotational_components = [
            ...
            for i in [0, 1] for sign in [((-1)**i) * np.sign(matrix[1 - i, i])]
        ]
