...
def logo_to_circle():
    ...
    small_circle = Circle(density=CIRCLE_DENSITY,
                          color='skyblue').scale(LOGO_RADIUS).set_color(
                              DARK_BROWN,
                              lambda x_y_z: x_y_z[0] < 0 and x_y_z[1] > 0)
...
def count_sections(*radians):
    ...
    for x in range(1, len(points)):
        if x == 1:
            ...
            sc.add(dots[x])
        new_lines = Mobject(*[Line(points[x], points[y]) for y in range(x)])
...
def interesting_problems():
    ...
    fermat_copies, face_copies, tb_copies, sb_copies = (Mobject(
        *[deepcopy(mob).scale(0.5).shift(locale)
          for locale in locales]) for mob in [fermat, face, tb, sb])

    sc.add(face, tb)
    sc.animate(ShowCreation(fermat, run_time=1))
    sc.add(fermat)
    sc.wait()
    sc.animate(Transform(deepcopy(fermat).repeat(len(locales)), fermat_copies),
               FadeIn(face_copies, run_time=1.0))
    sc.animate(FadeIn(tb_copies))
    sc.wait()
    sc.animate(Transform(tb, sb), Transform(tb_copies, sb_copies))
    return sc
...
def different_points(radians1, radians2):
    ...
    circle = Circle(density=CIRCLE_DENSITY).scale(RADIUS)
    sc.add(circle)
    points1, points2 = ([(RADIUS * np.cos(angle), RADIUS * np.sin(angle), 0)
                         for angle in radians]
                        for radians in (radians1, radians2))
    dots1, dots2 = (Mobject(*[Dot(point) for point in points])
...
def next_few_videos(*radians):
    ...
    frame = VideoIcon().set_color("black", lambda point: get_norm(point) < 0.5)
