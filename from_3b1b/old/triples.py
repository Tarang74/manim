    ...
    def construct(self):
        ...
        for a, b, c in triples:
            ...
            for n, color in zip([a, b, c], SIDE_COLORS):
                square_group = VGroup(*[
                    square.copy().shift(x * RIGHT + y * UP) for x in range(n)
                    for y in range(n)
                ])
            ...
            c_square.rotate(hyp_line.get_angle(),
            ...
            if c in [5, 13, 25]:
                ...
                i_list = [
                    i for i in range(c**2)
                    if (i % c) in keys and (i // c) in keys
                ]
            else:
            ...
            equation = TexMobject(str(a), "^2", "+", str(b), "^2", "=", str(c),
                                  "^2")
            ...
            self.play(FadeIn(triangle), self.teacher.change_mode,
                      "raise_right_hand")
...
class CompareToFermatsLastTheorem(TeacherStudentsScene):
    def construct(self):
        expressions = [
            TexMobject("a", "^%d" % d, "+", "b", "^%d" % d, "=", "c",
                       "^%d" % d) for d in range(2, 9)
        ]
        ...
        top_text = top_brace.get_text("Abundant integer solutions",
                                      buff=SMALL_BUFF)
...
class AskAboutFavoriteProof(TeacherStudentsScene):
    def construct(self):
        self.student_says("What's you're \\\\ favorite proof?",
...
class PythagoreanProof(Scene):
    ...
    def show_proof(self):
        ...
        big_squares = VGroup(
            *list(map(self.get_big_square, [triangle, triangle_copy])))
        negative_space_words = TextMobject("Same negative \\\\ space")
        ...
        self.play(FadeIn(big_squares), Write(negative_space_words),
                  ShowCreation(double_arrow), *list(map(FadeOut, squares)))
        self.wait(2)
        self.play(*it.chain(
    ...
    def get_big_square(self, triangle):
        ...
        square.replace(VGroup(triangle, triangle.added_triangles),
                       stretch=True)
    ...
    def get_added_triangles_to_c_square(self, triangle, c_square):
        return VGroup(*[
            triangle.copy().rotate(i * np.pi / 2,
                                   about_point=c_square.get_center())
            for i in range(1, 4)
        ])
...
class ReframeOnLattice(PiCreatureScene):
    ...
    def resize_plane(self):
        ...
        for coord in coordinate_labels:
            ...
            coord.target.next_to(new_plane.coords_to_point(x, 0),
                                 DOWN,
                                 buff=SMALL_BUFF)

        self.play(MoveToTarget(self.plane),
    ...
    def walk_through_square_geometrically(self):
        ...
        for method, args in method_args_list:
            self.play(Animation(ghost_plane),
    ...
    def get_all_plane_dots(self):
        x_min, y_min = list(
            map(
                int,
                self.plane.point_to_coords(FRAME_X_RADIUS * LEFT +
                                           FRAME_Y_RADIUS * DOWN)))
        x_max, y_max = list(
            map(
                int,
                self.plane.point_to_coords(FRAME_X_RADIUS * RIGHT +
                                           FRAME_Y_RADIUS * UP)))
        result = VGroup(*[
            Dot(
                self.plane.coords_to_point(x, y),
                radius=self.dot_radius,
                color=self.dot_color,
            ) for x in range(int(x_min),
                             int(x_max) + 1)
            for y in range(int(y_min),
                           int(y_max) + 1)
        ])
...
class OneMoreExample(Scene):
    ...
    def show_triangle(self):
        ...
        self.play(FadeIn(triangle),
                  Animation(
                      VGroup(self.line, self.dot, self.number_label[1],
                             *self.distance_label[1:])),
                  run_time=2)
        self.wait(2)
...
class WriteGeneralFormula(GeneralExample):
    ...
    def expand_square(self):
        ...
        top_line.next_to(rect.get_top(), DOWN)
        second_line = TexMobject("\\big(", "u^2 - v^2", "\\big)", "+",
                                 "\\big(", "2uv", "\\big)", "i")
        for i, j in (1, 0), (5, 1):
        ...
        self.wait()
        self.play(real_part.copy().next_to, real_part_line.copy(), DOWN,
                  SMALL_BUFF, ShowCreation(real_part_line))
        self.wait()
        self.play(
            FadeOut(
                VGroup(self.example_label, self.example_dot, self.example_line,
                       self.z_to_z_squared, self.z_to_z_squared_arrow)),
            imag_part.copy().next_to, imag_part_line.copy(), RIGHT, SMALL_BUFF,
    ...
    def show_uv_to_triples(self):
        ...
        uv_title.next_to(h_line.point_from_proportion(1. / 6), UP, SMALL_BUFF)
        triple_title.next_to(h_line.point_from_proportion(2. / 3), UP,
                             SMALL_BUFF)
        ...
        self.play(
            *list(map(FadeIn, [rect, h_line, v_line, uv_title, triple_title])))
        self.play(*[
...
class VisualizeZSquared(Scene):
    ...
    def add_plane(self):
        width = (FRAME_X_RADIUS +
                 abs(self.plane_center[0])) / self.final_unit_size
        ...
        coordinate_labels = VGroup()
        z_list = np.append(self.x_label_range,
    ...
    def draw_arrows(self):
        ...
        for z in z_list:
            ...
            square_label = TexMobject(complex_string_with_i(z**2))
            for label, point in (z_label, z_point), (square_label,
                                                     square_point):
                if abs(z) > 2:
                    vect = RIGHT
                else:
            ...
            self.play(
                ReplacementTransform(z_dot.copy(), square_dot, path_arc=angle))
            self.play(FadeIn(square_label))
            self.wait()
            self.play(FadeOut(z_label), FadeOut(square_label),
                      Animation(arrow))
    ...
    def draw_dots(self):
        min_corner, max_corner = [
            self.background_plane.point_to_coords(u * FRAME_X_RADIUS * RIGHT +
                                                  u * FRAME_Y_RADIUS * UP)
            for u in (-1, 1)
        ]
    ...
    def apply_transformation(self):
        ...
        self.play(Homotopy(self.homotopy, self.color_grid),
    ...
    def get_triangles(self, z_list):
        ...
        for z in z_list:
            ...
            a_label, b_label, c_label = labels = [
                TexMobject(str(num)) for num in (a, b, c)
            ]
...
class AskAboutHittingAllPoints(TeacherStudentsScene):
    def construct(self):
        self.student_says("Does this hit \\\\ all pythagorean triples?",
...
class PointsWeMiss(VisualizeZSquared):
    ...
    def add_dots(self):
        z_list = [
            complex(x, y)**2 for x in self.dot_x_range
    ...
    def show_missing_point(self):
        ...
        labels = VGroup(
            *[TexMobject(complex_string_with_i(z)) for z in z_list])
...
class PointsWeMissAreMultiplesOfOnesWeHit(TeacherStudentsScene):
    def construct(self):
        words = TextMobject("Every point we", "miss",
                            "is \\\\ a multiple of one we", "hit")
...
class DrawRadialLines(PointsWeMiss):
    ...
    def resize_plane(self):
        everything = VGroup(*self.get_top_level_mobjects())
        everything.scale(self.final_unit_size / self.initial_unit_size,
    ...
    def show_single_line(self):
        ...
        self.play(dot.scale_in_place, 2, dot.set_color, RED)
    ...
    def show_all_lines(self):
        ...
        self.play(
            ReplacementTransform(seed_dots,
                                 self.lines,
                                 run_time=3,
                                 lag_ratio=0.5))
        self.play(
...
class RationalPointsOnUnitCircle(DrawRadialLines):
    ...
    def divide_by_c_squared(self):
        ...
        second_line = TexMobject("\\left(", "{a", "\\over", "c}", "\\right)",
                                 "^2", "+", "\\left(", "{b", "\\over", "c}",
                                 "\\right)", "^2", "=", "1")
        ...
        circle_label = TextMobject("All $x+yi$ where \\\\", "$x^2 + y^2 = 1$")
        ...
        circle_arrow = Arrow(circle_label.get_bottom(),
                             self.unit_circle.point_from_proportion(0.45),
                             color=GREEN)

        self.play(FadeIn(top_rect), FadeIn(top_line))
        self.wait()
        self.play(*[ReplacementTransform(top_rect, second_rect)] + [
            ReplacementTransform(top_line.get_parts_by_tex(tex,
                                                           substring=False),
            ...
            ReplacementTransform(top_line.get_parts_by_tex("1"),
                                 second_line.get_parts_by_tex("1"),
                                 run_time=2)
        ] + [
            Write(second_line.get_parts_by_tex(tex),
                  run_time=2,
                  rate_func=squish_rate_func(smooth, 0, 0.5)) for tex in (
                      "(",
                      ")",
                      "over",
                  )
    ...
    def from_rational_point_to_triple(self):
        ...
        self.play(
            ReplacementTransform(self.curr_example_point_group,
        ...
        distance_label = TexMobject(str(int(np.sqrt(x**2 + y**2))))
        distance_label.add_background_rectangle()
        distance_label.next_to(line.get_center(), UP + LEFT, SMALL_BUFF)

        self.play(
            ReplacementTransform(rational_point_group, integer_point_group))
...
class ProjectPointsOntoUnitCircle(DrawRadialLines):
    ...
    def project_all_dots(self):
        ...
        for dot in dots:
            ...
            dot.target.scale(unit_length / distance,
                             about_point=self.plane_center)
            dot.target.set_width(0.01)

        self.play(LaggedStartMap(MoveToTarget, dots, run_time=3,
                                 lag_ratio=0.2))
...
class SupposeMissingPoint(PointsWeMiss):
    ...
    def draw_missing_triple(self):
        ...
        triangle.add(a, b, c)
        words = TextMobject("If we missed \\\\ a triple \\dots")
    ...
    def project_onto_unit_circle(self):
        ...
        arrow = Arrow(rational_point_word.get_bottom(),
                      dot.target,
                      buff=SMALL_BUFF)
...
class FinalProof(RationalPointsOnUnitCircle):
    ...
    def show_all_rational_slopes(self):
        ...
        for u in range(2, 7):
            for v in range(1, u):
                ...
                dot = Dot(point, color=YELLOW)
                line = Line(self.background_plane.number_to_point(-1),
                            point,
                            color=self.neg_one_dot.get_color())
                line.add(dot)

                label = TexMobject("\\text{Slope = }", str(v), "/", str(u))
                label.add_background_rectangle()
                label.next_to(self.background_plane.coords_to_point(1, 1.5),
                              RIGHT)
    ...
    def square_example_point(self):
        ...
        two_theta_arrow = Arrow(two_theta.get_right(),
        ...
        z_to_z_squared_arrow = Arrow(point,
                                     square_point,
                                     path_arc=np.pi / 3,
                                     color=WHITE)
        z_to_z_squared = TexMobject("z", "\\to", "z^2")
        ...
        z_to_z_squared.set_color_by_tex("z^2", MAROON_B)
        z_to_z_squared.add_background_rectangle()
        z_to_z_squared.next_to(z_to_z_squared_arrow.point_from_proportion(0.5),
                               RIGHT, SMALL_BUFF)
    ...
    def project_onto_circle(self):
        ...
        line.generate_target()
        line.target.scale(radius / line.get_length(),
    ...
    def show_same_slope(self):
        line = Line(self.neg_one_dot.get_center(),
        ...
        self.play(ShowCreation(line))
        self.wait()
        self.play(theta_group_copy.shift,
    ...
    def write_v_over_u_slope(self):
        ...
        self.wait()
        self.play(*[
            ReplacementTransform(self.uv_label.get_part_by_tex(tex).copy(),
                                 rhs.get_part_by_tex(tex),
                                 run_time=2) for tex in ("u", "v")
...
class BitOfCircleGeometry(Scene):
    def construct(self):
        ...
        p0, p1, p2 = [
            circle.point_from_proportion(alpha) for alpha in (0, 0.15, 0.55)
        ]
        O = circle.get_center()
        ...
        for point, tex, color in (O, "2", MAROON_B), (p2, "", RED):
            ...
            arc = Arc(angle=line2.get_angle() - line1.get_angle(),
