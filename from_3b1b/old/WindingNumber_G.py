...
class IntroSceneWrapper(PiCreatureScene):
    ...
    def introduce_two_words(self):
        ...
        self.wait()
        self.play(morty.change, "raise_right_hand", main_topic.next_to,
                  meta_topic.get_top(), UP, MED_SMALL_BUFF,
    ...
    def describe_main_topic(self):
        ...
        self.play(LaggedStartMap(FadeIn, xy_equation, run_time=1),
        ...
        for rainbow_solver in rainbow_solver1, rainbow_solver2:
            self.play(
                Transform(solver, rainbow_solver, run_time=2, lag_ratio=0.5))
...
class Introduce1DFunctionCase(Scene):
    ...
    def show_two_graphs(self):
        axes = self.axes
        f_graph = axes.get_graph(lambda x: 2 * x * (x - 0.75) * (x - 1.5) + 1,
                                 color=BLUE)
        g_graph = axes.get_graph(lambda x: 1.8 * np.cos(TAU * x / 2),
                                 color=YELLOW)

        label_x_corod = 2
        f_label = TexMobject("f(x)")
        f_label.match_color(f_graph)
        f_label.next_to(axes.input_to_graph_point(label_x_corod, f_graph),
                        LEFT)

        g_label = TexMobject("g(x)")
        g_label.match_color(g_graph)
        g_label.next_to(axes.input_to_graph_point(label_x_corod, g_graph), UP,
                        SMALL_BUFF)
        ...
        equation_arrow = Arrow(equation.get_bottom(),
                               cross_point,
                               buff=SMALL_BUFF,
                               color=WHITE)
        ...
        for graph, label in (f_graph, f_label), (g_graph, g_label):
            self.play(ShowCreation(graph),
                      ...
                      run_time=2)
        self.wait()
        self.play(ReplacementTransform(r_v_line.copy().fade(1), v_line),
        ...
        self.set_variables_as_attrs(f_graph,
                                    f_label,
                                    g_graph,
                                    g_label,
    ...
    def transition_to_sqrt_2_case(self):
        ...
        squared_label.next_to(axes.input_to_graph_point(2, squared_graph),
                              RIGHT)
        ...
        self.play(ReplacementTransform(g_graph, two_graph),
    ...
    def show_example_binary_search(self):
        self.binary_search(self.squared_graph,
                           self.two_graph,
                           x0=1,
                           x1=2,
                           n_iterations=8)
    ...
    def binary_search(
        ...
        midpoint_line_update = UpdateFromFunc(midpoint_line,
                                              lambda l: l.move_to(rect))
        ...
        for x in range(n_iterations - 1):
            ...
            leftovers_mid = self.compare_graphs_at_x(f_graph, g_graph, x_mid,
                                                     **kwargs)
            ...
            if mention_signs:
                ...
                if x < n_iterations_with_sign_mention:
                    added_anims = [
                        word.next_to, leftovers_mid[0].get_end(), -edge
                    ]
                elif word in self.camera.extract_mobject_family_members(
                        self.mobjects):
            ...
            self.play(MoveToTarget(rect), midpoint_line_update, decimal_update,
                      Animation(all_leftovers), FadeOut(to_fade), *added_anims)
            if zoom:
                ...
                everything.target[decimal_index].scale(1. / factor,
                                                       about_edge=UP)
                everything.target[midpoint_line_index].scale(1. / factor)
                if factor > 1:
                    self.play(everything.scale, factor,
...
class PiCreaturesAreIntrigued(AltTeacherStudentsScene):
    def construct(self):
        self.teacher_says("You can extend \\\\ this to 2d",
                          bubble_kwargs={
                              "width": 4,
                              "height": 3
                          })
...
class TransitionFromEquationSolverToZeroFinder(Introduce1DFunctionCase):
    ...
    def transition_to_difference_graph(self):
        ...
        new_graph = axes.get_graph(lambda x: f_graph.underlying_function(x) -
                                   g_graph.underlying_function(x),
                                   color=GREEN)
...
class RewriteEquationWithTeacher(AltTeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.play(
            Transform(*root_two_equations, run_time=1.5, path_arc=TAU / 2))
...
class InputOutputScene(Scene):
    CONFIG = {
        ...
        "output_scalar":
        10,
        "non_renormalized_func":
        plane_func_by_wind_spec(
            (-2, -1, 2),
            (1, 1, 1),
            (2, -2, -1),
        ),
    ...
    def get_colorings(self):
        ...
        out_cmos = ColorMappedObjectsScene(func=lambda p: (scalar * (p[
            0] - self.x_shift), scalar * (p[1] + self.y_shift)))
    ...
    def get_planes(self):
        ...
        for plane, vect, text, color in zip(planes, vects, label_texts,
                                            label_colors):
            plane.stretch_to_fit_width(self.plane_width)
            plane.add_coordinates(x_vals=list(range(-2, 3)),
            ...
            for submob in plane.get_family():
                if isinstance(submob, TexMobject) and hasattr(
                        submob, "background_rectangle"):
    ...
    def get_dots(self, input_plane, output_plane):
        ...
        for x in np.arange(x_min, x_max + step, step):
            for y in np.arange(y_max, y_min - step, -step):
                ...
                dot.target_color = rgba_to_color(
                    point_to_rgba(
...
class IntroduceInputOutputScene(InputOutputScene):
    ...
    def map_single_point_to_point(self):
        ...
        self.play(GrowFromCenter(in_dot), GrowArrow(input_label_arrow),
                  FadeIn(input_label))
        self.wait()
        self.play(
        ...
        for vect in compass_directions(4, UP + RIGHT):
            self.play(
                Rotating(in_dot,
                         about_point=in_dot.get_corner(vect),
                         radians=TAU,
                         run_time=1))
        self.wait()
        for coords in (-2, 2), (-2, -2), (2, -2), (1.5, 1.5):
            self.play(in_dot.move_to,
...
class IntroduceVectorField(IntroduceInputOutputScene):
    ...
    def construct(self):
        ...
        continual_updates = [
            out_dot_continual_update, continual_out_vector_update,
    ...
    def ask_about_2d_functions(self):
        ...
        for x in np.arange(-2.5, 3.0, 0.5):
            for y in np.arange(-2.5, 3.0, 0.5):
                ...
                dot.generate_target()
                dot.target.move_to(
                    out_plane.coords_to_point(x + 0.25 * np.cos(5 * y),
                                              y + 0.25 * np.sin(3 * x)))
                dots.add(dot)
...
class DotsHoppingToColor(InputOutputScene):
    ...
    def construct(self):
        ...
        inspector.add(*inspector.copy().set_color(BLACK).shift((TAU / 24) *
                                                               UP))
        ...
        def update_inspector_image(inspector_image):
            inspector_image.move_to(self.point_function(
                inspector.get_center()))

        inspector_image_update_anim = UpdateFromFunc(inspector_image,
                                                     update_inspector_image)
        ...
        # Show black zero
        zeros = tuple(
            it.starmap(input_plane.coords_to_point, [
                (-2., -1),
                (1, 1),
                (2, -2),
            ]))
        for x in range(2):
            for zero in zeros:
                path = ParametricFunction(bezier([
                    inspector.get_center(),
                    input_plane.coords_to_point(0, 0), zero
                ]),
                                          t_min=0,
                                          t_max=1)
                self.play(
        ...
        for dot in dots:
            dot.scale(1.5)
        self.play(FadeOut(input_coloring), input_plane.white_parts.set_color,
                  WHITE, LaggedStartMap(GrowFromCenter, dots))
        self.wait()
        random.shuffle(dots.submobjects)
        ...
        for x, y, width, height in quads:
            self.play(square.stretch_to_fit_width, width,
                      square.stretch_to_fit_height, height, square.move_to,
...
class Rearrange2DEquation(AltTeacherStudentsScene):
    def construct(self):
        f_tex, g_tex, h_tex = [
            "%s(\\text{2d point})" % char for char in ("f", "g", "h")
        ]
...
class SearchForZerosInInputSpace(ColorMappedObjectsScene):
    ...
    def construct(self):
        ...
        points = [
            4 * RIGHT + UP, 2 * RIGHT + 2 * DOWN, 2 * LEFT + 2 * DOWN,
            3 * RIGHT + 2.5 * DOWN
        ]
        for point in points:
...
class OneDRegionBoundary(Scene):
    ...
    def construct(self):
        ...
        alt_graphs = [
            self.get_graph_part(*points) for points in [
                (-1, -2),
                (-1, -1, -1),
                (1, 1, 1),
                (-0.75, 0, 1.75),
        ...
        region = Rectangle(stroke_width=0,
                           fill_color=YELLOW,
                           fill_opacity=0.5,
        ...
        x0_arrow, x1_arrow = arrows = VGroup(*[
            Arrow(axes.coords_to_point(x, 0),
        ...
        self.wait()
        self.play(
            *it.chain(list(map(GrowArrow, arrows)), list(map(Write, signs))))
        self.wait()
        self.play(
            ShowCreation(graph),
    ...
    def get_graph_part(self, *interim_values):
        ...
        result.set_points_smoothly([
            self.axes.coords_to_point(x, fx) for x, fx in zip(
                np.linspace(self.x0, self.x1, len(values)), values)
        ])
        return result
...
class DirectionOfA2DFunctionAlongABoundary(InputOutputScene):
    def construct(self):
        ...
        out_vect_update_anim = UpdateFromFunc(
            out_vect, lambda ov: ov.put_start_and_end_on(
                output_plane.coords_to_point(0, 0), rect_image.points[-1]).
            set_color(get_color()))
...
class HypothesisAboutFullyColoredBoundary(ColorMappedObjectsScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeIn, hypothesis[1]),
...
class ForeverNarrowingLoop(InputOutputScene):
    ...
    def construct(self):
        ...
        if self.start_around_target:
            ...
            circle.next_to(input_coloring.get_corner(self.input_plane_corner),
                           -self.input_plane_corner, SMALL_BUFF)
        ...
        circle_image_update_anim = UpdateFromFunc(circle_image,
                                                  update_circle_image)
...
class FailureOfComposition(ColorMappedObjectsScene):
    CONFIG = {
        "func": lambda p: (np.cos(TAU * p[1] / 3.5), np.sin(TAU * p[1] / 3.5))
    }
    ...
    def construct(self):
        ...
        equation = VGroup(no_answers_in_equation[0], plus,
        ...
        for square, char in zip(small_squares, [x, y]):
            ...
            self.play(small_squares.restore, ShowPassingFlash(ghost),
                      Write(char))
        self.wait()
...
class PathContainingZero(InputOutputScene, PiCreatureScene):
    ...
    def draw_path_hitting_zero(self):
        ...
        path.set_points_smoothly(
            list(
                it.starmap(self.input_plane.coords_to_point,
                           [(1, 2.5), (2.5, 2.5), (2, 0.5), (1, 1), (0.5, 1),
                            (0.5, 2), (1, 2.5)])))
    ...
    def comment_on_zero(self):
        ...
        words = TextMobject("Output is zero \\\\", "which has no direction")
        origin = self.output_plane.coords_to_point(0, 0)
        words.to_edge(DOWN, buff=LARGE_BUFF)
        background_rect = BackgroundRectangle(words,
                                              buff=SMALL_BUFF,
                                              opacity=1.0)
        ...
        self.play(Write(words[1]), morty.change, "pleading",
...
class TransitionFromPathsToBoundaries(ColorMappedObjectsScene):
    ...
    def construct(self):
        ...
        if self.include_question_mark:
            sum_tex += "\\, ?"
        for square, tex in (left_square, "x"), (right_square,
                                                "y"), (joint_rect, sum_tex):
        ...
        for path in path1, path2:
            ...
            self.play(MoveAlongPath(dot, path.copy()),
                      ShowCreation(path),
                      run_time=2)
        ...
        #Add regions
        self.play(FadeOut(paths), FadeOut(joint_path.brace), dot.move_to,
                  path1.get_start())
        for square in squares:
        ...
        self.play(
            ...
            Transform(left_square.label[1],
                      joint_rect.label[1][0],
                      path_arc=TAU / 6), FadeIn(joint_rect.label[1][1]),
            FadeIn(joint_rect.label[1][3:]), FadeOut(right_square.label[0]),
            Transform(right_square.label[1],
                      joint_rect.label[1][2],
                      path_arc=TAU / 6),
            MoveAlongPath(dot,
                          joint_rect,
                          run_time=6,
    ...
    def get_dot_and_add_continual_animations(self):
        ...
        def get_total_winding(dt=0):
            ...
            possible_windings = [
                np.floor(self.curr_winding) + k + rev for k in (-1, 0, 1)
            ]
            i = np.argmin(
                [abs(pw - self.curr_winding) for pw in possible_windings])
        ...
        update_dot_color = Mobject.add_updater(
            dot,
            lambda d: d.set_fill(get_output_color(), self.dot_fill_opacity))
...
class BreakDownLoopWithNonzeroWinding(TransitionFromPathsToBoundaries):
    def construct(self):
        ...
        for rect, tex in (left_square, "x"), (right_square, "y"), (joint_rect,
                                                                   "3"):
        ...
        arrows = VGroup(*[
            Arrow(both_cannot_be_zero.get_bottom(),
                  var.get_top(),
                  buff=SMALL_BUFF) for var in (x, y)
        ])
...
class BackToEquationSolving(AltTeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            *
            [ApplyMethod(pi.look_at, self.screen) for pi in self.pi_creatures])
        self.wait(3)
...
class MonomialTerm(PathContainingZero):
    ...
    def show_winding(self):
        ...
        out_dot = Dot()
        update_out_dot = UpdateFromFunc(out_dot,
                                        lambda d: d.move_to(get_out_point()))

        buff = self.label_buff

        def generate_label_update(label, point_func, origin):
            return UpdateFromFunc(
                label, lambda m: m.move_to((1 + buff) * point_func() - buff *
                                           origin, self.label_move_to_corner))
...
class PolynomialTerms(MonomialTerm):
    CONFIG = {
        "non_renormalized_func":
        plane_func_from_complex_func(lambda z: z**5 - z - 1),
        "full_func_label":
        "f(x) = x^5 - x - 1",
        "func_label":
        "x^5 + \\cdots",
        ...
        "label_buff":
        0.15,
        "label_move_to_corner":
        DOWN + LEFT,
        "should_end_with_rescaling":
        False,
    }
    ...
    def sweep_through_loop_interior(self):
        ...
        line, line_target = [
            Line(
                ...
                path_arc=u * TAU / 2,
                n_arc_anchors=40,
                background_image_file=self.input_coloring.
        ...
        update_out_line = UpdateFromFunc(
            out_line,
            lambda m: m.set_points(line.points).apply_function(self.
                                                               point_function),
        )

        self.play(
            Transform(line, line_target, run_time=10,
                      rate_func=there_and_back), update_out_line, morty.change,
            "hooray")
        self.wait()
    ...
    def cinch_loop(self):
        ...
        update_out_loop = UpdateFromFunc(
            out_loop, lambda m: m.set_points(loop.points).apply_function(
                self.point_function))
        ...
        self.play(
            ApplyMethod(
                loop.scale,
                0, {"about_point": self.input_plane.coords_to_point(0.2, 1)},
                run_time=12,
                rate_func=bezier([0, 0, 1, 1])), update_out_loop, morty.change,
            "hooray")
        self.wait()
...
class ShowComplexFunction(Scene):
    def construct(self):
        ...
        self.wait(2)
        self.play(MoveToTarget(fx_label),
...
class WindingNumbersInInputOutputContext(PathContainingZero):
    ...
    def construct(self):
        ...
        in_loop.move_to(
            self.input_plane.coords_to_point(*self.in_loop_center_coords))
        in_loop.match_background_image_file(self.input_coloring)

        out_loop = in_loop.copy()
        out_loop.match_background_image_file(self.output_coloring)
        update_out_loop = Mobject.add_updater(
            out_loop, lambda m: m.set_points(in_loop.points).apply_function(
                self.point_function))
        ...
        update_out_arrow = Mobject.add_updater(
            out_arrow, lambda a: a.put_start_and_end_on(
        ...
        words = TextMobject("How many times does \\\\ the output wind around?")
        ...
        self.play(ShowCreation(in_loop),
...
class AllOfTheVideos(Scene):
    ...
    def construct(self):
        thumbnail_dir = os.path.join(MEDIA_DIR,
                                     "3b1b_videos/Winding/OldThumbnails")
        n = 4
        images = Group(*[
...
class EndingCredits(Scene):
    def construct(self):
        text = TextMobject("Written and animated by: \\\\",
                           "Sridhar Ramesh \\\\", "Grant Sanderson")
...
class MentionQAndA(Scene):
    def construct(self):
        title = TextMobject("Q\\&A with ", "Ben", "and", "Sridhar\\\\", "at",
                            "Patreon")
...
class InfiniteListOfTopics(Scene):
    def construct(self):
        ...
        lines = VGroup(*[
            TextMobject(words).scale(0.5) for words in [
                "Winding number",
                "Laplace transform",
                "Wallis product",
                "Quantum information",
...
class ManyIterations(Scene):
    def construct(self):
        words = VGroup(*[
            TextMobject(word, alignment="") for word in [
                "Winding numbers, v1",
...
class MentionFree(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(items[:2].shift, UP, FadeInFromDown(items[2]), morty.change,
                  "surprised")
        self.wait(4)
...
class Thumbnail(SearchSpacePerimeterVsArea):
    CONFIG = {
        ...
        18,
        "func":
        plane_func_by_wind_spec((-3, -1.3, 2), (0.1, 0.2, 1), (2.8, -2, 1)),
    }
