...
class Piano(ImageMobject):
    ...
    def split(self):
        ...
        for count in range(14):
            ...
            key.add_points(self.points[(self.points[:, 0] > x0) *
                                       (self.points[:, 0] < x1)])
            keys.append(key)
        return keys
...
class Vibrate(Animation):
    ...
    def __init__(self, **kwargs):
        ...
        def func(x, t):
            return sum([(self.amplitude / ((k + 1)**2.5)) *
                        np.sin(2 * mult * t) * np.sin(k * mult * x)
    ...
    def interpolate_mobject(self, alpha):
        ...
        self.mobject.add_points(
            [[x * self.radius,
              self.func(x, alpha * self.run_time) + y, 0]
...
class IntervalScene(NumberLineScene):
    ...
    def add_open_interval(self, num, width, color=None, run_time=0):
        ...
        interval_line = Line(center_point + spatial_width * LEFT / 2,
        ...
        if run_time > 0:
            ...
            self.play(Transform(squished_interval, open_interval),
...
class TwoChallenges(Scene):
    def construct(self):
        two_challenges = TextMobject("Two Challenges",
                                     size="\\Huge").to_edge(UP)
...
class ChallengeOne(Scene):
    def construct(self):
        ...
        self.bottom_vibration = Vibrate(num_periods=1,
                                        run_time=3.0,
                                        center=DOWN,
                                        color=start_color)
        top_vibrations = [
            Vibrate(num_periods=freq,
                    run_time=3.0,
                    center=2 * UP,
                    color=next(colors))
    ...
    def continuously_vary_frequency(self, top_vib_1, top_vib_2):
        ...
        self.play(self.bottom_vibration, Transform(arrow1, arrow2, **kwargs),
...
class QuestionAndAnswer(Scene):
    def construct(self):
        ...
        twotwenty = TexMobject("220").scale(0.25).next_to(
            string1.mobject, LEFT)
        r220 = TexMobject("r\\times220").scale(0.25).next_to(
            string2.mobject, LEFT)
        question = TextMobject(
            "For what values of $r$ will the frequencies 220~Hz and \
            $r\\times$220~Hz sound nice together?").next_to(Q)
        answer = TextMobject(
            ["When $r$ is", "sufficiently close to", "a rational number"],
            size="\\small").scale(1.5)
        answer.next_to(A)
        correction1 = TextMobject("with sufficiently low denominator",
                                  size="\\small").scale(1.5)
        ...
        self.play(Transform(Point(answer[1].get_left()), answer[1]),
                  Transform(temp_answer_end, answer[2]), string1, string2)
        self.play(string1, string2, run_time=3.0)


class PlaySimpleRatio(Scene):
    args_list = [(Fraction(3, 2), "green"), (Fraction(4, 3), "purple"),
                 (Fraction(8, 5), "skyblue"), (Fraction(211, 198), "orange"),
    ...
    def construct(self, fraction, color):
        string1 = Vibrate(num_periods=1,
                          run_time=5.0,
                          center=DOWN,
                          color="blue")
        string2 = Vibrate(num_periods=fraction,
                          run_time=5.0,
                          center=2 * UP,
                          color=color)
...
class LongSine(Mobject1D):
    def generate_points(self):
        self.add_points([(x, np.sin(2 * np.pi * x), 0)
...
class DecomposeTwoFrequencies(Scene):
    def construct(self):
        ...
        self.add(line)
        self.play(
            ApplyMethod(comp.shift, 15 * LEFT, run_time=7.5, rate_func=linear))
...
class PatternInFrequencies(Scene):
    ...
    def construct(self, num1, num2, color):
        ...
        self.play(FlashOnXProximity(big_line, -2, *anim_mobs, **kwargs),
                  *anims)
...
class CompareFractionComplexity(Scene):
    def construct(self):
        ...
        self.play(FadeIn(indicates), ShowCreation(arrow3),
                  ShowCreation(arrow4))
        self.wait()
...
class IrrationalGang(Scene):
    def construct(self):
        ...
        eyes.scale(0.5)
        zeta3.add(
            eyes.next_to(zeta3, UP, buff=0).shift(0.3 * LEFT + 0.08 * DOWN))
...
class PianoTuning(Scene):
    def construct(self):
        ...
        semicircles = Mobject(
            *[deepcopy(semicircle).shift(jump * k * RIGHT) for k in range(23)])
    ...
    def show_interval(self, interval, half_steps):
        ...
        semicircles = self.semicircles[half_notes_to_base:half_notes_to_base +
                                       half_steps]
        product = TexMobject(
            ["\\left(2^{\\left(\\frac{1}{12}\\right)}\\right)"] * half_steps,
            size="\\small").next_to(self.piano, UP, buff=1.0)
        approximate_form = TexMobject("\\approx" +
                                      str(2**(float(half_steps) / 12)))
...
class PowersOfTwelfthRoot(Scene):
    def construct(self):
        ...
        for i in range(1, num_terms + 1):
            ...
            mob_list.append(
                Mobject(*[
                    term, approx_copy, approx_form, words, frac_mob,
                    error_string
                ]))
...
class AllValuesBetween1And2(NumberLineScene):
    def construct(self):
        ...
        one, two, irr = list(
            map(self.number_line.number_to_point, [1, 2, irrational]))
        top_arrow = Arrow(one + UP, one)
        bot_arrow = Arrow(irr + 2 * DOWN, irr)
        r = TexMobject("r").next_to(top_arrow, UP)
        irr_mob = TexMobject(str(irrational) + "\\dots").next_to(
            bot_arrow, DOWN)
        ...
        self.play(*[
            ApplyMethod(mob.shift, RIGHT, **kwargs) for mob in (r, top_arrow)
        ])
        self.wait()
        ...
        for approx in approximations:
            ...
            self.play(Transform(top_arrow, new_arrow),
...
class CoveringSetsWithOpenIntervals(IntervalScene):
    def construct(self):
        ...
        theorems = [
            TextMobject(words).shift(UP) for words in [
                "Heine-Borel Theorem", "Lebesgue's Number Lemma",
                "Vitali Covering Lemma", "Besicovitch Covering Theorem",
                "$\\vdots$"
            ]
        ]
...
class ShowAllFractions(IntervalScene):
    def construct(self):
        IntervalScene.construct(self)
        self.show_all_fractions(num_fractions=100,
                                remove_as_you_go=False,
                                pause_time=0.3)
        self.wait()
        self.play(*[
            CounterclockwiseTransform(mob, Point(mob.get_center()))
            for mob in self.mobjects if isinstance(mob, ImageMobject)
        ])
        self.wait()
...
class StepsToSolution(IntervalScene):
    ...
    def enumerate_rationals(self):
        ...
        for frac, count in zip(rationals(), list(range(1, 28))):
            ...
            comma = TextMobject(",").next_to(mob_copy,
                                             buff=0.1,
                                             aligned_edge=DOWN)
    ...
    def assign_intervals_to_rationals(self):
        ...
        self.play(
            *
            [ApplyMethod(mob.shift, FRAME_WIDTH * RIGHT) for mob in to_remove])
    ...
    def stretch_intervals(self):
        for interval, count in zip(self.intervals, it.count(1)):
            self.play(ApplyMethod(interval.scale_in_place, 1.0 / (count**2)),
                      run_time=1.0 / count)
        self.wait()
