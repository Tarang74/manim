...
class IntroducePutnam(Scene):
    ...
    def construct(self):
        ...
        question_groups = VGroup(*[
            VGroup(*[TextMobject("%s%d)" % (c, i))
                     for i in range(1, 7)]).arrange(DOWN, buff=MED_LARGE_BUFF)
            for c in ("A", "B")
        ...
        difficulty_strings = [
            "Pretty hard", "Hard", "Harder", "Very hard", "Ughhh",
            "Can I go home?"
        ]
        ...
        self.play(ReplacementTransform(six_hours, three_hours),
        ...
        self.wait()
        self.play(title.next_to, median_words.copy(), LEFT, LARGE_BUFF,
                  MoveToTarget(out_of_120), Write(median_words))
...
class NatureOf5sAnd6s(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(
            test.scale, self.test_scale_val, test.to_corner, UP + LEFT,
            FadeIn(self.teacher),
...
class HowDoYouStart(TeacherStudentsScene):
    def construct(self):
        self.student_says("How do you even start?",
...
class TwoDCase(Scene):
    ...
    def simplify_further(self):
        ...
        bubble = SpeechBubble(direction=RIGHT, height=3, width=3)
        bubble.pin_to(morty)
        bubble.to_edge(LEFT, SMALL_BUFF)
        bubble.write("Simplify \\\\ more!")

        self.play(FadeIn(morty))
        self.play(morty.change, "hooray", ShowCreation(bubble),
                  Write(bubble.content))
        self.play(Blink(morty))
        self.wait()
        self.play(morty.change, "happy", morty.fade, 1,
    ...
    def fix_two_points_in_place(self):
        ...
        self.play(
            LaggedStartMap(ApplyMethod, push_pins, lambda mob:
                           (mob.restore, )))
    ...
    def note_special_region(self):
        ...
        self.change_point_mobs([0, 0, arc.angle], ShowCreation(arc,
                                                               run_time=2))
    ...
    def draw_lines_through_center(self):
        ...
        self.wait()
        self.play(
            all_arcs.space_out_submobjects,
            1.5,
        ...
        self.wait()
        self.change_point_mobs([0, 0, np.mean(angles[:2]) + np.pi - angles[2]])
        self.wait()
        for x in range(3):
    ...
    def ask_about_probability_p3_lands_in_this_arc(self):
        ...
        answer = TexMobject("{\\text{Length of arc}", "\\over",
                            "\\text{Circumference}}")
        ...
        self.play(question.next_to, answer, UP, LARGE_BUFF, Write(equals),
                  FadeIn(answer))
    ...
    def various_arc_sizes_for_p1_p2_placements(self):
        ...
        self.play(*list(
            map(FadeOut, [self.push_pins, self.triangle, self.arc_lines])))
        ...
        for angle_pair in angle_pairs:
            self.change_point_mobs_to_angles(
                angle_pair,
                VGroup(elbow, ninety_degrees).fade,
                1,
            )
    ...
    def ask_about_average_arc_size(self):
        ...
        self.play(GrowFromCenter(brace), Write(average))
        for x in range(6):
            self.change_point_mobs_to_angles(2 * np.pi * np.random.random(2))
        self.change_point_mobs_to_angles([1.2 * np.pi, 0.3 * np.pi])
        self.wait()
    ...
    def overall_probability(self):
        ...
        self.play(FadeIn(triangle), Animation(point_mobs))
        self.update_animations.append(self.triangle_update)
        self.have_p3_jump_around_randomly(8, wait_time=0.25)
        self.play(
            ReplacementTransform(self.proportion.copy(), VGroup(one_fourth)))
    ...
    def get_point_mob_labels(self):
        point_labels = VGroup(*[
            TexMobject("P_%d" % (i + 1)) for i in range(len(self.point_mobs))
        ])
    ...
    def get_center_lines(self):
        ...
        lines = VGroup()
        for angle in angles[:2]:
            line = DashedLine(self.radius * RIGHT, self.radius * LEFT)
            line.rotate(angle)
            line.shift(self.center)
    ...
    def get_center_lines_update(self, point_mobs, center_lines):
        def update_lines(center_lines):
            for point_mob, line in zip(point_mobs, center_lines):
                ...
                line.rotate_in_place(angle_of_vector(point) - line.get_angle())
    ...
    def change_point_mobs(self, d_thetas, *added_anims, **kwargs):
        anims = it.chain(self.update_animations, [
    ...
    def change_point_mobs_to_angles(self, target_angles, *added_anims,
                                    **kwargs):
        angles = self.get_point_mob_angles()
        n_added_targets = len(angles) - len(target_angles)
        target_angles = list(target_angles) + list(angles[-n_added_targets:])
        self.change_point_mobs(
            [ta - a for a, ta in zip(angles, target_angles)], *added_anims,
            **kwargs)
    ...
    def have_p3_jump_around_randomly(self,
                                     n_jumps,
                                     wait_time=0.75,
                                     run_time=0):
        for x in range(n_jumps):
            self.change_point_mobs([0, 0, 2 * np.pi * random.random()],
                                   run_time=run_time)
            self.wait(wait_time)
...
class AverageSizeOfSphericalTriangleSectionSupplement(Scene):
    def construct(self):
        words = TextMobject("Average size of \\\\",
                            "this section",
                            "?",
                            arg_separator="")
...
class RevisitTwoDCase(TwoDCase):
    ...
    def setup_circle(self):
        ...
        self.add(center_dot, circle, triangle, point_mobs, point_labels)
        self.add_foreground_mobjects(center_dot)
        self.set_variables_as_attrs(circle, center_dot)

    def show_probability(self):
        title = TexMobject("P(\\text{triangle contains the center})", "=",
                           "1/4")
        title.to_edge(UP, buff=MED_SMALL_BUFF)
        ...
        self.n_in = 0
        self.n_out = 0
        frac = TexMobject("{0", "\\over", "\\quad 0", "+", "0 \\quad}", "=")
        ...
        def place_random_triangles(n, wait_time):
            for x in range(n):
                ...
                for num, placeholder, position in zip(nums, placeholders,
                                                      positions):
    ...
    def add_lines_and_comment_on_them(self):
        ...
        self.wait(2)
        self.play(
            center_lines.restore, center_lines.fade, 1,
            *list(
                map(FadeOut, [
                    rect,
                    words2,
    ...
    def rewrite_random_procedure(self):
        ...
        words = VGroup(*list(
            map(TextMobject, [
                "Choose 3 random points", "Choose 2 random lines",
        ...
        self.play(Write(random_procedure), ShowCreation(underline))
        ...
        self.play(FadeIn(words[1]), LaggedStartMap(GrowFromCenter,
                                                   center_lines))
        for x in range(3):
        ...
        self.play(p3_group.restore, p3_group.set_fill, YELLOW, 1)
        self.wait()
...
class ThisIsWhereItGetsGood(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("This is where \\\\ things get good",
                          target_mode="hooray")
...
class ContrastTwoRandomProcesses(TwoDCase):
    ...
    def construct(self):
        ...
        left_circles = VGroup(
            ...
            VGroup(circle, center_lines, point_label_groups[2]).copy(),
            VGroup(circle, center_lines, *point_label_groups[2::-1]).copy(),
        )
...
class Formality(TeacherStudentsScene):
    def construct(self):
        ...
        self.student_says("How would you \\\\ write that down?",
                          target_mode="sassy")
        self.change_student_modes("confused", "sassy", "erm")
        self.wait()
        self.play(Write(words), FadeOut(self.students[1].bubble),
...
class ProblemSolvingTakeaways(Scene):
    def construct(self):
        ...
        points = VGroup(*[
            TextMobject(string, alignment="") for string in [
        ...
        points.arrange(DOWN, buff=LARGE_BUFF, aligned_edge=LEFT)
...
class BrilliantPuzzle(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            LaggedStartMap(ApplyMethod, students, lambda m:
                           (m.change, "horrified", m.test)))
        self.wait()
        self.play(
            LaggedStartMap(ApplyMethod, students, lambda m:
                           (m.change, "conniving")))
    ...
    def get_arrow_swap_anim(self, arrow):
        ...
        arrow.target.rotate(target_angle,
...
class AddedPromoWords(Scene):
    def construct(self):
        words = TextMobject("First", "$2^8$", "vistors get",
                            "$(e^\\pi - \\pi)\\%$", "off")
        words.set_width(FRAME_WIDTH - 1)
        words.to_edge(DOWN)
