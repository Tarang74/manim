...
class ProbabalisticVectorCloud(ProbabalisticMobjectCloud):
    ...
    def update_mobject_by_point(self, vector, point):
        vector.put_start_and_end_on(self.center_func(), point)
...
class RadarPulseSingleton(ContinualAnimation):
    ...
    def update_mobject(self, dt):
        ...
        if self.reflection_distance is not None:
            ...
            #Check if done
            arc_point = arc.get_edge_center(-self.direction)
            if np.dot(arc_point, self.direction) < np.dot(
                    self.start_center, self.direction):
                self.finished = True
                self.arc.fade(1)
...
class MultipleFlashes(Succession):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Succession.__init__(
            self, *[Flash(*args, **kwargs) for x in range(self.num_flashes)])
...
class MentionUncertaintyPrinciple(TeacherStudentsScene):
    def construct(self):
        ...
        vector_cloud = ProbabalisticVectorCloud(
            ...
            center_func=lambda: dot_cloud.gaussian_distribution_wrapper.
            get_parameters()[0],
        )
        ...
        def get_brace_text_group_update(gdw, vect, text, color):
            ...
            text = brace.get_tex("2\\sigma_{\\text{%s}}" % text,
                                 buff=SMALL_BUFF)
        ...
        dot_brace_anim = get_brace_text_group_update(
            dot_cloud.gaussian_distribution_wrapper, DOWN, "position",
            dot_cloud.color)
        vector_brace_anim = get_brace_text_group_update(
            vector_cloud.gaussian_distribution_wrapper, UP, "momentum",
            vector_cloud.color)

        self.add(title)
        self.add(dot_cloud)
        self.play(Write(title), self.teacher.change, "raise_right_hand",
                  self.get_student_changes(*["pondering"] * 3))
        self.play(Write(dot_brace_anim.mobject, run_time=1))
        ...
        self.play(FadeIn(vector_brace_anim.mobject))
...
class FourierTradeoff(Scene):
    ...
    def construct(self):
        ...
        def get_wave_packet_function():
            factor = 1. / get_width()
            return lambda t: (factor**0.25) * np.cos(4 * TAU * t) * np.exp(
                -factor * (t - time_mean)**2)
        ...
        fourier_graph_update = UpdateFromFunc(
            fourier_graph, lambda g: Transform(
        ...
        anims = [ReplacementTransform(wave_packet.copy(), fourier_graph)]
        if self.show_text:
            anims += [GrowArrow(arrow), Write(fourier_words, run_time=1)]
        self.play(*anims)
        # self.play(FadeOut(arrow))
        self.wait()
        for width in self.widths:
            self.play(width_tracker.set_value,
                      width,
                      wave_packet_update,
...
class ShowPlan(PiCreatureScene):
    ...
    def play_sound_anims(self, word):
        ...
        wave_fader = UpdateFromAlphaFunc(
            wave, lambda w, a: w.set_stroke(width=3 * a))
        checkmark = self.get_checkmark(word)

        self.add(wave)
        self.add_foreground_mobjects(rect, word)
        self.play(Animation(word), wave_fader, morty.change,
                  "raise_right_hand", word)
        self.wait(2)
    ...
    def play_doppler_anims(self, word):
        ...
        while not pulse.is_finished() and next(count) < 15:
            self.play(morty.look_at, pulse.mobject, run_time=0.5)
        self.play(
            Write(checkmark),
            UpdateFromAlphaFunc(target,
                                lambda m, a: m.set_fill(opacity=1 - a)),
            FadeOut(radar_dish), morty.change, "happy")
        self.wait()
    ...
    def play_quantum_anims(self, word):
        ...
        self.wait()
        self.play(Write(ish), morty.change, 'maybe')
        self.wait(6)
...
class TwoCarsAtRedLight(Scene):
    ...
    def pull_up_behind(self):
        ...
        source_point = VectorizedPoint(traffic_light[2].get_right())
        screen = Line(ORIGIN, UP)
        screen.next_to(source_point, RIGHT, LARGE_BUFF)
        red_light = Spotlight(color=RED,
        ...
        #Setup cars
        car1, car2 = cars = self.cars = VGroup(*[Car() for x in range(2)])
        cars.arrange(RIGHT, buff=LARGE_BUFF)
        cars.next_to(traffic_light, LEFT, buff=LARGE_BUFF, aligned_edge=DOWN)
        ...
        self.play(
            ...
            ), Animation(traffic_light), self.get_flashes(car2, num_flashes=3),
            MoveCar(
                car1,
                car1.start_point,
                run_time=3,
    ...
    def flash_in_sync_short_time(self):
        ...
        text.align_to(axes.coords_to_point(0.25, 0), LEFT)

        self.play(self.get_flashes(car1, num_flashes=2),
        ...
        self.play(self.get_flashes(car1, num_flashes=3),
        ...
        self.time_graph_label = VGroup(brace, text)
    ...
    def show_low_confidence(self):
        ...
        peak_point = frequency_axes.input_to_graph_point(1, frequency_graph)
        ...
        self.play(
            ReplacementTransform(self.time_axes.copy(), frequency_axes),
            ReplacementTransform(self.time_graph.copy(), frequency_graph),
        )
        ...
        self.frequency_graph_label = VGroup(label, arrow)
    ...
    def flash_in_sync_long_time(self):
        ...
        self.play(
            ...
            UpdateFromFunc(
                ...
                lambda m: m.move_to(self.time_axes.x_axis.get_right(), LEFT)),
            ShowCreation(
                new_time_graph,
                run_time=n_spikes,
                rate_func=linear,
            ), ApplyMethod(
                frequency_graph.stretch,
                0.1,
                0,
                run_time=n_spikes,
    ...
    def show_high_confidence(self):
        ...
        self.play(GrowFromCenter(brace), Write(text, run_time=1),
                  *list(map(self.get_flashes, self.cars)))
        self.play(
            *[self.get_flashes(car, num_flashes=10) for car in self.cars])
    ...
    def get_multispike_function(self, spike_times):
        return lambda x: sum(
            [1.25 * np.exp(-100 * (x - m)**2) for m in spike_times])
    ...
    def construct(self):
        ...
        for width, phrase in zip(widths, phrases):
            ...
            if width is widths[0]:
                ...
                self.play(GrowFromCenter(brace), Write(phrase, run_time=1))
            else:
                self.play(change_width_anim(width),
        ...
        short_signal_words = TextMobject("Short", "signal", "correlates",
                                         "with", "wide range",
                                         "of frequencies")
        long_signal_words = TextMobject("Only", "wide", "signals", "correlate",
                                        "with a", "short range",
                                        "of frequencies")
        ...
        long_graph = FunctionGraph(lambda x: 0.5 * np.sin(freq * x),
                                   x_min=-FRAME_WIDTH,
                                   x_max=FRAME_WIDTH,
                                   n_components=0.001)
        ...
        self.play(ShowCreation(long_graph),
                  *list(map(FadeOut, [last_brace, last_phrase])))
        self.play(Write(short_signal_words), change_width_anim(widths[2]))
        self.play(long_graph.stretch,
                  0.35,
                  0,
                  long_graph.set_color,
                  GREEN,
                  run_time=5,
                  rate_func=wiggle)
        self.wait()
...
class CrossOutDefinitenessAndCertainty(TeacherStudentsScene):
    def construct(self):
        ...
        self.add(words)
        self.play(self.teacher.change, "sassy", ShowCreation(crosses[0]))
        self.play(self.get_student_changes(*3 * ["erm"]),
...
class BringInFourierTranform(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(self.teacher.change, "raise_right_hand", fourier.restore)
...
class FourierRecapScene(DrawFrequencyPlot):
    ...
    def preview_fourier_plot(self):
        ...
        fourier_graph = self.get_fourier_transform_graph(time_graph)
        ...
        self.play(
            ...
            ReplacementTransform(time_graph.copy(), fourier_graph, run_time=2),
            ReplacementTransform(signal_label.copy(),
                                 fourier_label,
                                 run_time=2,
                                 rate_func=squish_rate_func(smooth, 0.5, 1)),
        )
        ...
        self.play(
            LaggedStartMap(
                Indicate,
                self.frequency_axes.x_axis.numbers,
                run_time=4,
                rate_func=wiggle,
            ))
        self.wait()
    ...
    def wrap_signal_around_circle(self):
        ...
        self.play(*self.get_vector_animations(time_graph), run_time=15)
    ...
    def match_winding_to_beat_frequency(self):
        ...
        self.play(*self.get_vector_animations(self.time_graph,
...
class CenterOfMassDescription(FourierRecapScene):
    def construct(self):
        ...
        self.wait()
        self.play(MoveToTarget(circle_plane), com_dot.move_to,
        ...
        self.play(FadeOut(com_dot.length_label_group), Animation(line),
                  ShowCreation(arc), Write(theta))
        self.play(Rotate(VGroup(line, com_dot), angle, about_point=ORIGIN),
                  Transform(arc, alt_arc),
                  theta.move_to,
                  1.5 * alt_arc.point_from_proportion(0.5),
                  rate_func=there_and_back,
                  run_time=4)
        self.wait()
...
class LongAndShortSignalsInWindingMachine(FourierRecapScene):
    ...
    def extend_for_long_time(self):
        ...
        new_freq = 5.1
        long_pol_graph = self.get_polarized_mobject(long_time_graph,
                                                    freq=new_freq)
        fourier_graph = self.fourier_graph

        self.change_frequency(new_freq)
        self.play(FadeOut(self.graph), FadeOut(self.graph.polarized_mobject),
                  FadeOut(fourier_graph))
        self.play(ShowCreation(long_time_graph, rate_func=linear),
    ...
    def note_sharp_fourier_peak(self):
        fourier_graph = self.get_fourier_transform_graph(
            self.time_graph, num_graph_points=self.num_fourier_graph_points)
        self.fourier_graph = fourier_graph
    ...
    def very_short_signal(self):
        ...
        new_freq = 5.1
        short_pol_graph = self.get_polarized_mobject(short_time_graph,
                                                     freq=new_freq)
    ...
    def note_wide_fourier_peak(self):
        fourier_graph = self.get_fourier_transform_graph(
            self.graph, num_graph_points=self.num_fourier_graph_points)
        self.fourier_graph = fourier_graph
    ...
    def note_fourier_peak(self, fourier_graph, freq1, freq2):
        ...
        self.change_frequency(
            ...
            added_anims=[
                ...
                UpdateFromAlphaFunc(arrow, lambda m, a: m.set_fill(opacity=a)),
            ],
            run_time=3,
        )
        self.wait()
        self.change_frequency(
            freq2,
            added_anims=[MaintainPositionRelativeTo(arrow, dot)],
            run_time=3)
        self.wait()
        self.play(*list(map(FadeOut, [dot, arrow, self.center_of_mass_dot])))
        #This is not great...
...
class FocusRectangleInsert(FourierRecapScene):
    ...
    def construct(self):
        ...
        self.add(rect)
        self.wait()
        self.play(rect.stretch_to_fit_width,
                  self.target_width,
...
class MentionDopplerRadar(TeacherStudentsScene):
    def construct(self):
        ...
        look_at_anims = [
            Mobject.add_updater(pi, lambda pi: pi.look_at(pulse.mobject))
            for pi in self.get_pi_creatures()
        ]

        self.add(dish, plane, pulse, *look_at_anims)
        self.play(self.teacher.change, "hooray", words.restore)
        ...
        self.play(
            UpdateFromAlphaFunc(VGroup(plane, dish),
                                lambda m, a: m.set_fill(opacity=1 - a)))
...
class IntroduceDopplerRadar(Scene):
    ...
    def setup_axes(self):
        ...
        axes = Axes(x_min=0, x_max=10, y_min=-1.5, y_max=1.5)
        axes.move_to(DOWN)
        time_label = TextMobject("Time")
    ...
    def measure_distance_with_time(self):
        ...
        double_arrow = DoubleArrow(pulse_graph.peak_point,
        ...
        self.play(Write(echo_graph.label),
                  GrowArrow(echo_graph.arrow),
                  run_time=1)
        self.wait()
        self.play(GrowFromCenter(double_arrow), FadeIn(distance_text))
        self.wait()
        ...
        self.play(
            LaggedStartMap(
                FadeOut,
                VGroup(sum_graph, randy, pulse_graph.arrow, pulse_graph.label,
                       echo_graph.arrow, echo_graph.label, double_arrow,
                       distance_text)))
    ...
    def show_frequency_shift(self):
        ...
        self.play(
            UpdateFromAlphaFunc(plane, lambda m, a: m.set_fill(opacity=a)))
        self.play(
        ...
        self.play(
            UpdateFromAlphaFunc(plane, lambda m, a: m.set_fill(opacity=1 - a)))
    ...
    def show_frequency_shift_in_fourier(self):
        ...
        echo_label = VGroup(self.echo_brace, self.echo_text, self.echo_subtext)
        ...
        v_line_pair = VGroup(
            *[v_line.copy().shift(u * 0.6 * RIGHT) for u in (-1, 1)])
        v_line = VGroup(v_line)

        double_arrow = DoubleArrow(frequency_axes.coords_to_point(8, 0.007),
                                   frequency_axes.coords_to_point(16, 0.007),
                                   buff=0,
                                   color=WHITE)

        self.play(self.time_graph_group.to_edge, UP,
                  ApplyMethod(self.dish.shift, 2 * UP, remover=True),
                  FadeIn(frequency_axes))
        self.wait()
        self.play(
            FadeOut(sum_graph),
            FadeOut(echo_label),
            pulse_graph.set_stroke,
            {"width": 3},
        )
        self.play(
            ReplacementTransform(pulse_label[0].copy(),
        ...
        self.play(
            FadeOut(pulse_graph), FadeIn(sum_graph),
            ReplacementTransform(pulse_graph.fourier_transform,
    ...
    def get_frequency_pulse_graph(self, x, freq=50, **kwargs):
        return self.get_graph(self.get_frequency_pulse_function(x, freq),
...
class DopplerFormulaInsert(Scene):
    def construct(self):
        formula = TexMobject("f_{\\text{echo}", "=",
...
class MentionPRFNuance(TeacherStudentsScene):
    def construct(self):
        title = TextMobject("Speed of light", "$\\gg$", "Speed of a plane")
        title.to_edge(UP)
        self.add(title)
        ...
        displayed_doppler_shift = TextMobject("How I'm showing the \\\\",
                                              "Doppler shift")
        actual_doppler_shift = TextMobject("Actual\\\\", "Doppler shift")
        doppler_shift_words = VGroup(displayed_doppler_shift,
                                     actual_doppler_shift)
        ...
        self.play(Animation(pulse_graph),
                  self.teacher.change,
                  "raise_right_hand",
                  run_time=1)
        self.play(
        ...
        self.play(UpdateFromAlphaFunc(
            ...
            lambda g, a: Transform(
                g,
                self.get_spike_graph(interpolate(shift_x, pulse_x + 0.01, a), )
                .match_style(shift_graph)).update(1),
        ),
                  UpdateFromFunc(
                      brace, lambda b: b.match_width(peak_points, stretch=True)
                      .next_to(peak_points, UP, SMALL_BUFF)),
                  Transform(displayed_doppler_shift,
                            actual_doppler_shift,
                            rate_func=squish_rate_func(smooth, 0.3, 0.6)),
                  run_time=3)
        self.wait(2)

        everything = VGroup(title, axes, pulse_graph, shift_graph, brace,
                            displayed_doppler_shift)
        ...
        self.teacher_says(
            ...
            added_anims=[
                everything.scale, 0.4, everything.to_corner, UP + LEFT,
                UpdateFromAlphaFunc(rect,
                                    lambda m, a: m.set_stroke(width=2 * a))
            ],
        )
...
class TimeAndFrequencyGivePositionAndVelocity(IntroduceDopplerRadar):
    def construct(self):
        ...
        delta_x = 0.7
        brace = Brace(
            Line(axes.coords_to_point(x - delta_x, 1),
                 axes.coords_to_point(x + delta_x, 1)), UP)
...
class RadarOperatorUncertainty(Scene):
    def construct(self):
        ...
        self.add(vector_cloud)
        for i in range(3):
            for plane_factor, vector_factor, freq in (0.05, 10,
                                                      0.01), (20, 0.1, 0.1):
...
class AmbiguityInLongEchos(IntroduceDopplerRadar, PiCreatureScene):
    ...
    def introduce_multiple_objects(self):
        ...
        graphs = VGroup(*[self.get_frequency_pulse_graph(x) for x in x_coords])
        graphs.set_color_by_gradient(BLUE, YELLOW)
        sum_graph = self.axes.get_graph(
            lambda t: sum([graph.underlying_function(t) for graph in graphs]),
            num_graph_points=1000)

        noise_function = lambda t: np.sum(
            [0.5 * np.sin(f * t) / f for f in (2, 3, 5, 7, 11, 13)])
        noisy_graph = self.axes.get_graph(
            lambda t: sum_graph.underlying_function(t) *
            (1 + noise_function(t)),
            num_graph_points=1000)
        ...
        self.play(LaggedStartMap(GrowFromCenter, objects[1:]),
                  FadeOut(curr_graph), randy.change, "pondering")
        self.add(*pulses)
        self.wait(0.5)
    ...
    def fourier_transform_of_one_pulse(self):
        ...
        self.play(ApplyMethod(VGroup(self.axes, first_echo_graph).to_edge,
                              UP, {"buff": SMALL_BUFF},
                              rate_func=squish_rate_func(smooth, 0.5, 1)),
                  LaggedStartMap(FadeOut, self.objects),
                  LaggedStartMap(
                      FadeOut,
                      VGroup(self.curr_graph, self.dish, self.pi_creature)),
                  run_time=2)
        ...
        fourier_graph = get_fourier_graph(
            frequency_axes,
            first_echo_graph.underlying_function,
            t_min=0,
            t_max=25,
        ...
        self.play(
            ApplyFunction(
                lambda m: m.move_to(fourier_graph.saved_state).shift(-h_vect).
                fade(1),
        ...
        self.wait()
        self.play(ShowCreation(v_line))
        self.play(ReplacementTransform(VGroup(v_line), v_lines), rect.restore)
        self.wait()
    ...
    def show_echos_of_moving_objects(self):
        ...
        movements = self.object_movements = [
            always_shift(obj, direction=v / get_norm(v), rate=get_norm(v))
            for v, obj in zip(object_velocities, objects)
        ]
        pulses = self.get_pulses()
        continual_anims = pulses + movements

        self.play(FadeOut(self.axes), FadeOut(self.first_echo_graph),
                  LaggedStartMap(FadeIn, objects), FadeIn(self.dish))
    ...
    def overlapping_frequenies_of_various_objects(self):
        ...
        self.play(
            ReplacementTransform(VGroup(fourier_graph),
                                 shifted_graphs,
                                 lag_ratio=0.5,
                                 run_time=2))
        self.wait()
        self.play(
            shifted_graphs.arrange,
            DOWN,
            shifted_graphs.move_to,
            fourier_graph,
            DOWN,
        )
        self.wait()
    ...
    def get_pulses(self):
        return [
            self.get_pulse(self.dish.copy().shift(0.01 * obj.get_center()[0]),
    ...
    def get_shifted_frequency_graphs(self, fourier_graph):
        ...
        def get_func(v):
            return lambda f: fourier_graph.underlying_function(
                np.clip(
                    f - 5 * v[0],
                    frequency_axes.x_min,
        ...
        shifted_graphs = VGroup(
            *list(map(get_graph, list(map(get_func, self.object_velocities)))))
    ...
    def get_sum_graph(self, axes, graphs):
        ...
        return axes.get_graph(lambda t: sum([func(t) for func in funcs]), )
...
class SummarizeFourierTradeoffForDoppler(Scene):
    def construct(self):
        ...
        time_label = TextMobject("Time")
        frequency_label = TextMobject("Frequency")
        for label, axes in (time_label, time_axes), (frequency_label,
                                                     frequency_axes):
        ...
        def get_bottom_graphs():
            a = np.exp(a_mob.get_center()[0])
            graphs = VGroup(*[
                frequency_axes.get_graph(
                    lambda t: np.exp(-(5. / a) * (t - v)**2)) for v in v_values
            ])
        ...
        update_top_graphs = Mobject.add_updater(
            top_graphs, lambda g: Transform(g, get_top_graphs()).update(1))
...
class IntroduceDeBroglie(Scene):
    ...
    def construct(self):
        ...
        #Time line
        time_line = NumberLine(x_min=1900,
                               x_max=1935,
                               tick_frequency=1,
        ...
        def update_words(words):
            ...
            if text not in words.get_tex_string():
                words.__init__(text)
                words.scale(text_scale_val)
            words.move_to(interpolate(arrow.get_top(), date.get_bottom(), 0.5))

        update_words(words)
        self.play(FadeIn(time_line),
                  GrowArrow(arrow),
                  Write(words),
                  Write(date),
                  run_time=1)
        ...
        self.play(
            arrow.next_to,
            time_line.number_to_point(1924),
            UP,
            ChangingDecimal(date,
                            get_year,
        ...
        self.play(
            ...
            VGroup(arrow, words, date).shift, MED_LARGE_BUFF * UP, *[
                ApplyFunction(lambda m: m.rotate(TAU / 4).set_stroke(width=0),
                              mob,
                              remover=True) for mob in time_line.tick_marks
            ])
        ...
        for x in range(3):
            ...
            self.play(ApplyMethod(particle.move_to,
                                  axes.coords_to_point(22, 0),
                                  rate_func=linear),
    ...
    ###
    def get_wave_update_animation(self,
                                  axes,
                                  particle,
                                  re_line=None,
                                  im_line=None):
    ...
    def get_wave(self,
        ...
        def func(x):
            dispersion = fdiv(1., self.dispersion_factor) * (np.sqrt(
                1. / (1 + t0**2)))
            wave_part = complex_to_real_func(
                np.exp(complex(0, TAU * freq * (x - dispersion))))
    ...
    def get_wave_pair(self, axes, particle, colors=None, **kwargs):
        ...
        return VGroup(*[
            self.get_wave(axes, particle, C_to_R, color=color, **kwargs) for
            C_to_R, color in zip([lambda z: z.imag, lambda z: z.real], colors)
        ])
...
class ShowMomentumFormula(IntroduceDeBroglie, TeacherStudentsScene):
    ...
    def introduce_formula(self):
        ...
        p_arrow = Arrow(p_words.get_bottom(),
                        p.get_corner(UP + LEFT),
                        buff=SMALL_BUFF)
        ...
        xi_arrow = Arrow(xi_words.get_bottom(),
                         xi.get_corner(UP + RIGHT),
                         buff=SMALL_BUFF)
        ...
        v_lines.arrange(RIGHT,
                        buff=float(axes.x_axis.unit_size) /
                        self.default_wave_frequency)
        ...
        self.play(
            self.teacher.change, "raise_right_hand", GrowArrow(p_arrow),
            Succession(
                Write,
                p_words,
                ApplyMethod,
        ...
        self.play(Write(xi_words), GrowArrow(xi_arrow),
        ...
        self.play(LaggedStartMap(ShowCreation, v_lines),
                  self.get_student_changes(*["pondering"] * 3))
    ...
    def react_to_claim(self):
        ...
        self.play(full_formula.scale, 0, full_formula.move_to,
                  student.eyes.get_bottom() + SMALL_BUFF * DOWN,
                  Animation(student.bubble), **kwargs)
        ...
        self.student_says("Physics is \\\\ just weird",
                          bubble_kwargs={
                              "height": 2.5,
                              "width": 3
                          },
...
class AskPhysicists(PiCreatureScene):
    def construct(self):
        ...
        self.pi_creature_says(morty, "So...why?", target_mode="maybe")
        self.wait(2)
        self.play(
    ...
    def create_pi_creatures(self):
        ...
        physies = VGroup(*[
            PiCreature(color=c).flip() for c in (GREY, LIGHT_GREY, DARK_GREY)
        ])
...
class SortOfDopplerEffect(PiCreatureScene):
    ...
    def setup(self):
        ...
        self.camera = MovingCamera(rect, **self.camera_config)
    ...
    def construct(self):
        ...
        def get_wave():
            ...
            omega = self.omega
            color = interpolate_color(BLUE, RED, (k - 2) / 2.0)
            ...
            return VGroup(
                graph, *[
                    Arrow(x * RIGHT, x * RIGHT + func(x) * UP, color=color)
                    for x in np.arange(-4 * FRAME_X_RADIUS, FRAME_X_RADIUS,
                                       self.arrow_spacing)
                ])
            return
        ...
        randy = self.pi_creature
        randy_look_at = Mobject.add_updater(randy, lambda r: r.look_at(rect))
        ...
        ref_frame2_follow = Mobject.add_updater(ref_frame2,
                                                lambda m: m.next_to(rect, UP))
        ref_frame_1_continual_anim = ContinualAnimation(ref_frame1)

        self.add(t_tracker, wave_update, rect_movement, randy_look_at,
        ...
        self.play(
            UpdateFromAlphaFunc(
                screen_rect, lambda m, a: m.move_to(
                    interpolate(start_center, rect.get_center(), a))),
            k_tracker.shift,
            2 * RIGHT,
        )
        self.play(MaintainPositionRelativeTo(screen_rect, rect, run_time=4), )
        self.play(
            screen_rect.move_to,
            rect.get_right() + FRAME_X_RADIUS * LEFT,
            k_tracker.shift,
            2 * LEFT,
        )
...
class HangingWeightsScene(MovingCameraScene):
    ...
    def setup_springs(self):
        ...
        self.play(ShowCreation(ceiling), LaggedStartMap(ShowCreation, springs))
    ...
    def introduce(self):
        ...
        self.play(*[
            UpdateFromAlphaFunc(weight,
                                lambda w, a: w.set_width(2 * interpolate(
                                    w.start_radius, w.target_radius, a)),
                                run_time=2) for weight in self.weights
        ])
        self.play(FadeOut(arrows))
        self.wait(3)

    def show_analogy_with_electron(self):
        words = TextMobject("Analogous to the energy of a particle \\\\",
    ...
    def metaphor_for_something(self):
        ...
        self.play(Animation(de_broglie.backdrop, remover=True),
                  FadeIn(de_broglie))
        self.play(Write(words))
        self.wait(7)
        self.play(FadeOut(words), Animation(einstein.backdrop, remover=True),
                  FadeIn(einstein))
        self.wait(2)
    ...
    def moving_reference_frame(self):
            ...
            self.play(
                UpdateFromAlphaFunc(
                    camera_frame, lambda c, a: c.set_height(
                        interpolate(original_height, 0.95 * rect.get_height(),
                                    a)).move_to(
                                        interpolate(original_center,
                                                    rect.get_center(), a))),
                ApplyMethod(self.k_tracker.shift, RIGHT))
            self.play(
                MaintainPositionRelativeTo(camera_frame, rect, run_time=6))
            self.play(camera_frame.set_height, original_height,
        ...
        self.wait()
        self.play(
            UpdateFromAlphaFunc(rect, lambda m, a: m.set_stroke(width=2 *
                                                                (1 - a))))
        ...
        self.play(
            UpdateFromAlphaFunc(rect, lambda m, a: m.set_stroke(width=2 * a)))
        ...
        def update(mob, dt):
            ...
            if mob.curr_anim:
                ...
                if mob.curr_anim_time > mob.curr_anim.run_time:
                    ...
                    return
                mob.curr_anim.update(mob.curr_anim_time /
                                     mob.curr_anim.run_time)
...
class FourierTransformOfWaveFunction(Scene):
    ...
    def take_fourier_transform(self):
        ...
        self.fourier_graph_update_animation = UpdateFromFunc(
            fourier_graph, lambda m: Transform(
                m, get_wave_function_fourier_graph()).update(1))
        ...
        self.play(ReplacementTransform(self.axes.copy(), frequency_axes))
        self.play(
    ...
    def show_correlations_with_pure_frequencies(self):
        ...
        freq_label.next_to(v_line,
                           UP,
                           SMALL_BUFF,
                           submobject_to_align=f_equals[0])

        self.play(ShowCreation(sinusoid), ShowCreation(v_line),
                  Write(freq_label, run_time=1), FadeOut(self.fourier_label))
        last_f = 1
        for f in 1.4, 0.7, 1:
            self.play(
                sinusoid.stretch,
            ...
            last_f = f
        self.play(*list(map(FadeOut, [sinusoid, v_line, freq_label])))
    ...
    def this_is_momentum(self):
        ...
        formula.next_to(self.frequency_x_axis_label, UP)
        ...
        brace.add(words)
        for k in 2, 0.5, 1:
            self.play(self.k_tracker.set_value,
                      k,
    ...
    def show_tradeoff(self):
        for a in 5, 0.1, 0.01, 10, 0.5:
            self.play(ApplyMethod(self.a_tracker.set_value, a,
                                  run_time=2), self.wave_update_animation,
                      self.fourier_graph_update_animation)
            self.wait()
    ...
    def get_wave(self, axes):
        return axes.get_graph(self.get_wave_func(),
...
class MusicalNote(AddingPureFrequencies):
    def construct(self):
        ...
        graph = axes.get_graph(
            lambda x: op.mul(
                np.exp(-0.2 * (x - 4)**2),
                0.3 * (np.cos(2 * TAU * x) + np.cos(3 * TAU * x) + np.cos(
                    5 * TAU * x)),
            ), )
...
class AskAboutUncertainty(TeacherStudentsScene):
    def construct(self):
        self.student_says("What does this have \\\\ to do with ``certainty''",
...
class ProbabalisticDetection(FourierTransformOfWaveFunction):
    ...
    def setup_wave(self):
        ...
        squared_bell_curve = axes.get_graph(
            lambda x: bell_curve.underlying_function(x)**2).match_style(
                bell_curve)
    ...
    def detect_only_single_points(self):
        ...
        gdw_anim = Mobject.add_updater(
            gdw, lambda m: m.set_width(2.0 / (self.a_tracker.get_value()**
                                              (0.5))).move_to(rect))
        ...
        self.play(GrowFromCenter(brace), rect.restore, Write(question))
        self.wait()
    ...
    def show_probability_distribution(self):
        ...
        self.play(FadeIn(VGroup(axes, wave)), question_group.next_to, v_lines,
                  UP, {"buff": 0}, *list(map(ShowCreation, v_lines)))
        self.wait(10)

    def show_concentration_of_the_wave(self):
        self.play(
            self.a_tracker.set_value,
            5,
            self.wave_update_animation,
        )
        self.wait(10)
...
class HeisenbergCommentTodos(TODOStub):
    CONFIG = {"message": "Insert position-momentum trade-off"}
    ...
    def construct(self):
        ...
        self.play(
            PiCreatureSays(
                ...
                "Well, yes and no",
                target_mode="sassy",
            ), RemovePiCreatureBubble(other, target_mode="erm"))
        self.wait(4)
...
class OneLevelDeeper(Scene):
    def construct(self):
        ...
        randomness = ProbabalisticMobjectCloud(TextMobject("Randomness"),
                                               n_copies=5,
                                               time_per_change=0.05)
        ...
        self.wait(4)
        self.play(
            FadeIn(wave_words), GrowArrow(arrow),
            ApplyMethod(
                gdw.next_to,
                wave_words,
                DOWN,
...
class BetterTranslation(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            GrowArrow(translation_arrow), FadeIn(translation_words),
            ReplacementTransform(german_word.copy().fade(1), translation))
...
class ThinkOfHeisenbergUncertainty(PiCreatureScene):
    def construct(self):
        ...
        vector_cloud = ProbabalisticVectorCloud(center_func=dot_gdw.get_center)
        vector_gdw = vector_cloud.gaussian_distribution_wrapper
        ...
        continual_anims = [
            ...
            Mobject.add_updater(
                dot_gdw, lambda d: d.set_width(
                    (np.cos(freq * time_tracker.get_value()) + 1.1) / 2)),
            Mobject.add_updater(
                vector_gdw, lambda d: d.set_width(
                    (-np.cos(freq * time_tracker.get_value()) + 1.1) / 2)),
            dot_cloud, vector_cloud
        ]
        self.add(*continual_anims)

        position, momentum, time, frequency = list(
            map(TextMobject, ["Position", "Momentum", "Time", "Frequency"]))
        ...
        self.wait(10)
        self.play(
            ApplyMethod(VGroup(dot_gdw, vector_gdw).shift,
                        FRAME_X_RADIUS * RIGHT,
                        rate_func=running_start))
        self.remove(*continual_anims)
        self.play(morty.change, "raise_left_hand", groups,
                  FadeIn(
                      groups,
                      lag_ratio=0.5,
                      run_time=3,
...
class PatreonMention(PatreonThanks):
    def construct(self):
        ...
        self.play(DrawBorderThenFill(patreon_logo), morty.change, "gracious")
...
class Promotion(PiCreatureScene):
    ...
    def construct(self):
        ...
        url = TextMobject("AoPS.com/", "3b1b", arg_separator="")
        url.to_corner(UP + LEFT)
        url_rect = Rectangle(color=BLUE)
        url_rect.replace(url.get_part_by_tex("3b1b"), stretch=True)
        ...
        self.play(
            self.pi_creature.change_mode, "raise_right_hand", *[
                DrawBorderThenFill(submob,
                                   run_time=2,
                                   rate_func=squish_rate_func(
                                       double_smooth, a, a + 0.5))
                for submob, a in zip(aops_logo,
                                     np.linspace(0, 0.5, len(aops_logo)))
            ])
        self.play(
            words.scale,
            0.75,
        ...
        self.play(
            LaggedStartMap(
                ...
                aops_logo,
                lambda mob:
                (lambda m: m.shift(0.2 * UP).set_color(YELLOW), mob),
                rate_func=there_and_back,
                run_time=1,
        ...
        self.play(
            ...
            ), morty.change, "happy", aops_logo.shift, 1.5 * UP + 0.5 * RIGHT)
        self.play(Blink(mathy))
        self.wait()
        self.play(
            RemovePiCreatureBubble(mathy, target_mode="raise_right_hand"),
            aops_logo.to_corner,
            UP + RIGHT,
            aops_logo.shift,
            MED_SMALL_BUFF * DOWN,
        ...
        self.wait()
        self.play(
            Animation(BackgroundRectangle(book, fill_opacity=1), remover=True),
            FadeOut(book),
        )
        ...
        self.play(
            FadeOut(url_rect),
            url.get_part_by_tex("3b1b").set_color,
            BLUE,
        )
        self.wait(15)
...
class Thumbnail(Scene):
    def construct(self):
        ...
        dot_cloud = ProbabalisticDotCloud(n_copies=1000, )
        dot_gdw = dot_cloud.gaussian_distribution_wrapper
        ...
        axes = Axes(x_min=-6, x_max=6, x_axis_config={"unit_size": 0.25})
        graphs = VGroup(
            *[axes.get_graph(get_func(a)) for a in (
                10,
                3,
                1,
                0.3,
        ...
        frequency_axes = Axes(x_min=0, x_max=2, x_axis_config={"unit_size": 1})
