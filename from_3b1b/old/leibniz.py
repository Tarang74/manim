...
class LatticePointScene(Scene):
    ...
    def setup(self):
        ...
        plane = ComplexPlane(y_radius=self.y_radius,
    ...
    def setup_lattice_points(self):
        ...
        self.lattice_points.sort(lambda p: get_norm(p - self.plane_center))
    ...
    def get_radial_line_with_label(self, radius=None, color=None):
        ...
        radial_line = Line(self.plane_center,
    ...
    def get_lattice_points_on_r_squared_circle(self, r_squared):
        ...
        points.sort(lambda p: angle_of_vector(p - self.plane_center) %
                    (2 * np.pi))
        return points
    ...
    def draw_lattice_points(self, points=None, run_time=4):
        ...
        self.play(*[
            DrawBorderThenFill(
                ...
                run_time=run_time,
                rate_func=squish_rate_func(double_smooth, a, a + 0.25),
            ) for dot, a in zip(points, np.linspace(0, 0.75, len(points)))
        ])
    ...
    def add_axis_labels(self, spacing=2):
        ...
        x_labels = VGroup(*[TexMobject(str(x)) for x in x_range])
        y_labels = VGroup(*[TexMobject(str(y) + "i") for y in y_range])
        ...
        for labels, ticks in (x_labels, x_ticks), (y_labels, y_ticks):
            ...
            for tex_mob, tick in zip(labels, ticks):
                ...
                tex_mob.next_to(tick,
                                tick.get_start() - tick.get_end(), SMALL_BUFF)
...
class Introduction(PiCreatureScene):
    ...
    def introduce_three_objects(self):
        ...
        self.wait()
        self.play(Write(primes, run_time=2), morty.change_mode, "happy",
                  video.set_height, FRAME_WIDTH, video.center, video.set_fill,
                  None, 0)
        self.wait()
        self.play(Write(plane, run_time=2), morty.change, "raise_right_hand")
        self.wait()
        self.remove(morty)
        morty = morty.copy()
        self.add(morty)
        self.play(
            ReplacementTransform(morty.body,
                                 pi_group.get_part_by_tex("pi"),
                                 run_time=1),
            FadeOut(VGroup(morty.eyes, morty.mouth)),
            Write(VGroup(*pi_group[1:])))
        self.wait(2)
        self.play(plane.set_width, pi_group.get_width(), plane.next_to,
                  pi_group, DOWN, MED_LARGE_BUFF)

    def show_screen(self):
        screen = ScreenRectangle(height=4.3)
        screen.to_edge(LEFT)
        titles = VGroup(TextMobject("From zeta video"),
                        TextMobject("Coming up"))
        for title in titles:
            title.next_to(screen, UP)
            title.set_color(YELLOW)
        self.play(ShowCreation(screen), FadeIn(titles[0]))
        self.show_frame()
        self.wait(2)
...
class ShowSum(TeacherStudentsScene):
    ...
    def say_words(self):
        ...
        self.change_student_modes("hooray", "sassy", "angry")
        self.wait(2)
    ...
    def show_sum(self):
        ...
        sum_arrow = Arrow(lhs.get_part_by_tex("pi").get_bottom(), sum_point)
        fading_terms = [
            TexMobject(sign + "\\frac{1}{%d}" % (2 * n + 1))
            for n, sign in zip(list(range(self.num_terms_to_add)),
                               it.cycle("+-"))
        ]
        ...
        for term, arrow, last_arrow, fading_term, last_fading_term in zip(
            ...
            if term:
                ...
                run_time *= 0.8
        self.play(FadeOut(arrow), FadeOut(fading_term), dot.move_to, sum_point)
...
class ShowCalculus(PiCreatureScene):
    def construct(self):
        ...
        int1 = TexMobject("= \\int_0^1 (1 - x^2 + x^4 - \\dots )\\,dx")
        int2 = TexMobject("= \\int_0^1 \\frac{1}{1+x^2}\\,dx")
        ...
        rhs_group.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        rhs_group.shift(
        ...
        for rhs, mode in zip(rhs_group, modes):
            self.play(FadeIn(rhs), self.pi_creature.change, mode)
            self.wait()
        self.change_mode("maybe")
        self.wait()
        self.look_at(rhs_group[-1])
        self.wait()
        self.pi_creature_says("Where's the \\\\ circle?",
                              bubble_kwargs={
                                  "width": 4,
                                  "height": 3
                              },
...
class CertainRegularityInPrimes(LatticePointScene):
    ...
    def add_pi_formula(self):
        formula = TexMobject("\\frac{\\pi}{4}", "=", "1", "-", "\\frac{1}{3}",
                             "+", "\\frac{1}{5}", "-", "\\frac{1}{7}",
                             "+\\cdots")
    ...
    def walk_through_primes(self):
        primes = self.primes
        lines_and_labels = [
            self.get_radial_line_with_label(np.sqrt(p)) for p in primes
        ]
        lines, labels = list(zip(*lines_and_labels))
        circles = [self.get_circle(np.sqrt(p)) for p in primes]
        dots_list = [
            self.get_lattice_points_on_r_squared_circle(p) for p in primes
        ]
        groups = [
            VGroup(*mobs) for mobs in zip(lines, labels, circles, dots_list)
        ]
...
class Outline(PiCreatureScene):
    ...
    def generate_list(self):
        steps = VGroup(
            ...
            TexMobject("2. \\text{ Things like }17 = ", "4", "^2 + ", "1",
                       "^2"),
            TexMobject("3. \\text{ Things like }17 = (", "4", " + ", "i", ")(",
                       "4", " - ", "i", ")"),
        ...
        steps.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
    ...
    def wonder_at_pi(self):
        ...
        self.think("Who am I really?",
                   look_at_arg=question,
                   added_anims=[FadeIn(question)])
        self.wait(2)
        self.play(RemovePiCreatureBubble(self.pi_creature), question.to_corner,
                  UP + RIGHT)
    ...
    def count_lattice_points(self):
        ...
        lattice_points.sort(lambda p: get_norm(p - plane_center))
        ...
        self.play(*[
            ...
            for dot, a in zip(lattice_points,
                              np.linspace(0, 0.75, len(lattice_points)))
        ])
        self.play(FadeIn(step))
        self.wait()
        self.play(lattice_group.set_height, 2.5, lattice_group.next_to,
                  self.question, DOWN, lattice_group.to_edge, RIGHT)
    ...
    def show_chi(self):
        ...
        chis = VGroup(*[TexMobject("\\chi(%d)" % n) for n in input_range])
        ...
        self.play(*[
            FadeIn(mob, run_time=3, lag_ratio=0.5)
    ...
    def show_complicated_formula(self):
        ...
        self.play(MoveToTarget(pi), Write(rhs))
    ...
    def show_last_step(self):
        ...
        self.play(Write(expression), self.pi_creature.change_mode, "pondering")
        self.wait(3)
...
class CountLatticePoints(LatticePointScene):
    ...
    def introduce_lattice_point(self):
        ...
        h_line = Line(ORIGIN, self.plane.coords_to_point(x, 0), color=GREEN)
        v_line = Line(h_line.get_end(),
                      self.plane.coords_to_point(x, y),
                      color=RED)
        ...
        self.play(*[
            ApplyMethod(dot.set_fill,
                        None,
                        0,
                        run_time=3,
                        rate_func=squish_rate_func(
                            lambda t: 1 - there_and_back(t), a, a + 0.5),
                        remover=True)
    ...
    def draw_lattice_points_in_circle(self):
        ...
        radius_label = brace.get_text(str(self.max_lattice_point_radius),
                                      buff=SMALL_BUFF)
    ...
    def turn_points_int_units_of_area(self):
        ...
        squares = VGroup(
            *[square.copy().move_to(point) for point in self.lattice_points])
    ...
    def write_pi_R_squared(self):
        equations = VGroup(*[
            VGroup(
                TextMobject("\\# Lattice points\\\\",
                            "within radius ",
                            R,
                            alignment=""),
                TexMobject("\\approx \\pi", "(", R, ")^2")).arrange(RIGHT)
        ...
        self.play(FadeIn(background), Write(radius_10_eq))
        self.wait(2)
        self.play(ReplacementTransform(radius_10_eq.copy(), radius_million_eq))
        self.play(FadeIn(brace))
        self.wait(3)
    ...
    def allude_to_alternate_counting_method(self):
        alt_count = TextMobject("(...something else...)",
                                "$R^2$",
                                "=",
                                arg_separator="")
        ...
        self.play(FadeOut(self.million_group), Write(alt_count),
                  ReplacementTransform(self.radius_10_eq, self.radius_R_eq))
        self.wait(2)
        self.play(final_group.arrange, RIGHT, final_group.next_to, ORIGIN, UP)
...
class SoYouPlay(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("So you play!", run_time=2)
...
class CountThroughRings(LatticePointScene):
    ...
    def preview_rings(self):
        radii = list(set([np.sqrt(p.r_squared) for p in self.lattice_points]))
        radii.sort()
        circles = VGroup(*[self.get_circle(radius=r) for r in radii])
    ...
    def isolate_single_ring(self):
        x, y = self.example_coords
        example_circle = self.circles[self.radii.index(np.sqrt(x**2 + y**2))]
        self.circles.remove(example_circle)
        points_on_example_circle = self.get_lattice_points_on_r_squared_circle(
            x**2 + y**2).copy()

        self.play(FadeOut(self.circles), self.lattice_points.set_fill, GREY,
                  0.5, Animation(points_on_example_circle))
        self.wait()
    ...
    def show_specific_lattice_point_distance(self):
        x, y = self.example_coords
        dot = Dot(self.plane.coords_to_point(x, y),
                  color=self.dot_color,
                  radius=self.dot_radius)
        ...
        for mob in distance, distance_num:
            ...
            mob.rotate(radial_line.get_angle(), about_point=mob.get_bottom())
        ...
        a_num, b_num = [
            TexMobject(str(coord))[0] for coord in self.example_coords
        ]
        a_num.move_to(a, UP)
        ...
        self.play(*list(
            map(FadeOut, [
                self.example_circle, self.points_on_example_circle, distance,
                a, b, radial_line, h_line, v_line, label
            ])))
    ...
    def count_through_rings(self):
        ...
        left_list = VGroup(*[
            TexMobject("\\sqrt{%d} \\Rightarrow" % n, str(count)) for n, count
            in zip(list(range(self.num_rings_to_show_explicitly)), counts)
        ])
        left_counts = VGroup()
        left_roots = VGroup()
        ...
        top_list = VGroup(*[TexMobject("%d, " % count) for count in counts])
        ...
        for r_squared, count_mob, root in zip(it.count(), left_counts,
                                              left_roots):
            self.show_ring_count(r_squared,
                                 count_mob,
                                 added_anims=[FadeIn(root)])
            self.wait(2)
        self.play(
            FadeOut(left_roots), FadeIn(top_rect), *[
                ReplacementTransform(lc, VGroup(tc), path_arc=np.pi / 2)
                for lc, tc in zip(left_counts, top_list)
            ])
        for r_squared in range(len(left_counts),
                               self.max_lattice_point_radius**2 + 1):
    ...
    def show_ring_count(self,
        ...
        root.set_width(min(0.7 * radial_line.get_width(), root.get_width()))
        ...
        if radius_squared == 0:
            ...
            self.play(DrawBorderThenFill(points, stroke_color=PINK),
                      *added_anims,
                      run_time=run_time)
            self.play(ReplacementTransform(points.copy(), target))
            return
        points.set_fill(YELLOW, 1)
        self.play(Transform(self.little_circle, circle),
...
class LookAtExampleRing(LatticePointScene):
    ...
    def analyze_25(self):
        ...
        labels = [
            TexMobject("(", str(x), ",", str(y), ")") for x, y in coords_list
        ]
        for label in labels:
            label.x = label[1]
            label.y = label[3]
        ...
        sums_of_squares = [
            TexMobject(special_str(x), "^2", "+", special_str(y), "^2", "= 25")
        ...
        self.play(ShowCreation(radius), Write(root_label))
        self.play(
            ShowCreation(circle),
            Rotating(
                radius,
        ...
        self.play(Write(curr_label), curr_label.point.set_color, PINK)
        x, y = curr_label.x.copy(), curr_label.y.copy()
        self.play(
            Transform(x, curr_sum_of_squares.x),
            Transform(y, curr_sum_of_squares.y),
        )
        self.play(Write(curr_sum_of_squares), Animation(VGroup(x, y)))
        self.remove(x, y)
        self.wait()

        for label, sum_of_squares in zip(labels, sums_of_squares)[1:]:
            self.play(ReplacementTransform(curr_label,
                                           label), label.point.set_color, PINK,
                      curr_label.point.set_color, self.dot_color)
            curr_label = label
            self.play(ReplacementTransform(curr_sum_of_squares,
                                           sum_of_squares))
        ...
        for i, point in enumerate(points.target):
            point.move_to(self.plane.coords_to_point(i % 3, i // 3))
        ...
        self.play(*list(
            map(FadeOut, [
                curr_label, curr_sum_of_squares, circle, points, radius,
                root_label
            ])))
    ...
    def analyze_11(self):
        ...
        self.play(Write(root_label), ShowCreation(radius), run_time=1)
        self.play(
            ShowCreation(circle),
            Rotating(
                radius,
    ...
    def construct(self):
        ...
        self.play(
            ...
            RemovePiCreatureBubble(self.teacher,
                                   target_mode="raise_right_hand"))
        self.change_student_modes(*["thinking"] * 3, look_at_arg=plane)
        self.wait(3)
...
class IntroduceComplexConjugate(LatticePointScene):
    ...
    def write_points_with_complex_coords(self):
        ...
        ticks = VGroup(*[
            Line(ORIGIN, MED_SMALL_BUFF *
                 RIGHT).move_to(self.plane.coords_to_point(0, y))
            for y in y_range
        ])
        imag_coords = VGroup()
        ...
        self.add(dot)
        self.play(ShowCreation(h_arrow), Write(x_coord))
        self.play(ShowCreation(v_arrow), Write(y_coord))
    ...
    def introduce_complex_conjugate(self):
        ...
        dot = Dot(self.plane.coords_to_point(x, -y), color=self.dot_color)
        ...
        self.play(FadeIn(equation, run_time=3, lag_ratio=0.5))
        self.wait(2)
        self.play(GrowFromCenter(brace), Write(conjugate_words, run_time=2))
        self.wait()
        self.play(*[
            ReplacementTransform(m1.copy(), m2) for m1, m2 in [
                (self.example_dot, dot),
    ...
    def expand_algebraically(self):
        x, y = self.example_coords
        expansion = VGroup(TexMobject(str(x), "^2"),
                           TexMobject("-", "(", str(y), "i", ")^2"))
        expansion.arrange(RIGHT, buff=SMALL_BUFF)
        expansion.next_to(VGroup(*self.equation[-2:]), DOWN, LARGE_BUFF)
        ...
        self.wait()
        self.play(*list(
            map(FadeOut, [expansion[0].rect, expansion[1].rect, expansion])))
    ...
    def show_geometrically(self):
        ...
        for dot in dots:
            dot.line = Line(self.plane_center, dot.get_center(), color=BLUE)
            dot.angle = dot.line.get_angle()
            dot.arc = Arc(dot.angle, radius=0.75, color=YELLOW)
            ...
            dot.magnitude_word.next_to(ORIGIN,
        ...
        twenty_five_label.next_to(self.plane.coords_to_point(25, 0), DOWN)

        self.play(ShowCreation(top_dot.line))
        mover = VGroup(top_dot.line.copy().set_color(PINK), top_dot.copy())
        self.play(FadeIn(top_dot.magnitude_word, lag_ratio=0.5))
        self.wait()
        self.play(ShowCreation(top_dot.arc))
        self.wait(2)
        self.play(ShowCreation(low_dot.line))
        self.play(ReplacementTransform(top_dot.arc, low_dot.arc),
                  FadeIn(low_dot.rotate_word))
        self.play(Rotate(mover, low_dot.angle, about_point=self.plane_center),
                  run_time=2)
        self.play(
            FadeOut(low_dot.arc),
        ...
        self.play(mover[0].scale_about_point,
                  5,
                  self.plane_center,
                  mover[1].move_to,
...
class NameGaussianIntegers(LatticePointScene):
    ...
    def add_a_plus_bi(self):
        ...
        arrows = VGroup(*[
            Arrow(integers.get_top(), mob, tip_length=0.15) for mob in (a, b)
        ])
    ...
    def add_name(self):
        gauss_name = TextMobject("Carl Friedrich Gauss")
        ...
        self.play(FadeIn(gauss_name, run_time=2, lag_ratio=0.5))
        self.wait(3)
    ...
    def restrict_to_one_circle(self):
        ...
        self.play(
            FadeOut(self.lattice_points), ShowCreation(circle),
            Rotating(radius,
                     run_time=1,
                     rate_func=smooth,
                     about_point=self.plane_center),
    ...
    def show_question_algebraically(self):
        for i, dot in enumerate(self.circle_dots):
            ...
            label.next_to(
                dot,
                dot.get_center() - self.plane_center + SMALL_BUFF *
                (UP + RIGHT),
                buff=0,
            )
            label.add_background_rectangle()
        ...
        for dot in self.circle_dots:
            ...
            if last_dot is None:
                ...
                anims += [
                    last_dot.set_color,
                    self.dot_color,
                    last_dot.conjugate_dot.set_color,
                    self.dot_color,
...
class FactorOrdinaryNumber(TeacherStudentsScene):
    def construct(self):
        equation = TexMobject("2{,}250", "=", "2 \\cdot 3^2 \\cdot 5^3")
        equation.next_to(self.get_pi_creatures(), UP, LARGE_BUFF)
        number = equation[0]
        alt_rhs_list = list(
            it.starmap(TexMobject, [
                ("\\ne", "2^2 \\cdot 563"),
        ...
        self.play(self.teacher.change_mode, "raise_right_hand", Write(number))
        self.wait(2)
        self.play(number.restore, Write(VGroup(*equation[1:])),
                  Write(title[1]))
...
class IntroduceGaussianPrimes(LatticePointScene, PiCreatureScene):
    ...
    def construct(self):
        ...
        dots = [
            Dot(self.plane.coords_to_point(*coords)) for coords in [
                (5, 0),
                (2, 1),
                (2, -1),
                (-1, 2),
        ...
        arc_angle = 0.8 * np.pi
        times_i_arc = Arrow(p1_dot.get_top(),
                            p3_dot.get_top(),
                            path_arc=arc_angle)
        times_neg_i_arc = Arrow(p2_dot.get_bottom(),
                                p4_dot.get_bottom(),
                                path_arc=-arc_angle)
        times_i = TexMobject("\\times i")
        times_i.add_background_rectangle()
        times_i.next_to(times_i_arc.point_from_proportion(0.5), UP)
        times_neg_i = TexMobject("\\times (-i)")
        times_neg_i.add_background_rectangle()
        times_neg_i.next_to(times_neg_i_arc.point_from_proportion(0.5), DOWN)
        VGroup(times_i, times_neg_i, times_i_arc,
               times_neg_i_arc).set_color(MAROON_B)
        ...
        factorization = TexMobject("5", "= (2+i)(2-i)")
        ...
        for alt_factorization in neg_alt_factorization, i_alt_factorization:
            alt_factorization.next_to(factorization.get_part_by_tex("="),
                                      DOWN,
                                      aligned_edge=LEFT)
        ...
        for dot in dots:
            dot.add(
                Line(self.plane_center,
                     dot.get_center(),
                     color=dot.get_color()))

        self.add(factorization)
        self.play(DrawBorderThenFill(five_dot), FadeIn(five_label))
        self.wait()
        self.play(
            ReplacementTransform(
                VGroup(five_dot).copy(), VGroup(p1_dot, p2_dot)))
        ...
        self.wait()
        self.play(FadeOut(neg_alt_factorization), *[m.restore for m in movers])
        self.wait()

        ##Show i_alternate expression
        self.play(
            ShowCreation(times_i_arc), FadeIn(times_i), *[
                ReplacementTransform(mob1.copy(), mob2, path_arc=np.pi / 2)
                for mob1, mob2 in [
                    (p1_dot, p3_dot),
                    (p1_label, p3_label),
                ]
        ...
        self.wait()
        self.play(
            ShowCreation(times_neg_i_arc), FadeIn(times_neg_i), *[
                ReplacementTransform(mob1.copy(), mob2, path_arc=-np.pi / 2)
                for mob1, mob2 in [
                    (p2_dot, p4_dot),
                    (p2_label, p4_label),
...
class FromIntegerFactorsToGaussianFactors(TeacherStudentsScene):
    def construct(self):
        expression = TexMobject("30", "=", "2", "\\cdot", "3", "\\cdot", "5")
        ...
        for mob, vect in (two, DOWN), (five, UP):
            ...
            mob.arrows = VGroup(*[
                Arrow(mob.get_edge_center(vect),
                      factor.get_edge_center(-vect),
                      color=mob.get_color(),
                      tip_length=0.15) for factor in mob.factors
            ])

        self.add(expression)
        for mob in two, five:
            self.play(ReplacementTransform(mob.copy(), mob.factors),
        ...
        self.teacher_says("Now for a \\\\ surprising fact...",
...
class FactorizationPattern(Scene):
    ...
    def show_one_mod_four_primes(self):
        ...
        dots = VGroup(
            *
            [Dot(self.number_line.number_to_point(prime)) for prime in primes])
        ...
        self.play(FadeIn(factorizations, run_time=2, lag_ratio=0.5))
        self.wait(4)
    ...
    def show_three_mod_four_primes(self):
        ...
        word_arrows = VGroup(*[
            Line(words.get_bottom(),
                 p_mob.get_top(),
                 color=p_mob.get_color(),
                 buff=MED_SMALL_BUFF) for p_mob in prime_mobs
        ])
        ...
        self.wait()
        self.play(Write(words), *list(map(ShowCreation, word_arrows)))
        self.wait(4)
    ...
    def ask_why_this_is_true(self):
        ...
        self.play(*list(
            map(FadeOut,
                [randy, randy.bubble, randy.bubble.content, links_text])))
    ...
    def show_two(self):
        ...
        time_i_arrow = Arrow(factors[1].get_bottom(),
        ...
        self.play(Write(two), ShowCreation(arrow), DrawBorderThenFill(two_dot))
        self.wait()
        self.play(MoveToTarget(mover), Write(factorization))
...
class FactorTwo(LatticePointScene):
    ...
    def construct(self):
        ...
        factor_dots = VGroup(
            *[Dot(self.plane.coords_to_point(1, u)) for u in (1, -1)])
        ...
        factor_labels = VGroup(*[
            ...
            for tex, dot, vect in zip(["1+i", "1-i"], factor_dots, [UP, DOWN])
        ])
        ...
        self.play(*[
            ReplacementTransform(VGroup(mob1.copy()), mob2) for mob1, mob2 in [
...
class IntroduceRecipe(Scene):
    ...
    def add_title(self):
        title = TexMobject("\\text{Recipe for }", "a", "+", "b", "i",
                           "\\text{ satisfying }", "(", "a", "+", "b", "i",
                           ")", "(", "a", "-", "b", "i", ")", "=",
                           self.N_string)
    ...
    def show_ordinary_factorization(self):
        ...
        factorization = TexMobject(*it.chain(
            *list(zip(symbols, list(map(str, factors))))))
        factorization.next_to(N_mob.target, RIGHT)

        self.play(MoveToTarget(N_mob, run_time=2, path_arc=-np.pi / 6))
    ...
    def subfactor_ordinary_factorization(self):
        ...
        factorization.next_to(self.integer_factorization,
                              DOWN,
                              aligned_edge=LEFT)
    ...
    def take_product_of_columns(self):
        ...
        for arrow, product, group in zip(arrows, products, factor_groups):
            ...
            self.play(ReplacementTransform(group.copy(), VGroup(product)))
            self.wait()
        self.wait(3)
    ...
    def get_T_chart(self):
        ...
        h_lines = VGroup(
            *
            [Line(ORIGIN, self.T_chart_width * RIGHT / 2.0) for x in range(2)])
    ...
    def get_product_mobjects(self):
        ...
        for factors, line in zip(factor_groups, self.multiplication_lines):
            product = reduce(op.mul,
                             [factor.underlying_number for factor in factors])
            color = average_color(*[factor.get_color() for factor in factors])
            product_mob = TexMobject(self.complex_number_to_tex(product))
    ...
    def write_last_step(self):
        ...
        final_step = TextMobject("Multiply by $1$, $i$, $-1$ or $-i$")
...
class StateThreeChoices(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("$5^2$ gives 3 choices.")
        self.wait(3)
...
class ThreeOutputsAsLatticePoints(LatticePointScene):
    ...
    def add_dots_and_labels(self):
        ...
        for dot, label in zip(dots, labels):
            self.play(
                FadeIn(label),
                DrawBorderThenFill(dot, stroke_color=PINK, stroke_width=4))
        self.wait(2)
...
class ThreeOutputsAsLatticePointsContinued(ThreeOutputsAsLatticePoints):
    ...
    def show_multiplication_by_units(self):
        ...
        for dot in original_dots:
            ...
            dot.add(line)
        words_group = VGroup(*[
            TextMobject("Multiply by $%s$" % s) for s in ("1", "i", "-1", "-i")
        ])
        ...
        self.play(FadeIn(words_group[0]), *list(map(ShowCreation, lines)))
        for words, angle, color in zip(words_group[1:], angles,
                                       self.colors[1:]):
            self.play(FadeIn(words))
            dots_copy = original_dots.copy()
            self.play(dots_copy.rotate,
                      angle,
                      dots_copy.set_color,
                      color,
                      path_arc=angle)
...
class StateFourChoices(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("$5^3$ gives 4 choices.")
        self.wait(3)
...
class Show125Circle(ThreeOutputsAsLatticePointsContinued):
    ...
    def draw_circle(self):
        ...
        self.play(Write(root_label), ShowCreation(radial_line))
        self.add_foreground_mobject(root_label)
        self.play(
            Rotating(radial_line,
                     rate_func=smooth,
                     about_point=self.plane_center),
            ShowCreation(circle),
            run_time=2,
        )
        group = VGroup(self.plane, radial_line, circle, root_label)
...
class RecipeFor375(IntroduceRecipe):
    ...
    def express_trouble_with_three(self):
        ...
        self.play(FadeIn(morty))
        self.play(MoveToTarget(three), morty.change, "angry", three.target)
        self.play(Blink(morty))
        self.wait()
        for factors in self.left_factors, self.right_factors:
            self.play(three.next_to, factors, DOWN, morty.change, "sassy",
                      factors.get_bottom())
            self.wait()
...
class Show375Circle(LatticePointScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(radial_line), Write(root_label, run_time=1))
        self.add_foreground_mobject(root_label)
        self.play(
            Rotating(radial_line,
                     rate_func=smooth,
        ...
        group = VGroup(self.plane, radial_line, root_label, circle)
        self.wait(2)
...
class RecipeFor1125(IntroduceRecipe):
    ...
    def write_last_step(self):
        words = TextMobject("Multiply by \\\\ ", "$1$, $i$, $-1$ or $-i$")
        words.scale(0.7)
...
class SummarizeCountingRule(Show125Circle):
    ...
    def add_count_words(self):
        words = TextMobject("\\# Lattice points \\\\ on $\\sqrt{%s}$ circle" %
                            self.N_str)
        words.to_corner(UP + LEFT)
        words.add_background_rectangle()
        self.add(words)
    ...
    def draw_circle(self):
        ...
        dots = VGroup(*[
            Dot(self.plane.coords_to_point(*coords),
                radius=self.dot_radius,
                color=self.dot_color) for coords in self.coords_list
        ])
        ...
        self.play(
            LaggedStartMap(DrawBorderThenFill,
                           dots,
                           stroke_width=4,
                           stroke_color=PINK))
        self.wait(2)
    ...
    def add_full_screen_rect(self):
        rect = FullScreenFadeRectangle(fill_opacity=self.rect_opacity)
        self.play(FadeIn(rect), Animation(self.count_words))

    def talk_through_rules(self):
        factorization = TexMobject("N =", "3", "^4", "\\cdot", "5", "^3",
                                   "\\cdot", "13", "^2")
        ...
        five_choices = five_brace.get_tex("(", "3", "+", "1", ")")
        thirteen_choices = thirteen_brace.get_tex("(", "2", "+", "1", ")")
        ...
        self.wait()
        for choices, power in (five_choices, five_power), (thirteen_choices,
                                                           thirteen_power):
            self.play(Write(VGroup(choices[0], *choices[2:])),
                      ReplacementTransform(power.copy(), choices[1]))
        self.wait()
        self.play(three.set_color, RED, FadeIn(three_brace))
        self.wait()
        self.play(Write(VGroup(three_choices[0], three_choices[2])),
                  ReplacementTransform(three_power.copy(), three_choices[1]))
        self.wait()

        movers = three_power, three_choices
        for mob in movers:
            mob.save_state()
        self.play(Transform(three_power, alt_three_power, path_arc=np.pi),
                  Transform(three_choices, alt_three_choices))
        self.wait()
        self.play(*[mob.restore for mob in movers], path_arc=-np.pi)
        self.wait()

        equals_four = TexMobject("=", "4")
        four = equals_four.get_part_by_tex("4")
        four.set_color(YELLOW)
        final_choice_words = TextMobject("Mutiply",
                                         "by $1$, $i$, $-1$ or $-i$")
        final_choice_words.set_color(YELLOW)
        final_choice_words.next_to(four, DOWN, LARGE_BUFF, LEFT)
        final_choice_words.to_edge(RIGHT)
        final_choice_arrow = Arrow(final_choice_words[0].get_top(),
                                   four.get_bottom(),
                                   buff=SMALL_BUFF)
        ...
        self.play(self.count_words.next_to, equals_four, LEFT,
                  MoveToTarget(choices_copy), FadeIn(equals_four))
    ...
    def ask_about_two(self):
        ...
        self.play(FadeIn(randy))
        self.play(
            PiCreatureBubbleIntroduction(randy,
                                         "What about \\\\ factors of 2?",
                                         bubble_class=ThoughtBubble,
                                         bubble_kwargs={
                                             "height": 3,
                                             "width": 3
...
class RecipeFor10(IntroduceRecipe):
    ...
    def swap_two_factors(self):
        ...
        self.play(ShowCreation(arrow), Write(times_i, run_time=1))
        self.wait()
        self.play(curr_product.to_edge, LEFT)
        self.swap_factors_at_index(0)
        new_arrow = Arrow(self.result_surrounding_rect,
                          curr_product,
                          buff=SMALL_BUFF)
...
class FactorsOfTwoNeitherHelpNorHurt(TeacherStudentsScene):
    def construct(self):
        words = TextMobject("Factors of", "$2^k$",
...
class EffectOfPowersOfTwo(LatticePointScene):
    ...
    def construct(self):
        ...
        radial_lines, root_labels = list(
            zip(*list(map(self.get_radial_line_with_label, radii))))
        dots_list = list(
            map(self.get_lattice_points_on_r_squared_circle,
                self.square_radii))
        groups = [
            VGroup(*mobs)
...
class IntroduceChi(FactorizationPattern):
    CONFIG = {
        "numbers_list":
        [list(range(i, 36, d)) for i, d in [(1, 4), (3, 4), (2, 2)]],
    ...
    def add_define_chi_label(self):
        ...
        chi_expressions = VGroup(*[
            ...
            for numbers, color in zip(self.numbers_list, self.colors)
        ])
        chi_expressions.scale(0.9)
        chi_expressions.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        chi_expressions.to_corner(UP + RIGHT)
        brace = Brace(chi_expressions, LEFT)
        label.next_to(brace, LEFT)

        self.play(Write(label), GrowFromCenter(brace))
    ...
    def describe_values(self, index):
        ...
        dots, arrows, labels = self.get_dots_arrows_and_labels(numbers, color)
        ...
        self.play(
            Write(
                VGroup(*[
                    part for part in chi_expression
                    if part not in chi_expression.inputs
                ])), *[
                    ReplacementTransform(label.copy(), num_mob)
                    for label, num_mob in zip(labels, chi_expression.inputs)
                ])
        self.wait()
    ...
    def show_multiplicative(self):
        ...
        expressions = VGroup()
        for x, y in pairs:
            expression = TexMobject("\\chi(%d)" % x, "\\cdot", "\\chi(%d)" % y,
                                    "=", "\\chi(%d)" % (x * y))
            braces = [Brace(expression[i], UP) for i in (0, 2, 4)]
        ...
        self.play(FadeIn(expression), self.morty.change, "pondering",
                  expression)
        self.wait(2)
    ...
    def get_chi_expression(self, numbers, color, num_terms=4):
        ...
        result = TexMobject(*it.chain(*[["\\chi(", str(n), ")", "="]
...
class WriteCountingRuleWithChi(SummarizeCountingRule):
    CONFIG = {"colors": [GREEN, RED, YELLOW]}
    ...
    def add_factorization_and_rule(self):
        ...
        count.next_to(factorization.get_part_by_tex("="),
                      DOWN,
                      buff=LARGE_BUFF,
                      aligned_edge=LEFT)

        self.play(FadeIn(factorization), self.count_words.next_to, count, LEFT)
        self.wait()
        self.play(*[
            ReplacementTransform(
                VGroup(factorization.get_part_by_tex(
                    tex, substring=False)).copy(), part)
    ...
    def write_chi_expression(self):
        ...
        expression.arrange(DOWN, buff=MED_SMALL_BUFF, aligned_edge=LEFT)
        ...
        self.play(*[
            ApplyMethod(
                c_part.move_to, e_part, LEFT, path_arc=-np.pi / 2, run_time=2)
            for c_part, e_part in zip(count_copy, expression)
        ])
        self.wait()
        self.play(ReplacementTransform(count_copy, expression, run_time=2))
        self.wait()
    ...
    def walk_through_expression_terms(self):
        ...
        groups = [
            ...
            for index, tex1, tex2 in [(-1, "5", "^3"), (-2, "3",
                                                        "^4"), (-3, "2", "^2")]
        ]
        evaluation_strings = [
            "(1+1+1+1)",
        ...
        for group, tex in zip(groups, evaluation_strings):
            ...
            if base.get_tex_string() is "3":
                ...
                self.play(
                    ...
                    Transform(VGroup(*evaluation[-3:-1]),
                              evaluation_point), evaluation[-1].next_to,
                    evaluation_point, RIGHT, SMALL_BUFF,
                    Transform(VGroup(*chi_sum[-6:-1]), chi_sum_point),
            ...
            self.play(
                FadeOut(VGroup(rect, brace, evaluation_rect, evaluation)),
                Animation(group))
...
class ExpandCountWith45(SummarizeCountingRule):
    ...
    def add_factorization_and_count(self):
        ...
        braces = VGroup(*[Brace(part, UP) for part in expression[1:]])
        for brace, num, color in zip(braces, [1, 2], [RED, GREEN]):
            num_mob = brace.get_tex(str(num), buff=SMALL_BUFF)
            num_mob.set_color(color)
            brace.add(num_mob)

        self.play(FadeIn(factorization), self.count_words.next_to, expression,
                  LEFT)
        self.wait()
        self.play(*[
            ReplacementTransform(
    ...
    def expand_expression(self):
        equals, four, lp, rp = list(
            map(TexMobject, ["=", "4", "\\big(", "\\big)"]))
        expansion = VGroup(equals, four, lp)
        chi_pairs = list(
            it.product(*[factor[1::2] for factor in self.chi_expression[1:]]))
        ...
        self.play(
            FadeIn(rect), *[
                ReplacementTransform(self.chi_expression[i][j].copy(), mob)
                for i, j, mob in [
                    (0, 0, equals),
                    (0, 1, four),
                    (1, 0, lp),
    ...
    def show_divisor_sum(self):
        ...
        self.play(FadeIn(rect), Write(divisor_sum, run_time=2))
        self.play(
            LaggedStartMap(
                MoveToTarget,
                prime_pairs,
...
class CountLatticePointsInBigCircle(LatticePointScene):
    ...
    def introduce_points(self):
        ...
        self.wait()
        self.play(
            FadeOut(R_rect), FadeIn(pi_R_squared_rect),
    ...
    def show_rings(self):
        ...
        rings = VGroup(*[self.get_circle(radius=np.sqrt(N)) for N in N_range])
        rings.set_color_by_gradient(TEAL, GREEN)
        rings.set_stroke(width=2)
        dot_groups = VGroup(
            *[self.get_lattice_points_on_r_squared_circle(N) for N in N_range])
        radicals = self.get_radicals()

        self.play(LaggedStartMap(FadeIn, rings),
                  Animation(self.lattice_points),
    ...
    def ignore_center_dot(self):
        ...
        self.wait()
        self.play(*[
            ApplyMethod(m.scale, 0.5) for m in [
                self.plane, self.circle, self.radius, self.rings,
                self.lattice_points
            ]
        ])
        new_dots.scale(0.5)
        ...
        self.play(
            ApplyMethod(
                VGroup(self.circle, self.radius).scale_in_place,
                2,
                rate_func=linear,
            ),
            LaggedStartMap(
    ...
    def get_radicals():
        radicals = VGroup(
            *[TexMobject("\\sqrt{%d}" % N) for N in range(1, 13)])
        radicals.add(TexMobject("\\vdots"), TexMobject("\\sqrt{R^2}"))
...
class AddUpGrid(Scene):
    ...
    def add_row_lines(self):
        ...
        row_lines = VGroup(*[
            h_line.copy().next_to(
                radical, DOWN, buff=SMALL_BUFF, aligned_edge=LEFT)
    ...
    def add_chi_sums(self):
        ...
        for N, radical in zip(list(range(1, 13)), self.radicals):
            arrow, four, lp, rp = list(
                map(TexMobject, ["\\Rightarrow", "4", "\\big(", "\\big)"]))
            fours.add(four)
            parens.add(lp, rp)
            arrows.add(arrow)
    ...
    def talk_through_rows(self):
        ...
        self.play(FadeIn(composite_rects), randy.look_at,
                  composite_rects.get_bottom())
        self.wait(2)
        self.play(
    ...
    def organize_into_columns(self):
        ...
        for plus in self.plusses:
            ...
            plus.target.next_to(plus.chi_mob.target, RIGHT, SMALL_BUFF)
    ...
    def collapse_columns(self):
        ...
        for d in range(1, 7):
            ...
            term = VGroup(TexMobject(*R_args),
                          TexMobject("\\chi(", str(d), ")"), TexMobject("+"))
        ...
        for column, term in zip(chi_mob_columns, full_sum):
            ...
            self.wait()
            self.play(
                ReplacementTransform(column.copy(),
                                     VGroup(term[1]),
                                     run_time=2),
                Write(term[0]),
                Write(term[2]),
            ...
            if term is full_sum[2]:
                ...
                self.play(*[
                    ApplyMethod(m.shift, vect) for m in [
                        self.count_rect,
                        self.count_words,
                    ] + list(full_sum[:3])
                ])
    ...
    def factor_out_R(self):
        ...
        new_sum = VGroup(self.corner_four.target, R_squared, lp)

        R_fracs, chi_terms, plusses = full_sum_parts = [
            VGroup(*[term[i] for term in self.full_sum]) for i in range(3)
        ]
        targets = []
        ...
        self.add(R_squared)
        self.wait()
        self.play(MoveToTarget(self.corner_four, run_time=2),
    ...
    def show_chi_sum_values(self):
        ...
        alt_rhs.scale(0.9)
        alt_rhs.next_to(self.count_words[-1],
                        DOWN,
                        buff=LARGE_BUFF,
                        aligned_edge=LEFT)
        ...
        self.play(*list(
            map(FadeOut, [
                self.chi_mobs, self.plusses, self.arrows, self.radicals,
                self.row_lines
            ])) + [
    ...
    def compare_to_pi_R_squared(self):
        approx, pi, R_squared = area_rhs = TexMobject("\\approx", "\\pi",
                                                      "R^2")
        area_rhs.next_to(self.alt_rhs, RIGHT)

        brace = Brace(VGroup(self.alt_rhs, area_rhs), DOWN)
        brace.add(brace.get_text("Arbitrarily good as $R \\to \\infty$"))

        pi_sum = TexMobject("4", self.alt_rhs[-1].get_tex_string(), "=",
                            "\\pi")
        pi_sum.scale(0.9)
        pi = pi_sum.get_part_by_tex("pi")
        pi.scale(2, about_point=pi.get_left())
        pi.set_color(YELLOW)
        pi_sum.shift(self.alt_rhs[-1].get_bottom(), MED_SMALL_BUFF * DOWN,
                     -pi_sum[1].get_top())
    ...
    def celebrate(self):
        ...
        self.play(*[
            ApplyMethod(pi.change, "hooray", self.alt_rhs) for pi in creatures
        ])
        self.wait()
        for i in 0, 2, 3:
...
class IntersectionOfTwoFields(TeacherStudentsScene):
    def construct(self):
        circles = VGroup()
        for vect, color, adj in (LEFT, BLUE, "Algebraic"), (RIGHT, YELLOW,
                                                            "Analytic"):
            ...
            circle.title = title
            circles.add(circle)
        new_number_systems = TextMobject("New \\\\ number systems")
        gaussian_integers = TextMobject("e.g. Gaussian \\\\ integers")
        ...
        for circle in circles:
            self.play(Write(circle.title, run_time=2),
        ...
        for circle in circles:
            for word in circle.words:
                self.play(
                    Write(word, run_time=2), self.teacher.change, "speaking",
                    *[
                        ApplyMethod(pi.change, "pondering")
                        for pi in self.get_students()
                    ])
                self.wait()
        self.play(Write(mid_words), self.teacher.change, "raise_right_hand")
        self.change_student_modes(*["thinking"] * 3, look_at_arg=mid_words)
        self.wait(3)
...
class Sponsorship(PiCreatureScene):
    def construct(self):
        morty = self.pi_creature
        logo = SVGMobject(file_name="remix_logo", )
        logo.set_height(1)
        logo.center()
        ...
        self.wait()
        self.play(ShowCreation(rect), logo.scale, 0.8, logo.to_corner,
                  UP + RIGHT, morty.change, "happy")
        self.wait()
        self.play(Write(url))
        self.wait(3)
