...
class TauPoem(Scene):
    ...
    def add_line_and_number(self):
        ...
        first_word, rest_of_line = TextMobject(
            [line_str[:index],
             line_str[index:]]).to_edge(UP).shift(BUFF * DOWN).split()
    ...
    def first_word_to_last_digit(self):
        if self.line_num == 19:
            ...
            self.play(
                ApplyMethod(self.first_digits.shift, shift_val, run_time=2.0))
            self.wait(2)
        self.play_over_time_range(
            0, 2,
            Transform(deepcopy(self.first_word),
                      self.number_word,
                      rate_func=squish_rate_func(smooth)))
        self.play_over_time_range(
            2, 4,
            Transform(self.number_word,
                      self.new_digit,
                      rate_func=squish_rate_func(smooth)))
    ...
    def line1(self):
        ...
        self.wait()
        self.play(
            CounterclockwiseTransform(two_pi,
                                      sphere,
                                      rate_func=lambda t: 2 * smooth(t / 2)))
        self.remove(two_pi)
        self.play(
            CounterclockwiseTransform(sphere,
                                      tau,
                                      rate_func=lambda t: 2 *
                                      (smooth(t / 2 + 0.5) - 0.5)))
        self.remove(sphere)
        self.add(tau)
        self.wait()
    ...
    def line2(self):
        tau = TauCreature()
        tau.make_sad()
        tau.mouth.points = np.array(
            sorted(tau.mouth.points, lambda p0, p1: cmp(p0[0], p1[0])))
    ...
    def line3(self):
        ...
        self.play(*[
            Transform(*parts) for parts in zip(tau.white_parts, pi.white_parts)
        ])
    ...
    def line11(self):
        ...
        self.play(
            *
            [Transform(*mobs, run_time=2.0) for mobs in zip(formulae, f_copy)])
    ...
    def line12(self):
        ...
        self.add(axes, grid)
        self.play(
            TransformAnimations(ShowCreation(sine),
                                ShowCreation(
                                    deepcopy(sine).shift(
                                        2 * np.pi * interval_size * RIGHT)),
                                run_time=2.0,
                                rate_func=smooth), ShowCreation(circle))
        self.play(
    ...
    def line13(self):
        formula = form_start, two_pi, form_end = TexMobject(
            ["\\hat{f^{(n)}}(\\xi) = (", "2\\pi",
             "i\\xi)^n \\hat{f}(\\xi)"]).shift(DOWN).split()
    ...
    def line16(self):
        self.add(TexMobject("\\frac{1}{2}\\tau r^2").scale(2).shift(DOWN))
    ...
    def line18(self):
        ...
        self.wait(0.8)
        self.play(*[
            Transform(*eyes, run_time=0.2, rate_func=rush_into) for eyes in [
        ...
        self.play(*[
            Transform(*eyes, run_time=0.2, rate_func=rush_from) for eyes in [
        ...
        self.play(Transform(tau.arm, mad_tau.arm),
