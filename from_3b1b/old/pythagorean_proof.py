...
class AddParallelLines(DrawAllThreeSquares):
    ...
    def construct(self, num, trim):
        ...
        shift_pairs = [(4 * RIGHT, 3 * UP), (ORIGIN, DOWN),
                       (3 * LEFT, 2 * DOWN)]
...
class DrawAllThreeSquaresWithMoreTriangles(DrawAllThreeSquares):
    args_list = [(1, True), (2, True), (3, True), (4, True), (5, True),
...
class ZoomInOnTroublePoint(Scene):
    ...
    def construct(self, with_labels, rotate):
        ...
        angle1_arc.filter_out(lambda x_y_z2: not (x_y_z2[0] > 0 and x_y_z2[
            1] > 0 and x_y_z2[1] < x_y_z2[0] / 3))
        angle2_arc.filter_out(lambda x_y_z3: not (x_y_z3[0] < 0 and x_y_z3[
            1] > 0 and x_y_z3[1] < -3 * x_y_z3[0]))
...
class DrawTriangleWithAngles(Scene):
    def construct(self):
        ...
        angle1_arc = Circle(
            radius=0.4,
            **kwargs).filter_out(lambda x_y_z: not (x_y_z[0] > 0 and x_y_z[
                1] < 0 and x_y_z[1] < -3 * x_y_z[0])).shift(vertices[1])
        angle2_arc = Circle(
            radius=0.2,
            **kwargs).filter_out(lambda x_y_z1: not (x_y_z1[0] < 0 and x_y_z1[
                1] > 0 and x_y_z1[1] < -3 * x_y_z1[0])).shift(vertices[2])
