...
class SideGigToFullTime(Scene):
    ...
    def income(self, morty):
        dollar_signs = VGroup(*[TexMobject("\\$") for x in range(10)])
        ...
        self.play(morty.change_mode, "happy", morty.look_at, dollar_signs,
                  dollar_signs.shift, LEFT, dollar_signs.set_color, GREEN)
        for x in range(5):
        ...
        self.play(
            ApplyMethod(dollar_signs.shift, (FRAME_Y_RADIUS + 1) * DOWN,
                        lag_ratio=0.5), morty.change_mode, "guilty",
...
class GrowingToDoList(Scene):
    def construct(self):
        ...
        self.play(morty.change_mode, "pleading", morty.look_at, lines,
                  Write(VGroup(*lines[3:]), ))
...
class TwoTypesOfVideos(Scene):
    def construct(self):
        ...
        series_list = VGroup(*[
            TextMobject("Essence of %s" % s) for s in [
                "linear algebra",
                "calculus",
                "probability",
                "real analysis",
        ...
        self.play(Blink(morty))
        self.wait()
        self.play(morty.change_mode, "raise_right_hand", morty.look_at, box,
                  ShowCreation(box))
        for x in range(3):
            self.wait(2)
            self.play(Blink(morty))
        ...
        for words, mode in pairs:
            self.play(Write(words), morty.change_mode, mode, morty.look_at,
                      words)
            self.wait()
...
class GrowRonaksSierpinski(Scene):
    ...
    def construct(self):
        ...
        run_time = 1
        for n, sierp_layer, dot_layer in zip(it.count(1), sierp_layers,
                                             dot_layers[1:]):
            self.play(ShowCreation(sierp_layer, lag_ratio=1),
...
class PatreonLogo(Scene):
    def construct(self):
        words1 = TextMobject("Support future\\\\", "3blue1brown videos")
        words2 = TextMobject("Early access to\\\\", "``Essence of'' series")
...
class MakeALotOfPiCreaturesHappy(Scene):
    def construct(self):
        ...
        pis = VGroup(*[
            VGroup(*[Randolph()
                     for x in range(7)]).arrange(RIGHT, buff=MED_LARGE_BUFF)
            for x in range(4)
        ]).arrange(DOWN, buff=MED_LARGE_BUFF)

        pi_list = list(
            it.chain(*[layer.submobjects for layer in pis.submobjects]))
...
class IntegrationByParts(Scene):
    def construct(self):
        ...
        self.play(ShowCreation(v_lines, run_time=2), Animation(curve),
                  Animation(rect))
        self.play(Write(equation[0]))
        self.wait()
        self.play(FadeIn(lower_left))
        self.play(ShowCreation(h_lines, run_time=2), Animation(curve),
                  Animation(rect))
    ...
    def draw_curve(self, curve):
        ...
        self.play(ShowCreation(curve),
                  UpdateFromFunc(dot, lambda d: d.move_to(curve.points[-1])),
