...
class Necklace(VMobject):
    ...
    def get_default_colors(self):
        result = list(
            it.chain(*[num * [color] for num, color in self.default_colors]))
...
class IntroduceStolenNecklaceProblem(ThreeDScene):
    ...
    def write_title(self):
        ...
        self.play(Write(title),
                  *[ApplyMethod(pi.look_at, title) for pi in self.thieves])
        self.title = title
    ...
    def introduce_necklace(self):
        ...
        self.play(
            ...
            *it.chain(*[[pi.change_mode, "conniving", pi.look_at, necklace]
                        for pi in self.thieves]))
        self.play(*[
            ApplyMethod(
                jewel.rotate_in_place, np.pi / 6, UP, rate_func=there_and_back)
            for jewel in jewels
        ])
    ...
    def divvy_by_cutting_all(self):
        ...
        for half_labels in both_half_labels:
            self.play(
                ReplacementTransform(enumeration_labels.copy(), half_labels))
        self.play(
            *[ApplyMethod(pi.change_mode, "pondering") for pi in thieves])
        self.wait()
    ...
    def divvy_with_n_cuts(
        ...
        jewel_types = self.jewel_types
        jewels = sorted(
            necklace.jewels, lambda m1, m2: cmp(m1.get_center()[0],
                                                m2.get_center()[0]))
        ...
        for group, choice in zip(subgroups, binary_choices):
            strand = Line(group[0].get_center(),
                          group[-1].get_center(),
        ...
        self.play(
            ...
            *it.chain(
                *[list(map(Animation, group)) for group in strand_groups]))
            ...
            for jewel_type in jewel_types:
                self.play(*[
                    ...
                    for jewel in jewel_type
                          ...
                          run_time=2)
            self.wait()
        self.play(FadeOut(v_lines), FadeIn(necklace.chain),
    ...
    def find_slice_indices(self, jewels, jewel_types):
        ...
        for slice_indices in it.combinations(list(range(1, len(jewels))),
                                             n_types):
            ...
            if self.forced_binary_choices is not None:
                ...
                all_binary_choices = it.product(*[list(range(2))] *
                                                (n_types + 1))
            for binary_choices in all_binary_choices:
                ...
                left_sets, right_sets = [[
                    subset for subset, index in zip(subsets, binary_choices)
...
class ThingToProve(PiCreatureScene):
    def construct(self):
        ...
        self.play(Write(top_words, run_time=2), self.pi_creature.change_mode,
                  "raise_right_hand")
...
class FiveJewelCase(IntroduceStolenNecklaceProblem):
    ...
    def add_title(self):
        ...
        title = TextMobject("%d jewel types, %d cuts" % (n_cuts, n_cuts))
        title.to_edge(UP)
        self.add(title)
...
class ShearThenProjectnOntoEquatorAntipodalCollisionIn3D(
...
class TemperaturePressurePlane(GraphScene):
    ...
    def wander_continuously(self):
        path = VMobject().set_points_smoothly([
            ORIGIN, 2 * UP + RIGHT, 2 * DOWN + RIGHT, 5 * RIGHT,
            4 * RIGHT + UP, 3 * RIGHT + 2 * DOWN, DOWN + LEFT, 2 * RIGHT
        ])
...
class WalkEquatorPostTransform(GraphScene):
    ...
    def walk_transverse_curve(self):
        ...
        self.play(UpdateFromAlphaFunc(dots, dot_update),
                  run_time=4,
                  rate_func=lambda t: 0.455 * smooth(t))
        self.play(dots.set_color,
                  YELLOW,
                  dots.scale_in_place,
                  1.2,
                  rate_func=there_and_back)
    ...
    def sphere_to_plane(self, point):
        x, y, z = point
        return np.array(
            [x - 2 * x * z + y + 1, y + 0.5 * y * np.cos(z * np.pi), 0])
    ...
    def get_great_arc_images(self):
        curves = VGroup(*[
            ParametricFunction(lambda t: self.sphere_point(t, s)).
...
class MentionGenerality(TeacherStudentsScene, ThreeDScene):
    def construct(self):
        ...
        self.play(
            FadeIn(necklace, run_time=2, lag_ratio=0.5), Write(arrow), *[
...
class WritePointCoordinates(Scene):
    ...
    def construct(self):
        ...
        arrow = Arrow(-self.corner,
                      self.corner,
                      stroke_width=8,
                      color=MAROON_B)
        x_component = self.corner[0] * RIGHT
        y_component = self.corner[1] * UP
        arrow.next_to(coords.get_edge_center(y_component),
                      y_component,
...
class GeneralizeBorsukUlam(Scene):
    ...
    def get_condition(self):
        squares = list(
            map(TexMobject, ["x_%d^2" % d for d in range(1, 1 + self.n_dims)]))
...
class MakeTwoJewelCaseContinuous(IntroduceStolenNecklaceProblem):
    ...
    def introduce_necklace(self):
        self.get_necklace(width=self.necklace_width, )
    ...
    def identify_necklace_with_unit_interval(self):
        ...
        self.wait()
        self.play(ShowCreation(interval.tick_marks), )
        self.wait()
    ...
    def color_necklace_by_indices(self, *indices):
        ...
        segments = VGroup(*[
            Line(chain.point_from_proportion(index / float(n_jewels)),
        ...
        self.play(
            ReplacementTransform(jewels, segments, lag_ratio=0.5, run_time=2))
        self.wait()
        return segments
    ...
    def find_continuous_fair_division(self):
        ...
        cut_proportions.append(
            [float(i) / n_jewels for i in slice_indices[1:-1]])
        cut_points = [
    ...
    def set_color_continuous_groups(self):
        ...
        weight_description = VGroup(*[
            VGroup(
                ...
                Jewel(color=jewel_type[0].get_color())).arrange()
    ...
    def shift_divide_off_tick_marks(self):
        ...
        for mob in left_segment, right_segment:
            ...
            self.add(mob.parts)
        restorers = [
            left_segment.parts, left_tick, right_segment.parts, right_tick
        ]
        for mob in restorers:
            mob.save_state()

        emerald_segments = VGroup(*[
            segment for segment in list(groups[0][0]) + list(groups[2][0])
        ...
        self.play(*[
            ApplyMethod(mob.shift, vect) for mob, vect in [
        ...
        label = VGroup(TexMobject("5\\left( 1/18 \\right)"),
                       Jewel(color=self.jewel_colors[1])).arrange()
        ...
        self.wait()
        self.play(emerald_segments.restore, *list(map(FadeOut,
                                                      [brace, label])))
...
class ThinkAboutTheChoices(TeacherStudentsScene):
    def construct(self):
        ...
        """)
        self.change_student_modes(*["pondering"] * 3,
                                  look_at_arg=FRAME_X_RADIUS * RIGHT +
                                  FRAME_Y_RADIUS * DOWN)
        self.wait(3)
...
class ChoicesInNecklaceCutting(ReconfigurableScene):
    ...
    def add_necklace(self):
        ...
        color_list = list(
            it.chain(
                *[num * [color] for num, color in zip(num_per_color, colors)]))
        ...
        segments = VGroup()
        for l_tick, r_tick, color in zip(tick_marks, tick_marks[1:],
                                         color_list):
            segment = Line(l_tick.get_center(),
                           r_tick.get_center(),
                           color=color)
    ...
    def choose_places_to_cut(self):
        ...
        self.division_points = list(
            it.chain([self.interval.get_left()], point_pairs[-1],
    ...
    def show_three_numbers_adding_to_one(self):
        ...
        braces = [
            Brace(Line(p1 + SMALL_BUFF * RIGHT / 2,
                       p2 + SMALL_BUFF * LEFT / 2))
        ...
        words = TextMobject("1) Choose", "$a$, $b$, $c$", "so that",
                            "$a+b+c = 1$")
        words[1].set_color(YELLOW)
        words[3].set_color(YELLOW)
        words.to_corner(UP + LEFT)

        self.play(*it.chain(*[[GrowFromCenter(brace),
        ...
        self.play(*list(
            map(FadeOut,
                list(braces) + [brace.concrete_label for brace in braces])))
    ...
    def make_binary_choice(self):
        ...
        words.next_to(self.choice_one_words,
                      DOWN,
                      buff=MED_LARGE_BUFF,
                      aligned_edge=LEFT)
    ...
    def get_boxes_and_labels(self):
        box = Rectangle(height=self.necklace.get_height() + SMALL_BUFF,
        ...
        labels = VGroup(*[
            TextMobject("Thief %d" %
                        (i + 1)).next_to(box, UP, aligned_edge=RIGHT)
    ...
    def get_choice_arrow_pairs(self, groups):
        ...
        arrow_pairs = [[arrow.copy(), arrow.copy().rotate(np.pi)]
                       for group in groups]
        pre_arrow_points = [
            VectorizedPoint(group.get_center()) for group in groups
        ]
    ...
    def wiggle_v_lines(self):
        self.play(*it.chain(
            *[[line.rotate_in_place, np.pi / 12, vect, line.set_color, RED]
...
class ChoicesForSpherePoint(GeneralizeBorsukUlam):
    ...
    def initialize_words(self):
        choice_one_words = TextMobject("1) Choose", "$x^2$, $y^2$, $z^2$",
        ...
        choice_two_words = TextMobject("2) Make a binary choice for each one")
        choice_two_words.next_to(choice_one_words,
                                 DOWN,
                                 buff=MED_LARGE_BUFF,
                                 aligned_edge=LEFT)
    ...
    def show_binary_choices(self):
        for choice in self.choices:
            ...
            arrows = VGroup(*[
                Arrow(choice.get_right(),
                      sqrt.get_left(),
                      color=WHITE,
                      tip_length=0.1,
                      buff=SMALL_BUFF) for sqrt in sqrts
            ])
...
class NecklaceDivisionSphereAssociation(ChoicesInNecklaceCutting):
    ...
    def add_sphere_point_label(self):
        label = TextMobject("$(x, y, z)$", "such that",
                            "$x^2 + y^2 + z^2 = 1$")
    ...
    def add_braces(self):
        ...
        braces = [
            Brace(
                ...
                UP) for p1, p2 in zip(points, points[1:])
        ]
        for char, brace, color, denom in zip("xyz", braces, self.xyz_colors,
                                             self.denoms):
            brace.label = brace.get_text("$%s^2$" % char,
                                         "$= 1/%d$" % denom,
                                         buff=SMALL_BUFF)
            brace.label.set_color(color)
            if brace.label.get_right()[0] > brace.get_right()[0]:
                brace.label.next_to(brace,
                                    UP,
                                    buff=SMALL_BUFF,
                                    aligned_edge=RIGHT)

        self.play(*it.chain(list(map(GrowFromCenter, braces)),
                            [Write(brace.label) for brace in braces]))
        self.wait()
    ...
    def show_binary_choice_association(self):
        ...
        for brace, denom, group, final_choice in quads:
            ...
            choices = [
                TexMobject(char, "=", sign, "\\sqrt{\\frac{1}{%d}}" % denom)
                for sign in ("+", "-")
            ]
            for choice, color in zip(choices, [GREEN, RED]):
                ...
                if choice.get_width() > 0.8 * group.get_width():
                    choice.next_to(group.get_right(),
                                   LEFT,
                                   buff=MED_SMALL_BUFF)
            ...
            self.wait()
            self.play(Transform(*choices), group.move_to,
                      group.target_points[1])
            self.wait()
            if final_choice == 0:
                self.play(Transform(choices[0], original_choices[0]),
                          group.move_to, group.target_points[0])
            self.swapping_anims += [
    ...
    def ask_about_antipodal_pairs(self):
        ...
        antipodal_tex = TexMobject("(x, y, z) \\rightarrow (-x, -y, -z)")
...
class TotalLengthOfEachJewelEquals(NecklaceDivisionSphereAssociation,
    ...
    def find_fair_division(self):
        ...
        groups = self.get_groups([0] + list(np.array(indices) + 1) +
                                 [len(segments)])
        self.add(*groups)
        binary_choice = [0, 1, 0]
    ...
    def get_fair_division_indices(self, colors):
        ...
        type_to_count = dict([(color, sum(colors == color))
    ...
    def demonstrate_fair_division(self):
        ...
        color_types = list(
            map(Color,
                set([segment.get_color().get_hex_l()
        ...
        for color in color_types:
            monochrome_groups = [
                VGroup(*[
                    segment for segment in segment_group
            ...
            for i, group in enumerate(monochrome_groups):
                ...
                label = VGroup(TextMobject("Thief %d" % (i + 1)),
...
class ShowFunctionDiagram(TotalLengthOfEachJewelEquals, ReconfigurableScene):
    ...
    def add_number_pair(self):
        plane_classes = [
            JewelPairPlane(skip_animations=True, thief_number=x)
            for x in (1, 2)
        ]
        ...
        arrow = Arrow(self.necklace.get_corner(DOWN + RIGHT),
                      self.example_coords,
                      color=YELLOW)
        ...
        self.wait(2)
        self.play(ApplyMethod(t1_plane.restore, path_arc=np.pi),
                  Animation(arrow))
        self.wait()
    ...
    def add_sphere_arrow(self):
        ...
        to_plane_arrow = Arrow(up_down_arrow.get_bottom() + DOWN + RIGHT,
                               self.example_coords,
                               color=YELLOW)
    ...
    def get_fair_division_indices(self, *args):
        if self.make_up_fair_division_indices:
            ...
            return TotalLengthOfEachJewelEquals.get_fair_division_indices(
                self, *args)
...
class WhatAboutGeneralCase(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.teacher_says("""Use Borsuk-Ulam for
...
class FourDBorsukUlam(GeneralizeBorsukUlam, PiCreatureScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(FadeIn(lhs), self.pi_creature.change_mode,
                  "raise_right_hand")
        self.play(ReplacementTransform(lhs.copy(), rhs), Write(eq))
        self.wait(2)
