...
class IntroduceCycle(WalkingRandolph):
    ...
    def construct(self):
        ...
        regions = [
            self.region_from_cycle(cycle) for cycle in encompassed_cycles
        ]
        ...
        for pair in self.spanning_tree_index_pairs:
            self.play(
                ShowCreation(Line(self.points[pair[0]],
                                  self.points[pair[1]]).set_color("yellow"),
                             run_time=run_time_per_branch))
            self.play(
...
class FacebookGraphAsAbstractSet(Scene):
    def construct(self):
        ...
        friends_mob = TexMobject(friends_string,
        ...
        lines = Mobject(
            ...
            Line(LEFT * FRAME_X_RADIUS + 3 * UP,
                 RIGHT * FRAME_X_RADIUS + 3 * UP)).set_color("white")
...
class ExamplesOfGraphs(GraphScene):
    def construct(self):
        ...
        vertical_line = Line((vert_line_x_val, FRAME_Y_RADIUS, 0),
    ...
    def handle_english_words(self, words1, words2):
        ...
        lines = [
            Line(*pair) for pair in [(words[0].get_center() +
                                      RIGHT * words[0].get_width() / 2,
                                      words[1].get_center() +
                                      LEFT * words[1].get_width() / 2),
                                     (words[1].get_center() +
                                      UP * words[1].get_height() / 2,
                                      words[2].get_center() +
                                      DOWN * words[2].get_height() / 2),
                                     (words[1].get_center() +
                                      DOWN * words[1].get_height() / 2,
                                      words[3].get_center() +
                                      UP * words[3].get_height() / 2)]
        ]
        ...
        self.play(*[
            Transform(
                ...
                Dot(v.get_center(), radius=3 *
                    Dot.DEFAULT_RADIUS).set_color("green"))
...
class MortimerCannotTraverseCycle(GraphScene):
    ...
    def construct(self):
        ...
        self.play(*[
            Transform(line,
                      deepcopy(edge).set_color(line.get_color()))
...
class TwoPropertiesOfSpanningTree(Scene):
    def construct(self):
        spanning, tree = TextMobject(["Spanning ", "Tree"],
                                     size="\\Huge").split()
        ...
        self.wait()
        for word, explanation, vect in [(spanning, spanning_explanation,
                                         0.5 * UP),
...
class TreeCountFormula(Scene):
    def construct(self):
        ...
        self.play(
            FadeOut(text), FadeIn(Mobject(*gs.edges + gs.vertices)), *[
                Transform(*pair)
