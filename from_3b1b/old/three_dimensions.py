...
class Stars(Mobject1D):
    ...
    def generate_points(self):
        ...
        self.add_points([(r * np.sin(phi) * np.cos(theta),
...
class CubeWithFaces(Mobject2D):
    def generate_points(self):
        self.add_points([
            sgn * np.array(coords) for x in np.arange(-1, 1, self.epsilon)
            for y in np.arange(x, 1, self.epsilon)
            for coords in it.permutations([x, y, 1]) for sgn in [-1, 1]
        ])
        self.pose_at_angle()
        self.set_color(BLUE)
...
class Cube(Mobject1D):
    def generate_points(self):
        self.add_points([
            ...
            for a, b, c in it.product([-1, 1], [-1, 1],
                                      np.arange(-1, 1, self.epsilon))
        ])
        self.pose_at_angle()
...
class Octohedron(Mobject1D):
    def generate_points(self):
        ...
        y = np.array([0, 1, 0])
        z = np.array([0, 0, 1])
        vertex_pairs = [(x + y, x - y), (x + y, -x + y), (-x - y, -x + y),
                        (-x - y, x - y)]
        vertex_pairs += [(b[0] * x + b[1] * y, b[2] * np.sqrt(2) * z)
...
class Sphere(Mobject2D):
    def generate_points(self):
        self.add_points([
            ...
            for theta in np.arange(0, 2 * np.pi, 2 * self.epsilon /
                                   np.sin(phi))
        ])
        self.set_color(BLUE)
