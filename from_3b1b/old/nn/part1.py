...
class MNistMobject(WrappedImage):
    def __init__(self, vect, **kwargs):
        WrappedImage.__init__(self, ImageMobject(layer_to_image_array(vect)),
                              **kwargs)
...
class NetworkScene(Scene):
    ...
    def show_activation_of_layer(self,
                                 layer_index,
                                 activation_vector,
                                 added_anims=None):
        if added_anims is None:
...
class ExampleThrees(PiCreatureScene):
    ...
    def show_initial_three(self):
        ...
        self.play(
            ...
            *it.chain(list(map(GrowFromCenter, braces)),
                      list(map(FadeIn, brace_labels))))
        self.wait()
        self.play(ShowCreation(bubble),
                  MoveToTarget(three_mob), FadeOut(braces),
                  FadeOut(brace_labels), randy.change, "pondering")
    ...
    def show_alternate_threes(self):
        ...
        for square in three.target[1]:
            ...
            square.set_fill(rgba_to_color(yellow_rgb * square_rgb),
                            opacity=0.5)
    ...
    def show_alternate_digits(self):
        ...
        alt_mobs = [
            WrappedImage(PixelsAsSquares(
                ImageMobject(layer_to_image_array(arr))),
                         color=LIGHT_GREY,
                         buff=0).replace(three) for arr in arrays
        ]

        self.play(randy.change, "sassy", Transform(three, alt_mobs[0]),
                  ShowCreation(cross))
        self.wait()
...
class BrainAndHow(Scene):
    def construct(self):
        ...
        for x in range(2):
            self.play(
                ShowPassingFlash(brain_outline, time_width=0.5, run_time=2))
        self.wait()
...
class WriteAProgram(Scene):
    def construct(self):
        ...
        self.play(LaggedStartMap(DrawBorderThenFill, three),
        ...
        for n in 8, 1, 5, 3:
            self.play(rect.move_to, choices[n],
...
class LayOutPlan(TeacherStudentsScene, NetworkScene):
    ...
    def show_words(self):
        ...
        self.play(words[0].restore, self.teacher.change, "raise_right_hand",
    ...
    def show_network(self):
        ...
        self.play(ReplacementTransform(VGroup(self.words[1].copy()),
                                       network_mob.layers),
    ...
    def show_math(self):
        equation = TexMobject("\\textbf{a}_{l+1}", "=", "\\sigma(", "W_l",
                              "\\textbf{a}_l", "+", "b_l", ")")
    ...
    def ask_about_layers(self):
        self.student_says("Why the layers?",
                          student_index=2,
    ...
    def show_videos(self):
        ...
        self.play(DrawBorderThenFill(videos[0]), FadeIn(structure),
...
class PreviewMNistNetwork(NetworkScene):
    ...
    def feed_in_image(self, in_vect):
        ...
        self.feed_forward(in_vect,
                          added_anims=[
                              self.get_image_to_layer_one_animation(
                                  image, start_neurons)
                          ])
        n = np.argmax([
...
class AlternateNeuralNetworks(PiCreatureScene):
    def construct(self):
        ...
        self.play(Write(examples[0], run_time=2), morty.change,
                  "raise_right_hand")
        self.wait()
        self.play(
            examples[0].shift,
        ...
        self.wait()
        self.play(examples.shift, UP, FadeIn(maybe_words), morty.change,
                  "maybe")
        self.wait(2)
...
class BreakDownName(Scene):
    ...
    def ask_questions(self):
        ...
        self.play(brain.set_height, 2, brain.shift, 2 * UP, brain.set_fill,
                  None, 1, randy.look, UP)
        brain_outline = brain.copy()
        brain_outline.set_fill(opacity=0)
        brain_outline.set_stroke(BLUE_B, 3)
        ...
        self.play(Write(q2, run_time=1), ShowCreation(a2), name[1].set_color,
                  q2.get_color())
        self.wait(2)

        self.play(*list(
            map(FadeOut, [name, randy, brain, q2, a1, a2, q1[0], q1[2]])))
    ...
    def show_neuron(self):
        ...
        self.play(ReplacementTransform(self.neuron_word, neuron_word),
                  ShowCreation(neuron))
...
class IntroduceEachLayer(PreviewMNistNetwork):
    ...
    def break_up_image_as_neurons(self):
        ...
        labels = VGroup(
            *[brace.get_tex("28", buff=SMALL_BUFF) for brace in braces])
    ...
    def show_activation_of_one_neuron(self):
        ...
        def change_activation(num):
            self.play(
                ...
                UpdateFromFunc(
                    example_num,
                    lambda m: m.set_fill(BLACK if example_neuron.
                                         get_fill_opacity() > 0.8 else WHITE)))
    ...
    def transform_into_full_network(self):
        ...
        rows = VGroup(
            *[VGroup(*neurons[28 * i:28 * (i + 1)]) for i in range(28)])

        self.play(FadeOut(self.braces), FadeOut(self.brace_labels),
                  FadeOut(VGroup(*self.num_pixels_equation[:-1])))

        self.play(rows.space_out_submobjects, 1.2)
        self.play(rows.arrange,
                  RIGHT,
                  buff=SMALL_BUFF,
                  path_arc=np.pi / 2,
                  run_time=2)
        ...
        self.play(
            ReplacementTransform(self.num_pixels_equation[-1],
                                 layer.brace_label),
            FadeIn(layer.brace),
        )
        self.play(layer.restore, FadeIn(self.corner_image))
        self.wait()
        for edge_group, layer in zip(network_mob.edge_groups,
    ...
    def show_output_layer(self):
        ...
        def change_activation(num):
            self.play(
                ...
                UpdateFromFunc(
                    activation, lambda m: m.set_fill(
...
class DiscussChoiceForHiddenLayers(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(FadeOut(rects),
                  ReplacementTransform(two_words, sixteen_words), neurons_anim)
        self.wait()
...
class MoreHonestMNistNetworkPreview(IntroduceEachLayer):
    ...
    def get_image_to_layer_one_animation(self, image, start_neurons):
        neurons = VGroup()
        for pixel in image:
            neuron = Circle(radius=pixel.get_width() / 2,
                            stroke_width=1,
                            stroke_color=WHITE,
                            fill_color=WHITE,
...
class AskAboutPropogationAndTraining(TeacherStudentsScene):
    def construct(self):
        self.student_says("How does one layer \\\\ influence the next?",
                          student_index=0,
                          run_time=1)
        self.wait()
        self.student_says("How does \\\\ training work?",
                          student_index=2,
                          run_time=1)
        self.wait(3)
...
class BreakUpMacroPatterns(IntroduceEachLayer):
    CONFIG = {
        ...
        "prefixes": [
            "nine", "eight", "four", "upper_loop", "right_line", "lower_loop",
            "horizontal_line", "upper_left_line"
        ]
    }
    ...
    def setup_needed_patterns(self):
        ...
        vects = [
            np.array(
                Image.open(get_full_raster_image_path("handwritten_" +
                                                      p), ))[:, :,
                                                             0].flatten() /
            255.0 for p in prefixes
        ]
    ...
    def setup_added_patterns(self):
        image_map = get_organized_images()
        two, three, five = mobs = [
            MNistMobject(image_map[n][0]) for n in (2, 3, 5)
        ]
    ...
    def show_nine(self):
        ...
        self.wait()
        self.play(Write(equation[1]), upper_loop[1].restore,
                  FadeIn(upper_loop[0]))
        self.wait()
        self.play(
            Write(equation[3]),
    ...
    def show_four(self):
        ...
        equation = self.get_equation(four, right_line, upper_left_line,
                                     horizontal_line)
    ...
    def show_second_to_last_layer(self):
        ...
        patterns = VGroup(self.upper_loop, self.lower_loop, self.right_line,
                          self.upper_left_line, self.horizontal_line,
    ...
    def get_equation(self, *mobs):
        equation = VGroup(
            ...
            TexMobject("=").scale(2),
            *list(
                it.chain(*[[m, TexMobject("+").scale(2)]
                           for m in mobs[1:-1]])) + [mobs[-1]])
...
class HowWouldYouRecognizeSubcomponent(TeacherStudentsScene):
    def construct(self):
        self.student_says("Okay, but recognizing loops \\\\",
                          "is just as hard!",
                          target_mode="sassy")
...
class BreakUpMicroPatterns(BreakUpMacroPatterns):
    ...
    def break_down_long_line(self):
        ...
        edges = Group(
            *[getattr(self, "right_line_edge%d" % d) for d in range(1, 4)])
        ...
        self.wait()
        self.play(LaggedStartMap(FadeIn, Group(*equation[1:])), randy.change,
                  "pondering", equation)
        self.wait(3)
...
class SecondLayerIsLittleEdgeLayer(IntroduceEachLayer):
    ...
    def show_propogation(self):
        ...
        def activate_layer(i):
            self.play(
                ShowCreationThenDestruction(edge_groups[i - 1],
                                            run_time=2,
                                            lag_ratio=0.5),
        ...
        self.play(
            MoveToTarget(neurons, remover=True, lag_ratio=0.5, run_time=2))
    ...
    def set_neurons_target(self, neurons, layer):
        ...
        Transform(VGroup(*neurons.target[n:-n]),
                  VectorizedPoint(layer.get_center())).update(1)
...
class ManyTasksBreakDownLikeThis(TeacherStudentsScene):
    def construct(self):
        ...
        sequence = VGroup(audio, get_arrow(), letters, get_arrow(), syllables,
                          get_arrow(), word)
        ...
        self.teacher_says("Many", "recognition", "tasks\\\\",
        ...
        self.play(self.teacher.bubble.content.fade, 1, ShowCreation(audio),
                  pre_word.shift, MED_SMALL_BUFF, DOWN)
        self.wait(2)
        self.play(
            RemovePiCreatureBubble(self.teacher), audio.restore,
            FadeIn(audio_label), *[
        ...
        self.wait()
        self.play(GrowFromPoint(arrows[1], arrows[1].get_start()),
                  LaggedStartMap(FadeIn, syllables, run_time=1))
        self.wait()
        self.play(GrowFromPoint(arrows[2], arrows[2].get_start()),
                  LaggedStartMap(FadeIn, word, run_time=1))
        self.wait()

    def get_wave_form(self):
        func = lambda x: abs(
            sum([(1. / n) * np.sin((n + 3) * x) for n in range(1, 5)]))
        result = VGroup(
            *
            [Line(func(x) * DOWN,
                  func(x) * UP) for x in np.arange(0, 4, 0.1)])
...
class IntroduceWeights(IntroduceEachLayer):
    ...
    def show_desired_pixel_region(self):
        ...
        self.play(pixels_to_detect.set_fill,
                  WHITE,
                  1,
                  lag_ratio=0.5,
                  run_time=2)
    ...
    def ask_about_parameters(self):
        ...
        arrow = Arrow(parameter_word.get_bottom(),
        ...
        for i, p_label in enumerate(p_labels[:-1]):
            ...
            changing_decimals.append(
                ChangingDecimal(decimal, get_alpha_func(i + 5)))
        for i, pixel in enumerate(pixels):
            pixel.func = get_alpha_func(i, pixel.get_fill_opacity())
        pixel_updates = [
            UpdateFromAlphaFunc(pixel,
        ...
        self.wait()
        self.play(*changing_decimals + pixel_updates,
                  run_time=5,
                  rate_func=linear)
    ...
    def show_weighted_sum(self):
        ...
        w_labels = VGroup(
            *[VGroup(*label[:-1]).copy() for label in self.w_labels])
        ...
        weighted_sum = VGroup(*it.chain(
            *[[w, a, TexMobject("+")] for w, a in zip(w_labels, a_labels)]))
        weighted_sum.add(TexMobject("\\cdots"), TexMobject("+"),
    ...
    def organize_weights_as_grid(self):
        ...
        self.play(
            FadeIn(VGroup(*weight_grid[len(decimals):]),
                   lag_ratio=0.5,
                   run_time=3), *[
    ...
    def make_most_weights_0(self):
        ...
        self.wait()
        self.play(
            ReplacementTransform(self.pixels_to_detect.copy(),
                                 self.weighted_sum,
                                 run_time=3,
                                 lag_ratio=0.5),
    ...
    def add_negative_weights_around_the_edge(self):
        ...
        self.play(*[
            ApplyMethod(weight_grid[28 * y + x].set_fill,
                        self.negative_weights_color, 0.5) for y in (6, 10)
    ...
    def get_surrounding_pixels_for_edge(self, pixels):
        d = int(np.sqrt(len(pixels)))
        return VGroup(*it.chain(
            *[pixels[d * n + d / 2 - 4:d * n + d / 2 + 4] for n in (6, 10)]))
...
class MotivateSquishing(Scene):
    ...
    def add_weighted_sum(self):
        weighted_sum = TexMobject(*it.chain(
            *[["w_%d" % d, "a_%d" % d, "+"]
              for d in range(1, 5)] + [["\\cdots", "+", "w_n", "a_n"]]))
    ...
    def show_real_number_line(self):
        ...
        self.play(
            Transform(arrow1, arrow2, run_time=5, rate_func=there_and_back))
    ...
    def show_interval(self):
        ...
        interval = Line(lower_number_line.number_to_point(0),
        ...
        self.play(
            ReplacementTransform(self.number_line.copy(), lower_number_line))
        self.play(
    ...
    def squish_into_interval(self):
        ...
        arrow = Arrow(line.numbers.get_bottom(),
                      line.target.get_top(),
                      color=YELLOW)
...
class IntroduceSigmoid(GraphScene):
    ...
    def add_title(self):
        ...
        equation = TexMobject("\\sigma(%s) = \\frac{1}{1+e^{-%s}}" %
                              (char, char))
    ...
    def add_graph(self):
        graph = self.get_graph(lambda x: 1. / (1 + np.exp(-x)), color=YELLOW)
    ...
    def show_part(self, x_min, x_max, color):
        line, graph_part = [
            self.get_graph(
                ...
                x_max=x_max,
                color=color,
            ).set_stroke(width=4) for func in (lambda x: 0, sigmoid)
        ]
...
class IncludeBias(IntroduceWeights):
    ...
    def setup_start(self):
        ...
        self.make_edges_weighted(edges,
                                 1.5 * np.random.random(len(edges)) - 0.5)

        Transform(self.network_mob.layers[0],
                  self.network_mob.get_active_layer(
                      0, np.random.random(16))).update(1)
    ...
    def comment_on_need_for_bias(self):
        ...
        words = TextMobject("Only activate meaningfully \\\\ when",
                            "weighted sum", "$> 10$")
        ...
        self.play(Write(words, run_time=2))
        self.play(
            ApplyMethod(colored_pixels.shift,
                        MED_LARGE_BUFF * UP,
                        rate_func=there_and_back,
                        run_time=2,
                        lag_ratio=0.5))
    ...
    def summarize_weights_and_biases(self):
        ...
        self.play(
            LaggedStartMap(ApplyMethod,
                           weight_grid,
                           lambda p:
                           (p.set_fill, random.choice([GREEN, GREEN, RED]),
                            random.random()),
                           rate_func=there_and_back,
                           lag_ratio=0.4,
                           run_time=4))
...
class ContinualEdgeUpdate(VGroup):
    ...
    def update_edges(self, dt):
        ...
        for edge in self.edges:
            ...
            alpha = (
                (self.internal_time - 1) % edge.cycle_time) / edge.cycle_time
            low_n = int(t) % len(edge.colors)
            high_n = int(t + 1) % len(edge.colors)
            color = interpolate_color(edge.colors[low_n], edge.colors[high_n],
                                      alpha)
...
class ShowRemainingNetwork(IntroduceWeights):
    ...
    def show_all_of_second_layer(self):
        ...
        for neuron in neurons[:7]:
            ...
            self.play(ShowCreation(edges, lag_ratio=0.5),
                      FadeIn(neuron),
                      *added_anims,
                      run_time=1.5)
            last_edges = edges
    ...
    def compute_layer_two_of_weights_and_biases_count(self):
        ...
        self.wait()
        self.play(ReplacementTransform(times_16.copy(), bias_count[0]),
                  FadeOut(bb1), ReplacementTransform(bb2, bias_count[1]),
                  FadeOut(bb3), LaggedStartMap(FadeOut, bias_arrows))
        self.wait()
    ...
    def show_remaining_layers(self):
        ...
        self.play(
            MoveToTarget(weights_count), MoveToTarget(bias_count),
            Write(added_weights, run_time=1), Write(added_biases, run_time=1),
            LaggedStartMap(
                ShowCreation,
                edges,
                run_time=4,
    ...
    def get_edge_weight_wandering_anim(self, edges):
        for edge in edges:
            edge.generate_target()
            edge.target.set_stroke(color=random.choice(
...
class ImagineSettingByHand(Scene):
    def construct(self):
        ...
        network_mob = NetworkMobject(Network(sizes=[8, 6, 6, 4]),
                                     neuron_stroke_color=WHITE)
...
class WhenTheNetworkFails(MoreHonestMNistNetworkPreview):
    ...
    def ask_about_weights(self):
        question = TextMobject("What weights are used here?\\\\",
                               "What are they doing?")
...
class IntroduceWeightMatrix(NetworkScene):
    ...
    def show_first_neuron_weighted_sum(self):
        ...
        w_labels = VGroup(
            *[TexMobject("w_{0, %d}" % d) for d in range(len(a_labels))])
        weighted_sum = VGroup()
        symbols = VGroup()
        ...
        VGroup(a1_label, equals,
               weighted_sum).scale(0.75, about_point=a1_label.get_left())

        w_labels.set_color(GREEN)
        w_labels.shift(0.6 * SMALL_BUFF * DOWN)
        a_labels.target.shift(0.5 * SMALL_BUFF * UP)

        self.play(Write(a1_label),
                  Write(equals),
                  neuron.set_fill,
                  None,
                  0.3,
    ...
    def organize_activations_into_column(self):
        ...
        self.play(
            LaggedStartMap(Indicate,
                           ...
                           rate_func=there_and_back,
                           run_time=1))
        self.play(MoveToTarget(a_labels),
    ...
    def organize_weights_as_matrix(self):
        ...
        dots_row = VGroup(*list(
            map(TexMobject, ["\\vdots", "\\vdots", "\\ddots", "\\vdots"])))
    ...
    def connect_weighted_sum_to_matrix_multiplication(self):
        ...
        self.wait(2)
        self.show_meaning_of_lower_rows(arrow, brace, top_row_rect,
                                        result_terms)
        self.play(*list(
            map(FadeOut,
    ...
    def write_clean_final_expression(self):
        ...
        for neuron in neurons:
            neuron_anims += [
                neuron.restore, neuron.set_fill, None,
                random.random()
            ]
        ...
        self.play(
            ReplacementTransform(
                VGroup(self.top_matrix_row, self.lower_matrix_rows,
                       self.matrix_brackets).copy(),
                VGroup(W),
            ))
        self.play(
            ReplacementTransform(
        ...
        self.play(
            ReplacementTransform(self.big_sigma_group.copy(),
                                 VGroup(sigma, lp, rp)))
        self.wait()
...
class EoLA3Wrapper(PiCreatureScene):
    def construct(self):
        ...
        self.play(ShowCreation(rect), FadeIn(title), morty.change,
                  "raise_right_hand", rect)
        self.wait(4)
...
class NeuronIsFunction(MoreHonestMNistNetworkPreview):
    ...
    def activate_network(self):
        ...
        for i, activation in enumerate(activations):
            ...
            Transform(layer,
                      self.network_mob.get_active_layer(i,
                                                        activation)).update(1)
        self.add(mnist_mob)
    ...
    def write_neuron_holds_a_number(self):
        ...
        self.play(
            neuron_group.restore, neuron_group.scale, 3, neuron_group.next_to,
            neuron_word, LEFT, FadeIn(neuron_word), GrowArrow(arrow),
            FadeIn(thing_words,
                   run_time=2,
    ...
    def show_neuron_as_function(self):
        ...
        self.play(FadeOut(self.network_mob),
    ...
    def network_is_a_function(self):
        ...
        self.play(ReplacementTransform(neuron_word, network_word),
                  FadeIn(func_tex))
...
class ComplicationIsReassuring(TeacherStudentsScene):
    def construct(self):
        self.student_says("It kind of has to \\\\ be complicated, right?",
...
class NextVideo(MoreHonestMNistNetworkPreview, PiCreatureScene):
    ...
    def show_video_neural_network(self):
        ...
        self.play(MoveToTarget(network_mob), MoveToTarget(video),
                  FadeOut(words), FadeOut(rect), morty.change,
                  "raise_left_hand")
...
class IntroduceReLU(IntroduceSigmoid):
    ...
    def old_school(self):
        ...
        arrow = Arrow(old_school.get_bottom(),
        ...
        self.wait(2)
        self.play(
            ApplyMethod(VGroup(cross, sigmoid_title).shift,
        ...
        self.play(
            ShowCreation(self.sigmoid_graph,
...
class CompareSigmoidReLUOnDeepNetworks(PiCreatureScene):
    def construct(self):
        ...
        self.play(FadeIn(sigmoid_name), ShowCreation(sigmoid_graph),
                  lisha.change, "raise_left_hand", morty.change, "pondering")
        self.play(Write(slow_learner, run_time=1), GrowArrow(slow_arrow))
        self.wait()
        self.play(FadeIn(relu_name), ShowCreation(relu_graph), lisha.change,
                  "raise_right_hand", morty.change, "thinking")
