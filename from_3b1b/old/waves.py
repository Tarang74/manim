...
class EMWave(ContinualAnimationGroup):
        ...
        for alpha in np.linspace(0, 1, self.n_vectors):
            ...
            E_ov = OscillatingVector(Vector(
                ...
                normal_vector=UP,
                                     ...
                                     A_vect=self.A_vect,
                                     **kwargs)
            M_ov = OscillatingVector(Vector(
                UP,
                color=M_COLOR,
                normal_vector=OUT,
            ),
                                     A_vect=rotate_vector(
                                         self.A_vect, np.pi / 2, RIGHT),
                                     **kwargs)
    ...
    def update_mobject(self, dt):
        if self.requires_start_up:
            ...
            middle_alpha = interpolate(0.4, 1.4,
            ...
            for ov, alpha in zip(ovs, np.linspace(0, 1, len(ovs))):
                epsilon = 0.0001
                new_amplitude = np.clip(new_smooth(middle_alpha - alpha),
                                        epsilon, 1)
...
class WavePacket(Animation):
    ...
    def interpolate_mobject(self, alpha):
        packet_center = interpolate(self.wave_packet_start,
                                    self.wave_packet_end, alpha)
        em_wave = self.em_wave
        for vect in self.vects:
            tail = vect.get_start()
            distance_from_packet = np.dot(tail - packet_center,
...
class PolarizingFilter(Circle):
    ...
    def __init__(self, **kwargs):
        ...
        if self.include_arrow_label:
            arrow_label = TexMobject("%.1f^\\circ" %
                                     (self.filter_angle * 180 / np.pi))
...
class FilterScene(ThreeDScene):
    ...
    def setup(self):
        ...
        self.add(self.axes)
        for x in range(
                len(self.filter_x_coordinates) - len(self.pol_filter_configs)):
            self.pol_filter_configs.append({})
        self.pol_filters = VGroup(
            *
            [PolarizingFilter(**config) for config in self.pol_filter_configs])
    ...
    def get_filter_absorption_animation(self, pol_filter, photon):
        ...
        return ApplyMethod(pol_filter.set_fill,
                           RED,
...
class DirectionOfPolarizationScene(FilterScene):
    ...
    def change_polarization_direction(self, angle, **kwargs):
        ...
        self.play(ApplyMethod(self.reference_line.rotate, angle, **kwargs),
                  *added_anims)
    ...
    def apply_filters(self):
        ...
        filters = sorted(
            self.pol_filters, lambda pf1, pf2: cmp(
                pf1.get_center()[0],
                pf2.get_center()[0],
            ))
        for pol_filter in filters:
            ...
            for vect_group, angle in zip(vect_groups, [0, -np.pi / 2]):
                ...
                for vect_mob in vect_group:
                    ...
                    vect_angle = angle_of_vector([vect[2], -vect[1]])
    ...
    def update_rectangles(self):
        if not hasattr(self,
                       "rectangles") or self.rectangles not in self.mobjects:
            return
        ...
        target_depth = max(
            2 * self.em_wave.amplitude * abs(np.cos(target_angle)), epsilon)
...
class WantToLearnQM(TeacherStudentsScene):
    def construct(self):
        question1 = TexMobject(
            "\\text{What does }\\qquad \\\\", "|\\!\\psi \\rangle", "=",
        ...
        question2 = TextMobject("Why are complex \\\\ numbers involved?")
        ...
        self.wait(2)
        self.teacher_says("First, lots and lots \\\\ of linear algebra",
...
class Goal(PiCreatureScene):
    def construct(self):
        ...
        self.play(goal.restore, randy.change, "raise_right_hand")
        self.play(Write(weirdness, run_time=2))
        self.play(ShowCreation(cross), randy.change, "sassy")
        self.wait()
        self.play(
            VGroup(weirdness, cross).shift, DOWN, Write(foundations,
                                                        run_time=2),
            randy.change, "happy")
        self.wait(2)
...
class AskWhatsDifferentInQM(TeacherStudentsScene):
    def construct(self):
        self.student_says("What's different in \\\\ quantum mechanics?")
...
class BellsWrapper(VideoWrapper):
    CONFIG = {"title": "Bell's inequalities"}


class FromOtherVideoWrapper(VideoWrapper):
    CONFIG = {"title": "See the other video..."}


class OriginOfQuantumMechanicsWrapper(VideoWrapper):
    CONFIG = {"title": "The origin of quantum mechanics"}
...
class IntroduceElectricField(PiCreatureScene):
    ...
    def write_title(self):
        ...
        title = TextMobject("Electro", "magnetic", " field", arg_separator="")
        ...
        self.wait()
        self.play(ShrinkToCenter(title[1]), Transform(title[0], electric))
    ...
    def let_particle_wander(self):
        ...
        particles = VGroup(
            self.particle,
            *[self.particle.copy().move_to(point) for point in points])
        for particle in particles:
            particle.velocity = np.zeros(3)

        self.play(FadeOut(self.force_vector),
    ...
    def get_vector_field(self):
        result = VGroup(*[
            self.get_vector(point) for x in np.arange(-9, 9)
            for y in np.arange(-5, 5) for point in [x * RIGHT + y * UP]
        ])
        shading_list = list(result)
        shading_list.sort(key=lambda m: m1.get_length())
...
class IntroduceMagneticField(IntroduceElectricField, ThreeDScene):
    CONFIG = {"vector_field_colors": [YELLOW_C, YELLOW_D]}
    ...
    def add_vector_field(self):
        ...
        self.play(LaggedStartMap(ShowCreation, vector_field, run_time=3),
    ...
    def show_force(self):
        ...
        F_vect = Vector(3 * np.cross(self.field_function(point), RIGHT),
                        color=GREEN)
        F_vect.shift(point)
        ...
        F_eq = TexMobject("=", "q", "\\textbf{v}", "\\times", "\\textbf{B}")
...
class CurlRelationBetweenFields(ThreeDScene):
    ...
    def loop_in_E(self):
        ...
        M_vect = Vector(IN, normal_vector=DOWN, color=M_COLOR)
        M_vect.shift(point)
        M_vect.save_state()
    ...
    def loop_in_M(self):
        M_vects = VGroup(*[
            Vector(rotate_vector(vect, np.pi / 2),
                   normal_vector=IN,
                   color=M_COLOR).shift(vect)
        ...
        self.play(ShowCreation(M_vects, run_time=2),
...
class IntroduceEMWave(ThreeDScene):
    CONFIG = {"EMWave_config": {"requires_start_up": True}}
...
class ListRelevantWaveIdeas(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(Animation(VectorizedPoint(wave_point)), self.teacher.change,
                  "raise_right_hand")
        self.wait(2)
        self.play(Write(title, run_time=2), ShowCreation(h_line))
        self.change_student_modes(
            *["pondering"] * 3,
            added_anims=[LaggedStartMap(FadeIn, topics, run_time=3)],
            look_at_arg=title)
        self.play(Animation(title), self.teacher.change, "happy")
        self.play(
            title[0].next_to,
            quantum.copy(),
            UP,
...
class DirectWaveOutOfScreen(IntroduceEMWave):
    ...
    def move_into_position(self):
        ...
        faded_vectors = VGroup(
            *[ov.vector for ov in self.em_wave.continual_animations[:-2]])
        self.move_camera(0.99 * np.pi / 2,
                         -0.01,
                         run_time=2,
...
class ShowVectorEquation(Scene):
    ...
    def add_plane(self):
        ...
        self.play(Write(xy_plane), Animation(self.vector))
        self.wait(2)
    ...
    def write_components(self):
        ...
        E, equals = E_equals = TexMobject("\\vec{\\textbf{E}}", "=")
        E.set_color(E_COLOR)
        ...
        group.next_to(self.horizontally_polarized_words, DOWN, MED_LARGE_BUFF,
                      RIGHT)
        ...
        def update_brace(brace):
            brace.stretch_to_fit_width(max(self.vector.get_width(), 0.001))
            brace.next_to(self.vector.get_center(), DOWN, SMALL_BUFF)
            return brace

        moving_brace = Mobject.add_updater(Brace(Line(LEFT, RIGHT), DOWN),
                                           update_brace)
    ...
    def show_graph(self):
        ...
        self.remove(new_ov)
        self.add(self.oscillating_vector)
        self.play(ReplacementTransform(*fx_group),
                  ReplacementTransform(*graphs), FadeOut(new_ov.vector),
                  FadeIn(self.vector))
        self.wait(4)
    ...
    def add_phi(self):
        ...
        self.play(Write(words, run_time=1), ShowCreation(arrow))
        self.wait(3)
    ...
    def add_amplitude(self):
        ...
        self.wait(2)
        self.play(x.next_to, A, RIGHT, SMALL_BUFF, corner_cos.next_to,
                  corner_A, RIGHT, SMALL_BUFF, FadeIn(all_As))
        x.add(A)
        corner_cos.add(corner_A)
        self.wait()
        factor = 0.5
        self.play(
            v_brace.stretch_in_place,
            factor,
            1,
            v_brace.move_to,
            v_brace.copy(),
    ...
    def add_kets(self):
        ...
        self.play(FadeOut(self.brackets),
    ...
    def switch_to_vertically_polarized_light(self):
        ...
        y.target = TexMobject("A_y", "\\cos(", "2\\pi", "f_y", "t", "+",
                              "\\phi_y", ")")
        ...
        self.play(
            ...
            Transform(self.h_brace, new_h_brace), self.h_brace.A.next_to,
            new_h_brace, RIGHT, SMALL_BUFF,
...
class ShowTipToTailSum(ShowVectorEquation):
    ...
    def add_kets(self):
        h_ket, v_ket = kets = VGroup(*[
            TexMobject("\\cos(", "2\\pi", "f", "t", ")",
                       "|\\!\\%sarrow\\rangle" % s) for s in ("right", "up")
        ])
        for ket in kets:
    ...
    def show_vector_sum(self):
        ...
        self.play(
            ...
            self.h_vector.set_fill,
            blue_black,
            self.v_vector.set_fill,
            blue_black,
        )
        self.wait()
        self.add(
    ...
    def write_superposition(self):
        ...
        self.play(Write(superposition_words, run_time=2),
                  MoveToTarget(ket_sum))
        self.wait(2)
        self.play(
            FadeIn(sum_word),
            superposition_words.shift,
            MED_LARGE_BUFF * DOWN,
            ket_sum.shift,
            MED_LARGE_BUFF * DOWN,
        )
        self.wait()
        self.play(ReplacementTransform(sum_word, weighted_sum_word))
        self.wait(2)
    ...
    def add_amplitudes(self):
        ...
        h_A_mob, v_A_mob = A_mobs = VGroup(*[
            TexMobject(str(A)).add_background_rectangle() for A in [h_A, v_A]
        ])
        A_mobs.scale(0.8)
        A_mobs.set_color(GREEN)
        h_A_mob.move_to(h_ket, LEFT)
        VGroup(h_ket.target, plus.target).next_to(h_A_mob, RIGHT, SMALL_BUFF)
        ...
        self.play(*it.chain(list(map(
            MoveToTarget, self.ket_sum)), list(map(Write, A_mobs)), [
                UpdateFromAlphaFunc(
                    ov.vector,
                    self.generate_A_update(ov, A * np.array(ov.A_vect),
                                           np.array(ov.A_vect)))
    ...
    def add_phase_shift(self):
        ...
        self.play(
            MoveToTarget(v_ket), Write(plus_phi),
            UpdateFromAlphaFunc(
                v_ov.vector,
                self.generate_phi_update(v_ov, np.array([0, np.pi / 2, 0]),
        ...
        self.play(
            UpdateFromAlphaFunc(
                ...
                )), ellipse.stretch, 0.25, 0, Transform(h_A_mob, new_h_A_mob))
        self.wait(8)
    ...
    def generate_A_update(self, ov, A_vect, prev_A_vect):
        def update(vect, alpha):
            ov.A_vect = interpolate(np.array(prev_A_vect), A_vect, alpha)
            return vect
    ...
    def generate_phi_update(self, ov, phi_vect, prev_phi_vect):
        def update(vect, alpha):
            ov.phi_vect = interpolate(prev_phi_vect, phi_vect, alpha)
            return vect
    ...
    def generate_dashed_line_update(self, v1, v2):
        def update_line(line):
            line.put_start_and_end_on_with_projection(*v1.get_start_and_end())
...
class FromBracketFootnote(Scene):
    def construct(self):
        words = TextMobject("From, ``Bra", "ket", "''", arg_separator="")
...
class AlternateBasis(ShowTipToTailSum):
    ...
    def rotate_plane(self):
        ...
        self.play(
            self.xy_plane.fade,
            0.5,
            self.xy_plane.coordinate_labels.fade,
            1,
            new_plane.restore,
            new_plane.rotate,
            np.pi / 4,
    ...
    def show_vertically_polarized(self):
        self.play(
            UpdateFromAlphaFunc(
                self.h_vector,
                self.generate_A_update(
                    self.h_oscillating_vector, np.array([0.7, 0.7, 0]),
                    np.array(self.h_oscillating_vector.A_vect))),
            UpdateFromAlphaFunc(
                self.v_vector,
                self.generate_A_update(
                    self.v_oscillating_vector, np.array([-0.7, 0.7, 0]),
                    np.array(self.v_oscillating_vector.A_vect))),
        )
        self.wait(8)
...
class ShowPolarizingFilter(DirectionOfPolarizationScene):
    ...
    def mention_energy_absorption(self):
        ...
        self.play(Write(words, run_time=2), *list(map(GrowFromCenter, lines)))
        self.wait(6)
    ...
    def write_as_superposition(self):
        ...
        self.move_camera(
            theta=-0.6 * np.pi,
            added_anims=[Rotate(superposition, -0.6 * np.pi, axis=OUT)])
        rect.set_stroke(YELLOW, 3)
        self.play(ShowCreation(rect))
        arrow = Arrow(rect.get_nadir(),
                      3 * RIGHT + 0.5 * OUT,
                      normal_vector=DOWN)
        ...
        for angle in np.pi / 3, -np.pi / 3, np.pi / 6:
            self.change_polarization_direction(angle)
            self.wait(2)
        self.play(FadeOut(superposition), FadeOut(arrow),
                  *[FadeOut(cu.mobject) for cu in continual_updates])
    ...
    def diagonal_filter(self):
        ...
        self.play(Rotate(self.pol_filter, np.pi / 4, axis=LEFT),
    ...
    def get_superposition_tex(self, angle, s1, s2):
        ...
        def generate_decimal_update(trig_func):
            def update_decimal(decimal):
                new_decimal = DecimalNumber(
                    abs(trig_func(self.reference_line.get_angle() - angle)))
        ...
        continual_updates = [
            Mobject.add_updater(
                A_x,
                generate_decimal_update(np.sin),
            ),
            Mobject.add_updater(
                A_y,
                generate_decimal_update(np.cos),
            ),
        ]
...
class EnergyOfWavesWavePortion(DirectWaveOutOfScreen):
    ...
    def add_components(self):
        ...
        self.play(FadeIn(h_wave.mobject), FadeIn(v_wave.mobject),
                  self.brace.A.move_to, self.brace, self.brace.A.shift,
                  SMALL_BUFF * (2 * UP + IN),
                  ReplacementTransform(self.brace, h_brace), Write(h_brace.A))
        self.wait(6)

        self.play(ReplacementTransform(h_brace.copy(), v_brace),
                  Write(v_brace.A))
        self.wait(6)
    ...
    def scale_wave(self, factor):
        def generate_vect_update(ov):
            ...
            def update(vect, alpha):
                ov.A_vect = interpolate(prev_A, new_A, alpha)
                return vect
        ...
        h_brace.target.stretch_about_point(factor, 1, h_brace.get_bottom())
        v_brace.generate_target()
        v_brace.target.stretch_about_point(factor, 2, v_brace.get_nadir())
        self.play(
            MoveToTarget(h_brace), MoveToTarget(v_brace), *[
...
class EnergyOfWavesTeacherPortion(TeacherStudentsScene):
    ...
    def show_energy_equation(self):
        ...
        energy = TexMobject("\\frac{\\text{Energy}}{\\text{Volume}}", "=",
                            "\\epsilon_0", "A", "^2")
        ...
        self.change_student_modes(*["pondering"] * 3, look_at_arg=dot)
        self.wait(2)
        ...
        self.play(
            ReplacementTransform(
                VGroup(*energy[-4:]).copy(), VGroup(*component_energy[:4])),
            ReplacementTransform(
                VGroup(*energy[-4:]).copy(), VGroup(*component_energy[4:])))
    ...
    def show_both_ways_of_thinking_about_it(self):
        ...
        b1, b2 = [ThoughtBubble(direction=v).scale(0.5) for v in (LEFT, RIGHT)]
        b1.pin_to(s1)
        b2.pin_to(s2)
        ...
        for b, s in (b1, s1), (b2, s2):
            self.play(ShowCreation(b), Write(b.content, run_time=2), s.change,
                      "thinking")
            self.wait(2)
        self.change_student_modes(
            *["plain"] * 3,
            look_at_arg=self.dot,
            added_anims=[self.teacher.change, "raise_right_hand", self.dot])
...
class DescribePhoton(ThreeDScene):
    ...
    def freeze_photon(self):
        self.play(self.photon, rate_func=lambda x: 0.55 * x, run_time=1)
    ...
    def reposition_to_face_photon_head_on(self):
        ...
        self.move_camera(
            phi=np.pi / 2,
            theta=0,
            added_anims=[Rotate(self.equation, -self.camera.get_theta())])
        self.play(Write(plane, run_time=1), Animation(self.equation))
    ...
    def show_components(self):
        ...
        self.play(
            self.equation.rect.set_stroke,
            BLUE,
            4,
            self.equation.words.set_fill,
            WHITE,
            1,
        )
        for part, arrow, d in (h_part, h_arrow, IN), (v_part, v_arrow, UP):
            self.play(part.next_to, arrow.get_center(), d, ShowCreation(arrow))
    ...
    def show_amplitude_and_phase(self):
        ...
        rhs = TexMobject("=", "A_x", "e", "^{i", "(2\\pi", "f", "t", "+",
                         "\\phi_x)}")
        A_rect = SurroundingRectangle(rhs.get_part_by_tex("A_x"),
                                      buff=0.5 * SMALL_BUFF)
        ...
        phase_rect = SurroundingRectangle(VGroup(*rhs[4:]),
                                          buff=0.5 * SMALL_BUFF)
        ...
        self.play(ReplacementTransform(alpha.copy(), new_alpha), FadeIn(rhs))
        for word, rect in A_group, phase_group:
            self.play(ShowCreation(rect), Write(word, run_time=1))
        self.wait()
    ...
    def change_basis(self):
        ...
        A = 1.9
        h_arrow.target = Vector(A * np.cos(np.pi / 12) *
                                rotate_vector(UP, np.pi / 6, RIGHT),
                                normal_vector=RIGHT,
                                color=TEAL)
        v_arrow.target = Vector(A * np.sin(np.pi / 12) *
                                rotate_vector(OUT, np.pi / 6, RIGHT),
                                normal_vector=RIGHT,
                                color=MAROON)
        ...
        h_part.target.next_to(h_arrow.target.get_center(), IN + UP, SMALL_BUFF)
        v_part.target.next_to(v_arrow.target.get_center(), UP, SMALL_BUFF)
    ...
    def write_components(self):
        ...
        self.play(GrowFromCenter(d_brace), Write(d_brace.label))
        self.wait()
        self.play(
    ...
    def describe_via_energy(self):
        ...
        parts = VGroup(
            *[VGroup(*energy[i1:i2]) for i1, i2 in zip(indices, indices[1:])])
        for part in parts:
            bg_rect = BackgroundRectangle(part)
        ...
        self.play(
            Transform(self.d_brace.label.copy(), one.copy(), remover=True),
            Write(parts[1], run_time=1),
        )
        self.wait()
        self.play(Transform(self.h_brace.label[1].copy(),
                            halves[0].copy(),
                            remover=True,
                            rate_func=squish_rate_func(smooth, 0, 0.75)),
                  Transform(self.v_brace.label[1].copy(),
                            halves[1].copy(),
                            remover=True,
    ...
    def components_not_possible_in_isolation(self):
        ...
        self.play(ShowCreation(rect), FadeIn(randy))
        self.play(
            randy.rotate,
            np.pi / 2,
            IN,
    ...
    def change_camera(self):
        ...
        self.move_camera(phi=0.8 * np.pi / 2, theta=-0.3 * np.pi, run_time=2)
        self.play(
            self.photon,
...
class ShootPhotonThroughFilter(DirectionOfPolarizationScene):
    ...
    def ask_what_would_happen(self):
        ...
        self.pol_filter.add(self.pol_filter.arrow.copy().rotate(
            np.pi / 2, OUT))
        ...
        self.play(photon, FadeIn(self.superposition_tex))
    ...
    def expect_half_energy_to_be_absorbed(self):
        ...
        self.play(FadeOut(lines), FadeOut(self.question),
    ...
    def probabalistic_passing_and_blocking(self):
        ...
        self.play(FadeIn(prob), self.get_blocked_photon(), absorption)
        ...
        for should_pass in bools:
            if should_pass:
                ...
                self.play(self.get_blocked_photon(),
                          Animation(self.axes),
                          absorption,
                          run_time=1)
    ...
    def note_change_in_polarization(self):
        words = TextMobject("``Collapses'' \\\\ from",
                            "$|\\!\\nearrow\\rangle$", "to",
                            "$|\\!\\uparrow\\rangle$")
...
class YouCanSeeTheCollapse(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("You can literally \\\\ \\emph{see} the collapse",
                          target_mode="hooray")
...
class ThreeFilters(ShootPhotonThroughFilter):
    ...
    def setup_lines(self):
        lines_group = VGroup(*[
            ...
            for pf1, pf2, ratio in zip([None] + list(self.pol_filters),
                                       list(self.pol_filters) +
                                       [None], [1, 1, 0.5, 0.25])
        ])
        lines = lines_group[0]
    ...
    def fifty_percent_pass_second(self):
        ...
        self.wait(2)
        self.play(FadeOut(l3), Animation(pf2), FadeOut(l2), Animation(pf1),
                  FadeOut(l1))
    ...
    def show_changed_to_diagonal(self):
        ...
        label = brace.get_text("Changed to", "$|\\!\\nearrow\\rangle$")
        ...
        self.move_camera(
            phi=np.pi / 2,
            theta=0,
            added_anims=[Animation(VGroup(*self.pol_filters[:2]))] + [
                Rotate(
                    v, np.pi / 2, axis=v.get_vector(), in_place=True, **kwargs)
                for v in photon.mobject
            ] + [
                Animation(self.pol_filters[2]),
                Rotate(label, np.pi / 2, axis=OUT, in_place=True, **kwargs),
            ],
            **kwargs)
        self.wait()
    ...
    def fifty_percent_to_pass_third(self):
        ...
        new_photon = self.get_photon(rate_func=lambda x: (1 - a) * x + a,
                                     run_time=1)

        self.revert_to_original_skipping_status()
        self.play(ShowCreation(arrow), Write(label, run_time=1))
    ...
    def show_lines_with_middle(self):
        ...
        self.play(FadeIn(l4), Animation(pf3), FadeIn(l3), Animation(pf2),
                  FadeIn(l2), Animation(pf1), FadeIn(l1),
    ...
    def remove_middle_then_put_back(self):
        ...
        self.remove(l2, l3)
        self.play(FadeOut(l4), Animation(pf3), FadeOut(l3),
                  ApplyMethod(mover.shift, 3 * OUT, rate_func=running_start),
                  ReplacementTransform(l2.copy(), mid_lines), Animation(pf1),
                  Animation(l1))
        self.play(ShowCreation(arrow), Write(labels[0], run_time=1))
        self.wait(2)
        self.play(FadeIn(l4), Animation(pf3), FadeOut(mid_lines),
                  FadeIn(l3), mover.shift, 3 * IN, FadeIn(l2), Animation(pf1),
                  Animation(l1))
    ...
    def get_lines(self, filter1=None, filter2=None, ratio=1.0):
        ...
        indices_to_block = random.sample(list(range(self.n_lines)), n_to_block)
...
class PhotonAtSlightAngle(ThreeFilters):
    ...
    def write_angle(self):
        ...
        self.play(FadeOut(self.pol_filter), ShowCreation(arc),
    ...
    def write_components(self):
        ...
        v_arrow = Vector(self.radius * np.cos(np.pi / 8) * UP, color=GREEN)
        ...
        sin_brace.label = sin_brace.get_tex("%.2f" % np.sin(np.pi / 8),
                                            buff=SMALL_BUFF)
        cos_brace = Brace(state.get_part_by_tex("cos"), DOWN, buff=SMALL_BUFF)
        cos_brace.label = cos_brace.get_tex("%.2f" % np.cos(np.pi / 8),
                                            buff=SMALL_BUFF)
        ...
        self.play(GrowFromCenter(d_brace), Write(d_brace.label))
        self.wait()
        self.play(GrowFromPoint(h_arrow, ORIGIN), Write(h_label, run_time=1))
        self.play(
            Write(VGroup(*state[:2])),
            ReplacementTransform(h_label.copy(), state.get_part_by_tex("sin")),
            ReplacementTransform(h_arrow.copy(),
                                 state.get_part_by_tex("rightarrow")),
            Write(state.get_part_by_tex("+")))
        self.play(GrowFromCenter(sin_brace), Write(sin_brace.label,
                                                   run_time=1))
        self.wait()
        self.play(GrowFromPoint(v_arrow, h_arrow.get_end()),
                  Write(v_label, run_time=1))
        self.play(
            ReplacementTransform(v_label.copy(), state.get_part_by_tex("cos")),
            ReplacementTransform(v_arrow.copy(),
                                 state.get_part_by_tex("uparrow")),
        )
        self.play(GrowFromCenter(cos_brace), Write(cos_brace.label,
                                                   run_time=1))
        self.wait()
    ...
    def classical_energy_conception(self):
        ...
        h_content = TexMobject("0.38", "^2", "= 0.15", "\\text{ energy}\\\\",
                               "\\text{in the }", "\\rightarrow",
                               "\\text{ direction}")
        ...
        v_content = TexMobject("0.92", "^2", "= 0.85", "\\text{ energy}\\\\",
                               "\\text{in the }", "\\uparrow",
                               "\\text{ direction}")
        ...
        self.play(
            ReplacementTransform(self.sin_brace.label.copy(), h_content[0]),
            ReplacementTransform(
        ...
        self.play(
            ReplacementTransform(self.cos_brace.label.copy(), v_content[0]),
            ReplacementTransform(
        ...
        self.wait(2)
        self.play(FadeOut(randy), FadeOut(bubble), FadeOut(v_content),
                  Transform(h_content, alt_h_content), FadeIn(self.pol_filter),
                  Animation(self.arc))
    ...
    def reposition_camera_back(self):
        self.move_camera(phi=0.8 * np.pi / 2,
                         theta=-0.6 * np.pi,
                         added_anims=[
    ...
    def rewrite_15_percent_meaning(self):
        ...
        self.play(
            finish_photon,
            ApplyMethod(self.pol_filter.set_color,
                        RED,
    ...
    def probabalistic_passing(self):
        ...
        self.play(ShowCreation(l2, run_time=0.5, **kwargs),
                  Animation(self.pol_filter), Animation(l1))
        self.wait()
    ...
    def get_blocked_photon(self, **kwargs):
        return self.get_photon(filter_distance=FRAME_X_RADIUS + 3,
                               get_filtered=True,
                               **kwargs)
...
class CompareWaveEquations(TeacherStudentsScene):
    ...
    def show_complex_plane(self):
        ...
        alpha_dot, beta_dot = [
            Dot(plane.coords_to_point(x, 0.5), radius=0.05, color=color)
        ...
        rhs = TexMobject("=", "A_y", "e", "^{i(", "2\\pi", "f", "t", "+",
                         "\\phi_y", ")}")
        rhs.scale(0.7)
        ...
        self.play(Write(rhs), ShowCreation(A_line), ShowCreation(arc))
        self.play(phi_copy.next_to, arc, RIGHT, SMALL_BUFF, phi_copy.shift,
                  0.5 * SMALL_BUFF * UP)
        self.play(
            A_copy.next_to,
            A_line.get_center(),
    ...
    def show_interpretations(self):
        ...
        self.play(FadeOut(self.state_words), Write(c_words),
                  self.teacher.change, "happy")
        self.change_student_modes(*["happy"] * 3, look_at_arg=c_words)
...
class CircularPhotons(ShootPhotonThroughFilter):
    ...
    def show_vertically_polarized_light(self):
        ...
        prob = TexMobject("P(", "\\text{passing}", ")", "=", "\\left(",
                          "\\frac{-i}{\\sqrt{2}}", "\\right)^2")
        ...
        v_photon = WavePacket(em_wave=em_wave,
    ...
    def get_circular_photon(self, **kwargs):
        ...
        photon.E_func = lambda x: np.exp(-0.25 *
                                         (2 * np.pi * x / photon.width)**2)
        return photon


class ClockwisePhotonInsert(Scene):
    def construct(self):
        eq = TexMobject("\\left| \\frac{-i}{\\sqrt{2}} \\right|^2")
        eq.set_color(BLUE)
