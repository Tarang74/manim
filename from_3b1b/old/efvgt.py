...
def get_confetti_animations(num_confetti_squares):
    ...
    confetti_spirils = [
        ConfettiSpiril(
            ...
            rate_func=squish_rate_func(lambda t: t, a, a + 0.5)) for a, square
        in zip(np.linspace(0, 0.5, num_confetti_squares), confetti_squares)
    ]
...
class Anniversary(TeacherStudentsScene):
        ...
        self.play(
            DrawBorderThenFill(
                ...
                Succession(Animation(pi, run_time=2),
                           ...
                           rate_func=linear) for pi in self.get_students()
            ] + [
    ...
    def get_party_hats(self):
        ...
        max_angle = np.pi / 6
        for hat in hats:
            hat.rotate(random.random() * 2 * max_angle - max_angle,
...
class JustGiveMeAQuickExplanation(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(*it.chain(*[[
            pi.change_mode, "hesitant", pi.look_at,
...
class QuickExplanation(ComplexTransformationScene):
    ...
    def add_vectors(self):
        ...
        circle = Circle(radius=self.z_to_point(1)[0],
                        color=self.position_color)

        self.wait(2)
        self.play(ShowCreation(s_vector))
        self.play(
            ReplacementTransform(s_vector.copy(), v_vector,
                                 path_arc=np.pi / 2))
        self.wait()
...
class SymmetriesOfSquare(ThreeDScene):
    ...
    def add_labels(self):
        ...
        self.play(FadeIn(self.square.randy), self.randy.change_mode, "happy",
    ...
    def show_full_group(self):
        ...
        all_squares = VGroup(
            *[self.square.copy().scale(0.5) for x in range(8)])
        ...
        self.play(
            *[ApplyMethod(mob.set_color, GREY) for mob in self.title[1:]])

        for square, angle in zip(all_squares[1:4],
        ...
        for square, axis in zip(bottom_squares,
    ...
    def show_top_actions(self):
        ...
        self.play(*[
            Rotate(square,
                   ...
                   **square.rotation_kwargs) for square in all_squares[1:4]
        ])
        self.wait()

    def show_bottom_actions(self):
        for square in self.all_squares[4:]:
            self.rotate_square(square=square,
    ...
    def name_dihedral_group(self):
        new_title = TextMobject("``Dihedral group'' of order 8")
    ...
    def get_rotation_arcs(self, square, angle, angle_buff=SMALL_BUFF):
        square_radius = get_norm(square.points[0] - square.get_center())
        arc = Arc(radius=square_radius + SMALL_BUFF,
        ...
        arcs = VGroup(
            arc,
            *[arc.copy().rotate(i * np.pi / 2) for i in angle_multiple_range])
...
class CircleSymmetries(Scene):
    ...
    def show_range_of_angles(self):
        ...
        for rate_func in smooth, lambda t: smooth(1 - t):
            self.play(Rotate(self.circle, 2 * np.pi - 0.001),
    ...
    def add_radial_line(self):
        radius = Line(self.circle.get_center(),
...
class GroupOfCubeSymmetries(ThreeDScene):
    ...
    def get_cube(self):
        ...
        cube.apply_function(lambda p: np.dot(p, pose_matrix.T),
                            maintain_smoothness=False)
        return cube

    def get_pose_matrix(self):
        return np.dot(rotation_matrix(np.pi / 8, UP),
...
class AddSquareSymmetries(SymmetriesOfSquare):
    def construct(self):
        ...
        self.wait()
        self.play(FadeOut(arcs))
        self.flip_square(square=square,
                         axis=UP,
        ...
        self.play(square.next_to, equals, LEFT, LARGE_BUFF, alt_square.restore,
                  Write(equals))
        self.flip_square(
            square=alt_square,
            axis=UP + RIGHT,
...
class AddCircleSymmetries(CircleSymmetries):
    def construct(self):
        ...
        self.play(circle.next_to, equals, LEFT, LARGE_BUFF, alt_circle.restore,
                  Write(equals))
...
class AddCubeSymmetries(GroupOfCubeSymmetries):
    ...
    def construct(self):
        ...
        equation = cube1, plus, cube2, equals, cube3 = VGroup(
            cube, TexMobject("+"), cube.copy(), TexMobject("="), cube.copy())
        ...
        self.play(cube_copy.move_to, cube2, cube_copy.set_fill, None,
                  self.cube_opacity, Write(plus))
        ...
        self.wait()
        for cube, (angle, axis) in zip([cube1, cube_copy, cube3],
                                       angle_axis_pairs):
            self.rotate_cube(cube,
                             -angle,
                             axis,
                             add_arrows=False,
                             rate_func=there_and_back,
    ...
    def rotate_cube(self, cube, angle, axis, add_arrows=True, **kwargs):
        ...
        if add_arrows:
            arrows = VGroup(*[
                Arc(start_angle=np.pi / 12 + a,
                    angle=5 * np.pi / 6,
                    color=YELLOW).add_tip() for a in (0, np.pi)
            ])
            arrows.set_height(1.5 * cube.get_height())
            z_to_axis = z_to_vector(axis)
            arrows.apply_function(lambda p: np.dot(p, z_to_axis.T),
                                  maintain_smoothness=False)
            arrows.move_to(cube)
            arrows.shift(-axis * cube.get_height() / 2 / get_norm(axis))
            anims += list(map(ShowCreation, arrows))
        anims.append(
            Rotate(cube, axis=axis, angle=angle, in_place=True, **kwargs))
...
class DihedralGroupStructure(SymmetriesOfSquare):
    ...
    def demonstrate_sum(self, angle_axis_pairs):
        angle_axis_pairs = list(angle_axis_pairs) + [
            get_composite_rotation_angle_and_axis(*list(zip(
                *angle_axis_pairs)))
        ]
        ...
        expression = s1, plus, s2, equals, s3 = VGroup(
            prototype_square,
            TexMobject("+").scale(2), prototype_square.copy(),
            TexMobject("=").scale(2), prototype_square.copy())
...
class ThisIsAVeryGeneralIdea(Scene):
    def construct(self):
        ...
        self.play(*it.chain(list(map(ShowCreation, sub_lines)),
...
class AdditiveGroupOfReals(Scene):
    ...
    def show_example_slides(self, *nums):
        for num in nums:
            ...
            self.play(self.number_line.shift,
    ...
    def comment_on_zero_as_identity(self):
        ...
        words.shift(line.get_top() + MED_SMALL_BUFF * UP -
                    words[0].get_bottom())
        ...
        for num_pair in num_pairs:
            ...
            for num in num_pair:
                ...
                if len(num_mobs) > 0:
                    ...
                    if x < last_num_mob.get_right(
                    )[0] and x > last_num_mob.get_left()[0]:
...
class AdditiveGroupOfComplexNumbers(ComplexTransformationScene):
    ...
    def show_vertical_slide(self):
        dots = VGroup(
            *[Dot(self.z_to_point(complex(0, i))) for i in range(1, 4)])
    ...
    def show_example_addition(self):
        ...
        label2 = TexMobject("%d %di" % (z2.real, z2.imag))
        ...
        real_arrow, imag_arrow = component_arrows = [
            Vector(self.z_to_point(z), color=ADDER_COLOR) for z in [
                z1.real + z2.real,
                complex(0, z1.imag + z2.imag),
            ]
        ]
        imag_arrow.shift(real_arrow.get_end())
        plus = TexMobject("+").next_to(real_arrow.get_center(), UP + RIGHT)
        ...
        self.play(
            ...
            ), Write(plus), *list(map(MoveToTarget, [rp1, rp2])))
        self.wait()
        self.play(
    ...
    def write_group_name(self):
        title = TextMobject("Additive", "group of", "complex numbers")
    ...
    def slide(self, z, *added_anims, **kwargs):
        ...
        self.play(ApplyMethod(self.plane.shift, self.z_to_point(z), **kwargs),
                  *added_anims)
...
class SchizophrenicNumbers(Scene):
    def construct(self):
        ...
        self.play(*[
            MoveToTarget(
                ...
                run_time=2,
            ) for number, alpha in zip(numbers,
    ...
    def blink_numbers(self, numbers):
        self.play(*[
            ...
            for num, alpha in zip(numbers[1:], 0.8 *
                                  np.random.random(len(numbers)))
        ])
...
class MultiplicativeGroupOfReals(AdditiveGroupOfReals):
    ...
    def add_number_line(self):
        ...
        self.one = [
            m for m in self.number_line.numbers if m.get_tex_string() is "1"
        ][0]
    ...
    def follow_one(self):
        ...
        three_line, half_line = [
            DashedLine(self.number_line.number_to_point(num),
                       self.shadow_line.number_to_point(num))
            for num in (3, 0.5)
        ]
        three_mob = [
            m for m in self.shadow_line.numbers if m.get_tex_string() == "3"
        ][0]
    ...
    def every_positive_number_association(self):
        ...
        self.wait()
        self.play(ReplacementTransform(third_line, one_line),
                  self.number_line.restore,
    ...
    def stretch(self, factor, run_time=2, **kwargs):
        ...
        self.play(Transform(self.number_line, target, **kwargs),
...
class MultiplicativeGroupOfComplexNumbers(AdditiveGroupOfComplexNumbers):
    ...
    def add_title(self):
        title = TextMobject("Multiplicative", "group of", "complex numbers")
        title.to_edge(UP)
    ...
    def show_action_at_i_again(self):
        neg_one_label = [
            m for m in self.real_labels if m.get_tex_string() == "-1"
        ][0]
        half_turn_arc = Arc(start_angle=np.pi / 12,
                            angle=10 * np.pi / 12,
        ...
        self.multiply_by_z(complex(0, 1), run_time=3)
        self.wait()
        self.play(Transform(self.turn_arrow, half_turn_arc,
                            path_arc=np.pi / 2))
        self.wait()
    ...
    def show_i_squared_is_negative_one(self):
        ...
        self.wait()
        for term in terms[:2]:
            self.multiply_by_z(complex(0, 1),
                               added_anims=[
                                   Animation(equation),
    ...
    def talk_through_specific_example(self):
        ...
        self.multiply_by_z(
            ...
            added_anims=[ShowCreation(arc, run_time=2),
                         Write(angle_label)])
    ...
    def multiply_by_z(self, z, run_time=2, **kwargs):
        ...
        self.play(Transform(self.plane, target, run_time=run_time, **kwargs),
                  *kwargs.get("added_anims", []))
...
class ExponentsAsRepeatedMultiplication(TeacherStudentsScene):
    ...
    def show_repeated_multiplication(self):
        ...
        self.play(*[
            ReplacementTransform(mob.copy(), term, run_time=2) for term, mob in
            zip(general_equation, [lhs, three_twos.exp, five_twos.exp])
        ])
        self.wait(2)
    ...
    def show_non_counting_exponents(self):
        self.play(self.expanded_exponential_rule.scale, 0.5,
        ...
        half_expression = TexMobject("\\big(", "2^{1/2}", "\\big)",
                                     "\\big(2^{1/2}\\big) = 2^{1}")
        neg_one_expression = TexMobject("\\big(", "2^{-1}", "\\big)",
                                        "\\big( 2^{1} \\big) = 2^{0}")
        ...
        self.wait()
        self.play(Transform(neg_power, neg_one_expression[1]),
        ...
        self.wait(2)
        self.play(imag_power.move_to, UP, imag_power.scale_in_place, 1.5,
                  imag_power.set_color, BLUE, self.exponential_rule.to_edge,
                  RIGHT,
        ...
        arrow = Arrow(group_theory_words,
...
class ExponentsAsHomomorphism(Scene):
    ...
    def comment_on_equation(self):
        equation = TexMobject("2", "^{x", "+", "y}", "=", "2^x", "2^y")
    ...
    def show_adders(self):
        ...
        self.play(adders.scale, 1.5, adders.center,
                  adders.space_out_submobjects, 2, adders.to_edge, UP,
                  adders.set_color, GREEN_B, FadeOut(self.lhs_brace_group),
                  Write(top_line))
        self.wait()
        for x in 3, 5, -8:
    ...
    def confused_at_mapping(self):
        arrow = Arrow(self.top_line.get_bottom()[1] * UP,
        ...
        self.wait()
        for x in 1, -2, 3, 1, -3:
            self.play(self.get_stretch_anim(self.bottom_line, 2**x),
                      self.top_line.shift,
                      x * RIGHT,
                      randy.look_at,
                      self.top_line,
    ...
    def talk_through_composition(self):
        ...
        self.play(self.equation.next_to, ORIGIN, LEFT, MED_LARGE_BUFF,
                  randy.change_mode, "pondering", randy.look_at, self.equation)
        self.wait()
        self.play(randy.look_at, self.top_line)
        self.show_composition(*inputs,
                              parallel_anims=list(
                                  map(MoveToTarget, self.adders)))
        self.play(
        ...
        self.show_composition(*inputs,
                              parallel_anims=list(
                                  map(MoveToTarget, self.multipliers)))
        self.wait()
    ...
    def show_composition(self, *inputs, **kwargs):
        ...
        for x, parallel_anim in zip(inputs, parallel_anims):
            ...
            self.wait()
        self.play(
            *[line.restore for line in (self.top_line, self.bottom_line)])
...
class DihedralCubeHomomorphism(GroupOfCubeSymmetries, SymmetriesOfSquare):
    def construct(self):
        ...
        VGroup(square, arrow, cube).next_to(equation,
                                            DOWN,
                                            buff=MED_LARGE_BUFF)
...
class ComplexExponentiationAdderHalf(ComplexExponentiationAbstract,
...
class ComplexExponentiationMultiplierHalf(ComplexExponentiationAbstract,
    ...
    def take_steps_for_e(self):
        ...
        for last_angle, angle in zip([0] + angles, angles):
            ...
            if curr_brace is None:
                ...
                self.play(ReplacementTransform(brace, curr_brace),
                          Write(curr_label))
...
class ECLPromo(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(logo_part2.scale_in_place, 0.5, logo_part2.to_edge, UP)
...
class ExpTransformation(ComplexTransformationScene):
    ...
    def construct(self):
        ...
        cylinder = self.plane.copy().apply_function(lambda x_y_z: np.array(
