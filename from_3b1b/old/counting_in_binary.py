...
class Hand(ImageMobject):
    ...
    def __init__(self, num, small=False, **kwargs):
        ...
        path = os.path.join(VIDEO_DIR, MOVIE_PREFIX, "images",
                            "Hand%d.png" % num)
        invert = False
...
class LeftHand(Hand):
    def __init__(self, num, **kwargs):
        Hand.__init__(self, read_reversed_binary(five_char_binary(num)),
                      **kwargs)
...
class ShowCounting(OverHand):
    def construct(self):
        ...
        self.frames = [
            disp.paint_mobject(self.get_counting_mob(32 * count // num_frames),
                               frame)
...
class CountTo1023(Scene):
    def construct(self):
        ...
        self.frames = [
            disp.paint_mobject(
                Mobject(rh_map[count % 32], lh_map[count // 32],
                        get_num(count))) for count in range(2**10)
        ]
...
class ShowReadingRule(Scene):
    ...
    def read_sample(self, num):
        ...
        if num in [6, 17]:
            hand.shift(0.8 * LEFT)
        sum_mobs = TexMobject(" + ".join([str(2**c)
                                          for c in counts]).split(" ") +
                              ["=%d" % num]).to_corner(UP + RIGHT).split()
        self.add(hand, *count_mobs)
        self.wait()
        self.play(*[
            Transform(count_mobs[n / 2], sum_mobs[n]) if n %
            2 == 0 and n / 2 < len(counts) else FadeIn(sum_mobs[n])
...
class ShowIncrementRule(Scene):
    def construct(self):
        ...
        ranges = [(0, 14, False), (14, 28, False), (12, 13, True),
                  (29, 31, True), (27, 29, True), (23, 24, True),
                  (14, 20, False), (20, 26, False)]
        oh = OverHand()
        for phrase, (start, end, pause) in zip(phrases, ranges):
            ...
            self.frames += [
                disp.paint_mobject(phrase, frame) for frame in
                oh.frames[COUNT_TO_FRAME_NUM[start]:COUNT_TO_FRAME_NUM[end]]
            ]
            if pause:
                self.frames += [self.frames[-1]] * int(
...
class MindFindsShortcuts(Scene):
    def construct(self):
        ...
        self.play(
            Transform(compound,
                      Arrow(hand, seven).set_color("yellow")),
            ShimmerIn(
                TextMobject("Directly recognize").shift(1.5 * DOWN +
                                                        2 * RIGHT)))
        self.wait()

        self.clear()
        hands = dict([(num, Hand(num).center().scale(0.5).shift(DOWN))
                      for num in [23, 16, 7]])
        ...
        for num in 7, 16:
            hands[num].add(
                TexMobject(str(num)).shift(hands[num].get_top() + 0.5 * UP))
        ...
        self.play(
            Transform(
                deepcopy(hands[16]).set_color("black").center().shift(
                    hands[23].get_center()), hands[16]),
            Transform(
                deepcopy(hands[7]).set_color("black").center().shift(
                    hands[23].get_center()), hands[7]), Animation(hands[23]),
            FadeIn(equals), FadeIn(plus))
        self.wait()
...
class CountingExampleSentence(ShowCounting):
    def construct(self):
        ...
        self.words = TextMobject(words.split(),
                                 size="\\Huge").scale(0.7).to_corner(
                                     UP + LEFT, buff=0.25).split()
...
class FinishCountingExampleSentence(Scene):
    def construct(self):
        ...
        words = TextMobject(words,
                            size="\\Huge").scale(0.7).to_corner(UP + LEFT,
                                                                buff=0.25)
        hand = Hand(18)
