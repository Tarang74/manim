...
class EquationSolver1d(GraphScene, ZoomedScene):
    ...
    def solveEquation(self):
        ...
        for i in range(self.num_iterations):
            ...
            x_guess_label_caption = TextMobject("New guess: x = ",
                                                fill_color=midColor)
            ...
            self.play(ReplacementTransform(leftBrace.copy(), midXBrace),
    ...
    def construct(self):
        ...
        for i in range(len(walk_coords)):
            ...
            if self.display_wind:
                ...
                start_wind = 0 if i + 1 in self.wind_reset_indices else number_update_func(
                    1)
    ...
    def construct(self):
        ...
        def Animate2dSolver(cur_depth,
            ...
            def draw_line_return_wind(start,
                ...
                a0 = alpha_winder(0)
                rebased_winder = lambda alpha: alpha_winder(alpha
                                                            ) - a0 + start_wind
                colored_line = Line(
            ...
            if self.show_cursor:
                ...
                cursor_anim = Succession(FadeIn(cursor, run_time=0.1),
            ...
            if total_wind == 0:
                ...
                children = [
                    Animate2dSolver(cur_depth=cur_depth + 1,
                                    rect=sub_rect,
                                    dim_to_split=1 - dim_to_split,
                                    sides_to_draw=[side_to_draw],
                                    manual_wind_override=child(
                                        manual_wind_override, index))
                    for (index,
                         (sub_rect,
        ...
        node = Animate2dSolver(cur_depth=0,
                               rect=rect,
                               dim_to_split=0,
                               sides_to_draw=[],
...
class OdometerScene(ColorMappedObjectsScene):
    ...
    def construct(self):
        ...
        self.play(FuncRotater(base_arrow,
                              rev_func=lambda x: -self.rotate_func(x)),
...
class SignsExplanation(Scene):
    def construct(self):
        ...
        pos_arrow = Arrow(num_line.number_to_point(0),
                          num_line.number_to_point(pos_num),
                          buff=0,
                          color=positive_color)
        neg_arrow = Arrow(num_line.number_to_point(0),
...
class HasItsLimitations(Scene):
    ...
    def construct(self):
        ...
        new_curved_arrow.shift((new_input_dot.get_center() -
                                new_curved_arrow.points[0]) - input_diff)
        ...
        dot_objects = Group(input_dot, input_label, output_dot, output_label,
                            curved_arrow)
        new_dot_objects = Group(new_input_dot, new_input_label, new_output_dot,
...
class Initial2dFuncSceneMorphing(Initial2dFuncSceneBase):
    ...
    def show_planes(self):
        ...
        self.play(ReplacementTransform(self.num_plane.copy(), right_plane),
...
class DemonstrateColorMapping(ColorMappedObjectsScene):
    ...
    def construct(self):
        ...
        if self.show_full_color_map:
            bright_background = Rectangle(width=2 * FRAME_X_RADIUS + 1,
...
class LoopSplitScene(ColorMappedObjectsScene):
    ...
    def construct(self):
        ...
        left_square_trips = [
            tl_line_trip, midline_left_trip, bl_line_trip, left_line_trip
        ]
        ...
        right_square_trips = [
            tr_line_trip, right_line_trip, br_line_trip, midline_right_trip
        ]
        ...
        def line_fade(start, end, mob):
            return UpdateFromAlphaFunc(
                mob, lambda m, a: m.set_stroke(width=interpolate(
                    start, end, a) * stroke_width))
...
diff_func = lambda x: np.cos(1.4 * (x - 0.1) *
...
class UhOhScene(EquationSolver2d):
    CONFIG = {
        "func": uhOhFunc,
        "manual_wind_override":
        (1, None, (1, None, (1, None, None))),  # Tailored to UhOhFunc above
        "show_winding_numbers": False,
        "num_iterations": 5,
    }
