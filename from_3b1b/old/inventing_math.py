...
PARTIAL_CONVERGENT_SUMS_TEXT = [
    "\\frac{1}{2}", "", "", ",\\quad", "\\frac{1}{2} + \\frac{1}{4}", "=",
    "\\frac{3}{4}", ",\\quad", "\\frac{1}{2} + \\frac{1}{4} + \\frac{1}{8}",
    "=", "\\frac{7}{8}", ",\\quad",
...
ALT_PARTIAL_SUM_TEXT = reduce(op.add, [
    ...
    for n in range(1,
                   len(CONVERGENT_SUM_TERMS) + 1)
]) + [
    "\\vdots", "&", "\\\\", "1.0", "&=",
    "+".join(CONVERGENT_SUM_TERMS) + "+\\cdots+\\frac{1}{2^n}+\\cdots"
]
...
def zero_to_one_interval():
    interval = NumberLine(radius=INTERVAL_RADIUS,
                          interval_size=2.0 * INTERVAL_RADIUS /
                          NUM_INTERVAL_TICKS)
...
class FlipThroughNumbers(Animation):
    ...
    def interpolate_mobject(self, alpha):
        new_number = self.function(self.start + int(alpha *
                                                    (self.end - self.start)))
...
class IntroduceDivergentSum(Scene):
    def construct(self):
        ...
        sum_value = None
        brace = Underbrace(equation[0].get_boundary_point(DOWN + LEFT),
                           equation[1].get_boundary_point(DOWN + RIGHT)).shift(
                               0.2 * DOWN)
        ...
        end_brace = deepcopy(brace).stretch_to_fit_width(
            max(ellipses.points[:, 0]) - min_x_coord).to_edge(
                LEFT, buff=FRAME_X_RADIUS + min_x_coord)
...
class ClearlyNonsense(Scene):
    def construct(self):
        ...
        neg_1_arrow = Arrow((-1, 0.3, 0),
                            tail=how_here.get_center() + 0.5 * DOWN)
        right_arrow = Arrow((FRAME_X_RADIUS - 0.5) * RIGHT + DOWN,
...
class OutlineOfVideo(Scene):
    def construct(self):
        ...
        overbrace = Underbrace(
            conv_sum.get_left(), conv_sum.get_right()).rotate(
                np.pi, RIGHT).shift(0.75 * UP * conv_sum.get_height())
        dots = conv_sum.split()[-2].set_color("green")
        dots.sort_points()
        arrow = Arrow(dots.get_bottom(), direction=UP + LEFT)
        ...
        for mob in div_sum, u_brace:
            mob.shift(DOWN)
        texts = [
            TextMobject(words).set_color("yellow") for words in [
                "1. Discover this", "2. Clarify what this means",
                "3. Discover this", ["4. Invent ", "\\textbf{new math}"]
            ]
        ]
        ...
        groups = [[texts[0], overbrace, conv_sum], [texts[1], arrow, dots],
                  [texts[2], u_brace, div_sum], [texts[3]]]
        for group in groups:
            self.play(*[DelayByOrder(FadeIn(element)) for element in group])
            self.wait()
...
class YouAsMathematician(Scene):
    def construct(self):
        ...
        explanation = TextMobject(
            ...
            size="\\small").shift([2, you.get_center()[1], 0])
        ...
        self.play(ShowCreation(arrow), BlinkPiCreature(you))
        self.wait()
        self.play(ShowCreation(bubble))
        for part in equation_parts:
            self.play(DelayByOrder(FadeIn(part)), run_time=0.5)
        self.wait()
        self.play(BlinkPiCreature(you), FadeOut(explanation), FadeOut(arrow))
        ...
        self.add(*dot_pair)
        two_arrows = [
            Arrow(x, direction=x).shift(UP).nudge() for x in (LEFT, RIGHT)
        ]
        ...
        self.play(ApplyPointwiseFunction(
            lambda p: 3 * FRAME_X_RADIUS * p / get_norm(p), everything),
                  *[
                      Transform(dot,
                                deepcopy(dot).shift(DOWN).scale(3))
                      for dot in dot_pair
                  ],
                  run_time=2.0)
        self.wait()
...
class ZoomInOnInterval(Scene):
    def construct(self):
        ...
        new_line.set_color(
            "black",
            lambda x_y_z1: x_y_z1[0] < 0 or x_y_z1[0] > 1 or x_y_z1[1] < -0.2)
        ...
        self.play(
            ...
            ApplyMethod(interval[0].apply_function,
                        squish,
...
class DanceDotOnInterval(Scene):
    def construct(self, mode):
        ...
        self.add(interval)
        self.play(*[ApplyMethod(dot.shift, DOWN) for dot in dots])
        self.wait()
        for count in range(num_written_terms):
            shift_val = 2 * RIGHT * INTERVAL_RADIUS * (1 - prop) * (prop**
                                                                    count)
            start = dots[0].get_center()
            line = Line(start, start + shift_val * RIGHT)
            line.set_color(next(color_range))
            self.play(ApplyMethod(dots[0].shift, shift_val),
                      ShowCreation(line))
            num = conv_sum[count]
            ...
            if num.get_width() > line.get_length():
                ...
                arrow = Arrow(line.get_center() + 2 * DOWN,
                              tail=num.get_center() +
                              0.5 * num.get_height() * UP)
            self.play(
    ...
    def write_partial_sums(self):
        ...
        for x in range(0, len(partial_sum_parts), 4):
            ...
            self.play(*[FadeIn(partial_sum_parts[y]) for y in range(x, x + 4)])
        self.wait(2)
...
class OrganizePartialSums(Scene):
    def construct(self):
        ...
        pure_sums = [
            partial_sum_parts[x] for x in range(0, len(partial_sum_parts), 4)
        ]
        ...
        self.wait()
        self.play(*[
            ClockwiseTransform(*pair) for pair in zip(pure_sums, new_pure_sums)
        ] + [
            FadeOut(mob) for mob in partial_sum_parts if mob not in pure_sums
        ])
        ...
        infinite_sum = TexMobject("".join(CONVERGENT_SUM_TEXT[:-1]),
                                  size="\\samll")
...
class SeeNumbersApproachOne(Scene):
    def construct(self):
        ...
        self.add(interval)
        self.play(ShowCreation(arrow), ShowCreation(dots), run_time=2.0)
        self.wait()
...
class OneAndInfiniteSumAreTheSameThing(Scene):
    def construct(self):
        one, equals, inf_sum = TexMobject(
            ["1", "=", "\\sum_{n=1}^\\infty \\frac{1}{2^n}"]).split()
        ...
        self.wait()
        self.play(ApplyMethod(one.shift, RIGHT),
...
class HowDoYouDefineInfiniteSums(Scene):
    def construct(self):
        ...
        text = TextMobject(["How", " do", " you,\\\\", "\\emph{define}"],
                           size="\\Huge").shift(UP).split()
...
class ListOfPartialSums(Scene):
    def construct(self):
        all_terms = np.array(
            TexMobject(ALT_PARTIAL_SUM_TEXT, size="\\large").split())
        numbers, equals, sums = [
            all_terms[list(range(k, 12, 3))] for k in (0, 1, 2)
        ]
        dots = all_terms[12]
        one = all_terms[-3]
        ...
        self.count(numbers, mode="show", display_numbers=False, run_time=1.0)
        self.play(ShowCreation(dots))
        self.wait()
        self.play(
...
class ShowDecreasingDistance(Scene):
    ...
    def construct(self, num):
        ...
        self.wait()
        self.play(ApplyMethod(vert_line0.shift, RIGHT),
                  Transform(
                      horiz_line,
                      Line(vert_line0.end + RIGHT,
                           vert_line1.end).set_color("green")),
                  ShowCreation(dots),
                  run_time=2.5)
        self.wait()
...
class CircleZoomInOnOne(Scene):
    def construct(self):
        ...
        self.play(Transform(circle,
                            Point(2 * RIGHT).set_color("white")),
                  run_time=5.0)

        self.play(*[DelayByOrder(FadeIn(mob)) for mob in (arrow, curr_num)])
        self.wait()
        for num in numbers[1:] + [text]:
            curr_num.points = np.array(list(reversed(curr_num.points)))
            self.play(
                ShowCreation(curr_num, rate_func=lambda t: smooth(1 - t)),
                ShowCreation(num))
...
class ZoomInOnOne(Scene):
    def construct(self):
        ...
        zero, one, two = [
            TexMobject(str(n)).scale(0.5).shift(0.4 * DOWN + 2 *
                                                (-1 + n) * RIGHT)
            for n in (0, 1, 2)
        ]
        self.play(FadeOut(nl_with_nums),
        ...
        for count in range(num_iterations):
            self.zoom_with_numbers(number_pairs[count],
                                   number_pairs[count + 1])
            self.clear()
            self.add(one)

    def zoom_with_numbers(self, numbers, next_numbers):
        all_numbers = [
            TexMobject(str(n_u[0])).scale(0.5).shift(0.4 * DOWN +
                                                     2 * n_u[1] * RIGHT)
            for n_u in zip(numbers + next_numbers, it.cycle([-1, 1]))
        ]
        ...
        number_lines = [
            NumberLine(
                interval_size=1,
                density=scale_factor * DEFAULT_POINT_DENSITY_1D).filter_out(
                    lambda x_y_z: abs(x_y_z[1]) > 0.1).scale(
                        1.0 / scale_factor**x) for x in range(num_levels)
        ]
        ...
        self.play(*[
            ...
            ApplyMethod(all_numbers[i].shift, 2 * LEFT * (scale_factor - 1) *
                        (-1)**i, **kwargs) for i in (0, 1)
        ] + [
...
class DefineInfiniteSum(Scene):
    ...
    def put_expression_in_corner(self):
        ...
        self.play(*[ShowCreation(line.set_color("white")) for line in lines])
        self.wait()
    ...
    def list_partial_sums(self):
        ...
        self.play(*[ApplyMethod(ess.set_color, "yellow") for ess in esses])
        ...
        self.play(*[FadeOut(term) for term in other_terms] + [
            ApplyFunction(lambda s: move_s(s, n), ess)
            for ess, n in zip(esses, it.count())
        ] + [FadeIn(number_line),
             FadeIn(ex), FadeIn(arrow)])

        lines = [
            Line(x + 0.25 * DOWN, x + 0.25 * UP).set_color("white")
            for y in [-1, -0.01, 1, 0.01] for x in [ex_point + y * RIGHT]
        ]
        self.play(
            *[Transform(lines[x], lines[x + 1], run_time=3.0) for x in (0, 2)])


class YouJustInventedSomeMath(Scene):
    def construct(self):
        text = TextMobject(["You ", "just ", "invented\\\\", "some ",
                            "math"]).split()
        for mob in text[:3]:
            mob.shift(UP)
        for mob in text[3:]:
...
class ChopIntervalInProportions(Scene):
    ...
    def construct(self, mode):
        ...
        if mode == "p":
            ...
            prop = 0.7
            left_terms = list(
                map(TexMobject, ["(1-p)", ["p", "(1-p)"]] +
                    [["p^%d" % (count), "(1-p)"]
                     for count in range(2, num_terms)]))
            right_terms = list(
                map(TexMobject, ["p"] + [["p", "^%d" % (count + 1)]
        ...
        for lt, rt, count in zip(left_terms, right_terms, it.count()):
            ...
            for term, brace, count2 in zip([lt, rt], braces, it.count()):
                ...
                if term.get_width() > brace.get_width():
                    ...
                    arrow = Arrow(brace.get_center() + 0.3 * UP,
            ...
            if brace_to_replace is not None:
                if mode == "p":
                    ...
                    if count == 1:
                        ...
                        new_term_to_replace.center().shift(last + UP +
                                                           0.3 * LEFT)
                        left_paren.center().shift(last + 1.1 * UP)
                        self.play(
                            FadeIn(lt[1]), FadeIn(rt[0]),
                            Transform(brace_to_replace.repeat(2),
                                      Mobject(*braces)), FadeIn(left_paren),
                            FadeIn(right_paren),
                            Transform(term_to_replace, new_term_to_replace),
                            *additional_anims)
                        self.wait()
                        self.play(
                            Transform(term_to_replace, Mobject(lt[0], rt[1])),
                            FadeOut(left_paren), FadeOut(right_paren))
                    ...
                    self.play(
                        Transform(brace_to_replace.repeat(2),
                                  Mobject(*braces)),
                        Transform(term_to_replace, Mobject(lt, rt)),
                        *additional_anims)
                self.remove(brace_to_replace, term_to_replace)
                self.add(lt, rt, *braces)
            else:
                self.play(*[FadeIn(mob)
                            for mob in braces + [lt, rt]] + additional_anims)
            self.wait()
        ...
        if mode == "9":
            ...
            self.play(Transform(right_terms[-1], split_100))
        self.wait()
...
class GeometricSum(RearrangeEquation):
    def construct(self):
        ...
        RearrangeEquation.construct(self,
                                    start_terms.split(" "),
                                    end_terms.split(" "),
                                    index_map,
                                    size="\\large",
...
class PlugNumbersIntoRightside(Scene):
    def construct(self):
        ...
        lhs, rhs = TexMobject(
            ...
            size="\\large").scale(scale_factor).split()
        rhs = TexMobject(["1 \\over 1 - ", "p"],
                         size="\\large").replace(rhs).split()
        num_strings = ["0.5", "3", "\pi", "(-1)", "3.7", "2", "0.2", "27", "i"]
        nums = [
        ...
        right_words = TextMobject(
            "This side makes sense for almost any value of $p$,").shift(2 * UP)
        left_words = TextMobject(
            "even if it seems like this side will not.").shift(2 * DOWN)
        right_words.add(
            Arrow(rhs[0].get_center(),
                  tail=right_words.get_center() + DOWN + RIGHT).nudge(0.5))
        left_words.add(
            Arrow(lhs.get_center() + 0.3 * DOWN,
...
class PlugInTwo(RearrangeEquation):
    def construct(self):
        num_written_terms = 4
        start_terms = reduce(op.add, [["2", "^%d" % n, "+"]
                                      for n in range(num_written_terms)
                                      ]) + ["\\cdots=", "\\frac{1}{1-2}"]
        end_terms = "1 + 2 + 4 + 8 + \\cdots= -1".split(" ")
        index_map = dict([(3 * n, 2 * n) for n in range(num_written_terms)] +
                         [(3 * n + 2, 2 * n + 1)
                          for n in range(num_written_terms)])
        index_map[-2] = -2
        index_map[-1] = -1
...
class ListPartialDivergentSums(Scene):
    args_list = [(lambda n: "1" if n % 2 == 0 else "(-1)", lambda n: "1"
                  if n % 2 == 0 else "0"),
                 (lambda n: "2^%d" % n if n > 1 else
                  ("1" if n == 0 else "2"), lambda n: str(2**(n + 1) - 1))]
    ...
    def construct(self, term_func, partial_sum_func):
        num_lines = 8
        rhss = [partial_sum_func(n) for n in range(num_lines)]
        lhss = [
            "&=" + \
        ...
        for x in range(0, len(terms), 2):
            self.play(ApplyMethod(terms[x].set_color, "green"), run_time=0.1)
        self.wait()
...
class SumPowersOfTwoAnimation(Scene):
    def construct(self):
        ...
        equation = TexMobject("1+2+4+\\cdots+2^n=2^{n+1} - 1",
                              size="\\Huge").shift(3 * UP)
        ...
        for n in range(1, iterations):
            ...
            new_topbrace = Underbrace(top_brace_left,
                                      right).rotate(np.pi, RIGHT)
            ...
            self.wait()
            self.play(Transform(topbrace, new_topbrace),
            ...
            self.remove(bottom_num, alt_bottom_num, top_sum, top_sum_end,
                        new_top_sum_end, alt_topbrace, alt_bottombrace)
...
class PretendTheyDoApproachNegativeOne(RearrangeEquation):
    def construct(self):
        ...
        da = "\\downarrow"
        columns = [
            TexMobject("\\\\".join([n_func(n)
                                    for n in range(num_lines)] + last_bits),
                       size="\\Large").to_corner(UP + LEFT)
        ...
        self.add(columns[1])
        self.wait()
        self.play(DelayByOrder(Transform(deepcopy(columns[0]), columns[-1])),
                  FadeIn(columns[2]))
        self.wait()
...
class DistanceBetweenRationalNumbers(Scene):
    def construct(self):
        ...
        arrows = [Arrow(direction, tail=ORIGIN) for direction in locii]
...
class DistanceIsAFunction(Scene):
    ...
    def construct(self, mode):
        ...
        if mode == "Random":
            ...
            self.add(dist)
            self.wait()
        example_mobs = [(TexMobject(tup[0]).shift(arg0.get_center()),
        ...
        for mobs in example_mobs:
            if previous:
                ...
                self.play(
                    DelayByOrder(Transform(previous[-1], mobs[-1], **kwargs)))
...
class ShiftInvarianceNumberLine(Scene):
    def construct(self):
        ...
        self.play(
            ...
            *[Transform(*pair) for pair in zip(dist0.split(), dist1.split())])
        self.wait()
...
class NameShiftInvarianceProperty(Scene):
    def construct(self):
        ...
        u_brace = Underbrace(mid_part.get_boundary_point(DOWN + LEFT),
                             mid_part.get_boundary_point(DOWN + RIGHT)).shift(
                                 0.3 * DOWN)
...
class TriangleInequality(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(*[ShowCreation(line) for line in (ab_line, bc_line)] +
                  [FadeIn(dist) for dist in (ab_copy, bc_copy)])
        self.wait()
        self.play(*[Transform(*pair) for pair in zip(all_copies, all_dists)] +
                  [FadeIn(mob) for mob in (plus, greater_than)])
        self.wait()
    ...
    def construct(self):
        ...
        for mob in contents:
            ...
            for first, second in [(curr, questions), (questions, mob)]:
                copy = deepcopy(first)
                self.play(DelayByOrder(Transform(copy, second, **kwargs)))
                self.remove(copy)
            self.add(mob)
            self.wait()
...
class RoomsAndSubrooms(Scene):
    def construct(self):
        ...
        rectangle_groups = [
            ...
            [Rectangle(6, 5).shift(a).set_color(colors[1]) for a in a_set],
            [
            ...
            [
                ...
                for a in a_set for b in b_set for c in c_set
            ]
        ]
...
class RoomsAndSubroomsWithNumbers(Scene):
    def construct(self):
        ...
        self.wait()
        num_mobs = self.draw_remaining_numbers(zero, power_mobs, rect_clusters)
        self.wait()
    ...
    def draw_remaining_rectangles(self, rectangles):
        ...
        shift_vals = [
            2 * (c2 - c1)[0] * RIGHT for c1, c2 in zip(centers[1:], centers)
        ]
    ...
    def adjust_power_mobs(self, zero, power_mobs, small_rects):
        ...
        self.play(
            Transform(zero, new_zero), FadeIn(dots), *[
                Transform(old, new)
    ...
    def show_distances(self, num_mobs, rect_clusters):
        ...
        for cluster, count in zip(rect_clusters, it.count()):
            ...
            self.add(*cluster)
            pairs = [
                a_b for a_b in it.combinations(list(range(16)), 2)
                if (a_b[0] - a_b[1]) % (2**count) == 0 and (a_b[0] - a_b[1]) %
                (2**(count + 1)) != 0
            ]
            for pair in sample(pairs, min(10, len(pairs))):
                ...
                self.play(*[
                    ApplyMethod(num_mobs[index].rotate_in_place,
                                np.pi / 10,
                                rate_func=wiggle) for index in pair
                ])
                self.wait()
                for index in pair:
    ...
    def construct(self):
        part0, arg0, part1, part2, arg1, part3 = TextMobject(
            ["dist(-1, ", "0000", ") = ", "dist(0, ", "0000",
             ")"]).scale(1.5).split()
        u_brace = Underbrace(part2.get_boundary_point(DOWN + LEFT),
                             part3.get_boundary_point(DOWN +
                                                      RIGHT)).shift(0.3 + DOWN)

        colors = list(get_room_colors())
        num_labels = len(colors)
        texts = [
            Mobject(parts[0], parts[1].set_color(color))
            for count, color in zip(it.count(), colors) for parts in [
                TextMobject([
        ...
        for n in range(1, 15):
            ...
            new_args = [
                TextMobject("$%d$" % k).scale(1.5) for k in (2**n - 1, 2**n)
            ]
...
class PAdicMetric(Scene):
    def construct(self):
        p_str, text = TextMobject(["$p$",
                                   "-adic metric"]).shift(2 * UP).split()
    ...
    def spill(self, start, arrow, end):
        ...
        self.play(
            ShowCreation(arrow, rate_func=squish_rate_func(smooth, 0.5, 1.0)),
            DelayByOrder(
                Transform(start,
...
class FuzzyDiscoveryToNewMath(Scene):
    def construct(self):
        ...
        lines = Mobject(
            ...
            Line(3 * UP + LEFT * FRAME_X_RADIUS,
                 3 * UP + RIGHT * FRAME_X_RADIUS))
        fuzzy_discoveries = [
        ...
        new_maths = [
            ...
            TextMobject("Realize exponentiation is doing something much \
        ...
        self.wait()
        self.play(DelayByOrder(Transform(deepcopy(fuzzy), new_math)))
        self.play(*[
...
class DiscoveryAndInvention(Scene):
    def construct(self):
        invention, vs, discovery = TextMobject(
            ["Invention ", "vs. ", "Discovery"]).split()
        nrd = TextMobject("Non-rigorous truths").shift(2 * UP)
        rt = TextMobject("Rigorous terms").shift(2 * DOWN)

        arrows = []
        self.add(discovery, vs, invention)
        self.wait()
        arrow = Arrow(nrd.get_bottom(), tail=discovery.get_top())
        self.play(FadeIn(nrd), ShowCreation(arrow))
        arrows.append(arrow)
        self.wait()
        arrow = Arrow(invention.get_top(), tail=nrd.get_bottom())
        self.play(ShowCreation(arrow))
        arrows.append(arrow)
        self.wait()
        arrow = Arrow(rt.get_top(), tail=invention.get_bottom())
        self.play(FadeIn(rt), ShowCreation(arrow))
        arrows.append(arrow)
        self.wait()
        arrow = Arrow(discovery.get_bottom(), tail=rt.get_top())
        self.play(ShowCreation(arrow))
        self.wait()
        arrows.append(arrow)
        for color in Color("yellow").range_to("red", 4):
            for arrow in arrows:
                self.play(ShowCreation(deepcopy(arrow).set_color(color)),
                          run_time=0.25)
