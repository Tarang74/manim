...
class StudentsFindThisConfusing(TeacherStudentsScene):
    def construct(self):
        ...
        self.student_says("What does this actually mean?",
                          student_index=2,
                          added_anims=[
                              question1.scale_in_place,
                              0.8,
                              question1.to_edge,
                              LEFT,
                              question1.shift,
        ...
        question2.target.next_to(question1,
                                 DOWN,
                                 aligned_edge=LEFT,
                                 buff=MED_SMALL_BUFF)
        equation = TexMobject("\\det\\left( %s \\right)=0" %
...
class EigenThingsArentAllThatBad(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("Eigen-things aren't \\\\ actually so bad",
                          target_mode="hooray")
        self.change_student_modes("pondering", "pondering", "erm")
        self.random_blink(4)
...
class IntroduceExampleTransformation(ExampleTranformationScene):
    def construct(self):
        ...
        self.play(*[
            Transform(*pair)
            for pair in [(i_coords_copy.rect, self.matrix.rect),
                         (i_coords_copy.get_brackets(),
                          self.matrix.get_brackets()),
                         (i_coords_copy.get_entries(),
                          VGroup(*self.matrix.get_mob_matrix()[:, 0]))]
        ])
        to_remove = self.get_mobjects_from_last_animation()
        self.play(
            Transform(j_coords.copy().get_entries(),
...
class IHatAsEigenVector(ExampleTranformationScene):
    ...
    def label_i_hat_landing_spot(self):
        ...
        self.play(
            ...
            Transform(VGroup(*matrix.get_mob_matrix()[:, 0]),
                      array.get_entries()),
        )
        self.wait()
...
class AllXAxisVectorsAreEigenvectors(ExampleTranformationScene):
    def construct(self):
        vectors = VGroup(*[
            ...
            for x in reversed(list(range(1,
                                         int(FRAME_X_RADIUS) + 1)))
            for u in [-1, 1]
        ])
...
class FullSneakyEigenspace(ExampleTranformationScene):
    def construct(self):
        ...
        vectors = VGroup(*[
            ...
            for x in reversed(np.arange(0.5, 5, 0.5)) for u in [-1, 1]
        ])
...
class NameEigenvectorsAndEigenvalues(ExampleTranformationScene):
    ...
    def construct(self):
        ...
        x_vectors = VGroup(*[
            self.add_vector(u * x * RIGHT, animate=False)
            for x in range(int(FRAME_X_RADIUS) + 1, 0, -1) for u in [-1, 1]
        ])
        x_vectors.set_color_by_gradient(YELLOW, X_COLOR)
        self.remove(x_vectors)
        sneak_vectors = VGroup(*[
            self.add_vector(u * x * (LEFT + UP), animate=False)
            for x in np.arange(int(FRAME_Y_RADIUS), 0, -0.5) for u in [-1, 1]
        ])
        ...
        for words in x_words, sneak_words:
            ...
            eigen_val_words = TextMobject("with eigenvalue ",
        ...
        non_eigen_span = Line(-FRAME_Y_RADIUS * non_eigen.get_end(),
        ...
        self.play(ShowCreation(non_eigen_span), Write(non_eigen_words),
                  Animation(non_eigen))
...
class EigenvalueNegativeOneHalf(LinearTransformationScene):
    ...
    def construct(self):
        ...
        span = Line(-FRAME_Y_RADIUS * vector.get_end(),
...
class ContrastMatrixUnderstandingWithEigenvalue(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(Write(axis_and_rotation),
                  *it.chain(*list(
                      zip(
...
class WordsOnComputation(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("I won't cover the full\\\\",
...
class SymbolicEigenvectors(Scene):
    ...
    def introduce_terms(self):
        self.expression = TexMobject("A", "\\vec{\\textbf{v}}", "=",
                                     "\\lambda", "\\vec{\\textbf{v}}")
        ...
        v_arrows = VGroup(
            *[Arrow(v_text.get_top(), v.get_bottom()) for v in vs])
        ...
        self.play(*list(
            map(FadeOut,
                [A_brace, A_text, lamb_brace, lamb_text, v_text, v_arrows])))
    ...
    def contrast_multiplication_types(self):
        ...
        for group in left_group, right_group:
            self.play(GrowFromCenter(group.brace), Write(group.text,
                                                         run_time=2))
        ...
        solve_text = TextMobject("Solve for \\\\", "$\\lambda$", "and",
                                 "$\\vec{\\textbf{v}}$")
        ...
        self.play(Transform(solve_text, bubble.content), morty.change_mode,
                  "sassy")
        self.play(Blink(morty))
        self.wait()
        self.play(*list(
            map(FadeOut, [
                left_group.brace, left_group.text, right_group.brace,
                right_group.text, morty, bubble, solve_text
            ])))
    ...
    def rewrite_righthand_side(self):
        ...
        lamb_copy = lamb.copy()
        scaling_by = VGroup(TextMobject("Scaling by "), lamb_copy)
        scaling_by.arrange()
        arrow = TexMobject("\\Updownarrow")
        matrix_multiplication = TextMobject("Matrix multiplication by")
        ...
        q_marks = VGroup(
            *
            [TexMobject("?").replace(entry) for entry in matrix.get_entries()])
        q_marks.set_color_by_gradient(X_COLOR, Y_COLOR, Z_COLOR)
        diag_entries = VGroup(
            *[matrix.get_mob_matrix()[i, i] for i in range(3)])
        ...
        self.wait()
        self.play(
            Transform(q_marks, matrix.get_entries(), lag_ratio=0.5,
                      run_time=2))
        self.remove(q_marks)
        self.add(*matrix.get_entries())
        self.wait()
        self.play(Transform(diag_entries.copy(), new_lamb),
        ...
        for mob in lamb, id_text, v2:
            mob.target = mob.copy()
        VGroup(l_paren, lamb.target, id_text.target, r_paren,
               v2.target).arrange().next_to(equals).shift(SMALL_BUFF * UP)
        self.play(Write(parens), *list(map(MoveToTarget, [lamb, id_text, v2])))
        self.wait()
        self.play(*list(
            map(FadeOut, [corner_group, id_brace, id_text_copy, new_lamb])))
        self.expression = VGroup(A, v1, equals,
                                 VGroup(l_paren, lamb, id_text, r_paren), v2)
    ...
    def reveal_as_linear_system(self):
        ...
        for mob in movers:
            mob.target = mob.copy()
        VGroup(A.target, v1.target, minus, lamb_group.target,
               v2.target).arrange().next_to(equals, LEFT)
        ...
        self.play(MoveToTarget(A), MoveToTarget(l_paren),
        ...
        matrix.scale(0.7)
        VGroup(matrix.get_brackets()[1],
               *matrix.get_mob_matrix()[:, 2]).shift(0.5 * RIGHT)
        ...
        vect_words = TextMobject("We want a nonzero solution for")
    ...
    def bring_in_determinant(self):
        ...
        equation = TexMobject("\\det(A-", "\\lambda", "I)", "=0")
...
class NonZeroSolutionsVisually(LinearTransformationScene):
    ...
    def construct(self):
        equation = TexMobject("(A-", "\\lambda", "I)", "\\vec{\\textbf{v}}",
                              "= \\vec{\\textbf{0}}")
        ...
        det_equation = TexMobject("\\text{Squishification} \\Rightarrow",
                                  "\\det", "(A-", "\\lambda", "I", ")", "=0")
...
class TweakLambda(LinearTransformationScene):
    ...
    def construct(self):
        ...
        for mob in self.plane_mobjects:
            mob.save_state()
        self.apply_transposed_matrix(self.t_matrix)
        self.play(Write(det_text), Write(equals), ShowCreation(det.rect),
                  Write(det))
    ...
    def range_lambda(self, start_val, end_val, run_time=3):
        ...
        transformations += [
            Transform(self.i_hat, Vector(matrix_transform(RIGHT),
                                         color=X_COLOR)),
...
class RevisitExampleTransformation(ExampleTranformationScene):
    ...
    def introduce_matrix(self):
        ...
        self.play(*[
            Transform(*pair)
            for pair in [(i_coords.rect, self.matrix.rect),
        ...
        self.play(
            ...
            Transform(j_coords.get_entries(),
                      VGroup(*self.matrix.get_mob_matrix()[:, 1])),
        )
        to_remove += self.get_mobjects_from_last_animation()
        self.wait()
    ...
    def get_lambda_to_diag_movements(self, lamb):
        ...
        result += list(
            map(Animation,
                [self.matrix.get_mob_matrix()[i, 1 - i] for i in range(2)]))
        self.diag_entries = [
    ...
    def show_determinant(self, to_fade=None):
        ...
        VGroup(lp1, three_minus_lamb.target, rp1, lp2, two_minus_lamb.target,
               rp2, minus, one.target, cdot,
               zero.target).arrange().next_to(equals)
        ...
        brace_text.target = brace.target.get_text("Quadratic polynomial in ",
                                                  "$\\lambda$")
        ...
        final_text = TexMobject("\\lambda", "=2", "\\text{ or }", "\\lambda",
                                "=3")
        ...
        self.wait()
        self.play(
            *it.chain(list(map(MoveToTarget, [brace, brace_text])),
                      list(map(FadeOut, [one, zero, minus, cdot, new_rect]))))
        self.wait()
        self.play(Write(equals_0))
        self.wait()
        self.play(Transform(brace, final_brace),
        ...
        self.play(*it.chain(list(map(FadeOut, faders)), [
            lambda_equals_two.scale_in_place, 1.3, lambda_equals_two.next_to,
            self.matrix, DOWN
        ]))
    ...
    def show_diagonally_altered_transform(self):
        ...
        vectors = VGroup(*[
            self.add_vector(u * x * (LEFT + UP), animate=False)
            for x in range(4, 0, -1) for u in [-1, 1]
        ])
        vectors.set_color_by_gradient(MAROON_B, YELLOW)
        vectors.save_state()
        self.play(ShowCreation(vectors, lag_ratio=0.5, run_time=2),
                  *list(map(Animation, self.foreground_mobjects)))
        self.wait()
        ...
        self.play(*it.chain([
            mob.restore
            for mob in (self.plane, self.i_hat, self.j_hat, vectors)
    ...
    def show_unaltered_transform(self):
        ...
        self.play(
            *it.chain(list(map(MoveToTarget, movers)),
                      list(map(FadeOut, faders)), [GrowFromCenter(brace)]))
...
class ShearExample(RevisitExampleTransformation):
    ...
    def point_out_eigenvectors(self):
        ...
        vectors.set_color_by_gradient(YELLOW, X_COLOR)
        words = VGroup(TextMobject("Eigenvectors"),
...
class IntroduceEigenbasis(TeacherStudentsScene):
    def construct(self):
        words1, words2 = list(
            map(TextMobject, [
                "Finish with ``eigenbasis.''", """Make sure you've
        ...
        self.play(self.get_teacher().change_mode, "sassy", Write(words2),
...
class BasisVectorsAreEigenvectors(LinearTransformationScene):
    ...
    def construct(self):
        ...
        words = TextMobject("What if both basis vectors \\\\",
                            "are eigenvectors?")
        for word in words:
            word.add_to_back(BackgroundRectangle(word))
        ...
        self.wait()
        diag_entries = VGroup(
            *[matrix.get_mob_matrix()[i, i] for i in range(2)])
        off_diag_entries = VGroup(
            *[matrix.get_mob_matrix()[1 - i, i] for i in range(2)])
        for entries in diag_entries, off_diag_entries:
            self.play(entries.scale_in_place,
                      1.3,
                      entries.set_color,
                      YELLOW,
                      run_time=2,
...
class DefineDiagonalMatrix(Scene):
    def construct(self):
        ...
        diag_entries = VGroup(
            *[matrix.get_mob_matrix()[i, i] for i in range(n_dims)])
        off_diag_entries = VGroup(*[
            matrix.get_mob_matrix()[i, j] for i in range(n_dims)
            for j in range(n_dims) if i != j
        ])
...
class RepeatedMultiplicationInAction(Scene):
    def construct(self):
        ...
        l_bracket.target.next_to(
            VGroup(*[scalar.target for scalar in scalars]), LEFT)
        ...
        for new_exp in range(2, 6):
            ...
            movers = []
            to_remove = []
            for v_entry, scalar, new_scalar in zip(vector.get_entries(),
                                                   scalars, new_scalars):
                ...
                scalar.target = scalar.copy()
                VGroup(scalar.target,
                       scalar.exp.target).next_to(v_entry,
                                                  LEFT,
                                                  aligned_edge=DOWN)
...
class RepeatedMultilpicationOfMatrices(Scene):
    ...
    def construct(self):
        ...
        if self.diagonal:
            ...
            VGroup(last_matrix.target, hundred.target,
                   hundred_copy.target).next_to(vector, LEFT)

            self.play(*it.chain(
                list(map(FadeOut,
                         [brace, brace_text[1]] + list(matrices[:-1]))),
...
class RepeatedMultilpicationOfNonDiagonalMatrices(
        RepeatedMultilpicationOfMatrices):
    CONFIG = {
        "matrix": [[3, 4], [1, 1]],
        "diagonal": False,
    }
...
class ChangeToEigenBasis(ExampleTranformationScene):
    ...
    def introduce_eigenvectors(self):
        x_vectors, v_vectors = [
            VGroup(*[
                ...
                for x in range(num, 0, -1) for u in [-1, 1]
    ...
    def write_change_of_basis_matrix(self):
        ...
        cob_matrix = Matrix(
            np.array([list(vect.entries.target) for vect in (b1, b2)]).T)
        ...
        inv_cob.target.next_to(self.matrix,
                               LEFT,
                               buff=neg_1.get_width() + 2 * SMALL_BUFF)
        neg_1.next_to(
        ...
        self.play(
            ...
            *list(
                map(Animation,
                    self.foreground_mobjects + self.moving_vectors)))
