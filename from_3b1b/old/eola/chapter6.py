...
class OpeningQuote(Scene):
    def construct(self):
        words = TextMobject("To ask the", "right question\\\\",
...
class NoComputations(TeacherStudentsScene):
    def construct(self):
        self.setup()
        self.student_says("Will you cover \\\\ computations?",
        ...
        self.play(*[
            ApplyMethod(student.change_mode, mode) for student, mode in zip(
        ...
        self.play(Transform(self.teacher.bubble.content, new_words),
...
class StockPrices(Scene):
    def construct(self):
        ...
        self.play(
            ShowCreation(VMobject(*stock_lines), run_time=3, lag_ratio=0.5))
        self.wait()
...
class SystemOfEquations(Scene):
    ...
    def get_equations(self):
        ...
        for row in mob_matrix:
            equation = VMobject(*it.chain(*list(
                zip(row, [v.copy() for v in variables],
    ...
    def describe_organization(self, equations):
        variables = VMobject(*it.chain(
            *[eq.split()[:-2] for eq in equations.split()]))
        ...
        xs, ys, zs = [
            VMobject(*[eq.split()[i] for eq in equations.split()])
            for i in (1, 4, 7)
        ]
        ...
        zero_circles = VMobject(*[
            Circle().replace(mob).scale_in_place(1.3) for mob in [
        ...
        self.play(ShowCreation(zero_circles),
    ...
    def factor_into_matrix(self, equations):
        ...
        all_brackets = [
            mob.get_brackets() for mob in (matrix, x_array, v_array)
        ]
        ...
        self.play(*it.chain(*[[
            ...
            for m1, m2 in zip(start_array.flatten(),
                              matrix_mobject.get_entries().split())
        ] for start_array, matrix_mobject in [(
            coefficients,
            matrix), (variable_arrays[0],
                      x_array), (variable_arrays[1],
                                 x_array), (variable_arrays[2],
                                            x_array), (rhs_entries,
                                                       v_array)]]))
    ...
    def label_matrix_product(self, matrix, x_array, v_array):
        ...
        compact_equation.submobjects.insert(
            2,
            TexMobject("=").next_to(x_array, RIGHT))
...
class ThinkAboutWhatsHappening(Scene):
    def construct(self):
        ...
        self.play(ApplyFunction(lambda m: m.shift(2 * DOWN).scale(5),
                                everything),
...
class SystemOfTwoEquationsTwoUnknowns(Scene):
    def construct(self):
        ...
        matrix_system = VMobject(matrix, x, TexMobject("="), v)
...
class ShowBijectivity(LinearTransformationScene):
    ...
    def construct(self):
        self.setup()
        vectors = VMobject(*[
            Vector([x, y]) for x, y in it.product(*[
                np.arange(-int(val) + 0.5,
                          int(val) + 0.5)
        ...
        titles = [
            TextMobject(["Each vector lands on\\\\", "exactly one vector"]),
            TextMobject(["Every vector has \\\\", "been landed on"])
        ]
        for title in titles:
            title.to_edge(UP)
...
class StartWithNonzeroDetCase(TeacherStudentsScene):
    def construct(self):
        words = TextMobject("Let's start with \\\\", "the",
                            "$\\det(A) \\ne 0$", "case")
...
class DeclareNewTransformation(TeacherStudentsScene):
    def construct(self):
        words = TextMobject("Playing a transformation in\\\\",
...
class MultiplyToIdentity(LinearTransformationScene):
    def construct(self):
        ...
        self.apply_inverse_transpose(self.t_matrix)
        self.wait()
        self.play(Transform(brace, product.brace),
                  Transform(text, product.text))
        self.wait()
        self.play(Write(identity.background), Write(identity.get_brackets()),
                  Write(eq), Transform(brace, identity.brace),
...
class SymbolicInversion(Scene):
    def construct(self):
        ...
        self.play(GrowFromCenter(product.brace),
                  Write(product.words, run_time=1), product.set_color, BLUE)
        self.wait()
        self.play(*[
        ...
        self.wait()
        self.play(
            ApplyFunction(lambda m: m.center().to_edge(UP),
    ...
    def construct(self):
        ...
        self.play(grid.restore,
                  *[
                  ...
                  run_time=3)
        multiple_outputs = TextMobject("Must map to \\\\", "multiple vectors")
...
class PrepareForRank(Scene):
    def construct(self):
        new_term, rank = words = TextMobject("New terminology: ", "rank")
        rank.set_color(TEAL)
...
class DefineRank(Scene):
    def construct(self):
        ...
        dims = TextMobject("Number of\\\\", "dimensions \\\\", "in the output")
...
class ColumnsRepresentBasisVectors(Scene):
    def construct(self):
        ...
        for i, words in enumerate([i_hat_words, j_hat_words]):
            ...
            self.play(
                Write(words, run_time=1), ShowCreation(arrow), *[
...
class TowDColumnsDontSpan(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(
            ApplyFunction(lambda m: m.scale(-1).shift(self.i_hat.get_end()),
                          self.j_hat))
...
class ThreeDColumnsDontSpan(Scene):
    def construct(self):
        ...
        matrix.set_column_colors(X_COLOR, Y_COLOR, Z_COLOR)
        brace = Brace(matrix)
        words = brace.get_text("Columns don't", "span \\\\",
                               "full output space")
...
class NameColumnSpace(Scene):
    def construct(self):
        ...
        span_text = TexMobject("\\text{Span}", "\\Big(",
                               matrix_to_tex_string([1, 2, 3]), ",",
                               matrix_to_tex_string([1, 2, 3]), ",",
                               matrix_to_tex_string([1, 2, 3]), "\\big)")
        for i in 1, -1:
            span_text[i].stretch(1.5, 1)
            span_text[i].do_in_place(span_text[i].set_height,
        ...
        self.wait()
        self.play(*[
            Transform(VMobject(*matrix.copy().get_mob_matrix()[:, i]),
                      col_arrays[i].get_entries()) for i in range(3)
        ])
        self.play(
            Write(span_text),
        ...
        words = TextMobject("To solve",
                            "$A\\vec{\\textbf{x}} = \\vec{\\textbf{v}}$,\\\\",
                            "$\\vec{\\textbf{v}}$", "must be in \\\\ the",
                            "column space.")
        ...
        rank_words = brace.get_text("Number of dimensions \\\\ is called",
                                    "``rank''")
    ...
    def cycle_through_span_possibilities(self, span_text):
        ...
        mob_matrix = np.array(
            [two_d_span[i].get_entries().split() for i in (2, 4)])
        ...
        self.add(span_text)
        mob_matrix = np.array(
            [span_text[i].get_entries().split() for i in (2, 4, 6)])
        self.replace_number_matrix(mob_matrix,
                                   [[1, 1, 0], [0, 1, 1], [1, 0, 1]])
        self.wait()
        self.replace_number_matrix(mob_matrix,
                                   [[1, 1, 0], [0, 1, 1], [-1, -2, -1]])
    ...
    def replace_number_matrix(self, matrix, new_numbers):
        ...
        targets = list(
            map(TexMobject, list(map(str,
                                     np.array(new_numbers).flatten()))))
...
class ZeroMatirx(LinearTransformationScene):
    def construct(self):
        origin = Dot(ORIGIN)
        self.play(
            Transform(VMobject(self.plane, self.i_hat, self.j_hat),
                      origin,
                      run_time=3))
        self.wait()
...
class NameFullRank(Scene):
    def construct(self):
        ...
        low_words = TextMobject("matrix is", "``full rank''")
...
class FullRankCase(LinearTransformationScene):
    ...
    def construct(self):
        t_matrices = [[[2, 1], [-3, 2]], [[1. / 2, 1], [1. / 3, -1. / 2]]]
        vector = Matrix([0, 0]).set_color(YELLOW)
        title = VMobject(TextMobject("Only"), vector, TextMobject("lands on"),
                         vector.copy())
...
class NameNullSpace(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.apply_transposed_matrix(self.t_matrix,
                                     added_anims=[
                                         ...
                                         ApplyMethod(null_space_label.scale,
                                                     0),
    ...
    def get_vectors(self, offset=0):
        ...
        vectors = VMobject(
            *[Vector(a * vect + offset) for a in np.linspace(-5, 5, 18)])
...
class OffsetNullSpace(NameNullSpace):
    def construct(self):
        ...
        dot = Dot(self.get_matrix_transformation(self.t_matrix)(x.get_end()),
                  color=RED)
...
class AddJustOneNullSpaceVector(NameNullSpace):
    def construct(self):
        ...
        self.apply_transposed_matrix(
            self.t_matrix,
            added_anims=[Transform(null_vector, dot),
                         Write(transformer)])
        self.play(Write(final_sum))
        self.wait()
        self.play(Write(brace))
        self.wait()
        words = TextMobject("$\\vec{\\textbf{x}}$", "and the",
...
class NullSpaceOffsetRule(Scene):
    def construct(self):
        ...
        x.set_color(PINK)
        v.set_color(YELLOW)
        A_text = TextMobject("When $A$ is not", "full rank")
        ...
        v_text = TextMobject("If", "$%s$" % vec("v"), "is in the",
                             "column space", "of $A$")
...
class WhatAboutNonsquareMatrices(TeacherStudentsScene):
    def construct(self):
        self.student_says("What about \\\\ nonsquare matrices?",
