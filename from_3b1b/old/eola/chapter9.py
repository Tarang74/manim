...
class LinearCombinationScene(LinearTransformationScene):
    ...
    def setup(self):
        ...
        self.i_hat.label = self.get_vector_label(self.i_hat, "\\hat{\\imath}",
                                                 "right")
        self.j_hat.label = self.get_vector_label(self.j_hat, "\\hat{\\jmath}",
                                                 "left")
    ...
    def show_linear_combination(
        ...
        for coord, basis, scalar in zip(coord_mobs, basis_vectors,
            ...
            new_label.move_to(basis.label,
                              aligned_edge=basis.get_center() -
                              basis.label.get_center())
...
class RemindOfCoordinates(LinearCombinationScene):
    ...
    def show_standard_coord_meaning(self, x_coord, y_coord):
        ...
        for line, coord, direction in (x_line, x_coord, DOWN), (y_line,
                                                                y_coord, LEFT):
            self.play(
                coord.set_color,
                line.get_color(),
    ...
    def show_abstract_scalar_idea(self, x_coord, y_coord):
        ...
        self.play(FadeOut(everything), x_coord.scale_in_place, 1.5,
                  x_coord.move_to, x_shift + 3 * UP, y_coord.scale_in_place,
                  1.5, y_coord.move_to, y_shift + 3 * UP, Write(words))
        ...
        self.wait()
        self.play(FadeOut(words), FadeIn(everything),
    ...
    def scale_basis_vectors(self, x_coord, y_coord):
        ...
        self.show_linear_combination(self.vector_coords,
    ...
    def list_implicit_assumptions(self, x_coord, y_coord):
        ...
        self.wait(2)
        keepers = VGroup(
            *[self.i_hat, self.j_hat, self.i_hat.label, self.j_hat.label])
        self.play(FadeOut(everything), Animation(keepers.copy()),
                  Animation(group))
        self.wait()
...
class NameCoordinateSystem(Scene):
    def construct(self):
        ...
        basis_words.shift(basis_group.get_bottom()[1] * UP +
                          MED_SMALL_BUFF * DOWN)
...
class JenniferScene(LinearCombinationScene):
    ...
    def setup(self):
        ...
        self.b1.label.next_to(self.b1.get_end() * 0.4, UP + LEFT,
                              SMALL_BUFF / 2)
        self.b2.label.next_to(self.b2.get_end(), DOWN + LEFT, buff=SMALL_BUFF)
        self.basis_vectors = VGroup(self.b1, self.b2, self.b1.label,
                                    self.b2.label)
...
class IntroduceJennifer(JenniferScene):
    ...
    def introduce_jenny(self):
        ...
        self.play(jenny.change_mode, "happy", jenny.look, UP + RIGHT,
                  FadeOut(name))
        self.wait()
    ...
    def show_v_from_both_perspectives(self):
        ...
        self.play(*it.chain(
            list(
                map(FadeIn, [
                    self.plane, self.i_hat, self.j_hat, self.i_hat.label,
                    self.j_hat.label, you
                ])),
        ...
        self.play(*it.chain(
            list(
                map(FadeOut, [
                    self.plane, self.i_hat, self.j_hat, self.i_hat.label,
                    self.j_hat.label, you.bubble, you.coords
                ])),
    ...
    def hacked_show_linear_combination(
        ...
        i_sym.set_color(X_COLOR).move_to(new_label1.target[1],
                                         aligned_edge=LEFT)
        j_sym.set_color(Y_COLOR).move_to(new_label2.target[1],
                                         aligned_edge=LEFT)
        ...
        self.play(
            ...
            self.you.change_mode, "speaking", self.jenny.change_mode, "erm",
            *list(
                map(MoveToTarget, [
    ...
    def how_we_label_her_basis(self):
        ...
        self.play(ShowCreation(you.bubble), Write(b1_coords), you.change_mode,
                  "speaking")
        self.play(b1_coords.mover.next_to, self.b1.get_end(), RIGHT,
                  b1_coords.mover.set_color, X_COLOR)
        self.play(Blink(you))
        self.wait()
        self.play(Transform(b1_coords, b2_coords))
        self.play(b2_coords.mover.next_to, self.b2.get_end(), LEFT,
        ...
        self.play(
            self.jenny_plane.restore, FadeOut(self.you.bubble),
            FadeOut(b1_coords), self.jenny.change_mode, "speaking",
...
class SpeakingDifferentLanguages(JenniferScene):
    def construct(self):
        ...
        for pi in you, jenny:
            self.play(pi.change_mode, "speaking" if pi is you else "sassy",
                      ShowCreation(pi.bubble), Write(pi.coords))
            self.play(Blink(pi))
        self.wait()
...
class SpaceHasNoGrid(LinearTransformationScene):
    ...
    def construct(self):
        ...
        words.to_edge(UP)
        self.play(Write(words), FadeOut(self.plane),
...
class JennysGrid(JenniferScene):
    def construct(self):
        ...
        for vect in self.b1, self.b2:
            self.play(ShowCreation(vect), Write(vect.label))
        self.wait()
        self.play(ShowCreation(self.jenny_plane, run_time=3,
                               lag_ratio=0.5), self.jenny.change_mode,
                  "speaking", self.jenny.look_at, ORIGIN, ShowCreation(bubble),
                  Write(bubble.content), Animation(self.basis_vectors))
        self.play(Blink(self.jenny))
...
class ShowOriginOfGrid(JenniferScene):
    def construct(self):
        ...
        self.wait()
        self.play(Transform(self.jenny_plane, self.plane),
...
class TranslateFromJenny(JenniferScene):
    ...
    def ask_question(self):
        self.play(self.jenny.change_mode, "pondering",
                  ShowCreation(self.jenny.bubble), Write(self.jenny.coords))
        ...
        self.play(self.you.change_mode, "confused",
                  ShowCreation(self.you.bubble), Write(self.you.coords))
        self.wait()
    ...
    def perform_arithmetic(self):
        ...
        movers = [
            self.you.coords, equals, jenny_x, self.b1_coords_mob, plus,
            jenny_y, self.b2_coords_mob, equals2, result
        ]
        for mover in movers:
        ...
        self.play(self.you.change_mode, "hooray",
        ...
        self.play(FadeOut(self.jenny.bubble), FadeOut(self.you.coords),
                  self.jenny.change_mode, "plain",
                  MoveToTarget(self.jenny.coords), FadeIn(matrix))
        self.wait()
...
class TalkThroughChangeOfBasisMatrix(JenniferScene):
    def construct(self):
        ...
        for vect in self.i_hat, self.j_hat:
            self.play(ShowCreation(vect), Write(vect.label))
        self.play(self.you.change_mode, "pondering",
                  ShowCreation(self.you.bubble), Write(basis_coords_pair))
        self.play(Blink(self.you))
        self.wait()

        self.add_foreground_mobject(self.jenny, self.you, self.you.bubble,
        ...
        self.apply_transposed_matrix(
            ...
            added_anims=[
                ...
                Transform(VGroup(*matrix_copy.get_mob_matrix()[:, 0]),
        ...
        self.apply_transposed_matrix(
            ...
            added_anims=[
                ...
                Transform(VGroup(*matrix_copy.get_mob_matrix()[:, 1]),
...
class FeelsBackwards(Scene):
    def construct(self):
        ...
        self.play(Swap(jenny_language,
                       our_language), bottom_arrow.scale_in_place, 0.8,
...
class RecallInverse(JenniferScene):
    def construct(self):
        ...
        self.play(GrowFromCenter(brace), Write(inverse_word),
...
class WorkOutInverseComputation(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(FadeIn(matrix), GrowFromCenter(matrix_brace),
                  Write(matrix_text))
        self.wait()
...
class SummarizeTranslationProcess(Scene):
    ...
    def show_translation(self):
        ...
        our_arrow = Arrow(our_words.get_top(),
                          our_vector.get_bottom(),
        ...
        self.play(
            VGroup(her_vector,
                   equals).next_to, A_inv, LEFT, her_arrow.rotate_in_place,
            -np.pi / 6, her_arrow.shift, MED_SMALL_BUFF * LEFT,
...
class RotationExample(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            Transform(i_coords.get_entries(),
    ...
    def add_words(self, matrix):
        follow_basis = TextMobject("Follow", "our choice",
                                   "\\\\ of basis vectors")
        ...
        record = TextMobject("Record using \\\\", "our coordinates")
...
class JennyWatchesRotation(JenniferScene):
    def construct(self):
        ...
        self.play(jenny.change_mode, "sassy", ShowCreation(jenny.bubble),
                  Write(matrix))
        self.play(*it.chain([
...
class AksAboutTranslatingColumns(TeacherStudentsScene):
    def construct(self):
        ...
        self.random_blink()
        words = TextMobject("\\centering Those columns still \\\\ represent ",
                            "our basis",
                            ", not ",
                            "hers",
                            arg_separator="")
...
class HowToTranslateAMatrix(Scene):
    def construct(self):
        ...
        brace_text_pairs = [
            ...
            (cob_matrix.top_brace, ("Same vector \\\\", "in", "our",
                                    "language")),
            (transform.brace, ("Transformation matrix \\\\", "in", "our",
                               "language")),
            (transform.top_brace, ("Transformed vector \\\\", "in", "our",
                                   "language")),
            (inv_cob.brace, ("Inverse \\\\", "change of basis \\\\",
                             "matrix")),
            (inv_cob.top_brace, ("Transformed vector \\\\", "in", "her",
                                 "language")),
            (final_top_brace, ("Transformation matrix \\\\", "in", "her",
                               "language"))
        ]
        ...
        def introduce(array):
            self.play(Write(array), Transform(brace, array.brace),
                      Transform(bottom_words, array.brace.text))
            self.wait()

        def echo_introduce(array):
            self.play(Transform(top_brace, array.top_brace),
...
class JennyWatchesRotationWithMatrixAndVector(JenniferScene):
    def construct(self):
        ...
        self.play(self.jenny.change_mode, "pondering", Write(equals),
                  Write(result))
        self.play(Blink(self.jenny))
        self.wait()
...
class MathematicalEmpathy(TeacherStudentsScene):
    def construct(self):
        words = TextMobject("\\centering An expression like", "$A^{-1} M A$",
