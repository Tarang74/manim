...
class AnotherFootnote(TeacherStudentsScene):
    def construct(self):
        ...
        self.teacher_says("More footnotes!",
...
class ColumnsRepresentBasisVectors(Scene):
    ...
    def put_in_thought_bubble(self):
        ...
        self.play(
            ApplyFunction(
                lambda m: bubble.position_mobject_inside(m.set_height(2.5)),
                everything), ShowCreation(bubble), randy.change_mode,
            "pondering")
...
class DescribeColumnsInSpecificTransformation(Scene):
    def construct(self):
        ...
        for col, char, vect in zip([i_col, j_col], ["i", "j"], [UP, DOWN]):
            ...
            col.arrow = Arrow(col.words.get_edge_center(-vect),
        ...
        for col in i_col, j_col:
            self.play(Write(col), ShowCreation(col.arrow),
...
class CountRowsAndColumns(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(rows_words[0].copy().move_to, title[0],
...
class MatrixInTheWild(Scene):
    def construct(self):
        ...
        VMobject(randy, bubble, matrix).to_corner(UP + LEFT,
                                                  buff=MED_SMALL_BUFF)
...
class ThreeDToTwoDOutput(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.add(title)
        i, j, k = it.starmap(self.add_vector, [([3, 1], X_COLOR),
                                               ([1, 2], Y_COLOR),
                                               ([-2, -2], Z_COLOR)])
        pairs = [(i, "L(\\hat\\imath)"), (j, "L(\\hat\\jmath)"),
                 (k, "L(\\hat k)")]
        for v, tex in pairs:
...
class TwoDTo1DTransformWithDots(TwoDTo1DTransform):
    def construct(self):
        ...
        words = TextMobject("Line of dots remains evenly spaced")
...
class DotProductPreview(VectorScene):
    ...
    def project_w(self):
        ...
        projected_w = Vector(self.v.get_end() * dot_product / (v_norm**2),
                             color=self.w.get_color())
        projection_line = Line(self.w.get_end(),
                               projected_w.get_end(),
                               color=GREY)
    ...
    def show_scaling(self):
        ...
        self.wait()
        self.play(Transform(start_brace, interim_brace),
