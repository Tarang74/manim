...
class ListSteps(Scene):
    ...
    def construct(self):
        ...
        step_2 = TextMobject("Next video: Deeper understanding with ",
        ...
        self.play(MoveToTarget(step_1), step_2.fade, randy.change_mode,
                  "happy")
        self.play(Blink(randy))
        self.play(Transform(step_1,
                            step_1.copy().restore().fade()),
                  MoveToTarget(step_2), randy.look, LEFT)
...
class SimpleDefine2dCrossProduct(LinearTransformationScene):
    ...
    def write_area_words(self):
        ...
        self.play(
            FadeIn(cross_rect), Write(times), *[
                ApplyMethod(vect.label.target.restore,
                            rate_func=lambda t: smooth(1 - t))
                for vect in (self.v, self.w)
            ])
        self.wait()
        self.play(
            ApplyFunction(lambda m: m.scale_in_place(1.2).set_color(RED),
                          times,
    ...
    def show_sign(self):
        ...
        for word in positive, negative:
            ...
            word.arrow = Arrow(word.get_top(),
                               word.get_top() + 1.5 * UP,
                               color=word.get_color())
            VGroup(word, word.arrow).next_to(self.area_words,
                                             DOWN,
                                             aligned_edge=LEFT,
                                             buff=SMALL_BUFF)
        ...
        self.remove(arc)
        self.play(FadeOut(positive), FadeOut(positive.arrow),
                  *[mob.restore for mob in (square, self.v, self.w)])
        arc = self.get_arc(self.v, self.w, radius=1.5)
        arc.set_color(RED)
        self.play(ShowCreation(arc))
        self.play(Write(negative), ShowCreation(negative.arrow),
                  Write(minus_sign), MoveToTarget(self.area_words))
        self.wait()
    ...
    def swap_v_and_w(self):
        ...
        self.play(MoveToTarget(self.square),
...
class ContrastDotAndCross(Scene):
    ...
    def add_t_chart(self):
        for word, vect, color in ("Dot", LEFT, BLUE_C), ("Cross", RIGHT,
                                                         YELLOW):
    ...
    def add_dot_products(self, max_width=FRAME_X_RADIUS - 1, dims=[2, 5]):
        ...
        dot_products = []
        for dim in dims:
            arrays = [[random.randint(0, 9) for in_count in range(dim)]
        ...
        for dot_prod in dot_products:
            self.play(Write(dot_prod.syms),
                      *[
                          Transform(e.copy(), e.target, path_arc=-np.pi / 6)
    ...
    def add_cross_product(self):
        ...
        arrays = [[random.randint(0, 9) for in_count in range(3)]
                  for out_count in range(2)]
        ...
        full_cross_product = VGroup(cross_product, TexMobject("="), result)
        full_cross_product.arrange()
        full_cross_product.scale(0.75)
        full_cross_product.next_to(self.r_h_line,
                                   DOWN,
                                   buff=MED_SMALL_BUFF / 2)
        full_cross_product.remove(result)
        self.play(Write(full_cross_product), )
        movements = []
...
class Define2dCrossProduct(LinearTransformationScene):
    ...
    def show_transformation(self):
        ...
        self.play(ShowCreation(i_hat), ShowCreation(j_hat),
                  Write(basis_labels))
        self.wait()
        ...
        self.play(Write(i_words), ShowCreation(i_words.arrow), col1.set_color,
                  X_COLOR)
        self.wait()
        self.play(Transform(i_words, j_words),
                  Transform(i_words.arrow, j_words.arrow), col2.set_color,
                  Y_COLOR)
        self.wait()
        ...
        self.play(ShowCreation(self.v), ShowCreation(self.w),
                  FadeIn(self.v.label), FadeIn(self.w.label),
                  FadeIn(self.v.coord_array), FadeIn(self.w.coord_array),
    ...
    def transform_square(self):
        ...
        vect_stuffs = VGroup(*it.chain(*[[m, m.label, m.coord_array]
        ...
        self.apply_transposed_matrix([self.v_coords, self.w_coords])
        self.wait()
        self.play(FadeOut(self.plane), Animation(self.matrix),
        ...
        area_arrow = Arrow(area_words.get_bottom(),
                           self.square.get_center(),
                           color=WHITE)
        self.play(GrowFromCenter(det_text_brace), Write(area_words),
        ...
        self.play(Transform(self.square.get_point_mobject(), pm),
                  path_arc=-np.pi / 2)
        self.wait()
        self.play(*list(
            map(FadeOut,
...
class TwoDCrossProductExample(Define2dCrossProduct):
    ...
    def construct(self):
        ...
        equation_start = VGroup(cross_product,
        ...
        equals, dot1, minus, dot2, equals_result = syms = VGroup(
            *list(map(TexMobject, ["=", "\\cdot", "-", "\\cdot",
                                   "=%d" % det])))

        equation_end = VGroup(equals, v1.target, dot1, w2.target, minus,
...
class BiggerWhenPerpendicular(LinearTransformationScene):
    ...
    def construct(self):
        ...
        start_square, end_square = [
            square.copy().apply_function(transform) for transform in transforms
        ]

        for vect in v, w:
            self.play(ShowCreation(vect))
        group.remove(bigger)
        self.play(FadeIn(group), ShowCreation(start_square),
...
class WriteCrossProductProperties(Scene):
    def construct(self):
        ...
        length_words = TextMobject("Length of ", p_cash, "\\\\ = ",
        ...
        length_words.next_to(VGroup(cross_product, vector),
                             DOWN,
                             buff=LARGE_BUFF)
        perpendicular = TextMobject("\\centering Perpendicular to", v_cash,
                                    "and", w_cash)
    ...
    def construct(self):
        ...
        result = Matrix([
            VGroup(
                entry_dict["positive"],
                entry_dict["negative"],
            ).arrange() for entry_dict in entry_dicts
        ])
        ...
        for e1, e1_target, e2, e2_target, syms in movement_sets:
            ...
            self.play(Transform(e1.copy(), e1_target),
...
class DeterminantTrick(Scene):
    def construct(self):
        ...
        det_text = get_det_text(matrix)
        equals = TexMobject("=")
        equation = VGroup(v, TexMobject("\\times"), w, equals,
                          VGroup(det_text, matrix))
        equation.arrange()
        ...
        for quint in quints:
            ...
            term = VGroup(basis.t, lp, quint[1].t, quint[2].t, minus,
...
class ThereIsAReason(TeacherStudentsScene):
    def construct(self):
        ...
        self.random_blink(2)
        words = TextMobject("\\centering but there is a\\\\", "reason",
                            "for doing it")
...
class CrossAndDualWords(Scene):
    def construct(self):
        ...
        VGroup(vector_word, cross).arrange(
            RIGHT, buff=MED_SMALL_BUFF).center().shift(LEFT).to_edge(UP)
