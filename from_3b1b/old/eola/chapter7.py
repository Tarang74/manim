...
def get_vect_mob_projection(vector_to_project, stable_vector):
    return Vector(get_projection(vector_to_project.get_end(),
                                 stable_vector.get_end()),
                  color=vector_to_project.get_color()).fade()
...
class ThisSeriesOrdering(RandolphScene):
    def construct(self):
        ...
        chapters = VMobject(*[
            TextMobject("\\small " + text) for text in [
        ...
        self.play(FadeIn(chapters, lag_ratio=0.5, run_time=3),
...
class OneMustViewThroughTransformations(TeacherStudentsScene):
    def construct(self):
        ...
        self.change_student_modes("pondering", "plain", "raise_right_hand")
        self.random_blink(2)
        words = TextMobject("First, the ",
                            "standard view...",
                            arg_separator="\n")
        self.teacher_says(words)
        self.random_blink(2)
...
class ShowNumericalDotProduct(Scene):
    ...
    def construct(self):
        ...
        dot = TexMobject("\\cdot")
        products = VGroup(*[
            VGroup(p1.copy(), dot.copy(), p2.copy()).arrange(RIGHT,
                                                             buff=SMALL_BUFF)
            for p1, p2 in pairs
        ])
        ...
        if self.write_dot_product_words:
            self.play(inter_array_dot.set_color, YELLOW, ShowCreation(arrow),
                      Write(dot_product_words, run_time=2))
            self.wait()
        self.play(
            Transform(VGroup(*it.starmap(Group, pairs)).copy(),
                      products,
                      path_arc=-np.pi / 2,
                      run_time=2))
...
class GeometricInterpretation(VectorScene):
    ...
    def add_vectors(self):
        ...
        for vect, char, direction in zip([self.v, self.w], ["v", "w"],
                                         [DOWN + RIGHT, DOWN]):
    ...
    def project(self):
        ...
        v_norm, w_norm = [
            get_norm(vect.get_end()) for vect in (self.v, self.w)
        ]
        projected = Vector(self.stable_vect.get_end() * dot_product /
                           (self.stable_vect.get_length()**2),
                           color=self.proj_vect.get_color())
        projection_line = Line(self.proj_vect.get_end(),
                               projected.get_end(),
                               color=GREY)
    ...
    def show_lengths(self):
        ...
        product = TextMobject("=",
                              "(",
                              "Length of projected $\\vec{\\textbf{%s}}$" %
                              proj_char,
                              ")",
                              "(",
        ...
        proj_brace, stable_brace = braces = [
            Brace(Line(ORIGIN,
                       vect.get_length() * RIGHT * sgn), UP)
            for vect in (self.proj_vect, self.stable_vect) for sgn in
            [np.sign(np.dot(vect.get_end(), self.stable_vect.get_end()))]
        ]
    ...
    def handle_possible_negative(self):
        ...
        words.set_color(RED)
        words.next_to(VMobject(*self.product[2:]),
                      DOWN,
                      buff=LARGE_BUFF,
                      aligned_edge=LEFT)
        ...
        self.play(Write(neg), ShowCreation(arrow),
                  VMobject(*self.product[1:]).next_to, neg, Write(words))
        self.wait()
...
class ShowQualitativeDotProductValues(VectorScene):
    def construct(self):
        ...
        words = list(
            map(TextMobject,
                ["Similar directions", "Perpendicular", "Opposing directions"
                 ]))
        ...
        w.rotate(-np.pi / 6)
        shadow = Vector(v.get_end() * np.dot(v.get_end(), w.get_end()) /
                        (v.get_length()**2),
                        color=MAROON_E,
...
class AskAboutSymmetry(TeacherStudentsScene):
    def construct(self):
        ...
        question = TexMobject("\\text{Why does }", v, "\\cdot", w, "=", w,
                              "\\cdot", v, "\\text{?}")
...
class SymmetricVAndW(VectorScene):
    def construct(self):
        ...
        line_of_symmetry = DashedLine(FRAME_X_RADIUS * LEFT,
        ...
        for vect in w, v:
            ...
            self.wait()
        self.play(
            *list(map(FadeOut, [v.proj, v.proj_line, w.proj, w.proj_line])))
    ...
    def show_doubling(self, v, w):
        ...
        new_v.proj.fade()
        new_v.proj_line = Line(new_v.get_end(),
                               new_v.proj.get_end(),
                               color=GREY)

        v_tex, w_tex = ["\\vec{\\textbf{%s}}" % c for c in ("v", "w")]
        equation = TexMobject("(", "2", v_tex, ")", "\\cdot", w_tex, "=", "2(",
                              v_tex, "\\cdot", w_tex, ")")
        ...
        self.play(Transform(*[VGroup(mob, mob.label) for mob in (v, new_v)]),
                  run_time=2)
        last_mob = self.get_mobjects_from_last_animation()[0]
        self.remove(last_mob)
        self.add(*last_mob)
        Transform(VGroup(v.proj, v.proj_line),
                  VGroup(new_v.proj, new_v.proj_line)).update(1)  ##Hacky
        ...
        for full_v, projector, stable in zip([False, True], [w, v], [v, w]):
            self.play(Transform(projector.copy(), projector.proj),
            ...
            self.play(*[v_part.restore for v_part in v_parts],
                      rate_func=there_and_back,
                      run_time=2)
            self.wait()
            self.play(
                *list(map(FadeOut, [projector.proj, projector.proj_line])))
...
class LurkingQuestion(TeacherStudentsScene):
    def construct(self):
        ...
        #     width = 6,
        # )
        self.change_student_modes("raise_right_hand", "confused",
                                  "raise_left_hand")
        self.random_blink(5)
        answer = TextMobject(
            """
            The most satisfactory
            answer comes from""", "duality")
...
class Symbolic2To1DTransform(Scene):
    def construct(self):
        ...
        VMobject(input_array, in_arrow, func, out_arrow,
                 output_array).arrange(RIGHT, buff=SMALL_BUFF)
        ...
        self.play(Transform(mover,
                            Dot().move_to(func)),
                  ShowCreation(in_arrow),
                  rate_func=rush_into,
                  run_time=0.5)
        self.play(Transform(mover, output_array),
...
class AdditivityProperty(TwoDToOneDScene):
    ...
    def get_symbols(self):
        v_tex, w_tex = ["\\vec{\\textbf{%s}}" % c for c in ("v", "w")]
        if self.sum_before:
            tex_mob = TexMobject("L(", v_tex, "+", w_tex, ")")
            result = VGroup(tex_mob[0], VGroup(*tex_mob[1:4]), tex_mob[4])
        else:
            tex_mob = TexMobject("L(", v_tex, ")", "+", "L(", w_tex, ")")
            result = VGroup(
...
class NonLinearFailsDotTest(TwoDTo1DTransformWithDots):
    def construct(self):
        ...
        new_dots = VMobject(*[
            Dot(func(dot.get_center()),
                color=dot.get_color(),
                radius=dot.radius) for dot in dots
        ])
        words = TextMobject("Line of dots", "do not", "remain evenly spaced")
        words.set_color_by_tex("do not", RED)
        words.next_to(line, UP, buff=MED_SMALL_BUFF)
        array_tex = matrix_to_tex_string(["x", "y"])
        equation = TexMobject("f", "\\left(%s \\right)" % array_tex,
                              " = x^2 - y^2")
...
class FollowVectorViaCoordinates(TwoDToOneDScene):
    ...
    def construct(self):
        ...
        if self.concrete:
            new_labels = [
                TexMobject("(%d)" % num) for num in self.t_matrix[:, 0]
            ]
        else:
            new_labels = [
                TexMobject("L(\\hat{\\%smath})" % char) for char in ("i", "j")
            ]
        ...
        for label, vect, direction in zip(new_labels,
                                          list(bases) + [v], [UP, DOWN, UP]):
...
class TwoDOneDMatrixMultiplication(Scene):
    ...
    def show_product(self, matrix, vector):
        ...
        pairs = [
            VMobject(e1.copy(), TexMobject("\\cdot"), e2.copy()).arrange(
        ...
        for starter_pair, pair in zip(starter_pairs, pairs):
            self.play(
                Transform(VMobject(*starter_pair).copy(),
                          pair,
                          path_arc=-np.pi / 2))
        self.wait()
...
class AssociationBetweenMatricesAndVectors(Scene):
    ...
    def construct(self):
        ...
        VGroup(matrices_words, arrow,
               vectors_words).arrange(buff=MED_SMALL_BUFF)
...
class SomeKindOfConnection(Scene):
    ...
    def construct(self):
        ...
        for array in matrix, v_coords:
            array.add_to_back(BackgroundRectangle(array))
        self.play(*list(map(ShowCreation, [plane, number_line, v_line])) +
...
class ProjectOntoUnitVectorNumberline(VectorScene):
    ...
    def setup(self):
        ...
        if self.zoom_factor != 1:
            ...
            self.play(
                *[Transform(mob, mob.target) for mob in self.get_mobjects()])
            self.wait()
    ...
    def construct(self):
        ...
        self.wait()
        self.play(self.number_line.set_stroke,
                  None,
                  2,
    ...
    def get_vectors(self, num_vectors=10, randomize=True):
        ...
        vectors = VGroup(
            *[Vector(x * RIGHT + y * UP) for x, y in zip(x_vals, y_vals)])
    ...
    def get_proj_lines(self, dots, proj_dots):
        return VGroup(*[
            DashedLine(d1.get_center(),
                       d2.get_center(),
                       buff=0,
                       color=d1.get_color(),
                       dash_length=0.15) for d1, d2 in zip(dots, proj_dots)
        ])
...
class ProjectionFunctionSymbol(Scene):
    def construct(self):
        v_tex = "\\vec{\\textbf{v}}"
        equation = TexMobject("P(", v_tex, ")=", "\\text{number }", v_tex,
                              "\\text{ lands on}")
        equation.set_color_by_tex(v_tex, YELLOW)
        equation.shift(2 * UP)
        words = TextMobject("This projection function is", "linear")
...
class ProjectSingleVectorOnUHat(ProjectOntoUnitVectorNumberline):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(FadeOut(v), FadeOut(v.proj_line), FadeOut(v.label),
                  Write(dot_product))
        self.wait()
...
class AskAboutProjectionMatrix(Scene):
    def construct(self):
        ...
        for b_words, q_mark, direction in zip(basis_words,
                                              matrix.get_entries(),
                                              [UP, DOWN]):
...
class ProjectBasisVectors(ProjectOntoUnitVectorNumberline):
    ...
    def show_symmetry(self, vect, coord, coord_landing_spot):
        ...
        for v in vect, self.u_hat:
            v.proj_line = DashedLine(v.get_end(),
                                     v.proj.get_end(),
                                     color=v.get_color())
            v.proj_line.fade()
        ...
        added_mobjects = [
            mob for mob in self.get_mobjects()
...
class ShowSingleProjection(ProjectBasisVectors):
    ...
    def construct(self):
        ...
        proj_line = DashedLine(vector.get_end(),
                               proj.get_end(),
...
class GeneralTwoDOneDMatrixMultiplication(TwoDOneDMatrixMultiplication):
    ...
    def construct(self):
        ...
        to_fade = [
            ...
            if isinstance(m, Brace) or isinstance(m, TextMobject)
        ]
        ...
        words = VGroup(TextMobject("Matrix-vector product"),
                       TexMobject("\\Updownarrow"), TextMobject("Dot product"))
        words[0].set_color(BLUE)
        words[2].set_color(GREEN)
        words.arrange(DOWN)
        words.to_edge(LEFT)
        self.play(everything.to_corner, UP + RIGHT,
...
class AskAboutNonUnitVectors(TeacherStudentsScene):
    def construct(self):
        self.student_says("What about \\\\ non-unit vectors",
                          target_mode="raise_left_hand")
        self.random_blink(2)
...
class ScaleUpUHat(ProjectOntoUnitVectorNumberline):
    ...
    def transform_basis_vectors(self):
        ...
        for b, char in zip(bases, ["x", "y"]):
            ...
            b.proj_line = DashedLine(b.get_end(),
                                     b.proj.get_end(),
                                     dash_length=0.05)
        ...
        for b in bases:
            ...
            self.wait()
            self.play(Transform(mover, b.scaled_proj),
    ...
    def transform_some_vector(self):
        words = TextMobject("\\centering Project\\\\", "then scale")
        ...
        proj_line = DashedLine(v.get_end(),
                               proj.get_end(),
                               color=v.get_color())
        mover = v.copy()
...
class TwoDOneDTransformationSeparateSpace(Scene):
    ...
    def construct(self):
        ...
        start_words = TextMobject("\\centering Any time you have a \\\\",
...
class IsntThisBeautiful(TeacherStudentsScene):
    def construct(self):
        ...
        self.random_blink()
        duality_words = TextMobject("It's called", "duality")
...
class WhatTheVectorWantsToBe(Scene):
    ...
    def construct(self):
        ...
        words = TextMobject("What the vector", "\\\\ wants", "to be")
