...
class OpeningQuote(Scene):
    def construct(self):
        words = TextMobject([
            "Lisa:", "Well, where's my dad?\\\\ \\\\", "Frink:",
...
class IHatJHatKHatWritten(Scene):
    def construct(self):
        for char, color in zip(["\\imath", "\\jmath", "k"],
...
class PutTogether3x3Matrix(Scene):
    ...
    def construct(self):
        ...
        start_entries = reduce(op.add,
                               [a.get_entries().split() for a in arrays])
...
class ShowVCoordinateMeaning(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Transform(coords.get_entries().copy(),
...
class ShowMatrixVectorMultiplication(Scene):
    def construct(self):
        ...
        sexts = list(
            zip(matrix.get_mob_matrix().transpose(), columns,
...
class ComputerGraphicsAndRobotics(Scene):
    def construct(self):
        mob = VMobject(TextMobject("Computer graphics"),
