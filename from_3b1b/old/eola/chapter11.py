...
class RevisitOriginalQuestion(TeacherStudentsScene):
    def construct(self):
        ...
        self.teacher_says(question,
                          added_anims=[
                              ApplyMethod(self.get_students()[i].change_mode,
                                          mode)
...
class WhatIsA2DVector(LinearTransformationScene):
    ...
    def introduce_vector_and_space(self):
        ...
        two_d_vector = TextMobject("``Two-dimensional ",
                                   "vector",
                                   "''",
                                   arg_separator="")
    ...
    def bring_in_students(self):
        ...
        for student in students:
            ...
            anims += [
                ...
                MoveToTarget(v),
                MoveToTarget(coords), student.change_mode, "erm",
        ...
        self.play(cs_student.change_mode, "speaking", ShowCreation(bubble),
        ...
        for index, dim, direction in (0, 4, RIGHT), (2, 100, LEFT):
            ...
            self.play(*[
                Transform(
                    ...
                    rate_func=squish_rate_func(there_and_back, i /
                                               (2. * num_entries), i /
                                               (2. * num_entries) + 0.5),
                    run_time=2) for i in range(num_entries)
            ])
            self.wait()
...
class ManyCoordinateSystems(LinearTransformationScene):
    ...
    def animate_coordinates(self):
        ...
        self.wait(2)
        self.play(self.i_hat.restore, self.j_hat.restore,
...
class DeterminantAndEigenvectorDontCare(LinearTransformationScene):
    ...
    def construct(self):
        words = TextMobject("Determinant", "and", "eigenvectors",
...
class WhatIsSpace(Scene):
    def construct(self):
        ...
        self.add(compy, physy)
        self.play(physy.change_mode, "pondering", ShowCreation(physy.bubble),
                  ShowCreation(vector))
        self.play(compy.change_mode, "sassy", ShowCreation(compy.bubble),
...
class OtherVectorishThings(TeacherStudentsScene):
    def construct(self):
        words = TextMobject("There are other\\\\", "vectorish", "things...")
        words.set_color_by_tex("vectorish", YELLOW)
        self.teacher_says(words)
        self.change_student_modes("pondering", "raise_right_hand", "erm")
        self.random_blink(2)
...
class FunctionGraphScene(Scene):
    CONFIG = {
        ...
        "default_functions":
        [lambda x: (x**3 - 9 * x) / 20., lambda x: -(x**2) / 8. + 1],
        "default_names": ["f", "g", "h"],
        "x_min":
        -4,
        "x_max":
...
class AddTwoFunctions(FunctionGraphScene):
    def construct(self):
        ...
        self.play(FadeOut(rect), Transform(sum_def, final_sum_def),
                  FadeOut(arrow))
    ...
    def get_sum_definition(self, input_mob):
        result = VGroup(*it.chain(TexMobject("(f+g)", "("), [input_mob.copy(
        )], TexMobject(")", "=", "f("), [input_mob.copy(
        )], TexMobject(")", "+", "g("), [input_mob.copy()], TexMobject(")")))
        result.arrange()
...
class ScaleFunction(FunctionGraphScene):
    def construct(self):
        graph = self.get_function_graph(lambda x: self.default_functions[0](x),
                                        animate=False)
        scaled_graph = self.get_function_graph(lambda x: graph.get_function()
                                               (x) * 2,
                                               animate=False,
                                               add=False)
...
class ScaleVectorByCoordinates(Scene):
    def construct(self):
        ...
        self.play(Transform(two.copy(), two_targets),
...
class ShowSlopes(Animation):
    ...
    def interpolate_mobject(self, alpha):
        ...
        low, high = list(
            map(f, np.clip([alpha - self.dx, alpha + self.dx], 0, 1)))
...
class TransformationsAndOperators(TeacherStudentsScene):
    def construct(self):
        ...
        bubble.set_fill(BLACK, opacity=1)
        bubble.write("Yup!")
        self.play(teacher.change_mode, "hooray", ShowCreation(bubble),
...
class ManyFunctions(FunctionGraphScene):
    def construct(self):
        ...
        for i in range(100):
            ...
            self.add_random_function(run_time=run_time,
...
class FormalDefinitionOfLinear(LinearTransformationScene):
    ...
    def write_properties(self):
        title = TextMobject("Formal definition of linearity")
    ...
    def show_additive_property(self):
        ...
        self.play(MoveToTarget(v_label_copy), MoveToTarget(w_label_copy),
                  Write(plus))
        ...
        self.wait()
        self.play(*it.chain(list(map(FadeOut, to_fade)),
                            list(map(Animation, self.foreground_mobjects))))
        self.plane.restore()
        self.play(FadeIn(self.plane),
                  *list(map(Animation, self.foreground_mobjects)))
    ...
    def show_scaling_property(self):
        ...
        self.play(Write(rhs))
        self.wait()
        faders = [scaled_v_label, scaled_v, v_copy, v, rhs
...
class CalcStudentsKnowThatDerivIsLinear(TeacherStudentsScene):
    def construct(self):
        words = TextMobject(
            ...
            know that""", "$\\dfrac{d}{dx}$", "is linear")
...
class DerivativeIsLinear(Scene):
    ...
    def prepare_text(self):
        v_tex, w_tex = ["\\vec{\\textbf{%s}}" % s for s in "vw"]
        additivity = TexMobject("L(", v_tex, "+", w_tex, ")", "=", "L(", v_tex,
                                ")+L(", w_tex, ")")
        scaling = TexMobject("L(", "c", v_tex, ")=", "c", "L(", v_tex, ")")
        ...
        deriv_additivity = TexMobject(deriv_tex, "(", "x^3", "+", "x^2", ")",
                                      "=", deriv_tex, "(", "x^3", ")", "+",
                                      deriv_tex, "(", "x^2", ")")
        deriv_scaling = TexMobject(deriv_tex, "(", "4", "x^3", ")", "=", "4",
                                   deriv_tex, "(", "x^3", ")")
...
class PolynomialsHaveArbitrarilyLargeDegree(Scene):
    def construct(self):
        polys = VGroup(*list(
            map(TexMobject, [
                "x^{300} + 9x^2", "4x^{4{,}000{,}000{,}000} + 1",
                "3x^{\\left(10^{100}\\right)}", "\\vdots"
            ])))
...
class IntroducePolynomialSpace(Scene):
    ...
    def show_polynomial_cloud(self):
        ...
        self.play(FadeOut(cloud), FadeOut(polys), self.poly1.next_to, ORIGIN,
                  LEFT, self.poly1.set_color, WHITE)
    ...
    def split_individual_polynomial(self):
        ...
        self.play(
            GrowFromCenter(brace), Write(brace.text), *[
    ...
    def list_basis_functions(self):
        ...
        basis_group = VGroup(self.poly1[7][1], self.poly1[4], self.poly1[1],
                             x_cubed).copy()
        basis_group.generate_target()
        basis_group.target.arrange(DOWN,
                                   buff=0.75 * LARGE_BUFF,
                                   aligned_edge=LEFT)
        basis_group.target.to_edge(RIGHT, buff=MED_LARGE_BUFF)
        dots = TexMobject("\\vdots")
        dots.next_to(basis_group.target,
                     DOWN,
                     buff=MED_SMALL_BUFF,
                     aligned_edge=LEFT)
        ...
        self.play(FadeIn(title), ShowCreation(h_line),
                  MoveToTarget(basis_group), Write(dots))
    ...
    def show_example_coordinates(self):
        ...
        self.play(MoveToTarget(self.poly1), Write(equals), FadeOut(self.brace),
        ...
        for i, mob in (2, VGroup(*self.poly2[3:5])), (7, self.poly2[0]):
            self.play(new_coords.get_entries()[i].scale_in_place,
                      1.3,
                      mob.scale_in_place,
                      1.3,
    ...
    def derivative_as_matrix(self):
        matrix = Matrix([[str(j) if j == i + 1 else "0"
                          for j in range(4)] + ["\\cdots"]
                         for i in range(4)] + [["\\vdots"] * 4 + ["\\ddots"]])
        matrix.shift(2 * LEFT)
        diag_entries = VGroup(
            *[matrix.get_mob_matrix()[i, i + 1] for i in range(3)])
        ##Horrible
        ...
        deriv.target.next_to(matrix.target,
                             UP,
                             buff=MED_SMALL_BUFF,
                             aligned_edge=LEFT)
        ...
        poly = TexMobject("(", "1", "x^3", "+", "5", "x^2", "+", "4", "x", "+",
                          "5", ")")
        ...
        poly_result = TexMobject("3", "x^2", "+", "10", "x", "+", "4")
        ...
        result_entries = []
        for entry, diag_entry, dot in zip(relevant_entries, diag_entries,
                                          dots):
        ...
        result_array = Matrix(
            result_entries +
            [TexMobject("0"), TexMobject("\\vdots")])
        result_array.next_to(eq2)

        rects = [
            Rectangle(color=YELLOW).replace(
                VGroup(*matrix.get_mob_matrix()[i, :]),
                stretch=True).stretch_in_place(1.1,
                                               0).stretch_in_place(1.3, 1)
            for i in range(3)
        ]
        ...
        tuples = list(
            zip(relevant_entries, diag_entries, result_entries, rects,
                result_terms, coefs[1:]))
        ...
        for entry, diag_entry, result_entry, rect, result_term, coef in tuples:
            ...
            self.wait()
            self.play(
                entry.scale_in_place,
                1.2,
                diag_entry.scale_in_place,
                1.2,
            )
            diag_entry_target, dot, entry_target = result_entry
            self.play(Transform(entry.copy(), entry_target),
                      Transform(diag_entry.copy(),
                                diag_entry_target), entry.scale_in_place,
                      1 / 1.2, diag_entry.scale_in_place, 1 / 1.2, Write(dot))
            self.wait()
...
class MatrixVectorMultiplicationAndDerivative(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(teacher.change_mode, "speaking", ShowCreation(bubble),
                  Write(group))
        self.random_blink()
        ...
        self.play(MoveToTarget(group), ShowCreation(h_line), Write(words),
...
class CompareTermsInLinearAlgebraToFunction(Scene):
    def construct(self):
        ...
        lin_alg_concepts = VGroup(*list(
            map(TextMobject, [
                "Linear transformations",
                "Dot products",
                "Eigenvectors",
            ])))
        function_concepts = VGroup(*list(
            map(TextMobject, [
                "Linear operators",
                "Inner products",
                "Eigenfunctions",
            ])))
        for concepts, vect in (lin_alg_concepts, LEFT), (function_concepts,
                                                         RIGHT):
...
class YouAsAMathematician(Scene):
    def construct(self):
        ...
        self.play(ShowCreation(arrow), mathy.change_mode, "wave_1", mathy.look,
                  OUT)
        ...
        self.play(equations.next_to, mathy.eyes, RIGHT, 2 * LARGE_BUFF,
                  mathy.change_mode, "confused", mathy.look, RIGHT,
                  Write(bubble.content))
        self.wait()
        self.play(Blink(mathy))
...
class ShowVectorSpaces(Scene):
    def construct(self):
        ...
        v_lines = [
            Line(h_line.get_center(),
                 FRAME_Y_RADIUS * DOWN).shift(vect * FRAME_X_RADIUS / 3.)
    ...
    def get_vectors(self, n_vectors=10):
        vectors = VGroup(*[
            ...
            for scalar, angle in zip(2 * np.random.random(n_vectors) +
                                     0.5, np.linspace(0, 6, n_vectors))
        ])
    ...
    def get_functions(self):
        ...
        axes.scale(0.3)
        functions = VGroup(*[
            FunctionGraph(func, x_min=-4, x_max=4) for func in [
                lambda x: x**3 - 9 * x,
                lambda x: x**3 - 4 * x,
                lambda x: x**2 - 1,
            ]
...
class MathematicianSpeakingToAll(Scene):
    def construct(self):
        ...
        self.play(ShowCreation(bubble), Write(bubble.content),
                  mathy.change_mode, "sassy", mathy.look_at, others)
        self.play(Blink(others[3]))
        self.wait()
        thought_bubble = mathy.get_bubble(ThoughtBubble)
        self.play(FadeOut(bubble.content), Transform(bubble, thought_bubble),
                  mathy.change_mode, "speaking", mathy.look_at, bubble,
...
class ListAxioms(Scene):
    def construct(self):
        ...
        axioms = VGroup(*it.starmap(TexMobject, [
            ("1. \\,", u_tex, "+", "(", v_tex, "+", w_tex, ")=(", u_tex, "+",
             v_tex, ")+", w_tex),
            ("2. \\,", v_tex, "+", w_tex, "=", w_tex, "+", v_tex),
            ("3. \\,", "\\text{There is a vector }", "\\textbf{0}",
             "\\text{ such that }", "\\textbf{0}+", v_tex, "=", v_tex,
             "\\text{ for all }", v_tex),
            ("4. \\,", "\\text{For every vector }", v_tex,
             "\\text{ there is a vector }", "-", v_tex, "\\text{ so that }",
             v_tex, "+", "(-", v_tex, ")=\\textbf{0}"),
            ("5. \\,", "a", "(", "b", v_tex, ")=(", "a", "b", ")", v_tex),
            ("6. \\,", "1", v_tex, "=", v_tex),
            ("7. \\,", "a", "(", v_tex, "+", w_tex, ")", "=", "a", v_tex, "+",
             "a", w_tex),
            ("8. \\,", "(", "a", "+", "b", ")", v_tex, "=", "a", v_tex, "+",
             "b", v_tex),
        ]))
        tex_color_pairs = [(v_tex, YELLOW), (w_tex, MAROON_B), (u_tex, PINK),
                           ("a", BLUE), ("b", GREEN)]
        for axiom in axioms:
        ...
        axioms_word.scale(2)
        axioms_word.shift(FRAME_X_RADIUS * RIGHT / 2,
                          FRAME_Y_RADIUS * DOWN / 2)
...
class AxiomsAreInterface(Scene):
    def construct(self):
        ...
        red_line = Line(rules_of_nature.get_left(),
        ...
        self.play(ShowCreation(double_arrow),
...
class VectorSpaceOfPiCreatures(Scene):
    ...
    def add_creatures(self):
        ...
        for pi in creatures:
            pi.change_mode(
                random.choice([
                    "pondering", "pondering", "happy", "happy", "happy",
                    "confused", "angry", "erm", "sassy", "hooray", "speaking",
                    "tired", "plain", "plain"
                ]))
            ...
            pi.set_color(
                random.choice([
                    BLUE_B, BLUE_C, BLUE_D, BLUE_E, MAROON_B, MAROON_C,
                    MAROON_D, MAROON_E, GREY_BROWN, GREY_BROWN, GREY, YELLOW_C,
                    YELLOW_D, YELLOW_E
                ]))
    ...
    def show_sum(self, creatures):
        ...
        self.play(Transform(pi1.copy(), sum_pi), Transform(pi2.copy(), sum_pi),
...
class MathematicianDoesntHaveToThinkAboutThat(Scene):
    def construct(self):
        ...
        self.play(mathy.change_mode, "hooray", ShowCreation(bubble),
                  Write(words))
        self.play(Blink(mathy))
        self.wait()
...
class TextbooksAreAbstract(TeacherStudentsScene):
    def construct(self):
        ...
        self.change_student_modes("pondering")
        self.random_blink(2)
        self.teacher_says("...then in some different\\\\",
                          "context, like a function space")
        self.change_student_modes(*["pondering"] * 2)
        self.random_blink()
        self.teacher_says("Only then should you\\\\",
...
class WhatIsThree(Scene):
    def construct(self):
        what_is, three, q_mark = words = TextMobject("What is ",
                                                     "3",
                                                     "?",
                                                     arg_separator="")
        words.scale(1.5)
        ...
        self.wait()
        self.play(*[
            Transform(trip, three, lag_ratio=0.5, run_time=2)
...
class AbstractionIsThePrice(Scene):
    def construct(self):
        words = TextMobject("Abstractness", "is the price\\\\"
                            "of", "generality")
...
class GoodLuck(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("Good luck with \\\\ your future learning!",
                          target_mode="hooray")
