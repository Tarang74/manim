...
class AboutLinearAlgebra(Scene):
    ...
    def show_dependencies(self):
        ...
        self.wait()
        self.play(ShowCreation(arrows, lag_ratio=0.5),
                  FadeIn(all_subs),
                  run_time=2)
        self.wait()
        self.linalg = linalg
    ...
    def get_eigen_question(self):
        result = TextMobject("What the heck \\\\ does ``eigen'' mean?", )
...
class NumericVsGeometric(Scene):
    ...
    def clear_way_for_geometric(self):
        ...
        self.play(
            Transform(self.vline, new_line), Transform(self.hline, new_line),
            ApplyMethod(self.numeric.shift, (FRAME_HEIGHT + 1) * DOWN),
            ApplyMethod(self.matrix_vector_product.shift,
                        (FRAME_HEIGHT + 1) * DOWN),
    ...
    def list_geometric_benefits(self):
        ...
        thoughts = [
            matrix_to_mobject(EXAMPLE_TRANFORM), bulb,
        ...
        for randy, thought in zip(randys, thoughts):
            ...
            if curr_randy:
                self.play(Transform(curr_randy, randy),
...
class LinAlgPyramid(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(*[
            ApplyMethod(m.set_color, DARK_GREY) for m in (words[0], rects[0])
        ])
        self.wait()
    ...
    def list_applications(self, top_mob):
        subjects = [
            TextMobject(word).to_corner(UP + RIGHT) for word in [
                "computer science",
                "engineering",
                "statistics",
                "economics",
...
class PhysicsExample(Scene):
    def construct(self):
        ...
        parabola = FunctionGraph(lambda x: (3 - x) * (3 + x) / 4,
                                 x_min=-4,
                                 x_max=4)
    ...
    def projectile(self, parabola):
        ...
        self.play(MoveAlongPath(dot, parabola.copy(), **kwargs),
...
class ProfessorsTry(Scene):
    def construct(self):
        ...
        self.add(randy, morty)
        self.play(ApplyMethod(morty.change_mode, "speaking"),
                  FadeIn(speech_bubble), FadeIn(words))
        self.play(Blink(randy))
        self.play(FadeIn(thought_bubble))
...
class TableOfContents(Scene):
    def construct(self):
        ...
        self.play(ApplyMethod(entry3.set_color, YELLOW),
                  ShowCreation(arrow),
                  Write(added_words),
                  run_time=1)
        self.wait()
...
class ResourceForTeachers(Scene):
    def construct(self):
        ...
        bubble.clear()
        randys = VMobject(
            *[Randolph(color=c) for c in (BLUE_D, BLUE_C, BLUE_E)])
    ...
    def construct(self):
        ...
        for word, student, mode in zip(words, students.split(), modes):
            ...
            if last_word:
                ...
                last_word = word
                last_arrow = arrow
            self.play(word_anim, arrow_anim,
