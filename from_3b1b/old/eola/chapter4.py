...
class OpeningQuote(Scene):
    def construct(self):
        words = TextMobject([
            ...
            "can be shortened by 50\\% if one", "throws the matrices out."
        ])
...
class RecapTime(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(ApplyMethod(everyone.fade, 0.7),
                  ApplyMethod(student.change_mode, "confused"))
        self.play(Blink(student))
        self.wait()
        self.play(
            ApplyFunction(lambda m: m.change_mode("pondering").look(LEFT),
                          student))
        self.play(Blink(student))
        self.wait()
...
class DeterminedByTwoBasisVectors(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.add_transformable_label(i_hat,
                                     "\\hat{\\imath}",
                                     "\\hat{\\imath}",
                                     color=X_COLOR)
        j_hat = self.add_vector([0, 1], color=Y_COLOR)
        self.add_transformable_label(j_hat,
                                     "\\hat{\\jmath}",
                                     "\\hat{\\jmath}",
                                     color=Y_COLOR)
...
class IntroduceIdeaOfComposition(RotationThenShear):
    ...
    def track_basis_vectors(self):
        ...
        words = TextMobject([
            "Record where", "$\\hat{\\imath}$", "and", "$\\hat{\\jmath}$",
            "land:"
        ])
        ...
        matrix = Matrix(
            np.append(i_coords.copy().get_mob_matrix(),
...
class MoreComplicatedExampleVisually(LinearTransformationScene):
    ...
    def get_matrices(self):
        ...
        comp_matrix.get_entries().set_submobject_colors_by_gradient(
            YELLOW, PINK)
        ...
        VMobject(m2_mob, m1_mob,
                 comp_matrix).arrange(buff=0.1).to_corner(UP +
                                                          LEFT).shift(DOWN)
...
class MoreComplicatedExampleNumerically(MoreComplicatedExampleVisually):
    ...
    def construct(self):
        ...
        for i, (col, char, color) in enumerate(triplets):
            ...
            self.play(ApplyMethod(comp_col.set_color, BLACK),
...
class GeneralMultiplication(MoreComplicatedExampleNumerically):
    def get_result(self):
        entries = list(map(TexMobject, ["ae+bg", "af+bh", "ce+dg", "cf+dh"]))
        for mob in entries:
    ...
    def get_matrices(self):
        ...
        self.play(
            ...
            *[
                ...
                for q_mark, entry in zip(comp.split()[1].get_entries().split(),
...
class AskAboutCommutativity(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(
            Transform(VMobject(eq, q_marks), VMobject(neq), lag_ratio=0.5))
        self.wait()
...
class NoticeTheLackOfComputations(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(*[
            ApplyMethod(pi.change_mode,
                        "pondering",
                        rate_func=squish_rate_func(
                            smooth, *np.clip(unit + 0.5 * i, 0, 1)))
...
class AskAssociativityQuestion(Scene):
    ...
    def move_matrix_parentheses(self, morty, matrices):
        ...
        lp, rp = parens.split()
        state1 = VMobject(VectorizedPoint(m1.get_left()), m1, m2,
                          VectorizedPoint(m2.get_right()), m3)
...
class ThreeSuccessiveTransformations(LinearTransformationScene):
    ...
    def construct(self):
        ...
        for t_matrix, sym in zip(self.t_matrices, [c, b, a]):
            self.play(brace.next_to, sym, DOWN, sym.set_color, YELLOW,
...
class ExplanationTrumpsProof(Scene):
    def construct(self):
        ...
        self.play(explanation.scale_in_place, 1.5, proof.scale_in_place, 0.7)
        self.wait()
