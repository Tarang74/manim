...
class CoordinatesAsScalars(VectorScene):
    ...
    def general_idea_of_scalars(self, array, vector):
        ...
        new_i_hat = Vector(self.vector_coords[0] * i_hat.get_end(),
                           color=X_COLOR)
        new_j_hat = Vector(self.vector_coords[1] * j_hat.get_end(),
                           color=Y_COLOR)
        ...
        self.wait()
        self.play(Transform(i_hat, new_i_hat),
        ...
        self.play(Transform(x, new_x), Transform(y, new_y), FadeOut(i_hat),
                  FadeOut(j_hat), Write(new_array.get_brackets()),
                  FadeIn(VMobject(*starting_mobjects)), FadeOut(title))
        self.remove(x, y)
        self.add(new_array)
        return new_array
...
class WhatIfWeChoseADifferentBasis(Scene):
    def construct(self):
        self.play(
            Write("What if we chose different basis vectors?", run_time=2))
        self.wait(2)
...
class ShowVaryingLinearCombinations(VectorScene):
    CONFIG = {
        ...
        "leave_sum_vector_copies":
        False,
        "start_with_non_sum_scaling":
        True,
        "finish_with_standard_basis_comparison":
        True,
        "finish_by_drawing_lines":
        False,
    }
    ...
    def construct(self):
        ...
        v1_label = self.label_vector(v1,
                                     self.vector1_label,
                                     color=self.vector1_color,
                                     buff_factor=3)
        v2_label = self.label_vector(v2,
                                     self.vector2_label,
                                     color=self.vector2_color,
                                     buff_factor=3)
        label_anims = [
    ...
    def get_scalar_anims(self, v1, v2, v1_label, v2_label):
        def get_val_func(vect):
            ...
            return lambda a: np.dot(original_vect,
                                    vect.get_end() - vect.get_start()
                                    ) / square_norm

        return [
            RangingValues(tracked_mobject=label,
    ...
    def get_rate_func_pair(self):
        return [
            squish_rate_func(smooth, a, b) for a, b in [(0, 0.7), (0.3, 1)]
        ]

    def initial_scaling(self, v1, v2, label_anims, scalar_anims):
        scalar_pair = self.scalar_pairs.pop(0)
        anims = [
            ApplyMethod(v.scale, s, rate_func=rf) for v, s, rf in zip(
                [v1, v2], scalar_pair, self.get_rate_func_pair())
        ]
    ...
    def scale_with_sum(self, v1, v2, label_anims, scalar_anims):
        ...
        while self.scalar_pairs:
            scalar_pair = self.scalar_pairs.pop(0)
            anims = [
                ApplyMethod(v.scale, s / s_old, rate_func=rf) for v, s, s_old,
                rf in zip([v1, v2], scalar_pair, self.last_scalar_pair,
    ...
    def standard_basis_comparison(self, label_anims, scalar_anims):
        ...
        array = Matrix([
            mob.copy().set_color(color) for mob, color in zip(
                alt_coords, [self.vector1_color, self.vector2_color])
        ])
        array.scale(0.8)
        array.to_edge(UP)
        array.shift(RIGHT)
        ...
        anims = [
            Transform(*pair) for pair in zip(alt_coords,
                                             array.get_mob_matrix().flatten())
        ]
        # anims += [
        #     FadeOut(a.mobject)
        ...
        ij_array, x_line, y_line = self.vector_to_coords(self.sum_vector,
                ...
                self.play(
                    ...
                    UpdateFromFunc(
                        line, lambda l: l.put_start_and_end_on(
                            curr_tip, self.sum_vector.get_end())), *aux_anims)
                self.add(line, v2)
            self.wait()
...
class VectorsToDotsScene(VectorScene):
    ...
    def construct(self):
        ...
        rate_functions = [
            squish_rate_func(smooth,
                             float(x) / (len(vectors) + 2), 1)
...
class VectorsOnALine(VectorsToDotsScene):
    def get_vectors(self):
        return [
            Vector(a * np.array([1.5, 1])) for a in np.linspace(
                -FRAME_Y_RADIUS, FRAME_Y_RADIUS, self.num_vectors)
        ]

    def play_final_animation(self, vectors, rate_functions):
        line_copies = [
            Line(vectors[0].get_end(), vectors[-1].get_end()) for v in vectors
        ]
        self.play(*[
...
class VectorsInThePlane(VectorsToDotsScene):
    ...
    def play_final_animation(self, vectors, rate_functions):
        h_line = Line(FRAME_X_RADIUS * RIGHT,
                      FRAME_X_RADIUS * LEFT,
                      stroke_width=0.5,
                      color=BLUE_E)
        v_line = Line(FRAME_Y_RADIUS * UP,
                      FRAME_Y_RADIUS * DOWN,
                      stroke_width=0.5,
                      color=BLUE_E)
        line_pairs = [
            VMobject(h_line.copy().shift(y),
                     v_line.copy().shift(x)) for v in vectors
...
class AlternateDefOfLinearlyDependent(Scene):
    def construct(self):
        ...
        added_words2 = TextMobject("is a = b = c = 0")

        scalar_specification = TextMobject("For some choice of $a$ and $b$")
        ...
        self.play(
            ApplyFunction(lambda m: m.scale(0.5).to_corner(UP + LEFT),
                          everything))
        self.wait()
    ...
    def get_equations(self):
        ...
        equation1.submobjects = list(
            it.chain([VectorizedPoint(u.get_center())],
                     equation1.submobjects[1:],
                     [VectorizedPoint(u.get_left())], [u]))
        equation2 = TexMobject([
