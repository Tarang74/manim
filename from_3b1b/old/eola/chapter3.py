...
class DescribeTransformation(Scene):
    ...
    def add_title(self):
        ...
        self.wait()
        self.play(GrowFromCenter(brace), Write(function),
    ...
    def show_function(self):
        ...
        num_outputs = VMobject(*[TexMobject(str(num**2)) for num in nums])
...
class ThinkinfOfFunctionsAsGraphs(VectorScene):
    def construct(self):
        ...
        self.play(ApplyPointwiseFunction(collapse_func, axes),
...
class TransformJustOneVector(VectorScene):
    def construct(self):
        ...
        t_matrix = [[0, -1], [2, -1]]
        v1 = Vector(v1_coords)
        v2 = Vector(np.dot(np.array(t_matrix).transpose(), v1_coords),
                    color=PINK)
        ...
        self.wait()
        self.remove(v2)
        self.play(Transform(v1.copy(), v2, path_arc=-np.pi / 2, run_time=3),
                  ApplyMethod(v1.fade))
        self.wait()
...
class TransformManyVectors(LinearTransformationScene):
    ...
    def construct(self):
        self.lock_in_faded_grid()
        vectors = VMobject(*[
            Vector([x, y]) for x in np.arange(-int(FRAME_X_RADIUS) + 0.5,
                                              int(FRAME_X_RADIUS) + 0.5)
            for y in np.arange(-int(FRAME_Y_RADIUS) + 0.5,
                               int(FRAME_Y_RADIUS) + 0.5)
        ])
        vectors.set_submobject_colors_by_gradient(PINK, YELLOW)
        t_matrix = self.transposed_matrix
        transformed_vectors = VMobject(*[
            Vector(np.dot(np.array(t_matrix).transpose(),
                          v.get_end()[:2]),
                   color=v.get_color()) for v in vectors.split()
        ])
...
class ApplyComplexFunction(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(
            ApplyMethod(self.plane.apply_complex_function,
                        self.function,
                        run_time=5,
                        path_arc=np.pi / 2))
        self.wait()
...
class LookToWordLinear(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(Transform(faded_linear, linear),
...
class IntroduceLinearTransformations(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.play(Write(lines_rule, run_time=2), )
        self.wait()
        self.play(Write(origin_rule, run_time=2), ShowCreation(arrow),
                  GrowFromCenter(dot))
        self.wait()
...
class SneakyNonlinearTransformationExplained(SneakyNonlinearTransformation):
    ...
    def setup(self):
        LinearTransformationScene.setup(self)
        diag = Line(FRAME_Y_RADIUS * LEFT + FRAME_Y_RADIUS * DOWN,
...
class Rotation(SimpleLinearTransformationScene):
    ...
    def construct(self):
        self.transposed_matrix = [[np.cos(self.angle),
                                   np.sin(self.angle)],
                                  [-np.sin(self.angle),
                                   np.cos(self.angle)]]
        SimpleLinearTransformationScene.construct(self)
...
class YetAnotherLinearTransformation(SimpleLinearTransformationScene):
    ...
    def construct(self):
        ...
        self.play(Write(words))
        self.wait()
        self.play(ApplyMethod(self.plane.fade, 0.7),
                  ApplyMethod(self.background_plane.fade, 0.7),
                  Write(formula, run_time=2), Animation(words))
        self.wait()
...
class FollowIHatJHat(LinearTransformationScene):
    ...
    def construct(self):
        ...
        i_label = self.label_vector(i_hat,
                                    "\\hat{\\imath}",
                                    color=X_COLOR,
                                    label_scale_factor=1)
        j_hat = self.add_vector([0, 1], color=Y_COLOR)
        j_label = self.label_vector(j_hat,
                                    "\\hat{\\jmath}",
                                    color=Y_COLOR,
                                    label_scale_factor=1)
...
class TrackBasisVectorsExample(LinearTransformationScene):
    ...
    def introduce_vector(self):
        ...
        pre_def = VMobject(
            ...
            VMobject(
                *[mob.copy() for mob in coords.get_mob_matrix().flatten()]))
    ...
    def show_linear_combination(self, clean_up=True):
        ...
        self.play(
            ApplyFunction(lambda m: m.scale(self.v_coords[0]).fade(0.3),
                          i_hat_copy))
        self.play(
            ApplyFunction(lambda m: m.scale(self.v_coords[1]).fade(0.3),
                          j_hat_copy))
...
class DeduceResultWithGeneralCoordinates(Scene):
    def construct(self):
        ...
        VMobject(row1.split()[0],
                 row2.split()[0],
                 row2.split()[1]).set_color(X_COLOR)
        VMobject(row1.split()[1],
                 row1.split()[4],
                 row2.split()[2],
                 row2.split()[5]).set_color(YELLOW)
        VMobject(row1.split()[3], row2.split()[4]).set_color(Y_COLOR)
        result = Matrix([row1, row2])
        result.show()
        vect_group = VMobject(vect, rto, x.copy(), i_coords.copy(), plus,
                              y.copy(), j_coords.copy(), equals, result)
...
class MatrixVectorMultiplication(LinearTransformationScene):
    ...
    def build_to_matrix(self):
        ...
        abstract_matrix = np.append(i_coords.get_mob_matrix(),
    ...
    def multiply_by_vector(self, matrix):
        ...
        formula = VMobject(v1.copy(), col1, TexMobject("+"), v2.copy(), col2)
    ...
    def reposition_matrix_and_vector(self, matrix, vector, formula):
        ...
        self.play(Transform(start_state, end_state, lag_ratio=0),
...
class ColumnsToBasisVectors(LinearTransformationScene):
    ...
    def move_matrix_columns(self, transposed_matrix, vector_coords=None):
        ...
        i_coords_start = VMobject(matrix_background.copy(), col1.copy(),
        ...
        j_coords_start = VMobject(matrix_background.copy(), col2.copy(),
                                  matrix_brackets.copy())
    ...
    def scale_and_add(self, vector, vector_coords):
        ...
        self.play(Transform(i_copy, i_target),
                  ApplyMethod(coord1.next_to, i_target.get_center(), DOWN))
        self.play(Transform(j_copy, j_target),
...
class Describe90DegreeRotation(LinearTransformationScene):
    ...
    def construct(self):
        ...
        for vect, color, col in [(self.i_hat, X_COLOR, col1),
...
class TransformationsAreFunctions(Scene):
    def construct(self):
        ...
        for v, a in [(starting_vector, start_arrow),
                     (ending_vector, ending_arrow)]:
...
class UsedToThinkinfOfFunctionsAsGraphs(VectorScene):
    ...
    def show_inputs_and_output(self):
        ...
        arrows = VMobject(
            *[Arrow(LEFT, RIGHT).next_to(mob) for mob in inputs.split()])
        outputs = VMobject(*[
...
class TryingToVisualizeFourDimensions(Scene):
    def construct(self):
        ...
        self.play(ApplyMethod(randy.change_mode, "pondering"),
                  ShowCreation(bubble), Write(thought))
...
class MoveAroundAllVectors(LinearTransformationScene):
    ...
    def construct(self):
        ...
        if self.focus_on_one_vector:
            ...
            self.play(
                *[FadeOut(v) for v in vectors.split() if v is not vector])
            self.wait()
