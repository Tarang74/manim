...
class Blob(Circle):
    ...
    def __init__(self, **kwargs):
        ...
        self.apply_complex_function(lambda z: z * (1 + self.random_nudge_size *
                                                   (random.random() - 0.5)))
...
class FlipSpaceOver(Scene):
    def construct(self):
        ...
        plane2 = NumberPlane(y_radius=FRAME_X_RADIUS,
                             color=RED_D,
...
class WriteNegativeDeterminant(NegativeDeterminantTransformation):
    def construct(self):
        ...
        det_text = get_det_text(matrix,
                                determinant=np.linalg.det(self.t_matrix))
...
class RightHandRule(Scene):
    ...
    def construct(self):
        ...
        v1_label, v2_label, v3_label = [
            TexMobject(label_tex).scale(1.5) for label_tex in self.labels_tex
        ]
        ...
        self.play(ShowCreation(hand.outline, run_time=2, rate_func=linear),
                  FadeIn(hand.inlines))
        self.wait()
