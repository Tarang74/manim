...
class AskQuestion(Scene):
    ...
    def construct(self):
        ...
        for word, part in zip(strings, question):
            ...
            self.play(LaggedStartMap(FadeIn, part),
...
class DisectQuestion(TeacherStudentsScene):
    ...
    def hold_up_question(self):
        question = TextMobject("What does it mean to", "have", "a", "Bitcoin?")
        question.set_color_by_tex("have", YELLOW)
    ...
    def list_topics(self):
        topics = TextMobject("Digital signatures, ", "Proof of work, ",
...
class CryptocurrencyEquation(Scene):
    def construct(self):
        parts = TextMobject("Ledger", "- Trust", "+ Cryptography",
                            "= Cryptocurrency")
...
class Hype(TeacherStudentsScene):
    def construct(self):
        ...
        phrases = list(
            map(TextMobject,
                ["I want some!", "I'll get rich, right?", "Buy them all!"]))
...
class NoCommentOnSpeculation(TeacherStudentsScene):
    def construct(self):
        ...
        group.add(cross)
        self.play(group.shift, FRAME_WIDTH * RIGHT, self.teacher.change,
                  "happy")
        self.wait()

        self.student_says("But...what are they?",
                          student_index=0,
        ...
        self.wait()
        self.play(
            VGroup(mining_graphic, mining_cross).shift, FRAME_WIDTH * RIGHT)
...
class LayOutPlan(LedgerScene):
    ...
    def ask_question(self):
        ...
        self.wait()
        self.play(group.scale, 0.7, group.next_to, ORIGIN, RIGHT,
                  group.to_edge, UP)
...
class IntroduceLedgerSystem(LedgerScene):
    ...
    def tally_it_all_up(self):
        ...
        for name in names:
            ...
            if name == "you":
                verb = verb[:-1]
            result = TextMobject(verb, "\\$%d" % abs(amount))
...
class AddFraudulentLine(LedgerScene):
    ...
    def alice_reacts(self):
        ...
        bubble.write("Hey!")
        self.play(Animation(self.bob.pupils), self.alice.change, "angry",
                  FadeIn(bubble), Write(bubble.content, run_time=1))
        self.wait(3)
...
class DescribeDigitalSignatures(LedgerScene):
    ...
    def generate_key_pairs(self):
        ...
        public_keys, private_keys = [
            VGroup(*[
                ...
                "pk", public_key_strings), ("sk", private_key_strings)]
        ]
        key_pairs = [
        ...
        self.play(
            ReplacementTransform(VGroup(VGroup(public.copy())), public_keys))
        self.play(
            ReplacementTransform(VGroup(VGroup(private.copy())), private_keys))
        self.wait()
    ...
    def keep_secret_key_secret(self):
        ...
        locks = VGroup(
            *[lock.copy().next_to(rect, LEFT, SMALL_BUFF) for rect in rects])
    ...
    def show_signing_functions(self):
        ...
        self.wait()
        self.play(*[
            FadeIn(part) for part in verify if part not in
            [verify.message, verify.signature, verify.pk, verify[-1]]
        ])
        self.wait()
        self.play(
...
class TryGuessingDigitalSignature(Scene):
    def construct(self):
        ...
        self.play(GrowFromCenter(brace),
                  FadeIn(zeros, lag_ratio=0.5, run_time=3))
        self.wait()
        for n in range(2**10):
            last_row = zeros[-1]
            binary = bin(n)[2:]
...
class WriteTwoTo256PossibleSignatures(Scene):
    def construct(self):
        words = TextMobject("$2^{256}$", "possible\\\\", "signatures")
        words.scale(2)
...
class FeelConfidentWithVerification(PiCreatureScene):
    ...
    def show_verification(self):
        ...
        self.add(verify)
        self.play(GrowFromCenter(brace), Write(signature),
...
class CharlieRacksUpDebt(SignedLedgerScene):
    ...
    def construct(self):
        ...
        self.play(*[
            ...
            for pi in self.pi_creatures if pi is not self.charlie
        ])
        self.play(self.charlie.shift,
...
class DontAllowOverdrawing(InitialProtocol):
    def construct(self):
        ...
        lines = list(
            map(self.get_new_item, [
                "Anyone can add lines to the Ledger \\,",
                "Only signed transactions are valid \\,", "No overspending"
            ]))
...
class LedgerWithInitialBuyIn(SignedLedgerScene):
    ...
    def everyone_buys_in(self):
        ...
        moneys = VGroup(*[TexMobject("\\$100") for pi in self.pi_creatures])
    ...
    def add_initial_lines(self):
        ...
        for name in self.get_names():
            new_line = TextMobject(name.capitalize(),
                                   "get" if name == "you" else "gets",
                                   "\\$100")
            new_line.set_color_by_tex(name.capitalize(),
    ...
    def running_balance(self):
        ...
        rect.stretch_to_fit_height(self.ledger.get_height() -
                                   2 * MED_SMALL_BUFF)
        ...
        lines = VGroup(
            *list(map(TextMobject, ["\\$100", "\\$50", "\\$0", "Overdrawn"])))
...
class RenameToLedgerDollars(LedgerScene):
    ...
    def rewrite_as_ledger_dollars(self):
        ...
        self.play(Write(ledger_dollars), FadeOut(self.bubble),
...
class ExchangeCashForLedgerDollars(LedgerScene):
    ...
    def add_title(self):
        ...
        title = TextMobject("Exchange", "LD", "for", "\\$\\$\\$")
    ...
    def give_ten_dollar_bill(self):
        ...
        self.play(ReplacementTransform(small_bill, bill), self.alice.change,
                  "raise_right_hand")
        self.play(ShowCreation(arrow))
...
class BitcoinIsALedger(Scene):
    ...
    def add_currency_to_tx_history(self):
        equation = TextMobject("Currency", "=", "Transaction history")
...
class TransitionToDistributedLedger(DistributedLedgerScene):
    ...
    def add_various_payements(self):
        payments = VGroup(*[
            self.add_payment_line_to_ledger(*payment) for payment in [
                ("Alice", "Bob", 20),
                ("Charlie", "You", 100),
                ("You", "Alice", 50),
                ("Bob", "You", 30),
    ...
    def broadcast_transaction(self):
        ...
        self.play(self.alice.change, "raise_right_hand", payment,
                  Write(payment, run_time=2))
        self.wait()
        self.play(
            ReplacementTransform(VGroup(payment),
                                 payment_copies,
                                 run_time=3,
                                 rate_func=squish_rate_func(smooth, 0.5, 1)),
            Broadcast(self.alice.get_corner(UP + RIGHT)), self.alice.change,
            "happy")
        self.wait()
...
class BobDoubtsBroadcastTransaction(DistributedLedgerScene):
    ...
    def bob_receives_transaction(self):
        ...
        payment = TextMobject("Alice", "pays", "Bob", "10 LD")
        payment.set_color_by_tex("Alice", self.alice.get_color())
        payment.set_color_by_tex("Bob", self.bob.get_color())
        payment.set_color_by_tex("LD", YELLOW)
        ...
        self.play(payment.scale, 0.3, payment.next_to, bob.ledger[-1], DOWN,
                  SMALL_BUFF)
...
class TrustComputationalWorkSupplement(Scene):
    def construct(self):
        words = TextMobject("Main tool: ", "Cryptographic hash functions")
...
class FraudIsInfeasible(Scene):
    def construct(self):
        words = TextMobject("Fraud", "$\\Leftrightarrow$",
...
class IntroduceSHA256(Scene):
    ...
    def introduce_evaluation(self):
        ...
        for brace, text in brace_text_pairs:
            if brace is digest_brace:
                self.play(
                    LaggedStartMap(FadeIn, digest, run_time=4, lag_ratio=0.05))
                self.wait()
        ...
        for new_group in groups[1:]:
            ...
            self.play(Transform(lhs,
                                new_lhs), message_brace.stretch_to_fit_width,
        ...
        new_sha_text = TextMobject("Cryptographic", "hash function")
    ...
    def guess_and_check(self):
        ...
        for x in range(32):
            message = "Guess \\#%d" % x
            lhs = TextMobject("SHA256(``", message, "'') = ", arg_separator="")
        ...
        for new_group in groups[1:]:
            ...
            self.play(
                Transform(group[1], new_group[1], run_time=1, lag_ratio=0.5))
...
class SHA256ToProofOfWork(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(Write(sha, run_time=1), self.teacher.change,
                  "raise_right_hand")
        self.play(ShowCreation(arrow))
...
class IntroduceNonceOnTrasactions(LedgerScene):
    ...
    def hash_with_nonce(self):
        ...
        self.play(
            LaggedStartMap(FadeIn, VGroup(special_word, nonce_brace, nonce)))
        self.wait()
        self.play(nonce.next_to, ledger.content, DOWN, MED_SMALL_BUFF, LEFT,
                  FadeOut(special_word), FadeOut(nonce_brace))
        ...
        self.play(
            LaggedStartMap(Transform,
                           decomposed_ledger.copy(),
                           lambda m: (m, point),
                           run_time=1))
        ...
        self.play(
            LaggedStartMap(ApplyMethod, zeros, lambda m:
                           (m.set_color, YELLOW)))
        self.wait(2)
    ...
    def guess_and_check(self):
        ...
        for x in range(1, 13):
            ...
            self.play(
                Transform(self.nonce, nonce, run_time=1 if x == 1 else 0.3))
            self.play(Transform(self.digest, digest, run_time=1,
                                lag_ratio=0.5))
        self.wait()
    ...
    def name_proof_of_work(self):
        ...
        arrow = Arrow(words.get_top(),
                      self.nonce.get_bottom(),
                      color=WHITE,
                      tip_length=0.15)
...
class IntroduceBlockChain(Scene):
    ...
    def setup(self):
        ...
        self.name_colors = [
            ls.get_color_from_name(name) for name in self.names
        ]
    ...
    def divide_ledger_into_blocks(self):
        ...
        arrows = VGroup(*[
            Arrow(blocks_word.get_top(),
                  block.get_bottom(),
                  buff=MED_LARGE_BUFF,
                  color=WHITE) for block in blocks
        ])

        self.play(LaggedStartMap(FadeIn, blocks))
        self.play(Write(blocks_word),
                  LaggedStartMap(
                      ShowCreation,
                      arrows,
                      run_time=1,
    ...
    def chain_blocks_together(self):
        ...
        for block in blocks:
                ...
                block.prev_hash.get_center()[1], 0
            ])
        ...
        for i in range(3):
            self.play(ReplacementTransform(digests[i], prev_hashes[i + 1]),
    ...
    def get_block(self):
        ...
        h_line1, h_line2 = [
            Line(rect.get_left(),
                 rect.get_right()).shift(0.3 * rect.get_height() * vect)
            for vect in (UP, DOWN)
        ]
...
class DistributedBlockChainScene(DistributedLedgerScene):
    ...
    def get_block_chain(self):
        ...
        for b1, b2 in zip(blocks, blocks[1:]):
            ...
            target_point = interpolate(b2.get_left(), b2.get_corner(UP + LEFT),
                                       0.8)
...
class IntroduceBlockCreator(DistributedBlockChainScene):
    ...
    def broadcast_transactions(self):
        ...
        for from_name, to_name, amount in payment_parts:
            ...
            payment = TextMobject(from_name, verb, to_name, "%d LD" % amount)
            payment.set_color_by_tex("LD", YELLOW)
            for name in self.get_names():
                payment.set_color_by_tex(name.capitalize(),
                                         self.get_color_from_name(name))
            payment.scale(0.7)
        ...
        payment_targets.next_to(self.block_creator_labels, RIGHT,
                                MED_LARGE_BUFF)
        payment_targets.shift(UP)

        anims = []
        alpha_range = np.linspace(0, 0.5, len(payments))
        for pi, payment, alpha in zip(self.pi_creatures, payments,
                                      alpha_range):
    ...
    def collect_transactions(self):
        ...
        self.play(FadeIn(block), MoveToTarget(payments), creator.change,
                  "raise_right_hand")
        self.wait()
        block.add(payments)
    ...
    def add_block_reward(self):
        ...
        new_transaction = TextMobject(
            self.block_creator_labels[0].get_tex_string(), "gets", "10 LD")
    ...
    def broadcast_block(self):
        ...
        self.play(*it.chain(arrow_creations, [
...
class MiningIsALottery(IntroduceBlockCreator):
    ...
    def create_pi_creatures(self):
        ...
        miners = VGroup(
            *[PiCreature(color=GREY) for n in range(self.n_miners)])
        miners.scale(0.5)
    ...
    def make_guesses(self):
        ...
        self.play(
            winner.change, "hooray", *[
                ApplyMethod(
                    VGroup(self.blocks[i], self.arrows[i], nonces[i],
                           digests[i]).fade, 0.7)
                for i in range(len(self.blocks)) if i is not winner_index
            ])
        self.play(
            *[ApplyMethod(loser.change, "angry", winner) for loser in losers])
        self.wait(2)
...
class TwoBlockChains(DistributedBlockChainScene):
    ...
    def defer_to_longer(self):
        randy = self.randy
        self.n_blocks -= 1
        block_chains = VGroup(self.block_chain,
        ...
        arrows = VGroup(*[
            Arrow(conflicting.get_bottom(),
                  block_chain.get_top(),
                  color=RED,
                  buff=MED_LARGE_BUFF) for block_chain in block_chains
        ])
        ...
        self.play(
            randy.change, "confused", chain, Broadcast(corner),
            ApplyMethod(chain.restore,
        ...
        self.to_fade = VGroup(conflicting, arrows, longer_chain_rect,
                              checkmark)
    ...
    def break_tie(self):
        ...
        block_chains.target.arrange(DOWN,
                                    buff=MED_LARGE_BUFF,
                                    aligned_edge=LEFT)
        ...
        self.play(
            ApplyMethod(arrow_block.next_to,
                        block_chains[0],
                        RIGHT,
                        0,
                        run_time=3,
        ...
        self.play(ShowCreation(rect), Write(checkmark), randy.change, "happy",
                  arrow_block)
        self.wait(2)
...
class AskAboutTrustingWork(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(3)
        self.teacher_says("Well, let's try\\\\ fooling someone",
...
class DoubleSpendingAttack(DistributedBlockChainScene):
    ...
    def show_fraudulent_block(self):
        ...
        self.play(ShowCreation(block), Write(block.content), self.alice.change,
                  "raise_left_hand")
    ...
    def send_to_bob(self):
        ...
        VGroup(arrow, block.target).next_to(self.bob.block_chain.blocks[-1],
                                            RIGHT,
                                            buff=0)
    ...
    def get_fraud_block(self):
        ...
        for word, vect, color in tuples:
            ...
            mob.next_to(block.get_edge_center(vect),
                        -vect,
        ...
        payment.set_color_by_tex("LD", YELLOW)
        payments = VGroup(TexMobject("\\vdots"), payment,
                          TexMobject("\\vdots"))
...
class AliceRacesOtherMiners(DoubleSpendingAttack):
    ...
    def initialize_characters(self):
        ...
        miners = VGroup(
            *[PiCreature(color=GREY) for x in range(self.n_miners)])
    ...
    def find_proof_of_work(self):
        ...
        self.play(Indicate(fraud_block.proof_of_work), self.alice.change,
                  "hooray")
        self.wait()
    ...
    def race_between_alice_and_miners(self):
        ...
        chain = self.block_chain
        winners = [
            "Alice", "Alice", "Miners", "Miners", "Miners", "Alice", "Miners",
            "Miners", "Miners", "Alice", "Miners", "Miners"
        ]
...
class WhenToTrustANewBlock(DistributedBlockChainScene):
    def construct(self):
        ...
        words = list(
            map(TextMobject, [
                "Don't trust yet", "Still don't trust", "...a little more...",
                "Maybe trust", "Probably safe", "Alright, you're good."
            ]))
...
class MainIdeas(Scene):
    def construct(self):
        ...
        ideas = VGroup(*[
            TextMobject("$\\cdot$ " + words) for words in [
                "Digital signatures",
...
class VariableProofOfWork(WhenToTrustANewBlock):
    ...
    def change_requirement(self):
        digest = self.digest
        requirement = TextMobject("Must start with \\\\", "60", "zeros")
...
class ShowGeometricSum(Scene):
    def construct(self):
        equation = TexMobject("210{,}000", "(", "50", "+", "25", "+", "12.5",
                              "+", "6.25", "+\\cdots", ")", "=",
                              "21{,}000{,}000")
...
class TransactionFeeExample(PiCreatureScene):
    def construct(self):
        ...
        incentive_words = TextMobject("Incentivizes miner \\\\ to include")
        incentive_words.next_to(rect, DOWN, buff=1.5)
        incentive_words.shift(2 * RIGHT)
        arrow = Arrow(incentive_words.get_top(),
                      rect.get_bottom(),
                      buff=MED_LARGE_BUFF)
        ...
        self.wait()
        self.play(Write(incentive_words), ShowCreation(arrow), alice.change,
                  "pondering")
        self.wait(2)
...
class ShowBitcoinBlockSize(LedgerScene):
    ...
    def construct(self):
        ...
        payments = VGroup(*[
            self.add_payment_line_to_ledger(*args) for args in [
                ("Alice", "Bob", "0.42"),
                ("You", "Charlie", "3.14"),
                ("Bob", "You", "2.72"),
                ("Alice", "Charlie", "4.67"),
        ...
        payments_rect = SurroundingRectangle(payments,
                                             color=WHITE,
                                             buff=MED_LARGE_BUFF)
...
class CurrentAverageFees(Scene):
    def construct(self):
        fees = TextMobject("Current average fees: ", "$\\sim 0.0013$ BTC",
                           "$\\approx$", "\\$3.39")
...
class ShowManyExchanges(Scene):
    ...
    def construct(self):
        ...
        exchanges = VGroup(*[
            VGroup(*[exchange.copy()
                     for x in range(3)]).arrange(RIGHT, buff=2 * LARGE_BUFF)
            for y in range(3)
...
class ProtocolLabs(PiCreatureScene):
    def construct(self):
        ...
        self.wait()
        self.play(logo.scale, 0.5, logo.to_corner, UP + LEFT,
