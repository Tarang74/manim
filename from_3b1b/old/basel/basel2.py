    ...
    def update(self):
        ...
        for face, (c1, c2) in zip(self, adjacent_pairs(corners)):
            ...
            for a1, a2 in zip(alphas, alphas[1:]):
                face.add(
                    Polygon(interpolate(source_point, c1, a1),
...
class IntroScene(PiCreatureScene):
    ...
    def build_up_euler_sum(self):
        ...
        euler_sum = self.euler_sum = TexMobject("1",
                                                "+",
                                                "{1 \\over 4}",
                                                "+",
                                                "{1 \\over 9}",
        ...
        for i in range(4):
            ...
            if i == 0:
                ...
                anims += [
                    ChangeDecimalToValue(partial_sum_decimal,
                                         series_terms[i + 1],
                                         run_time=1,
    ...
    def show_history(self):
        ...
        self.wait(2)
        self.play(ReplacementTransform(self.q_marks, pi_answer),
                  FadeIn(equals_sign), FadeOut(self.brace),
...
class MathematicalWebOfConnections(PiCreatureScene):
    ...
    def complain_that_pi_is_not_about_circles(self):
        ...
        words = self.words = TextMobject("I am not", "fundamentally \\\\",
                                         "about circles")
    ...
    def show_other_pi_formulas(self):
        ...
        self.play(jerk.change, "sassy", randy.change, "raise_right_hand",
                  FadeOut(jerk.bubble), words.next_to, jerk, UP,
    ...
    def show_web_of_connections(self):
        ...
        self.play(Write(title), LaggedStartMap(ShowCreation, edges,
                                               run_time=3),
...
class AskAboutBrightness(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.student_says("What do you mean \\\\ by ``brightness''?",
                          added_anims=[
...
class IntroduceScreen(Scene):
    ...
    def setup_elements(self):
        ...
        screen = self.screen = Rectangle(width=0.05,
        ...
        screen.save_state()
        self.play(FadeIn(morty), screen.match_height, morty.eyes,
    ...
    def setup_angle(self):
        ...
        angle_update_func = lambda x: self.light_source.spotlight.opening_angle(
        ) / DEGREES
    ...
    def shoot_rays(self, show_creation_kwargs=None):
        ...
        max_angle = np.max(
            [angle_of_vector(point - source_point) for point in screen.points])
        min_angle = np.min(
            [angle_of_vector(point - source_point) for point in screen.points])
        for ray in rays:
...
class EarthScene(IntroduceScreen):
    ...
    def construct(self):
        ...
        black_rect = Rectangle(height=FRAME_HEIGHT,
        ...
        sun = sun = LightSource(opacity_function=lambda r: 0.5,
...
class InverseSquareLaw(ThreeDScene):
    ...
    def move_screen_farther_away(self):
        ...
        self.shift_by_distance(
            1, GrowFromPoint(two_arrow, two_arrow.get_left()),
    ...
    def morph_into_3d(self):
        ...
        new_screen = Square(side_length=self.screen_height,
                            stroke_color=WHITE,
                            stroke_width=1,
                            fill_color=WHITE,
                            fill_opacity=0.5)
        ...
        three_arrow = DoubleArrow(source_point + 4 * DOWN,
                                  source_point + 4 * DOWN +
                                  3 * unit_distance * RIGHT,
                                  buff=0,
                                  color=WHITE)
        three = Integer(3)
...
class OtherInstanceOfInverseSquareLaw(Scene):
    def construct(self):
        ...
        items = VGroup(*[
            TextMobject("- %s" % s).scale(1) for s in [
                "Heat",
                "Sound",
                "Radio waves",
                "Electric fields",
        ...
        self.play(LaggedStartMap(FadeIn, items, run_time=4, lag_ratio=0.7),
    ...
    def construct(self):
        ...
        self.play(
            UpdateFromAlphaFunc(
                ...
                run_time=6,
                rate_func=bezier([0, 0, 1, 1])), morty.change, "pondering",
        ...
        self.play(ReplacementTransform(light_source, light_source_copies[0]),
...
class TwoLightSourcesScene(ManipulateLightsourceSetups):
    ...
    def construct(self):
        ...
        # IPT Theorem
        theorem = TexMobject("{1 \over ", "a^2}", "+", "{1 \over", "b^2}", "=",
                             "{1 \over", "h^2}")
        ...
        self.play(
            SwitchOn(lsC.ambient_light), FadeIn(lsC.lighthouse),
        ...
        self.play(Write(identical_lighthouses_words),
        ...
        for ls, part in (lsA, a_part), (lsB, b_part), (lsC, h_part):
            self.add(ls)
            self.play(SwitchOn(ls.ambient_light, run_time=2),
                      FadeIn(ls.lighthouse), part.restore)
        self.wait()
...
class SimpleIPTProof(Scene):
    def construct(self):
        ...
        triangle = VGroup(Line(C, A, color=BLUE), Line(C, B, color=RED),
                          Line(A, B, color=WHITE), Line(C, H, color=GREEN))
        ...
        argument_lines = VGroup(
            TexMobject("\\text{Area} = ", "{1 \\over 2}", "a", "b", "=",
                       "{1 \\over 2}", "c", "h"),
            TexMobject("\\Downarrow"),
            TexMobject("a^2", "b^2", "=", "c^2", "h^2"),
            TexMobject("\\Downarrow"),
            TexMobject("a^2", "b^2", "=", "(", "a^2", "+", "b^2", ")", "h^2"),
            TexMobject("\\Downarrow"),
            TexMobject(
                "{1 \\over ",
                "h^2}",
...
class InscribedeAngleThreorem(TeacherStudentsScene):
    def construct(self):
        ...
        theta_halves_update = UpdateFromFunc(
            theta_halves, lambda m: m.move_to(
                interpolate(
                    point.get_center(),
        ...
        self.play(ShowCreation(half_angle_mark), Write(theta_halves),
                  self.get_student_changes(*["pondering"] * 3))
...
class PondScene(ThreeDScene):
    def construct(self):
        ...
        self.play(lake0.restore, lake_word.scale, 0.5,
                  {"about_point": lake0.get_bottom()}, lake_word.fade, 1)
        ...
        self.play(FadeOut(one_left), FadeOut(one_right),
                  FadeOut(new_diameter_text), FadeOut(arc_left),
                  FadeOut(arc_right))
        ...
        def construction_step(n,
            ...
            if show_steps == True:
                self.zoomable_mobs.remove(self.hypotenuses, self.altitudes,
                                          self.inner_lake)
                self.play(FadeOut(self.hypotenuses), FadeOut(self.altitudes),
        ...
        self.add(self.inner_lake, self.outer_lake, self.legs, self.altitudes,
                 self.hypotenuses)
        ...
        my_triangle = triangle(self.light_sources[0].get_source_point(),
                               OBSERVER_POINT,
                               self.light_sources[1].get_source_point())

        angle_sign1 = right_angle(self.light_sources[0].get_source_point(),
                                  OBSERVER_POINT,
                                  self.light_sources[1].get_source_point(),
        ...
        angle_sign2 = right_angle(self.light_sources[1].get_source_point(),
                                  self.lake_center,
                                  OBSERVER_POINT,
                                  size=RIGHT_ANGLE_SIZE)
        ...
        self.play(ReplacementTransform(pond_sources, nl_sources),
        ...
        self.play(indicator.move_to, origin_point + scale * UP,
                  indicator_reading.move_to, origin_point + scale * UP,
                  FadeOut(open_sea), FadeOut(morty),
        ...
        self.play(FadeIn(covering_rectangle),
...
class FinalSumManipulationScene(PiCreatureScene):
    def construct(self):
        ...
        ls_template = LightSource(radius=1,
                                  num_levels=10,
                                  max_opacity_ambient=0.5,
                                  opacity_function=inverse_quadratic(
                                      1, 0.75, 1))
        ...
        for (ls, term) in zip(odd_lights.submobjects, odd_terms.submobjects):
            self.play(FadeIn(ls.lighthouse, run_time=switch_on_time),
        ...
        quarter_label = TexMobject("\\times {1\over 4}",
                                   fill_color=LIGHT_COLOR2,
        ...
        three_quarters_label = TexMobject("\\times {3\over 4}",
                                          fill_color=LIGHT_COLOR,
                                          stroke_color=LIGHT_COLOR)
        ...
        four_thirds_label = TexMobject("\\times {4\over 3}",
                                       fill_color=LIGHT_COLOR,
                                       stroke_color=LIGHT_COLOR)
        four_thirds_label.scale(0.7)
        four_thirds_label.next_to(four_thirds_arrow.get_center(), LEFT)
        self.play(FadeOut(quarter_label), FadeOut(quarter_arrow),
                  FadeOut(even_lights), FadeOut(even_terms))
        self.wait()
        ...
        self.play(FadeOut(self.number_line1), FadeOut(odd_lights),
                  FadeOut(self.number_line2), FadeOut(full_lights),
                  FadeIn(full_terms))
        self.wait()

        v = (sum_vertical_spacing + 0.5) * UP
        self.play(odd_terms.shift, v, result1.shift, v,
                  four_thirds_arrow.shift, v, four_thirds_label.shift, v,
                  odd_terms.shift, v, full_terms.shift, v)
        ...
        final_result = TexMobject("{\pi^2 \over 6}=",
                                  fill_color=LIGHT_COLOR3,
                                  stroke_color=LIGHT_COLOR3)
    ...
    def construct(self):
        ...
        def flash_arcs(n):
            ...
            for i in range(1, 2**n + 1):
                self.play(FadeOut(arcs[i - 1], run_time=FLASH_TIME),
...
class Credits(Scene):
    def construct(self):
        credits = VGroup(*[
            VGroup(*list(map(TextMobject, pair))) for pair in [
        ...
        self.wait()
        self.play(credits.next_to, patreon_logo.get_bottom(), DOWN,
                  MED_LARGE_BUFF, DrawBorderThenFill(patreon_logo))
        self.wait()
...
class Thumbnail(Scene):
    ...
    def construct(self):
        equation = TexMobject("1", "+", "{1\over 4}", "+", "{1\over 9}", "+",
                              "{1\over 16}", "+", "{1\over 25}", "+", "\cdots")
        equation.scale(1.8)
