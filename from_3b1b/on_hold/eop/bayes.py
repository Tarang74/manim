...
class IntroducePokerHand(PiCreatureScene, SampleSpaceScene):
    ...
    def add_cards(self):
        ...
        community_cards = VGroup(
            *list(map(PlayingCard, self.community_card_values)))
    ...
    def indicate_straight(self):
        ...
        straight_cards = VGroup(*it.chain(
            you.hand.target, [c.target for c in selected_community_cards]))
        straight_cards.submobjects.sort(key=lambda c: c.numerical_value)
        ...
        self.wait(2)
        self.play(selected_community_cards.restore, you.hand.restore,
    ...
    def show_flush_potential(self):
        ...
        for new_suit_pair in new_suit_pairs:
            ...
            self.play(Transform(hearts, new_symbols, lag_ratio=0.5))
            self.wait()
    ...
    def compute_flush_probability(self):
        you, her = self.you, self.her
        equation = TexMobject("{ {10 \\choose 2}", "\\over",
                              "{45 \\choose 2} }", "=", "{45 \\over 990}",
                              "\\approx", "4.5\\%")
        ...
        num_hearts.next_to(ten, UP, aligned_edge=LEFT)
        num_hearts.to_edge(UP)
        num_hearts.set_color(RED)
        num_hearts_arrow = Arrow(num_hearts.get_bottom(),
                                 ten.get_right(),
                                 color=RED,
                                 buff=SMALL_BUFF)
        ...
        num_cards_arrow = Arrow(num_cards,
                                fourty_five,
                                color=BLUE,
                                buff=SMALL_BUFF)
        ...
        self.play(FadeIn(num_cards), ShowCreation(num_cards_arrow),
    ...
    def show_flush_sample_space(self):
        ...
        self.play(*list(map(GrowFromCenter, [brace for brace in braces])))
        self.wait(2)
    ...
    def place_high_bet(self):
        ...
        pre_money = VGroup(*[
            VGroup(*[TexMobject("\\$")
                     for x in range(10)]).arrange(RIGHT, buff=SMALL_BUFF)
            for y in range(4)
    ...
    def move_community_cards_out_of_the_way(self):
        ...
        cards.target.arrange(
            RIGHT,
            buff=-cards[0].get_width() + MED_SMALL_BUFF,
        )
        ...
        self.play(self.deck.scale, 0.7, self.deck.next_to, cards.target, UP,
                  self.deck.to_edge, LEFT, self.sample_space.shift, 3 * DOWN,
                  MoveToTarget(cards))
    ...
    def name_bayes_rule(self):
        ...
        words = TextMobject("Maybe she really \\\\ does have a flush $\\dots$",
                            alignment="")
        words.scale(0.7)
        words.next_to(self.money, DOWN, aligned_edge=LEFT)

        self.play(Write(title, run_time=2), self.you.change_mode, "pondering")
        self.wait()
        self.play(FadeIn(subtitle))
        self.play(prior_word.set_color, GREEN)
        self.play(ShowCreation(rect), ShowCreation(arrow))
        self.wait(3)
    ...
    def get_hand(self, pi_creature, keys=None):
        if keys is not None:
            ...
            hand = VGroup(*[PlayingCard(turned_over=True) for x in range(2)])
        hand.scale(0.7)
        card1, card2 = hand
        vect = np.sign(pi_creature.get_center()[0]) * LEFT
        card2.move_to(card1)
        card2.shift(MED_SMALL_BUFF * RIGHT + SMALL_BUFF * DOWN)
        hand.next_to(pi_creature, vect, buff=MED_LARGE_BUFF, aligned_edge=UP)
        return hand


class HowDoesPokerWork(TeacherStudentsScene):
    def construct(self):
        self.student_says("Wait, how does \\\\ poker work again?",
...
class YourGutKnowsBayesRule(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("Your gut knows \\\\ Bayes' rule.", run_time=1)
...
class UpdatePokerPrior(SampleSpaceScene):
    ...
    def add_sample_space(self):
        ...
        self.play(LaggedStartMap(FadeIn, sample_space),
    ...
    def add_top_conditionals(self):
        ...
        explanation = TextMobject("Probability of", "high bet", "given",
                                  "flush")
    ...
    def react_to_top_conditionals(self):
        ...
        self.play(her.change_mode, "happy", Animation(glasses))
        self.wait(2)
    ...
    def add_bottom_conditionals(self):
        ...
        self.play(her.change_mode, "shruggie",
                  MaintainPositionRelativeTo(her.glasses, her.eyes))
        self.wait()
        self.play(*[
            ReplacementTransform(VGroup(*label[i1:i2]).copy(),
    ...
    def ask_where_conditionals_come_from(self):
        ...
        numbers = VGroup(self.top_conditional_rhs, self.bottom_conditional_rhs)
        numbers.save_state()
        arrows = VGroup(*[
            Arrow(
                numbers_word.get_left(), num.get_right(), buff=2 * SMALL_BUFF)
            for num in numbers
        ])

        questions = VGroup(*list(
            map(TextMobject, [
                "Does she bluff?", "How much does she have?",
                "Does she take risks?", "What's her model of me?", "\\vdots"
            ])))
        ...
        self.play(randy.change_mode, "confused", ShowCreation(bubble),
                  Write(words, run_time=2))
        self.play(*list(map(ShowCreation, arrows)))
        self.play(numbers.set_color, YELLOW)
        self.play(Blink(randy))
        self.play(randy.change_mode, "maybe")
        self.play(*list(map(FadeOut, [bubble, words, arrows])))
        for question in questions:
            self.play(FadeIn(question), randy.look_at, question)
        self.wait()
        self.play(Blink(randy))
        self.wait()
        self.play(randy.change_mode, "pondering", FadeOut(questions))
    ...
    def vary_conditionals(self):
        ...
        for i, value in enumerate(it.chain(*new_conditionals)):
            self.play(randy.look_at, rects[i % 2],
    ...
    def show_restricted_space(self):
        ...
        for submob in low_bet_space.target:
            submob.set_color(average_color(submob.get_color(), *[BLACK] * 4))
        arrows = VGroup(*[
            Arrow(words.get_left(),
        ...
        self.play(Write(words), *list(map(ShowCreation, arrows)))
        self.wait()
    ...
    def write_P_flush_given_bet(self):
        posterior_tex = TexMobject("P(", self.double_heart_template, "|",
                                   self.cash_string, ")")
        ...
        self.wait()
        self.play(*it.chain(list(map(Write, [frac_line, plus])),
    ...
    def compare_prior_to_posterior(self):
        ...
        post_arrow = Arrow(post_words[0].get_bottom(),
                           post_tex.get_top(),
                           color=WHITE)
        ...
        self.wait()
        self.play(Write(post_words), ShowCreation(post_arrow))
        self.wait()
    ...
    def preview_tweaks(self):
        ...
        for new_values in new_value_lists:
            for i, value in zip(list(range(2)), new_values):
                self.play(
                    *self.get_conditional_change_anims(i, value, post_rects))
            self.play(*self.get_prior_change_anims(new_values[-1], post_rects))
        self.wait(2)
    ...
    def tweak_non_flush_case(self):
        ...
        prior_rects = self.get_prior_rectangles()
        risk_averse_words = TextMobject("Suppose risk \\\\ averse \\dots")
        risk_averse_words.scale(0.7)
        risk_averse_words.next_to(her, DOWN)
        risk_averse_words.shift_onto_screen()

        arrows = VGroup(
            *[Arrow(ORIGIN, LEFT, tip_length=SMALL_BUFF) for x in range(3)])
        arrows.arrange(DOWN)
        ...
        self.wait()
        self.play(FadeOut(risk_averse_words),
    ...
    def tweak_flush_case(self):
        ...
        self.play(her.change_mode, "erm", Animation(her.glasses))
        self.play(*self.get_conditional_change_anims(0, 0.47, post_rects),
                  run_time=3)
        self.wait(3)
        self.play(*self.get_conditional_change_anims(0, 0.97, post_rects))
        self.wait()
    ...
    def tweak_prior(self):
        ...
        self.play(her.change_mode, "happy", Animation(her.glasses))
        self.play(*self.get_prior_change_anims(0.3, post_rects), run_time=2)
        self.wait(3)
        self.play(*self.get_prior_change_anims(1. / 22, post_rects),
                  run_time=2)
        self.play(*list(map(FadeOut, [her, her.glasses])))
    ...
    def compute_posterior(self):
        ...
        fraction = TexMobject("{(0.045)", "(0.97)", "\\over", "(0.995)",
                              "(0.3)", "+", "(0.045)", "(0.97)}")
        products = [
            VGroup(*[fraction.get_parts_by_tex(tex)[i] for tex in tex_list])
            for i, tex_list in [
                (0, ["0.045", "0.97"]),
                (0, ["0.995", "0.3"]),
                (1, ["0.045", "0.97"]),
        ...
        to_rhs_arrow = Arrow(fraction.get_bottom(), rhs.get_top(),
                             **arrow_kwargs)

        pre_top_rect_products = VGroup(prior_rhs_group[0],
                                       self.top_conditional_rhs)
        pre_bottom_rect_products = VGroup(prior_rhs_group[1],
        ...
        self.play(*[
            ReplacementTransform(
                ...
                term,
                run_time=2,
            ) for mob, term in zip(pre_top_rect_products, products[0])
        ])
        ...
        self.play(*[
            ReplacementTransform(
                ...
                term,
                run_time=2,
            ) for mob, term in zip(pre_bottom_rect_products, products[1])
        ])
        self.wait()
        for pair in zip(pre_bottom_rect_products, products[1]):
            self.play(*list(map(Indicate, pair)))
            self.wait()
        self.play(Write(fraction.get_part_by_tex("+")),
    ...
    def get_prior_labels(self, value):
        ...
        q_str = "%0.3f" % (1 - value)
        labels = [
            TexMobject("P(", s, self.double_heart_template, ")", "= ", num)
    ...
    def get_conditional_label(self, value, given_flush=True):
        label = TexMobject("P(", self.cash_string, "|",
                           "" if given_flush else "\\text{not }",
                           self.double_heart_template, ")", "=", str(value))
        self.insert_double_heart(label)
    ...
    def get_prior_change_anims(self, value, post_rects=None):
        ...
        anims = self.get_horizontal_division_change_animations(
            value, new_label_kwargs={"labels": self.get_prior_labels(value)})
...
class BayesRuleInMemory(Scene):
    def construct(self):
        ...
        self.add(randy)
        self.play(LaggedStartMap(FadeIn, rule), randy.change, "erm", rule)
        self.play(Blink(randy))
        self.play(
            ShowCreation(bubble),
            MoveToTarget(rule),
...
class NextVideoWrapper(TeacherStudentsScene):
    CONFIG = {"title": "Upcoming chapter: Bayesian networks"}
    ...
    def construct(self):
        ...
        self.play(title.restore, self.teacher.change, "raise_right_hand")
        self.play(ShowCreation(screen))
        self.change_student_modes(*["pondering"] * 3, look_at_arg=screen)
...
class BayesianNetworkPreview(Scene):
    ...
    def add_network(self):
        ...
        node.shift(2 * DOWN)
        nodes = VGroup(*[
            node.copy().shift(x * RIGHT + y * UP) for x, y in [
                (-1, 0),
                (1, 0),
                (-2, 2),
                (0, 2),
    ...
    def show_propogation(self, node):
        ...
        while curr_nodes:
            ...
            for node in curr_nodes:
                ...
                connected_nodes = [
                    n for n in it.chain(node.children, node.parents)
...
class GeneralizeBayesRule(SampleSpaceScene):
    ...
    def add_sample_space(self):
        sample_space = SampleSpace(full_space_config={
            "height": 3,
            "width": 3,
            "fill_opacity": 0
        })
        ...
        for tup in zip(parts, values, given_strs, color_pairs, vects):
            ...
            label = TexMobject("P(", "I", "|", given_str, "B", ")")
            label.scale(0.7)
            self.color_label(label)
            part.get_subdivision_braces_and_labels(part.vertical_parts,
                                                   [label], vect)
            sample_space.add(
    ...
    def add_title(self):
        title = TextMobject("Updating", "Beliefs", "from new", "Information")
    ...
    def add_posterior_rectangles(self):
        ...
        self.play(
            ReplacementTransform(prior_rects.copy(), post_rects, run_time=2))
        self.play(GrowFromCenter(braces), Write(label))
    ...
    def talk_through_terms(self):
        ...
        prior_arrow = Arrow(prior_word.get_bottom(),
                            prior.get_top(),
                            tip_length=0.15)
        posterior_arrow = Arrow(posterior_word.get_top(),
                                posterior.get_bottom(),
                                tip_length=0.15)
        ...
        for i in range(2):
            self.play(Indicate(self.sample_space[i][0], scale_factor=1))
        self.wait()
        self.play(
        ...
        self.posterior_label = VGroup(posterior_word, posterior_arrow,
                                      posterior_rect)
        self.likelihood = likelihood

    def name_likelihood(self):
        likelihoods = [
            self.sample_space[0].vertical_parts.labels[0], self.likelihood
        ]
        rects = [
            SurroundingRectangle(mob, buff=SMALL_BUFF) for mob in likelihoods
        ]
        name = TextMobject("Likelihood")
        name.scale(0.7)
        name.next_to(self.posterior_tex, UP, 1.5 * LARGE_BUFF)
        arrows = [
            Arrow(name, rect.get_edge_center(vect), tip_length=0.15)
        ...
        self.play(self.bayes_rule_words.to_edge, UP, Write(name),
    ...
    def show_space_restriction(self):
        prior_rects = self.get_prior_rectangles()
        non_I_rects = VGroup(*[self.sample_space[i][1] for i in range(2)])
        post_rects = self.post_rects
        ...
        self.play(
            ReplacementTransform(prior_rects.copy(), post_rects, run_time=2))
        self.play(*list(map(FadeIn, [post_rects.braces, post_rects.labels])))
        self.wait()
        ...
        self.play(*it.chain(
            self.get_division_change_animations(
                self.sample_space, self.sample_space.horizontal_parts, 0.1),
...
class MusicExample(SampleSpaceScene, PiCreatureScene):
    ...
    def introduce_musician(self):
        ...
        self.play(Animation(randy), DrawBorderThenFill(guitar),
    ...
    def add_prior(self):
        ...
        words = list(map(TextMobject, ["Blunt honesty", "Some confidence"]))
    ...
    def record_track(self):
        ...
        self.play(FadeIn(headphones), Animation(friends))
    ...
    def friend_gives_compliment(self):
        ...
        self.play(
            LaggedStartMap(ApplyMethod, friends, lambda pi:
                           (pi.change_mode, "conniving")))
        self.wait()
        self.play(
            ShowCreation(bubble),
            Write(bubble.content, run_time=1),
            ApplyMethod(friends[0].change_mode, "hooray"),
            LaggedStartMap(ApplyMethod, VGroup(*friends[1:]), lambda pi:
    ...
    def friends_dont_like(self):
        ...
        self.play(LaggedStartMap(MoveToTarget, friends))
        self.change_pi_creature_with_guitar("concerned_musician")
        self.wait()
    ...
    def false_compliment(self):
        ...
        self.play(friend.change_mode, "maybe", ShowCreation(bubble),
                  Write(content))
    ...
    def restrict_space(self):
        positive_space, negative_space = [
            VGroup(*[self.sample_space[i][j] for i in range(2)])
            for j in range(2)
        ]
    ...
    def show_posterior_probability(self):
        ...
        rhs.scale(0.7)
        rhs.next_to(post_tex, RIGHT)
        ratio = TexMobject("{(0.8)(0.9)", "\\over", "(0.8)(0.9)", "+",
                           "(0.2)(0.99)}")
        ratio.scale(0.6)
        ...
        to_ratio_arrow = Arrow(post_tex.get_bottom(), ratio.get_top(),
                               **arrow_kwargs)
        to_rhs_arrow = Arrow(ratio.get_top(), rhs[1].get_bottom(),
                             **arrow_kwargs)

        prior_rects = self.get_prior_rectangles()

        self.play(ShowCreation(to_ratio_arrow), FadeIn(ratio))
        self.wait(2)
    ...
    def intuition_of_positive_feedback(self):
        ...
        self.play(*list(
            map(FadeOut,
                [prior_rect, post_rect, self.ratio_group, self.post_rhs])))
    ...
    def get_negative_feedback(self):
        ...
        post_rects = self.get_posterior_rectangles()
        label = TexMobject("P(S | \\text{not } ", "\\checkmark", ")",
                           "\\approx", "0.98")
        label.scale(0.7)
        label.set_color_by_tex("\\checkmark", GREEN)
        braces, labels = self.get_posterior_rectangle_braces_and_labels(
        ...
        self.wait()
        self.play(
            ReplacementTransform(new_prior_rects.copy(),
                                 post_rects,
                                 run_time=2))
        self.play(GrowFromCenter(brace))
        self.wait(2)
    ...
    def get_conditional_label(self, value, given_suck=True):
        ...
        label = TexMobject("P(", positive_str, "|",
                           "" if given_suck else "\\text{not }", "S", ")", "=",
                           str(value))
...
class FinalWordsOnRule(SampleSpaceScene):
    ...
    def add_sample_space(self):
        ...
        sample_space[0].divide_vertically(top_conditional, colors=[GREEN, RED])
        sample_space[1].divide_vertically(bottom_condional,
                                          colors=[GREEN_E, RED_E])
        B = "\\text{Belief}"
        D = "\\text{Data}"
        P_B = TexMobject("P(", B, ")")
        P_D_given_B = TexMobject("P(", D, "|", B, ")")
        P_D_given_not_B = TexMobject("P(", D, "|", "\\text{not }", B, ")")
...
class FootnoteWrapper(NextVideoWrapper):
    CONFIG = {"title": "Thoughts on the classic Bayes example"}
