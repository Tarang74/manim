...
class Introduction(TeacherStudentsScene):
    ...
    def hold_up_example(self):
        ...
        self.add(name, randy)
        self.play(randy.change_mode, "sick", randy.set_color, SICKLY_GREEN)
    ...
    def comment_on_crazy_results(self):
        ...
        self.teacher_says("Who doesn't love \\\\ crazy results?",
                          target_mode="hooray",
    ...
    def put_it_first(self):
        ...
        bayes_to_intuition = VGroup(*list(
    ...
    def swap_example_order(self):
        ...
        self.play(disease_group.next_to, self.teacher.get_corner(UP + LEFT),
                  UP, disease_group.shift, LEFT, intuitive_examples.scale, 0.7,
    ...
    def other_culprit(self):
        ...
        new_group = VGroup(something_else, *self.bayes_to_intuition[1:])
...
class OneInOneThousandHaveDisease(Scene):
    def construct(self):
        ...
        all_creatures = VGroup(*[
            VGroup(*[creature.copy()
                     for y in range(25)]).arrange(DOWN, SMALL_BUFF)
            for x in range(40)
...
class TestNonDiseaseCase(TestScene):
    def construct(self):
        ...
        all_creatures = VGroup(*[
            VGroup(*[randy.copy() for y in range(10)]).arrange(DOWN)
            for y in range(10)
        ]).arrange(RIGHT)
        ...
        self.play(Write(result[1]))
        self.play(Blink(randy))
        self.play(ReplacementTransform(randy, all_creatures[0][0]),
                  LaggedStartMap(FadeIn, all_creatures, run_time=2),
                  FadeOut(result))
        self.play(ShowCreation(rect))
        self.play(last_guy.set_height, 2, last_guy.next_to, all_creatures,
                  RIGHT)
        ...
        arrow = Arrow(false_positive.get_bottom(),
                      result[1].get_top(),
                      buff=SMALL_BUFF)
...
class ReceivePositiveResults(TestScene):
    def construct(self):
        ...
        self.wait(2)
        self.play(randy.change, "pleading", accuracy,
...
class AskAboutRephrasingQuestion(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("What if we rephrased \\\\ the question?",
                          run_time=1)
        self.wait(3)
...
class RephraseQuestion(Scene):
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeIn, posterior),
...
class TryUnitSquareVisual(SampleSpaceScene):
    def construct(self):
        ...
        anims = self.get_division_change_animations(
            sample_space,
            sample_space.horizontal_parts,
            0.001,
            new_label_kwargs={"labels": final_labels})
    ...
    def add_prior_division(self):
        ...
        initial_labels, final_labels = [
            ...
            ).scale(0.7) for s1, s2 in (("", ""), ("= 0.001", "= 0.999"))
        ]
...
class ShowRestrictedSpace(Scene):
    ...
    def add_all_creatures(self):
        ...
        sick_one_arrow = Arrow(sick_one_words, sick_one, color=SICKLY_GREEN)
        ...
        self.play(FadeIn(sick_one_words), ShowCreation(sick_one_arrow))
        self.play(
            sick_one.set_height,
            2,
            sick_one.next_to,
    ...
    def show_accurate_positive_result(self):
        equation = TexMobject("P(", "\\text{Test positive }", "|",
                              "\\text{ sick}", ")", "=", "100\\%")
    ...
    def show_false_positive_conditional(self):
        ...
        self.play(ReplacementTransform(self.disease_conditional, equation))
        self.wait()
    ...
    def show_false_positive_individuals(self):
        ...
        false_positives = VGroup(*all_creatures[-1][1:1 +
        ...
        self.play(
            ...
            LaggedStartMap(ApplyMethod,
                           false_positives,
                           lambda pi:
                           (pi.set_color, self.false_positive_color),
                           run_time=1))
    ...
    def fade_out_negative_result_individuals(self):
        ...
        self.play(MoveToTarget(movers),
                  ReplacementTransform(self.false_positives_brace, brace),
                  self.false_positives_words.next_to, brace, DOWN)
        self.play(Write(words, run_time=2), ShowCreation(arrows[0]))
        self.play(Transform(*arrows, run_time=4, rate_func=there_and_back))
        self.play(*list(map(FadeOut, [words, arrows[0]])))

        self.brace = brace

    def show_posterior_probability(self):
        posterior = TexMobject("P(", "\\text{Sick }", "|",
    ...
    def get_all_creatures(self):
        ...
        all_creatures = VGroup(*[
            VGroup(*[creature.copy()
                     for y in range(self.n_rows)]).arrange(DOWN, SMALL_BUFF)
...
class DepressingForMedicalTestDesigners(TestScene):
    ...
    def show_99_percent_accuracy(self):
        ...
        checks = VGroup(*[
            VGroup(*[
                TexMobject("\\checkmark").set_color(GREEN) for y in range(10)
            ]).arrange(DOWN) for x in range(10)
        ]).arrange(RIGHT)
    ...
    def reject_test(self):
        ...
        self.play(FadeIn(result), randy.change_mode, "pondering")
        self.wait()
        self.say("Whatever, I'm 91\\% \\\\ sure that's wrong",
...
class HowMuchCanYouChangeThisPrior(ShowRestrictedSpace, PiCreatureScene):
    ...
    def show_subgroups(self):
        subgroups = VGroup(*[
            VGroup(*it.chain(self.pi_creatures[i:i +
                                               5], self.pi_creatures[i + 25:i +
                                                                     25 + 5:]))
            for i in range(0, 1000) if i % 5 == 0 and (i / 25) % 2 == 0
        ])
        ...
        rects = VGroup(*[
            SurroundingRectangle(group, buff=0, color=GREEN)
            for group in subgroups
        ])
        special_rect = SurroundingRectangle(special_group, buff=0, color=RED)
...
class ShowTheFormula(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        lhs_copy = lhs.copy()
        self.play(
            LaggedStartMap(FadeIn, initial_formula, lag_ratio=0.7),
...
class SourceOfConfusion(Scene):
    ...
    def ask_question(self):
        question = TextMobject("Where do math and \\\\ intuition disagree?")
        question.to_corner(UP + LEFT)
        question_arrow = Arrow(question.get_bottom(),
    ...
    def shift_arrow(self):
        new_arrow = Arrow(self.question.get_bottom(),
                          self.progression[0].get_top(),
                          color=WHITE)

        self.play(Transform(self.question_arrow, new_arrow))
        self.wait(2)
...
class StatisticsVsEmpathy(PiCreatureScene):
    def construct(self):
        ...
        sick_one = PiCreature()
        sick_one.scale(0.5)
        sick_group = VGroup(sick_one, VectorizedPoint(sick_one.get_bottom()))
        priors = VGroup(*[
        ...
        self.wait()
        self.play(prior.scale, 0.1, prior.set_fill, None, 0, prior.move_to,
                  randy.eyes)
        self.wait()
        self.play(
        ...
        self.play(sick_one.change_mode, "sick", sick_one.set_color,
                  SICKLY_GREEN)
        self.wait()

        probably_me = TextMobject("That's probably \\\\ me")
        probably_me.next_to(sick_one, DOWN)
        target_sick_group = VGroup(sick_one.copy(), probably_me)
...
class LessMedicalExample(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(ShowCreation(cross))
        self.play(FadeIn(telepathy), VGroup(disease, cross).shift, 2 * UP)
        self.wait()
...
class IntroduceTelepathyExample(StatisticsVsEmpathy):
    ...
    def generate_random_number(self):
        ...
        numbers = [Integer(random.choice(list(range(100)))) for x in range(30)]
    ...
    def ask_about_chances(self):
        probability = TexMobject("P(", "\\text{Telepath }", "|",
                                 "\\text{ Correct}", ")", "=", "???")
        ...
        self.wait()
        self.play(
            ReplacementTransform(self.title, VGroup(*it.chain(*probability))))
        self.wait()
...
class CompareNumbersInBothExamples(Scene):
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeIn, titles, lag_ratio=0.7),
...
class ExampleMeasuresDisbeliefInStatistics(Introduction):
    ...
    def write_new_theory(self):
        ...
        statistics_to_belief = TextMobject("Statistics ", "$\\leftrightarrow$",
                                           " Belief")
        ...
        self.play(LaggedStartMap(FadeIn, statistics_to_belief), cross.move_to,
                  arrow)
        self.change_student_modes(*["pondering"] * 3,
    ...
    def either_way(self):
        ...
        self.play(self.teacher.change_mode, "raise_right_hand",
                  self.teacher.look, UP + LEFT, s_to_b.next_to, self.teacher,
                  UP, s_to_b.shift, LEFT, b_to_i.shift, 2 * UP)
        self.wait(2)
