...
def get_quiz(*questions):
    ...
    content = VGroup(
        ...
        Line(q_mobs.get_left(), q_mobs.get_right()), q_mobs)
...
class Checkmark(TexMobjectFromPresetString):
    CONFIG = {"tex": "\\checkmark", "color": GREEN}


class Xmark(TexMobjectFromPresetString):
    CONFIG = {"tex": "\\times", "color": RED}
...
def get_slot_group(
    ...
    if include_qs:
        labels = VGroup(*[TextMobject("Q%d" % d) for d in range(1, n + 1)])
    else:
...
class DangerInProbability(Scene):
    def construct(self):
        ...
        self.play(warning.next_to, probability, UP, LARGE_BUFF,
...
class MeaningOfIndependence(SampleSpaceScene):
    ...
    def add_labeled_space(self):
        ...
        top_braces_and_labels, bottom_braces_and_labels = [
            part.get_subdivision_braces_and_labels(
                ...
                labels=["P(B | %s)" % s,
                        "P(\\overline B | %s)" % s],
                direction=vect) for part, s, vect in zip(
    ...
    def align_conditionals(self):
        line = Line(*[
            interpolate(self.sample_space.get_corner(vect + LEFT),
                        self.sample_space.get_corner(vect + RIGHT), 0.7)
            for vect in (UP, DOWN)
        ])
        ...
        self.play(*it.chain(self.get_top_conditional_change_anims(0.7),
    ...
    def relabel(self):
        ...
        ignored_braces, new_top_labels = self.sample_space[
            0].get_top_braces_and_labels(["P(B)", "P(\\overline B)"])
        equation = TexMobject("P(B | A) = P(B)")
        equation.scale(1.5)
        ...
        self.play(self.independence_word.next_to, equation, UP, MED_LARGE_BUFF,
                  Write(equation))
        self.wait()
    ...
    def no_independence(self):
        ...
        self.play(*it.chain(self.get_top_conditional_change_anims(
            0.9), self.get_bottom_conditional_change_anims(0.5), [
                self.independence_word.fade, 0.7, self.equation.fade, 0.7,
...
class IntroduceBinomial(Scene):
    ...
    def add_bar_chart(self):
        ...
        self.play(LaggedStartMap(FadeIn, VGroup(*it.chain(*chart)),
                                 run_time=2))

    def add_p_slider(self):
        interval = UnitInterval(color=LIGHT_GREY)
        interval.set_width(4)
        interval.next_to(VGroup(self.bar_chart.x_axis, self.bar_chart.y_axis),
                         UP, MED_LARGE_BUFF)
    ...
    def shift_weight_to_tails(self):
        ...
        self.play(
            ...
            ReplacementTransform(old_bars.copy().set_fill(opacity=0),
                                 new_bars))
...
class IntroduceQuiz(PiCreatureScene):
    ...
    def ask_about_probabilities(self):
        probabilities, abbreviated_probabilities = [
            VGroup(*[
                TexMobject("P(", s_tex, "=", str(score), ")",
                           rhs).set_color_by_tex_to_color_map({
                               str(score): YELLOW,
                               "text": GREEN,
                           }) for score in range(4)
            ]) for s_tex, rhs in [("\\text{Score}",
                                   "= \\, ???"), ("\\text{S}", "")]
        ]
        ...
        self.play(LaggedStartMap(FadeIn, probabilities,
                                 run_time=3), self.quiz.set_height,
                  0.7 * self.randy.get_height(), self.quiz.next_to, self.randy,
                  RIGHT, self.randy.change, "confused", probabilities)
        self.wait()
    ...
    def show_distribution(self):
        ...
        self.play(
            LaggedStartMap(Write,
                           VGroup(*[m for m in chart
                                    if m is not chart.bars])), )
        self.play(*[
            ReplacementTransform(
                bar.copy().stretch_to_fit_height(0).move_to(bar.get_bottom()),
                bar) for bar in chart.bars
        ])
        self.play(*[
            ReplacementTransform(p.copy(), short_p) for p, short_p in zip(
                self.probabilities,
    ...
    def show_single_question_probability(self):
        prob = TexMobject("P(", "\\text{Can answer a given question}", ")",
                          "= 0.8")
...
class BreakDownQuestionPatterns(IntroduceQuiz):
    ...
    def add_parts_from_last_scene(self):
        ...
        self.play(self.single_question_probability.scale, 0.8,
                  self.single_question_probability.to_corner, UP + LEFT,
                  chart_group.scale, 0.7, chart_group.get_top(),
                  chart_group.to_edge, LEFT, FadeOut(self.probabilities))
...
class AssociatePatternsWithScores(BreakDownQuestionPatterns):
    ...
    def show_score_groups(self):
        ...
        full_score_groups.arrange(DOWN,
                                  buff=MED_LARGE_BUFF,
                                  aligned_edge=RIGHT)
        ...
        for score, score_group in zip(scores, score_groups):
            ...
            self.wait()
            self.play(
                ReplacementTransform(score_group.copy(),
                                     score_group.organized),
    ...
    def think_about_binomial_patterns(self):
        ...
        self.play(FadeOut(self.slot_groups), FadeIn(self.randy),
                  FadeIn(bubble))
        self.play(
            self.randy.change,
            "pondering",
...
class BeforeCounting(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(triangle.scale, 0.5, triangle.to_corner, UP + RIGHT,
...
class TemptingButWrongCalculation(BreakDownQuestionPatterns):
    ...
    def write_simple_product(self):
        ...
        self.play(Write(lhs), ShowCreation(slot_group.lines),
...
class ThousandPossibleQuizzes(Scene):
    ...
    def show_division_by_first_question(self):
        ...
        for split, color, vect in (top_split, GREEN, UP), (bottom_split, RED,
                                                           DOWN):
        ...
        labels = VGroup()
        for num, b, split in (800, True, top_split), (200, False,
                                                      bottom_split):
            label = VGroup(
                TexMobject(str(num)),
    ...
    def second_division_among_first_wrong(self):
        ...
        slot_group = get_slot_group([False, True],
                                    buff=SMALL_BUFF,
                                    include_qs=False)
        ...
        self.play(bottom_label.shift, LEFT,
    ...
    def show_third_question_results(self):
        all_splits = VGroup(self.top_left_split, self.top_right_split,
...
class AccurateProductRule(SampleSpaceScene, ThreeDScene):
    ...
    def move_to_third_dimension(self):
        ...
        self.play(space.rotate, 0.9 * np.pi / 2, LEFT, space.rotate,
                  np.pi / 12, UP, space.to_corner, DOWN + RIGHT, LARGE_BUFF)
        space.remove(cubes)
        self.play(
            cubes[0].set_fill,
            None,
    ...
    def show_all_conditionals(self):
        ...
        for bool_list in it.product(*[[True, False]] * 3):
            ...
            for part, sub_list in zip(parts, sub_bool_lists):
                slot_group = get_slot_group(sub_list,
                                            buff=SMALL_BUFF,
                                            include_qs=False)
                slot_group.replace(part, dim_to_match=0)
        ...
        rect = SurroundingRectangle(VGroup(*equations[0][7:],
                                           *equations[-1][7:]),
                                    buff=SMALL_BUFF)
    ...
    def suggest_independence(self):
        ...
        self.play(
            PiCreatureSays(randy,
                           "Let's just assume \\\\ independence.",
...
class ComputeProbabilityOfOneWrong(Scene):
    ...
    def show_all_three_patterns(self):
        ...
        for i in range(2):
            self.play(
                ReplacementTransform(probabilities[i].copy(),
                                     probabilities[i + 1]))
        self.wait()
    ...
    def show_final_result(self):
        result = TexMobject("P(", "\\text{Score} = %s" % self.score, ")", "=",
...
class ShowFullDistribution(Scene):
    ...
    def show_bar_chart(self):
        ...
        new_p_terms = VGroup(
            *[TexMobject("P(", "S=%d" % k, ")") for k in range(4)])
        ...
        self.play(
            ReplacementTransform(value_mobs,
                                 chart.bars,
                                 lag_ratio=0.5,
                                 run_time=2))
        self.play(
    ...
    def show_alternate_values_of_p(self):
        new_prob = TexMobject("P(", "\\text{Correct}", ")", "=", "0.8")
...
class ProbablyWrong(TeacherStudentsScene):
    def construct(self):
        ...
        self.change_student_modes(*["angry"] * 3, run_time=1)
        self.wait()
    ...
    def show_distributions(self):
        ...
        for bar, old_bar in zip(bars, old_bars):
            for mob, vect in (bar, RIGHT), (old_bar, LEFT):
                mob.generate_target()
                mob.target.do_about_point(mob.get_corner(DOWN + vect),
...
class TeacherAssessingLiklihoodOfZero(TeacherStudentsScene):
    ...
    def show_independence_probability(self):
        ...
        self.play(self.teacher.change, "raise_right_hand", FadeIn(words),
                  Write(prob))
        self.wait()
...
class CorrelationsWith35Percent(ThousandPossibleQuizzes):
    ...
    def show_first_split(self):
        ...
        self.add(quizzes)
        self.wait()
        self.play(GrowFromCenter(brace), FadeIn(prop),
    ...
    def comment_on_final_size(self):
        ...
        words = TextMobject("Much more than ", "$(0.35)^3 \\approx 4.3\\%$")
...
class NameBinomial(Scene):
    ...
    def name_distribution(self):
        ...
        probability = TexMobject("P(", "\\checkmark", ")", "=", str(p))
    ...
    def add_quiz_questions(self):
        n = 10
        checkmarks = VGroup(
            *[TexMobject("\\checkmark").set_color(GREEN) for x in range(n)])
    ...
    def change_to_gender(self):
        ...
        girls, boys = [
            ...
            for template, group in [(female,
                                     self.checkmarks), (male, self.crosses)]
        ]
    ...
    def point_out_example_input(self):
        ...
        chart_rect = SurroundingRectangle(VGroup(chart.bars[n_girls],
                                                 chart.bar_labels[n_girls]),
                                          buff=SMALL_BUFF)

        prob = TexMobject("P(", "\\# \\text{Girls}", "=", "6", ")")
    ...
    def write_probability_of_girl(self):
        ...
        self.wait()
        self.play(
            ReplacementTransform(VGroup(probability.copy()),
                                 probability_copies),
...
class CycleThroughPatterns(NameBinomial):
    ...
    def construct(self):
        n = self.n
        k = self.k
        question = TextMobject("How many patterns have \\\\ %d " % k,
                               "$\\female$",
                               " and %d " % (n - k),
                               "$\\male$",
                               "?",
        ...
        for new_pattern in ProgressDisplay(patterns[1:]):
            self.play(*[
                Transform(getattr(pattern, attr),
                          getattr(new_pattern, attr),
                          path_arc=np.pi) for attr in ("boys", "girls")
            ])
...
class Compute6of10GirlsProbability(CycleThroughPatterns):
    ...
    def write_n_choose_k(self):
        ...
        computation = TexMobject("=\\frac{%s}{%s}" % (
        ...
        self.wait(2)
        self.play(ten_choose_six.next_to, computation.copy(), LEFT,
...
class ProbabilityOfAGivenBoyGirlPattern(CycleThroughPatterns):
    ...
    def write_total_count(self):
        count = TextMobject("${10 \\choose 6}$", " $= 210$", "total patterns.")
        count.to_edge(UP)
        self.add(count)
    ...
    def write_example_probability(self):
        ...
        final_probability = TexMobject("(0.49)^6", "(0.51)^4")
    ...
    def write_total_probability(self):
        ...
        self.play(Write(p_tex, run_time=2), self.final_probability.next_to,
...
class GeneralBinomialDistributionValues(Scene):
    ...
    def show_a_few_values(self):
        ...
        for i, bar in enumerate(chart.bars):
            prob = self.get_probability_expression("10",
                                                   str(i),
                                                   "(0.49)",
                                                   "(0.51)",
                                                   full=False)
    ...
    def compare_to_pascal_row(self):
        ...
        self.play(last_row.set_color_by_gradient, BLUE, YELLOW,
    ...
    def mention_center_concentration(self):
        ...
        bars.target.move_to(self.chart.x_axis.point_from_proportion(0.05),
                            DOWN)
        ...
        self.play(
            ...
            *list(
                map(FadeOut,
                    [brace, words, self.pascals_triangle, self.ten_choose_ks
                     ])))
    ...
    def generalize(self):
        ...
        self.play(ReplacementTransform(self.chart, alt_chart),
    ...
    def play_with_p_value(self):
        ...
        for new_p in new_p_values:
            ...
            self.play(self.chart.change_bar_values, values, triangle.move_to,
                      interval.number_to_point(p), DOWN)
            self.wait()

    #######

    def get_probability_expression(self,
                                   n="n",
                                   k="k",
                                   p="p",
                                   q="(1-p)",
...
class PointOutSimplicityOfFormula(TeacherStudentsScene,
                                  GeneralBinomialDistributionValues):
    def construct(self):
        ...
        self.change_student_modes(*["pondering"] * 3, look_at_arg=prob)
        self.wait()
        self.student_says("Simpler than I feared",
                          target_mode="hooray",
                          student_index=0,
...
class CorrectForDependence(NameBinomial):
    ...
    def get_arrow_flip_anims(self, indices):
        ...
        return [
            LaggedStartMap(MoveToTarget, mover, **kwargs) for mover in movers
        ]


class ButWhatsTheAnswer(TeacherStudentsScene):
    def construct(self):
        self.student_says("But what's the \\\\ actual answer?",
...
class AssumeOrderDoesntMatter(Scene):
    ...
    def show_equality(self):
        ...
        prob_groups = VGroup(*[
            VGroup(*list(
                map(get_probability_of_slot_group, [
        ...
        self.wait(2)
        self.play(
            FadeIn(VGroup(prob_groups[0], *prob_groups[2:]),
                   run_time=3,
                   lag_ratio=0.5))
        self.wait()
    ...
    def mention_correlation(self):
        ...
        question = TextMobject("But what is ",
                               "``correlation''",
                               "?",
                               arg_separator="")
        ...
        self.play(Write(question), assumption_group.next_to, bottom, DOWN,
                  LARGE_BUFF)
        self.wait()
    ...
    def coming_soon(self):
        self.play(
            ...
            ApplyMethod(self.question.center,
...
class CompareTwoSituations(PiCreatureScene):
    def construct(self):
        ...
        for screen, s in zip(screens, ["left", "right"]):
            self.play(randy.change, "raise_%s_hand" % s, screen,
                      ShowCreation(screen))
            self.wait(3)
...
class SkepticalOfDistributions(TeacherStudentsScene):
    ...
    def show_binomial(self):
        ...
        for values in binomial.values_list:
            ...
            self.wait()
        self.student_says("Is that valid?",
                          target_mode="sassy",
                          student_index=0,
                          run_time=1)
    ...
    def show_alternate_distributions(self):
        ...
        VGroup(poisson, poisson.title).next_to(self.students, UP,
                                               LARGE_BUFF).shift(RIGHT)
    ...
    def get_poisson(self):
        ...
        L = 2
        values = [
            np.exp(-L) * (L**k) / (scipy.special.gamma(k + 1)) for k in k_range
        ]
        chart = BarChart(values=values,
                         bar_names=k_range,
                         bar_colors=[RED, YELLOW])
        chart.set_height(self.chart_height)
        title = TextMobject("Poisson distribution \\\\",
