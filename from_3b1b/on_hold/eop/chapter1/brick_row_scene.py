...
class BrickRowScene(PiCreatureScene):
    ...
    def merge_tallies(self, row, target_pos=UP):
        ...
        if np.all(target_pos == DOWN):
            tally_targets = [
                rect.get_center() for rect in row.get_rects_for_level(r)
            ]
        ...
        for (tally, target) in zip(self.tallies_copy[:-1],
                                   tally_targets[1:-1]):
            anims.append(tally.move_anchor_to)
            anims.append(target)
    ...
    def construct(self):
        ...
        outcomes = self.row.get_outcome_rects_for_level(2,
                                                        with_labels=False,
                                                        inset=True)
        ...
        tally.next_to(rect, UP)
        decimal_tallies.add(tally)
        self.play(FadeIn(tally), FadeOut(grouped_outcomes[0]),
        ...
        tally.next_to(rect, UP)
        decimal_tallies.add(tally)
        self.play(FadeIn(tally), FadeOut(grouped_outcomes[1]),
        ...
        self.wait()
        self.play(FadeOut(single_flip_labels), FadeOut(new_heads),
                  FadeOut(single_flip_labels_copy), FadeOut(new_tails))
        self.wait()
        ...
        for (decimal_tally, tally_stack) in zip(decimal_tallies, self.tallies):
            anims.append(
                ApplyFunction(tally_stack.position_decimal_tally,
                              decimal_tally))
        ...
        rect_to_dice = self.row.get_outcome_rects_for_level(2,
                                                            with_labels=False,
                                                            inset=False)[1]
        N = 10
        dice_width = rect_to_dice.get_width() / N
        dice_height = rect_to_dice.get_height() / N
        prototype_dice = Rectangle(width=dice_width,
                                   height=dice_height,
                                   stroke_width=2,
                                   stroke_color=WHITE,
                                   fill_color=WHITE,
        ...
        self.play(FadeOut(outcomes), FadeOut(subdivs), FadeOut(all_dice),
                  FadeOut(table), FadeOut(coin1), FadeOut(coin2),
                  FadeIn(self.tallies))
        ...
        self.play(FadeIn(rect_left), FadeIn(rect_right), FadeIn(tally_left),
                  FadeIn(tally_right))
        ...
        self.play(MoveToTarget(rect_right), tally_right.move_to,
                  right_target_pos)
        ...
        # rest of the new row
        self.play(FadeIn(self.tallies[:1]), FadeIn(self.tallies[2:]),
                  FadeIn(self.tallies_copy[1:]))
        ...
        self.play(LaggedStartMap(ShowCreation, braces),
        ...
        self.play(
            Transform(grouped_outcomes[k][0],
                      grouped_target_outcomes[k][0][old_tally_sizes[k - 1]:]),
            Transform(grouped_outcomes_copy[k - 1][0],
                      grouped_target_outcomes[k][0][:old_tally_sizes[k - 1]]),
        )
...
class ShowProbsInBrickRow3(BrickRowScene):
    def construct(self):
        ...
        self.wait()
        self.play(LaggedStartMap(ShowCreation, braces, run_time=3),
...
class ShowOutcomesInBrickRow4(BrickRowScene):
    def construct(self):
        ...
        new_outcomes_left = VGroup(new_outcomes[0], new_outcomes[2],
                                   new_outcomes[3], new_outcomes[4],
                                   new_outcomes[8], new_outcomes[9],
                                   new_outcomes[10], new_outcomes[14])
        new_outcomes_right = VGroup(new_outcomes[1], new_outcomes[5],
                                    new_outcomes[6], new_outcomes[7],
                                    new_outcomes[11], new_outcomes[12],
                                    new_outcomes[13], new_outcomes[15])
        heads_labels = VGroup(
            *[outcome.label[-1] for outcome in new_outcomes_left])
        tails_labels = VGroup(
            *[outcome.label[-1] for outcome in new_outcomes_right])
        ...
        run_time = 0.5
        self.play(
            Transform(previous_outcomes[0],
                      new_outcomes_left[0],
                      run_time=run_time))
        self.play(
            Transform(previous_outcomes[1:4],
                      new_outcomes_left[1:4],
                      run_time=run_time))
        self.play(
            Transform(previous_outcomes[4:7],
                      new_outcomes_left[4:7],
                      run_time=run_time))
        self.play(
            Transform(previous_outcomes[7],
                      new_outcomes_left[7],
                      run_time=run_time))

        self.play(heads_labels.restore)
        self.play(
            Transform(previous_outcomes_copy[0],
                      new_outcomes_right[0],
                      run_time=run_time))
        self.play(
            Transform(previous_outcomes_copy[1:4],
                      new_outcomes_right[1:4],
                      run_time=run_time))
        self.play(
            Transform(previous_outcomes_copy[4:7],
                      new_outcomes_right[4:7],
                      run_time=run_time))
        self.play(
            Transform(previous_outcomes_copy[7],
                      new_outcomes_right[7],
                      run_time=run_time))
...
class SplitTalliesIntoBrickRow4(BrickRowScene):
    def construct(self):
        ...
        anims = []
        for (tally, rect) in zip(moving_tallies_left[1:],
                                 hypothetical_new_row.rects[1:-1]):
            anims.append(tally.move_to)
            anims.append(rect)
        for (tally, rect) in zip(moving_tallies_right[:-1],
                                 hypothetical_new_row.rects[1:-1]):
            anims.append(tally.move_to)
            anims.append(rect)
        self.play(*anims)
