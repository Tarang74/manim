...
def get_stack(
    ...
    for indices in it.combinations(list(range(n)), k):
        term = VGroup(
            *[obj1.copy() if i in indices else obj2.copy() for i in range(n)])
        if fixed_start:
...
def get_stacks(obj1, obj2, n, **kwargs):
    ...
    stacks.arrange(RIGHT, buff=MED_LARGE_BUFF, aligned_edge=DOWN)
    return stacks
...
class ExperienceProblemSolver(PiCreatureScene):
    ...
    def add_equation(self):
        equation = TexMobject("\\frac{x^3 + y^3}{(x+y)^2} + \\frac{3xy}{x+y}")
        equation.to_edge(UP)
    ...
    def jenny_solves(self):
        ...
        self.wait()
        self.play(randy.change, "confused", self.equation, Write(randy_words))
        ...
        self.wait()
        self.play(
            *list(map(FadeOut, [jenny.bubble, jenny_words, randy_words])))
    ...
    def no_genius(self):
        ...
        self.play(ShowCreation(cross), jenny.change, "sassy", cross,
                  randy.change, "happy")
        self.wait(2)
    ...
    def think_about_patterns(self):
        ...
        for row in rows:
            for num in row:
                ...
                num.set_color(interpolate_color(BLUE, YELLOW, n / 10.0))

        self.pi_creature_thinks(jenny,
                                "",
                                bubble_kwargs={
                                    "width": 5,
                                    "height": 4.2
                                ...
                                added_anims=[
                                    ...
                                    FadeOut(self.equation), randy.change,
                                    "plain"
                                ])
        rows.move_to(
        ...
        for last_row, curr_row in zip(rows, rows[1:]):
            self.play(*[
                Transform(last_row.copy(), VGroup(*mobs), remover=True)
...
class InitialFiveChooseThreeExample(Scene):
    ...
    def show_all_stacks(self):
        stacks = get_stacks(self.get_obj1(),
                            self.get_obj2(),
                            self.n,
        ...
        for stack in stacks:
            self.play(FadeIn(stack, run_time=0.2 * len(stack), lag_ratio=0.5))
        self.wait()
    ...
    def add_title(self):
        ...
        binomials = VGroup(
            *[TexMobject("%d \\choose %d" % (n, k)) for k in range(n + 1)])
        ...
        self.set_variables_as_attrs(n_choose_k, n_choose_k_words,
                                    binomial_equations)
    ...
    def count_chosen_stack(self):
        ...
        self.set_variables_as_attrs(stack_brace=brace, stack_count=number)
    ...
    def count_ways_to_fill_slots(self):
        ...
        for indices in it.combinations(list(range(5)), 3):
            ones = VGroup(
                *[self.get_obj1().next_to(lines[i], UP) for i in indices])
    ...
    def emphasize_pattern_over_number(self):
        ...
        self.play(morty.change, "speaking", Write(words, run_time=2))
        self.play(Blink(morty), morty.change, "happy")
...
class SixChooseThreeExample(InitialFiveChooseThreeExample):
    ...
    def show_stack(self):
        ...
        equation = TexMobject("{%d \\choose %d}" % (self.n, self.k), "=",
    ...
    def talk_through_one_line(self):
        ...
        self.play(GrowFromCenter(brace), Write(n_options), run_time=1)
        self.play(LaggedStartMap(GrowArrow, arrows), Write(choose_k,
                                                           run_time=1))
        self.wait(2)
    ...
    def count_stack(self):
        ...
        lhs = TexMobject("\\frac{6 \\cdot 5 \\cdot 3}{1 \\cdot 2 \\cdot 3} =")
...
class SixChooseThreeInOtherContext(Scene):
    ...
    def add_dots(self):
        ...
        self.dots = dots
        self.dot_to_dot_distance = get_norm(dots[1].get_center() -
                                            dots[0].get_center())
    ...
    def count_paths_to_three_three(self):
        ...
        for indices in it.combinations(list(range(6)), 3):
            ...
            arrows = VGroup(
                *[right.deepcopy() if b else up.deepcopy() for b in bools])
            ...
            if last_line is None:
                ...
                line.to_edge(UP)
                self.play(ShowCreation(arrows), ShowCreation(line))
            else:
...
class ProbabilityOfKWomenInGroupOfFive(Scene):
    ...
    def ask_question(self):
        ...
        lineup_point = 1.5 * UP
        prob_words = VGroup(*[
            TextMobject("Probability of", str(n), "women?").set_color_by_tex(
                str(n), YELLOW) for n in range(self.n_people_per_lineup + 1)
        ])
    ...
    def show_all_possibilities(self):
        ...
        n_possibilities = TexMobject("2 \\cdot", "2 \\cdot", "2 \\cdot",
                                     "2 \\cdot", "2", "\\text{ Possibilities}")
        n_possibilities.next_to(self.title, DOWN)
        twos = VGroup(*n_possibilities[-2::-1])
        twos.set_color(YELLOW)
        two_anims = [
            ReplacementTransform(VectorizedPoint(twos[0].get_center()),
                                 twos[0])
        ] + [
        ...
        self.play(ReplacementTransform(self.lineup, curr_lineup_group[0]),
    ...
    def stack_all_choices_by_number_of_women(self):
        ...
        stacks.target.arrange(RIGHT, buff=MED_LARGE_BUFF, aligned_edge=DOWN)
        stacks.target.to_edge(DOWN)

        self.play(MoveToTarget(stacks, run_time=2, path_arc=np.pi / 2))
        self.wait()
        ...
        for k, stack in enumerate(stacks):
            ...
            for lineup in stack:
                ...
                for item in lineup.items:
                    if "female" in item.get_tex_string():
                        women.add(item)
            equation = TexMobject("{%d \\choose %d}" % (n, k), "=",
                                  str(len(stack)))
            ...
            self.play(items.set_fill, None, 1, lines.set_stroke, WHITE, 3,
                      Write(equation, run_time=1))
            self.play(LaggedStartMap(Indicate, women,
                                     rate_func=there_and_back))
        self.wait()
    ...
    def remember_this_sensation(self):
        ...
        self.play(LaggedStartMap(Indicate, twos, rate_func=wiggle))
    ...
    def show_answer_to_question(self):
        stacks = self.stacks
        numbers = self.numbers
        n_possibilities = VGroup(self.n_possibilities[-1],
                                 self.n_possibilities[-3])
        n_possibilities_part_to_fade = VGroup(self.n_possibilities[-2],
                                              *self.n_possibilities[:-3])
        total = n_possibilities[-1]
        title = self.title
        n = self.n_people_per_lineup

        self.play(FadeOut(title), FadeOut(n_possibilities_part_to_fade),
                  n_possibilities.to_corner, UP + RIGHT)
        for k, stack, num in zip(it.count(), stacks, numbers):
            rect = SurroundingRectangle(stack)
            num.save_state()
            prob_words = TexMobject("P(", "\\#", "\\female", "=", str(k), ")"
                                    "=", "{\\quad \\over", "32}", "\\approx",
                                    "%0.3f" % (choose(n, k) / 32.0))
            ...
            self.play(num.next_to, frac_line, UP, SMALL_BUFF,
                      FadeIn(prob_words))
            self.play(ShowCreation(rect))
            self.wait(2)
            self.play(num.restore, FadeOut(rect), FadeOut(prob_words))
    ...
    def get_lineup(self, *mobjects, **kwargs):
        ...
        lines = VGroup(
            *[Line(ORIGIN, self.item_line_width * RIGHT) for mob in mobjects])
...
class AskAboutAllPossibilities(ProbabilityOfKWomenInGroupOfFive):
    def construct(self):
        ...
        for bits in it.product(*[[False, True]] * 5):
            mobs = [woman.copy() if bit else man.copy() for bit in bits]
...
class BuildFiveFromFour(ProbabilityOfKWomenInGroupOfFive):
    ...
    def show_all_configurations_of_four(self):
        ...
        n_possibilities = TexMobject("2 \\cdot", "2 \\cdot", "2 \\cdot", "2",
                                     "\\text{ Possibilities}")
        ...
        curr_lineup_group = lineup_groups[0]
        self.play(ShowCreation(curr_lineup_group[0]), )
        for i, lineup_group in enumerate(lineup_groups[1:]):
            anims = [ReplacementTransform(curr_lineup_group, lineup_group)]
            anims += two_anims[:i + 1]
            if i == 0:
    ...
    def split_into_two_possibilities(self):
        ...
        for stacks, sym in (top_stacks, Female()), (bottom_stacks, Male()):
            ...
            for stack in stacks:
                ...
                for lineup in stack:
                    new_lineup = self.get_lineup(*[
                        Female()
                        if "female" in item.get_tex_string() else Male()
                        for item in lineup.items
                    ] + [sym],
                                                 buff=SMALL_BUFF)
            ...
            new_stacks.arrange(RIGHT, buff=MED_LARGE_BUFF, aligned_edge=DOWN)
        ...
        for stacks in top_stacks, bottom_stacks:
            self.play(MoveToTarget(stacks), MoveToTarget(stacks.numbers))
        self.wait()
        ...
        self.play(
            top_stacks.shift,
            dist * RIGHT / 2,
            top_stacks.numbers.shift,
            dist * RIGHT / 2,
            bottom_stacks.shift,
            dist * LEFT / 2,
            bottom_stacks.numbers.shift,
            dist * LEFT / 2,
        )
        self.wait()
    ...
    def combine_stacks(self):
        ...
        for i, top_stack in enumerate(top_stacks[:-1]):
            ...
            new_number = TexMobject(
                str(len(top_stack) + len(bottom_stack) - 2))
            new_number.next_to(expr, UP)
            new_numbers.add(new_number)

            self.play(Write(plus), *list(map(MoveToTarget, movers)))
        self.play(
...
class BuildUpFromStart(Scene):
        ...
        for x in range(self.n_iterations):
            ...
            for stacks, i in (low_stacks, 0), (top_stacks, -1):
                ...
                for stack in stacks:
                    ...
                    new_stacks.add(new_stack)
                new_stacks.arrange(RIGHT, buff=LARGE_BUFF, aligned_edge=DOWN)
            ...
            #Shift
            dist = top_stacks[1].get_center()[0] - top_stacks[0].get_center(
            )[0]
            self.play(
                top_group.shift,
                dist * RIGHT / 2,
            ...
            self.play(
                ReplacementTransform(expressions,
                                     VGroup(*list(map(VGroup, new_numbers)))))
            self.wait(wait_time)
    ...
    def get_numbers(self, stacks):
        return VGroup(*[
            TexMobject(str(len(stack))).next_to(stack, UP) for stack in stacks
        ])
...
class IntroducePascalsTriangle(Scene):
    ...
    def show_sum_of_two_over_rule(self):
        ...
        self.play(
            ...
            low_row,
            LEFT,
            top_row_copy.next_to,
            top_row,
            UP,
            top_row_copy.align_to,
            low_row,
            RIGHT,
        )
        self.play(ShowCreation(h_line), Write(plus))
        self.wait(2)
        ...
        self.play(
            ...
            top_row.restore,
            Transform(top_row_copy, top_row.saved_state, remover=True),
    ...
    def keep_in_mind_what_these_mean(self):
        ...
        self.play(
            *list(map(FadeOut, [morty, morty.bubble, morty.bubble.content])))
    ...
    def show_9_choose_4_pattern(self):
        ...
        self.play(
            rows.shift,
            3 * UP,
            self.nine_choose_four_line.shift,
            2.5 * UP,
        )
        ...
        #Probability
        expr = TexMobject("P(4", "\\female", "\\text{ out of }", "9", ")", "=")
        ...
        show_random_lines(4)
        self.play(MoveToTarget(nine_choose_four_term), Write(over_512))
        show_random_lines(4)
        ...
        self.play(
            ...
            *list(
                map(FadeOut, [
                    expr, nine_choose_four_term, over_512, eq_result,
                    self.curr_line
                ])))
    ...
    def cap_off_triangle(self):
        ...
        for n in range(4, -1, -1):
            line = VGroup(*[random.choice([Male, Female])() for k in range(n)])
            if n == 0:
        ...
        for line_group in line_groups[1:]:
            self.play(ReplacementTransform(curr_line_group, line_group))
...
class StacksApproachBellCurve(Scene):
    ...
    def construct(self):
        ...
        for x in range(self.n_iterations):
            ...
            self.play(*list(
                map(MoveToTarget, [bars, bars_copy, numbers, numbers_copy])))
            self.play(
                bars.shift,
                distance * LEFT / 2,
                numbers.shift,
            ...
            for group in new_numbers, numbers.target[
                    1:], numbers_copy.target[:-1]:
                for num in group:
            ...
            numbers_copy.target[-1].next_to(bars_copy.target[-1], UP,
                                            SMALL_BUFF)
...
class ChooseThreeFromFive(InitialFiveChooseThreeExample, PiCreatureScene):
    ...
    def add_people(self):
        ...
        self.play(Write(braces), LaggedStartMap(FadeIn, people),
    ...
    def show_association_with_binary(self, index):
        ...
        self.wait()
        self.play(ReplacementTransform(line_rect, full_line_rect),
    ...
    def order_doesnt_matter(self):
        ...
        for indices in it.permutations(list(range(len(triplet)))):
            perm = triplet.copy()
            resorter = VGroup(*[perm[i] for i in indices])
        ...
        self.play(ReplacementTransform(self.name_triplets[0].copy(), triplet))
        curr_perm = permutations[0]
        self.play(ReplacementTransform(triplet.copy(), curr_perm),
                  Write(updownarrow))
        for i in range(8):
    ...
    def that_phrase_is_confusing(self):
        ...
        self.play(
            ...
            LaggedStartMap(ApplyMethod, self.people, lambda pi: (
                pi.change,
                "confused",
                odm_words,
            )),
        ...
        for x in range(2):
            ...
            word_arrow_groups.submobjects = [
                word_arrow_groups[j] for j in (1, 2, 0)
            ]
    ...
    def pattern_is_unambiguous(self):
        all_ones = VGroup()
        for line in self.stack:
            ones = VGroup(*[m for m in line
                            if "1" in m.get_tex_string()]).copy()
...
class SubsetProbabilityExample(ChooseThreeFromFive):
    ...
    def ask_question(self):
        pi_name_groups = VGroup(*[VGroup(pi, pi.name) for pi in self.people])

        words = TextMobject("Choose 3 people randomly.\\\\", "Probability",
                            "Ali", "is one of them?")
    ...
    def show_all_triplets(self):
        self.play(
            self.question.scale,
            0.75,
            self.question.to_corner,
            UP + RIGHT,
            VGroup(self.people, self.names, self.braces).to_edge,
            LEFT,
        )
        self.choose_triplets()

        brace = Brace(self.name_triplets, RIGHT)
        total_count = brace.get_tex("{5 \\choose 3}",
                                    "=",
                                    "10",
                                    buff=MED_LARGE_BUFF)
    ...
    def circle_those_with_ali(self):
        ...
        self.play(ShowCreation(rect), alis.set_color, YELLOW)
        ...
        self.add(arrows)
        self.wait()
        self.play(FadeIn(frac_lines), five_choose_three.next_to, frac_lines[0],
                  DOWN, SMALL_BUFF, ten.next_to, frac_lines[1], DOWN,
                  SMALL_BUFF, Write(four_choose_two))
        self.wait()
        self.play(ReplacementTransform(four_choose_two.copy(), six))
...
class StudentsGetConfused(PiCreatureScene):
    def construct(self):
        ...
        line = VGroup(Male(), Female(), Female(), Male(), Female())
    ...
    def create_pi_creatures(self):
        pis = VGroup(*[Randolph(color=color) for color in (BLUE_D, BLUE_B)])
        pis[1].flip()
...
class HowToComputeNChooseK(ChooseThreeFromFive):
    ...
    def setup_people(self):
        ...
        self.set_variables_as_attrs(names, people_group, people_braces=braces)
    ...
    def count_possibilities(self):
        ...
        for name, line, count, arrow in zip(chosen_names, lines, choice_counts,
            ...
            self.play(name.next_to, line, UP, SMALL_BUFF, GrowArrow(arrow))
            self.wait()
        ...
        choice_numbers = VGroup(
            *[cc.submobjects.pop(1) for cc in choice_counts])
    ...
    def show_permutations_of_ABC(self):
        ...
        for indices in list(it.permutations(list(range(3))))[1:n_perms]:
            self.play(*[
                ApplyMethod(
                    name.next_to, lines[i], UP, SMALL_BUFF, path_arc=np.pi)
    ...
    def count_permutations_of_ABC(self):
        ...
        for line, name, count, arrow in zip(lines, shuffled_names,
                                            choice_counts, arrows):
        ...
        self.fraction = VGroup(numerator, frac_line,
                               VGroup(choice_numbers, dots))

    def reset_stage(self):
        n, k = self.n, self.k
        n_choose_k_equals = TexMobject("{%d \\choose %d} =" % (n, k))
        ...
        self.play(self.people_group.to_edge, LEFT, FadeIn(n_choose_k_equals),
    ...
    def show_whats_being_counted(self):
        ...
        self.wait()
        self.play(LaggedStartMap(FadeIn, rects), Write(rhs[-1]))
        self.wait()
    ...
    def get_choice_counts(self, n, k):
        ...
        choice_counts = VGroup(*[
            TextMobject("(", str(n0), " choices", ")", arg_separator="")
    ...
    def get_lines_and_place_words(self):
        ...
        lines = VGroup(*[Line(ORIGIN, width * RIGHT) for x in range(k)])
        lines.arrange(RIGHT)
        lines.next_to(ORIGIN, DOWN, buff=LARGE_BUFF)
        place_words = VGroup(*[
            TexMobject("%d^\\text{%s}" % (i + 1, s)) for i, s in zip(
                list(range(k)), it.chain(["st", "nd", "rd"], it.repeat("th")))
        ])
...
class NineChooseFourExample(HowToComputeNChooseK):
    ...
    def show_n_choose_k_pattern(self):
        ...
        stack = get_stack(
            ...
            TexMobject("0").set_color(BLUE), n, k)
        l = len(stack)
        n_stacks = 6
        columns = VGroup(*[
        ...
        columns.arrange(RIGHT, aligned_edge=UP, buff=MED_LARGE_BUFF)
        ...
        self.wait(2)
        self.play(FadeOut(stack, lag_ratio=0.5, run_time=2))
    ...
    def choose_k_people(self):
        ...
        self.play(
            FadeIn(choose_k),
            LaggedStartMap(ApplyMethod, chosen_subset, lambda m:
                           (m.shift, MED_LARGE_BUFF * DOWN)))
        self.wait()
        self.play(
            chosen_subset.shift,
            MED_LARGE_BUFF * UP,
    ...
    def finish_computation(self):
        ...
        self.play(
            ...
            FadeOut(self.lines),
            FadeOut(self.place_words),
            self.n_choose_k.next_to,
            equals,
            LEFT,
            self.fraction.next_to,
            equals,
            RIGHT,
            FadeIn(equals),
        )
        ...
        self.wait()
        self.play(ShowCreation(eight.cross), ShowCreation(four.cross),
                  FadeIn(two))
        self.wait()
        self.play(Write(rhs))
        self.wait()
...
class WeirdKindOfCancelation(TeacherStudentsScene):
    def construct(self):
        fraction = TexMobject("{5 \\cdot 4 \\cdot 3", "\\text{ ordered}",
                              "\\text{ triplets}", "\\over",
                              "1 \\cdot 2 \\cdot 3",
                              "\\text{ orderings \\;\\qquad}}")
        ...
        names = VGroup(
            *list(map(TextMobject, ["Ali", "Ben", "Cam", "Denis", "Evan"])))
        names.arrange(RIGHT)
        ...
        self.play(
            ...
            LaggedStartMap(FadeIn, fraction, run_time=1), FadeIn(braces),
            LaggedStartMap(FadeIn, names))
        self.change_student_modes(*["pondering"] * 3, look_at_arg=fraction)
...
class ShowFormula(Scene):
    def construct(self):
        specific_formula = TexMobject("{9 \\choose 4}", "=",
                                      "{9 \\cdot 8 \\cdot 7 \\cdot 6",
                                      "\\over",
                                      "4 \\cdot 3 \\cdot 2 \\cdot 1}")
        general_formula = TexMobject("{n \\choose k}", "=",
...
class SumsToPowerOf2(Scene):
    ...
    def setup_stacks(self):
        ...
        numbers = VGroup(
            *[TexMobject(str(choose(self.n, k))) for k in range(self.n + 1)])
    ...
    def count_32(self):
        ...
        sum_group = VGroup(*it.chain(*list(zip(numbers.target, plusses))))
        ...
        self.set_variables_as_attrs(plusses,
                                    sum_result,
                                    rhs,
    ...
    def show_sum_as_n_choose_k(self):
        ...
        sum_group = VGroup(*it.chain(*list(zip(fractions, plusses.target))))
        sum_group.arrange(RIGHT, buff=2 * SMALL_BUFF)
        sum_group.next_to(rhs, LEFT)
        sum_group.shift(0.5 * SMALL_BUFF * DOWN)

        self.play(Transform(n_choose_k_terms, fractions),
    ...
    def show_alternate_sum(self):
        ...
        plusses = VGroup(*[TexMobject("+")
                           for f in fractions[:-1]] + [TexMobject("=")])
        sum_group = VGroup(*it.chain(*list(zip(fractions, plusses))))
        sum_group.arrange(RIGHT)
        sum_group.next_to(self.n_choose_k_terms,
                          DOWN,
                          aligned_edge=LEFT,
                          buff=LARGE_BUFF)
        sum_group.shift(SMALL_BUFF * DOWN)
        rhs = TexMobject(str(2**self.alt_n), "=", "2^{%d}" % (self.alt_n))
...
class AskWhyTheyAreCalledBinomial(TeacherStudentsScene):
    def construct(self):
        example_binomials = VGroup(
            *[TexMobject("(x+y)^%d" % d) for d in range(2, 7)])
        example_binomials.arrange(UP)
        example_binomials.next_to(self.teacher.get_corner(UP + LEFT), UP)
        ...
        #Name themn
        self.play(Write(binomial_word), LaggedStartMap(GrowArrow, arrows))
        self.change_student_modes(*["pondering"] * 3)
...
class NextVideo(Scene):
    def construct(self):
        ...
        self.play(Write(title), ShowCreation(screen))
        self.wait()
