...
class DifferentConceptions(Scene):
    ...
    def intro_vector(self):
        ...
        self.wait(2)
        self.play(FadeOut(plane), FadeOut(labels),
    ...
    def physics_conception(self, creatures, original_vector):
        ...
        self.play(Transform(original_vector, vector),
        ...
        self.play(Rotate(vector, np.pi / 3, in_place=True),
                  Write(direction),
                  run_time=1)
        ...
        self.wait()
        self.play(Write(two_dimensional),
    ...
    def cs_conception(self, creatures):
        ...
        vectors = VMobject(*list(
            map(matrix_to_mobject, [
                [2, 1],
                [5, 0, 0, -3],
                [2.3, -7.1, 0.1],
            ])))
    ...
    def add_scaling(self, arrows, syms, arrays):
        s_arrows = VMobject(TexMobject("2"),
                            Vector([1, 1]).set_color(YELLOW), TexMobject("="),
    ...
    def restore_creatures(self, creatures):
        self.play(*[
            ApplyFunction(lambda m: m.change_mode("plain").set_color(m.color),
                          pi) for pi in creatures
...
class HelpsToHaveOneThought(Scene):
    def construct(self):
        ...
        self.play(ShowCreation(speech_bubble), Transform(morty, new_morty),
                  Write(words))
        self.wait(2)
        self.play(FadeOut(speech_bubble), FadeOut(words),
                  ApplyMethod(randy.change_mode, "pondering"),
                  ShowCreation(thought_bubble), Write(thought))
        self.wait(2)
...
class ListsOfNumbersAddOn(Scene):
    def construct(self):
        arrays = VMobject(
            *list(map(matrix_to_mobject, [[-2, 3], [1, 2], [2, -1], [4, 0]])))
...
class CoordinateSystemWalkthrough(VectorScene):
    ...
    def introduce_coordinate_plane(self):
        ...
        self.play(*list(map(GrowFromCenter, plane_lines)) +
    ...
    def show_vector_coordinates(self):
        ...
        self.wait()
        self.play(
            ApplyFunction(lambda m: m.scale_in_place(1.25).set_color(YELLOW),
                          array.get_brackets(),
...
class VectorAddition(VectorScene):
    ...
    def ask_why(self, v1, v2, v_sum):
        ...
        self.wait(2)
        self.play(Transform(v2, new_v2), Transform(v_sum, new_v_sum),
...
class ItDoesntMatterWhich(Scene):
    def construct(self):
        ...
        back_and_forth = VMobject(new_arrow, TexMobject("\\Leftrightarrow"),
                                  matrix_to_mobject([2, 1]))
        back_and_forth.arrange(LEFT).center()
        self.wait()
        self.play(ApplyMethod(physy.change_mode, "speaking"),
                  ShowCreation(physy_speech),
                  Write(physy_statement),
                  run_time=1)
        self.play(Blink(compy))
        self.play(ApplyMethod(physy.change_mode, "sassy"),
                  ApplyMethod(compy.change_mode, "speaking"),
                  FadeOut(physy_speech), ShowCreation(compy_speech),
...
class DataAnalyst(Scene):
    def construct(self):
        plane = NumberPlane()
        ellipse = ParametricFunction(lambda x: 2 * np.cos(x) *
                                     (UP + RIGHT) + np.sin(x) * (UP + LEFT),
                                     color=PINK,
                                     t_max=2 * np.pi)
        ellipse_points = [
            ellipse.point_from_proportion(x) for x in np.arange(0, 1, 1. / 20)
        ]
        string_vects = [
        ...
        self.add(vects)
        self.wait()
        self.play(ApplyMethod(plane.fade, 0.7),
