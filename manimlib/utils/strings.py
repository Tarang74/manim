...
def to_camel_case(name):
    return "".join(
        [[c for c in part
          if c not in string.punctuation + string.whitespace].capitalize()
         for part in name.split("_")])


def initials(name, sep_values=[" ", "_"]):
    return "".join([(s[0] if s else "")
...
def split_string_to_isolate_substrings(full_string, *substrings_to_isolate):
    ...
    all_substrings = list(
        it.chain(*list(
            zip(full_string.split(substring_to_isolate),
                it.repeat(substring_to_isolate)))))
    all_substrings.pop(-1)
    all_substrings = [s for s in all_substrings if s != ""]
    return split_string_list_to_isolate_substrings(all_substrings,
                                                   *substrings_to_isolate[1:])


def split_string_list_to_isolate_substrings(string_list,
