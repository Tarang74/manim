...
def generate_tex_file(expression, template_tex_file_body):
    result = os.path.join(
        consts.TEX_DIR, tex_hash(expression, template_tex_file_body)) + ".tex"
    ...
    if not os.path.exists(result):
        commands = [
            ...
            "-output-directory=\"{}\"".format(
                consts.TEX_DIR), "\"{}\"".format(tex_file), ">", os.devnull
...
def dvi_to_svg(dvi_file, regen_if_exists=False):
    ...
    if not os.path.exists(result):
        commands = [
            "dvisvgm", "\"{}\"".format(dvi_file), "-n", "-v", "0", "-o",
            "\"{}\"".format(result), ">", os.devnull
        ]
