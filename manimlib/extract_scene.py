...
def open_file_if_needed(file_writer, **config):
    ...
    open_file = any(
        [config["open_video_upon_completion"], config["show_file_in_finder"]])
    if open_file:
...
def prompt_user_for_choice(scene_classes):
    ...
    try:
        ...
        return [
            num_to_class[int(num_str)] for num_str in user_input.split(",")
        ]
    except KeyError:
...
def get_scenes_to_render(scene_classes, config):
    ...
    for scene_name in config["scene_names"]:
        ...
        if not found and (scene_name != ""):
            print(
                manimlib.constants.SCENE_NOT_FOUND_MESSAGE.format(scene_name),
                file=sys.stderr)
    if result:
        return result
    return [
        scene_classes[0]
    ] if len(scene_classes) == 1 else prompt_user_for_choice(scene_classes)
